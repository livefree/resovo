/**
 * ScrollRow — 前台卡片尺寸体系横滚布局原语（共享组件，ADR-214 Amendment A1 D-214-A1-6）
 *
 * **平级 CardGrid**：CardGrid 服务网格档（standard，列数派生）；ScrollRow 服务横滚（scroll 档，卡定宽）。
 * 横滚行布局统一原语——容器 flex + overflow-x + scroll-snap，每个 child 自动包裹为定宽 item。
 * 消费 CARD-SIZE-SSR 经 `[locale]/layout.tsx` **全局 `:root`** 注入的 scroll 档 DB 真源变量
 * `--card-w-scroll`（卡定宽）/ `--card-gap-scroll`（间距）；`card-size-fetch` helper 内置
 * `CARD_SIZE_DEFAULTS` 降级，生产恒有值（取代各横滚行手写 flex+overflow inline 骨架）。
 *
 ...
 *   `.scroll-row__item` 仅加渲染韧性兜底 `var(--card-w-scroll, 170px)`（170=scroll 档 DB 默认，对齐
 *   CardGrid `var(--cg-cols,2)` 防御姿态，消除非注入子树/Storybook/隔离单测下卡宽飘移）——
 *   兜底为渲染韧性、非配置真源（不违 D-214-5）；缺兜底的退化形态仅"卡宽变内容宽、横滚仍可用"，
 *   危害低于网格坍塌，故与 CardGrid 不对称合理。
 *
