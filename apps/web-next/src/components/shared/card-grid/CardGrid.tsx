 ...
 *
 * **仅服务网格档（standard/compact）**：scroll 横滚非网格、无列概念，由 CARD-SIZE-SCROLL 独立处理。
 * 故 sizeClass 类型收紧为 `Exclude<CardSizeClass,'scroll'>`——对 D-214-7 字面 `sizeClass: CardSizeClass`
 * 的有意收紧（类型层挡住把 scroll 误传进网格，优于运行时忽略/抛错）。
 *
 * **响应式 + 防溢出契约**见 globals.css `.card-grid`（D-214-4/7/10）：
 *   列数 mobile=2 / ≥640px=3 / ≥1024px=DB 注入值；`repeat(…, minmax(0,1fr))` + `> * { min-width:0 }`
 *   防 1fr 轨道 auto 最小值被海报固有宽/长标题撑破溢出（Codex-R2）。
 *
