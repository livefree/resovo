...
export const VE = {
  ...
  tmdb: {
    sectionTitle: 'TMDB 来源关系',
    mediaType: { movie: '电影', tv: '剧集' },
    searchPlaceholder: '输入标题搜索 TMDB（留空用本片标题）',
    yearPlaceholder: '年份（可选）',
    fieldsLabel: '应用字段',
    fieldLabels: {
      title: '标题',
      title_original: '原始标题',
    ...
    errors: {
      searchFailed: 'TMDB 搜索失败，请重试',
      confirmFailed: '确认失败',
      rejectFailed: '拒绝失败',
      tmdb_exact_conflict: '该 TMDB 条目已绑定到其他作品',
      tmdb_kind_conflict: 'TMDB 条目类型与现有绑定冲突',
      tmdb_fetch_failed: '拉取 TMDB 详情失败，请重试',
    },
  },
} as const
