...
export interface AutoCrawlConfig {
  ...
  /** ADR-155 D-155-6 / EP-1C-CLEANUP-A/C：多 dailyTime 支持（主字段 / min 1 max 24 / 各时间同日各触发一次 / 反序列化永远非空 兜底 ['03:00']）。CLEANUP-C：dailyTime alias 已删。 */
...
// ADR-155 D-155-3 / EP-3a + EP-3b-1 + HOTFIX-E/F：range 选项扩展 4 → 8（加 '5m' 最小粒度 + 12h/24h/7d）
