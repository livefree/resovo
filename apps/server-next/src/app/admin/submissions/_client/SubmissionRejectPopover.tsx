...
/**
 * SubmissionRejectPopover.tsx — 单条投稿拒绝原因输入弹层（CHG-SN-5-01）
 *
...
const REJECT_TEMPLATES = ['来源无法访问', '内容与视频不符', '重复提交', '格式不支持'] as const
...
interface SubmissionRejectPopoverProps {
...
export function SubmissionRejectPopover({
  trigger,
  pending,
  onConfirm,
  'data-testid': testId,
}: SubmissionRejectPopoverProps) {
  ...
  return (
    <Popover
      ...
      trigger={trigger}
      content={
        <div style={PANEL_STYLE} data-testid={testId ?? 'submission-reject-popover'}>
          ...
          <div style={TEMPLATES_ROW_STYLE}>
            {REJECT_TEMPLATES.map((tpl) => (
              <button
                ...
                data-testid={`submission-reject-template-${tpl}`}
              >
                {tpl}
              </button>
            ))}
          ...
          <AdminInput
            ...
            maxLength={200}
            size="sm"
            data-testid="submission-reject-reason-input"
          />
          <div style={FOOTER_STYLE}>
            <AdminButton
              ...
              size="sm"
              onClick={close}
              data-testid="submission-reject-cancel"
            >
              取消
            </AdminButton>
            <AdminButton
              ...
              data-testid="submission-reject-confirm"
            >
              拒绝
            </AdminButton>
          </div>
