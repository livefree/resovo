...
const CHIP_BASE: React.CSSProperties = {
  ...
  fontSize: 'var(--font-size-xxs)', borderRadius: 'var(--radius-full)', fontWeight: 500,
}
...
const ACTION_BTN: React.CSSProperties = {
  padding: '6px 12px', fontSize: 'var(--font-size-xs)', border: '1px solid var(--border-subtle)',
...
const INPUT_ROW: React.CSSProperties = {
  flex: 1, padding: '6px 8px', fontSize: 'var(--font-size-sm-tight)',
...
const OK_PILL: React.CSSProperties = {
  display: 'inline-flex', padding: '1px 7px', fontSize: 'var(--font-size-2xs)',
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 'var(--font-size-2xs)', color: 'var(--fg-muted)' }}>{item.year}{item.sub_title ? ` · ${item.sub_title}` : ''}</div>
      </div>
      <span style={{ fontSize: 'var(--font-size-2xs)', color: 'var(--fg-muted)', fontFamily: 'monospace' }}>ID {item.id}</span>
...
export function TabDouban({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      {/* 匹配状态 */}
      <div>
        <div style={{ fontSize: 'var(--font-size-sm-tight)', fontWeight: 600, marginBottom: '8px' }}>{m.matchStatus}</div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', background: 'var(--bg-surface-raised)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-subtle)' }}>
          ...
          {doubanId && (
            <span style={{ fontSize: 'var(--font-size-xxs)', color: 'var(--fg-muted)' }}>豆瓣 ID <code>{doubanId}</code></span>
          )}
          <span style={{ flex: 1 }} />
          {!isPending && (
            <span style={{ fontSize: 'var(--font-size-xxs)', color: 'var(--fg-muted)' }}>{m.notPendingNote}</span>
          )}
        </div>
      </div>
      ...
      {(doubanStatus === 'candidate' || doubanStatus === 'matched') && (
        <div>
          <div style={{ fontSize: 'var(--font-size-sm-tight)', fontWeight: 600, marginBottom: '8px' }}>{m.candidateSection}</div>
          ...
          {state.candidate && (
            <div style={{ background: 'var(--bg-surface-raised)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-subtle)', overflow: 'hidden' }}>
              <div style={{ padding: '10px 12px', borderBottom: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', gap: '10px' }}>
                <div style={{ flex: 1 }}>
                  ...
                  {state.candidate.confidence != null && (
                    <span style={{ marginLeft: '8px', fontSize: 'var(--font-size-xxs)', color: 'var(--state-success-fg)', fontWeight: 600 }}>
              ...
              {state.candidate.diffs.length > 0 && (
                <div>
                  <div style={{ display: 'grid', gridTemplateColumns: '72px 1fr 1fr 72px', padding: '5px 12px', background: 'var(--bg-surface-raised)', fontSize: 'var(--font-size-2xs)', fontWeight: 600, color: 'var(--fg-muted)', letterSpacing: '.5px', textTransform: 'uppercase', borderBottom: '1px solid var(--border-subtle)' }}>
                    <span>{m.columns.field}</span><span>{m.columns.local}</span><span>{m.columns.douban}</span><span />
                  </div>
                  {state.candidate.diffs.map((d, i) => (
                    <div key={d.field} style={{ display: 'grid', gridTemplateColumns: '72px 1fr 1fr 72px', padding: '7px 12px', alignItems: 'center', fontSize: 'var(--font-size-xs)', borderBottom: i < state.candidate!.diffs.length - 1 ? '1px solid var(--border-subtle)' : 'none' }}>
                      <span style={{ color: 'var(--fg-muted)', fontWeight: 600, fontSize: 'var(--font-size-xxs)' }}>{d.label}</span>
          ...
          {state.actionError && (
            <div style={{ marginTop: '8px', fontSize: 'var(--font-size-xs)', color: 'var(--state-error-fg)' }}>{state.actionError}</div>
          )}
        </div>
      )}
      ...
      {showSearch && (
        <div>
          ...
          {state.searchError && (
            <div style={{ fontSize: 'var(--font-size-xs)', color: 'var(--state-error-fg)', marginBottom: '8px' }}>{state.searchError}</div>
          )}
          ...
          {!state.searching && state.searchResults.length === 0 && !state.searchError && (
            <p style={{ fontSize: 'var(--font-size-xs)', color: 'var(--fg-muted)' }}>无搜索结果。</p>
          )}
        </div>
      )}
      ...
      {/* 手动搜索 */}
      <div>
        <label style={{ fontSize: 'var(--font-size-xs)', fontWeight: 500, color: 'var(--fg-muted)', display: 'block', marginBottom: '4px' }}>
          {m.searchSection}
        </label>
