...
const SLOT_BTN: React.CSSProperties = {
  flex: 1, padding: '4px 0', fontSize: 'var(--font-size-xxs)', border: '1px solid var(--border-subtle)',
  borderRadius: 'var(--radius-sm)', background: 'var(--bg-surface)', color: 'var(--fg-muted)',
  cursor: 'pointer',
}
const PRESENT_PILL: React.CSSProperties = {
  display: 'inline-flex', alignItems: 'center', padding: '1px 6px',
  fontSize: 'var(--font-size-2xs)', borderRadius: 'var(--radius-full)',
...
const INPUT_INLINE: React.CSSProperties = {
  flex: 1, padding: '4px 6px', fontSize: 'var(--font-size-xxs)',
    ...
    <div style={SLOT_CARD}>
      <div style={{ aspectRatio: KIND_ASPECT[kind], background: 'var(--bg-surface)', position: 'relative', overflow: 'hidden' }}>
        {hasImage
          ...
          : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--fg-subtle)', fontSize: 'var(--font-size-xxs)' }}>暂无图片</div>
        }
      </div>
      <div style={{ padding: '8px 10px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginBottom: '2px' }}>
          <span style={{ fontSize: 'var(--font-size-xs)', fontWeight: 600 }}>{slot.label}</span>
        ...
        <div style={{ fontSize: 'var(--font-size-2xs)', color: 'var(--fg-muted)', marginBottom: '6px' }}>{slot.desc}</div>
        {updateError && (
          <div style={{ fontSize: 'var(--font-size-2xs)', color: 'var(--state-error-fg)', marginBottom: '4px' }}>{updateError}</div>
        )}
        {editing ? (
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
        ...
        <span style={{ fontSize: 'var(--font-size-xxs)', color: 'var(--fg-muted)' }}>{m.uploadedCount(uploadedCount, KIND_ORDER.length)}</span>
      </div>
