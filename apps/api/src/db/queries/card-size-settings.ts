// card-size-settings.ts — card_size_settings 表 DB 查询（ADR-214 D-214-3 + Amendment A2，仿 home-section-settings.ts）
// 所有 SQL 参数化，不拼接字符串（db-rules.md）
// 单行全局 seed 恒存在（migration 124→125→126，size_class='global'）；不可删（无 DELETE 路径）
...
const COLUMNS = `id, size_class, card_width_px, gap_px,
...
/** 全量（A2 单行全局；DB 按 size_class 字典序返回） */
 ...
 * sizeClass 不可改（业务键）；卡宽范围由 DB CHECK + 调用方 zod 双层守（CARD-SIZE-SERVICE-ADMIN）。
 */
