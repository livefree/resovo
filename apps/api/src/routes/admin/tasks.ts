 ...
 *
 * 统一任务控制端点：top bar 任务抽屉聚合 crawler_runs + bull jobs 成统一 AdminTaskItem，
...
/** 按 TaskAggregator id 方案分派：`bull-{queue}-{jobId}` → bull job，否则 → crawler runId。 */
function parseTaskId(id: string): TaskTarget {
  const m = id.match(/^bull-(crawler|maintenance)-(.+)$/)
  if (m) return { kind: 'bull_job', queue: m[1] as BullQueueName, jobId: m[2]! }
