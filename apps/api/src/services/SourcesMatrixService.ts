...
import {
  ...
  type RouteReprobeResult,
  type RouteDeleteResult,
} from '@/api/services/sources-matrix.schemas'
// route 层从 sources-matrix.schemas 直接 import 7 schema（不再经 Service 转出）
// CHG-357 / arch-reviewer I1：probeOne / renderCheckOne 迁至 SourceProbeService
//   解 file-size BLOCKER (551→<400 行) + 配合 batch internal 抽取
import {
  SourceProbeService,
  type SingleSourceProbeResult,
  type SingleSourceRenderCheckResult,
} from '@/api/services/SourceProbeService'

// 重新导出兼容既有 import path（路由层 / 测试均通过 SourcesMatrixService import 类型）
export type { SingleSourceProbeResult, SingleSourceRenderCheckResult }
...
// Zod 端点契约 schema / 行级 mutation 结果 DTO / aggregateSignal 已拆至 sources-matrix.schemas.ts
// （CHG-VSR-3 Codex review FIX 解 500 行硬限）；route 层直接 import schema，本文件 import 结果 DTO + aggregateSignal。
// CHG-357：SingleSourceProbeResult / SingleSourceRenderCheckResult 迁至 SourceProbeService.ts（顶部 re-export 兼容）。
...
export class SourcesMatrixService {
  ...
  // ── ADR-158 单源 probe / render-check：委托 SourceProbeService ───
  // CHG-357 / arch-reviewer I1: 抽出 SourceProbeService 解 file-size BLOCKER
  // 既有 route 层调用契约保持不变（路由 import { SourcesMatrixService } 仍可调 .probeOne）
  ...
  async probeOne(
    ...
    const probeSvc = new SourceProbeService(this.db)
    return probeSvc.probeOne(sourceId, actorId, requestId)
  }
