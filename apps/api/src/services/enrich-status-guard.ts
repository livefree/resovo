/**
 * enrich-status-guard.ts — enrich 写列一致性守卫（ADR-219 D-219-3 / META-57）
 *
 ...
 * （ADR-219 D-219-3 / Codex r1 B-2：完全停写 + 列冻结留 META-60，读路径全迁后）。
 *
 * applied 判据与 DC-219-1 `videoRefAppliedSql` **同源**（共享 `VIDEO_REF_APPLIED_MATCH_STATUSES`
 ...
 * 复用既有 `findPrimaryVideoExternalRef`（primary ref）+ DC-219-1 `isVideoRefApplied`（applied 判定，
