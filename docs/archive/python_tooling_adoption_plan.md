...
1. 仅当仓库中**首次引入 Python 业务代码**（非 `node_modules`、非第三方 vendored 文件）时，才要求立即完成最小接入。
2. 在触发前，不做“空接入”、不引入额外 Python 工具配置，保持现有 TS/Node 流程不受影响。
3. 一旦触发，当前任务内必须完成：
   - `pyproject.toml` 最小配置
   - `uv + ruff + ty` 可执行链路
   - 至少一轮 `uv run ruff ...` 与 `uv run ty check` 验证

## 1. 现状与约束

1. 当前仓库主体是前后端 TypeScript/Node.js 工程，Python 不是现有主技术栈。
2. 规范引入方式必须是“最小侵入增量改造”，不能影响现有 `npm` 工作流。
3. 触发条件严格限定为“首次引入 Python 业务代码”。
