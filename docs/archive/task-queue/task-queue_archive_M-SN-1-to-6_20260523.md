# Resovo（流光）— 任务序列池归档 · M-SN-1 ~ M-SN-6（2026-04-28 ~ 2026-05-17）

> archived_at: 2026-05-23
> archived_from: docs/task-queue.md
> archived_range: SEQ-20260428-01（M-SN-1 工程骨架）~ CHG-SN-6-29-FOLLOWUP（M-SN-6 关闭复核）
> 行号映射（归档前 task-queue.md 行号）：88 ~ 3947
> milestone 覆盖：M-SN-1 / M-SN-2 / M-SN-3 / M-SN-4 / M-SN-5 / M-SN-6
> 续档：M-SN-7 跟踪卡 + 设计稿对齐重做 + M-SN-6 milestone 关闭声明 + M-SN-8 全部活跃序列保留在 docs/task-queue.md

本文件仅作历史追溯；活跃任务序列见 `docs/task-queue.md`。
上一段归档（SEQ-20260319-01 ~ SEQ-20260426-01）见同目录 `task-queue_archive_20260427.md`。
