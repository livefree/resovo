# Resovo（流光）— 任务序列池归档 · M-SN-7 跟踪卡 ~ SEQ-20260531-01（2026-05-17 ~ 2026-06-01）

> archived_at: 2026-06-05
> archived_from: docs/task-queue.md
> archived_range: 两段——A 段：SEQ-20260529-02/20260530-01..03（META P1-P3）+ CHORE-TEST-BASELINE-20260529 + CHORE-TEST-CRAWLER-TZ-FLAKY + M-SN-7 跟踪卡 + 设计稿对齐重做 + M-SN-6 关闭声明 + SEQ-20260521-01/02/03/04/06；B 段：SEQ-20260525-CRAWLER-W1/W2 + SEQ-20260526-CRAWLER-W3-FIX + SEQ-20260526/27-ENUMS-SSOT + SEQ-20260527/28-MOD-WAVE1..4 + SEQ-20260529-01 + SEQ-20260530-04..07 + SEQ-20260531-01
> 行号映射（归档前 task-queue.md 行号）：A 段 93 ~ 908 / B 段 984 ~ 2512（SEQ-20260524-01 留存活跃文件，原行 909 ~ 983）
> 状态说明：全部序列已 ✅ 收口（含 ⛔ NEGATED / DEFER 裁决项）。部分序列**内部子条目状态标记滞后**于序列头收口声明（如 SEQ-20260526-CRAWLER-W3-FIX 内 EP-1/2/3 仍标 ⬜/🟡），以**序列头状态 + docs/changelog.md（含分段归档）条目**为准。
> 残留提示：① META-13（Face 4 source 徽标）= 用户未纳入的可选 backlog（SEQ-20260530-02）；② META-15-A TMDB ⛔ DEFER（待「TMDB API」一并，SEQ-20260530-04）；③ CHG-SN-7-LOW-4 useDoubleConfirm（第 3 处复用时条件触发）、CHG-SN-7-MISC-SHELL-NAV-COUNTS（按需立卡）等条件触发卡见各原文。
> 续档：SEQ-20260524-01（M-SN-9 容器 🟡）+ SEQ-20260601-01 起全部序列保留在 docs/task-queue.md

本文件仅作历史追溯；活跃任务序列见 `docs/task-queue.md`。
上一段归档（M-SN-1 ~ M-SN-6）见同目录 `task-queue_archive_M-SN-1-to-6_20260523.md`。
...
- **方案/ADR 全文**：`docs/archive/2026Q2/external-metadata-ux-overhaul_20260529.md` + `docs/decisions.md` ADR-170。
...
- **方案/ADR 全文**：`docs/archive/2026Q2/external-metadata-ux-overhaul_20260529.md` §3.3/§3.4 + ADR-172 骨架（§13）。
...
- **台账**：`docs/archive/2026Q2/known-failing-tests_20260529.md`（已更新为 ✅ 已修复）
...
- **状态**：✅ 已完成（2026-06-04）
- **台账**：`docs/archive/2026Q2/known-failing-tests_20260529.md` §Flaky（已同步标记加固完成）
...
- **方案/ADR**：`docs/archive/2026Q2/external-metadata-ux-overhaul_20260529.md` §2 + §13 ADR-168 骨架（D-168-1..6 已锁；at-rest 加密 NEGATED for P1）。
