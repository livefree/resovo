...
| **CHG-SN-7-REDO-02-A** | migration 065 + types + actionType `user_submission.action` + targetKind `user_submission` + audit RETRO 4 真源同步（R-MID-1 第 15 次）+ audit content assertion test（≥ 5 case）| 新建 4 文件 + 改 4 文件 / 含 backfill 历史失效源举报 | opus-4-7 | 0.4w | 🔴 P0 |
| **CHG-SN-7-REDO-02-B** | apps/api：6 端点（list / detail / process / reject / batch ×2）+ service + queries + audit 写入 + ≥ 10 case 单测 | userSubmissions queries + service + route（admin only / moderator+） | opus-4-7 | 0.7w | 🔴 P0 |
| **CHG-SN-7-REDO-02-PRE-CARD-PRIMITIVE** | admin-ui Card / Segment / QuoteBlock primitive 调研 | 0.1w / Opus / 缺则起独立 PRE 共享 primitive 卡 | Opus 调研 | 0.1w | 🔴 P0（C 前置）|
| **CHG-SN-7-REDO-02-C** | 前端：新页面 `/admin/user-submissions`（4 Segment + Card list / spec §5.13 / 32px 状态 icon box + poster + quote block + 3 按钮）| 复用 admin-ui primitives | opus-4-7 | 0.8w | 🔴 P0 |
| **CHG-SN-7-REDO-02-D** | 旧 `/admin/submissions*` 改 thin alias 转发新端点 + 旧 SubmissionsListClient deprecation banner | M-SN-9 退役卡承担最终删除（Y1） | Haiku | 0.2w | 🔴 P0 |
| **CHG-SN-7-REDO-02-E** | RETRO 4 真源验证 + verify:adr-contracts + e2e | content assertion test 实测 PASS | Sonnet | 0.3w | 🔴 P0 |
| **CHG-SN-7-REDO-02-F** | Opus 验收（spec §5.13 100% + ADR-124 9 节闭环 + ≥ A−） | spawn arch-reviewer Opus | Opus 验收 | 0.2w | 🔴 P0 |
