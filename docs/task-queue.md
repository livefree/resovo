...
- **依赖**：CHG-UX-01
- **建议模型**：sonnet
- **变更原因**：用户反馈核心痛点（topbar 4 图标 + 搜索框点击靶子无反馈）
- **文件范围**：
  - `packages/admin-ui/src/shell/topbar.tsx`（IconButton `data-interactive="icon"`；SEARCH_TRIGGER `data-interactive="trigger"`）
- **完成判据**：4 个 icon button + 搜索框在 dark / light 双模式下 hover 都有可见反差

### CHG-UX-04 · dropdown trigger / staff-note edit / VideoFilterFields hover
- **状态**：📝 待开工
- **依赖**：CHG-UX-01
- **建议模型**：sonnet
- **变更原因**：与 topbar trigger 同语义场景统一
- **文件范围**：
  - `packages/admin-ui/src/components/data-table/views-menu.tsx`（TRIGGER `data-interactive="trigger"`）
  - `packages/admin-ui/src/components/feedback/staff-note-bar.tsx`（EDIT_TRIGGER `data-interactive="icon"`）
  - `apps/server-next/src/app/admin/videos/_client/VideoFilterFields.tsx`（各 input/select `data-interactive="trigger"`）
...
- **建议模型**：sonnet
- **文件范围**：
  - `packages/admin-ui/src/components/data-table/data-table.tsx`（表头 sort/filter button `data-interactive="icon"`；row hover 80ms → `var(--duration-fast)`）
  - `packages/admin-ui/src/components/data-table/dt-styles.tsx`（chip/btn 加 `data-interactive="chip"`）
  - `packages/admin-ui/src/components/data-table/pagination-foot.tsx`（pagesize 容器整体 trigger 触发面）
- **完成判据**：表头排序按钮 hover 可见；行 hover 时长统一；pager / chip 样式不回归
