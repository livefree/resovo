...
6. M5-CARD-SKELETON-01 — Skeleton primitive + 三档门槛（状态：✅ 完成）
   - 创建时间：2026-04-20 19:00
...
2. M5-ADMIN-BANNER-01 — Banner 后台管理（状态：⚠️ 核心完成，两个验收项延期）
   - 创建时间：2026-04-20 19:00
   - 计划开始：M5-API-BANNER-01 ✅ 后
   - 实际开始：2026-04-21 15:00
   - 完成时间：2026-04-21 15:20（核心功能）
   - 延期项：① 图片上传（无基础设施，需单独任务）② Banner 管理 E2E（需单独任务）
   - 建议模型：claude-sonnet-4-6
   - 规模：M（~150 分钟）
   - 依赖：M5-API-BANNER-01 ✅
   - **文件范围**：
...
3. M5-PAGE-BANNER-FE-01 — HeroBanner 前端重塑（状态：✅ 已完成）
   - 创建时间：2026-04-20 19:00
...
4. M5-PAGE-GRID-01 — 分类页 Grid 重塑（Sibling 首激活）（状态：✅ 已完成）
   - 创建时间：2026-04-20 19:00
...
5. M5-PAGE-SEARCH-01 — 搜索页重塑（状态：✅ 已完成）
   - 创建时间：2026-04-20 19:00
...
7. ADMIN-15 — 审核区线路分组按 `source_name + source_site_key`（状态：✅ 已完成 2026-04-22）
   - 创建时间：2026-04-22
   - 实际开始：2026-04-22
   - 完成时间：2026-04-22
   - 执行模型：claude-opus-4-7
...
10. CRAWLER-08 — `source_category` 改存 `vod_class` + 主链路切 `mapSourceCategory()`（状态：✅ 已完成 2026-04-22）
    - 创建时间：2026-04-22
    - 实际开始：2026-04-22
    - 完成时间：2026-04-22
    - 执行模型：claude-opus-4-7
...
5. HANDOFF-14 — Shelf 骨架：4 种 template + empty placeholder（状态：✅ 已完成）
   - 创建时间：2026-04-23 12:00
   - 实际开始：2026-04-23
   - 完成时间：2026-04-23
   - 建议模型：sonnet
...
7. HANDOFF-16 — Search 浮层 + 搜索页（状态：✅ 已完成）
   - 实际开始：2026-04-23 17:10
   - 完成时间：2026-04-23 17:45

8. HANDOFF-17 — Detail 页（状态：✅ 已完成）
   - 创建时间：2026-04-23 12:00
   - 实际开始：2026-04-23 18:00
   - 完成时间：2026-04-23 18:30
   - 建议模型：sonnet
...
3. **HANDOFF-21 — Nav 交互修正（右对齐 + 分类同源 + 搜索宽度 + ⌘K 适配 + 更多展开）**（状态：✅ 完成，2026-04-24）
   - 创建时间：2026-04-24
   - 实际开始时间：2026-04-24
   - 完成时间：2026-04-24
   - 执行模型：claude-sonnet-4-6
...
6d. **HANDOFF-29 — 详情页播放源选择器**（状态：✅ 已完成，2026-04-24）
   - 创建时间：2026-04-24
   - 建议模型：sonnet
   - 估时：0.4d
...
#### HANDOFF-31 — MiniPlayer 非交互基础层（状态：✅ 已完成）

- **创建时间**：2026-04-24（任务拆分重构：2026-04-25）
- **建议模型**：sonnet
- **估时**：0.6d
- **前置依赖**：REVIEW-C PASS
- **修复项**：原缺陷 #2 / #9 / #12（P0 基础层）
- **UI 规格基准**：`docs/handoff_mini_player/miniPlayer_ui_contract_20260425.md`（含 §13 v1.1 修订，**开工前必读**）

> 本任务只建立"容器骨架 + store + token + drag 修正"，**不引入真实 `<video>` 元素**。视频与交互核心在 HANDOFF-32 实现。
...
- `apps/web-next/src/stores/playerStore.ts`（新增 action）：
  - **新增 `releaseMiniPlayer()` action**（UI Contract §13.4）：
    ...
    ```
  - 确认 `setCurrentTime(t: number)` 已存在（playerStore line 91）
  - 确认 `isPlaying` 字段已存在

- `apps/web-next/src/app/globals.css`（新增 `/* HANDOFF-31 */` token 区块）：
  - 逐一检查 UI Contract §8 全部 14 个 `--player-mini-*` token，补齐 globals.css 中缺失项
...
- `apps/web-next/src/lib/mini-player/drag.ts`（改动）：
  - `attachViewportResizeWatcher`：resize 时无论是否越界，若处于 corner-dock 状态，均按 `(corner, margin)` 重新计算 `left/top` 并写入 DOM
  - 确保 `MiniGeometryV1` 中持久化 `corner` 字段
  - 拖拽结束 snap 算法：以容器中心点判断最近角，写入 `corner` 字段

- `apps/web-next/src/app/[locale]/_lib/player/MiniPlayer.tsx`（骨架重构，不含 video）：
  - **几何骨架**（UI Contract §2）：
    - 容器高度 Collapsed = `videoH`（`width × 9/16`）；Expanded = `videoH + 44px`
    - Header 32px：`position: absolute; top: 0`，叠加视频占位区，不占容器高度
    - Expanded 时视频占位区 `bottom: 44px`，控制栏占位区 `position: absolute; bottom: 0`（HANDOFF-32 填充内容）
  - **`handleClose()` 实现**：`video.pause()` → `video.src = ''` → `releaseMiniPlayer()`（video ref 在 HANDOFF-32 接入，此阶段 ref 为 null 时跳过 video 操作）
  - **移动端防护**（UI Contract §13.6）：
    - mount 时 MQ 匹配 → 调 `handleClose()`
    - MQ 变化事件也触发 `handleClose()`
  - **Esc 关闭**：`keydown` 监听调 `handleClose()`（替换原 `closeHost()`）
  - **m 键静音占位**：`keydown` 监听，HANDOFF-32 接入 video ref 后实现 toggle mute
  - **移除 PiP 代码**：删除 MiniPlayer.tsx 内任何 `pip` / `requestPictureInPicture` 相关逻辑
  - **data-testid 占位**：`mini-player-return-btn` / `mini-player-toggle-expand` / `mini-player-play-pause` / `mini-player-progress` / `mini-player-mute` / `mini-loading` / `mini-error` / `mini-no-source`
  - **ARIA 骨架**（UI Contract §9）：`role="region" aria-label="迷你播放器"`

**验收要点**：
- [ ] `releaseMiniPlayer()` action 存在于 store，调用后 `shortId === null` 且 `hostMode === 'closed'`
- [ ] globals.css 中 14 个 `--player-mini-*` token 全部存在，无硬编码颜色
- [ ] drag.ts：resize 时 corner-dock 位置重新计算，`corner` 字段持久化
- [ ] MiniPlayer 骨架渲染：Collapsed 高度正确，header 覆盖定位正确（可视化确认）
- [ ] 移动端 MQ 触发时调 `releaseMiniPlayer()`（非 CSS 隐藏）
- [ ] Esc 关闭调用 `handleClose()`
- [ ] MiniPlayer.tsx 无任何 `pip` / `requestPictureInPicture` 字样（grep 验证）
- [ ] typecheck + lint 通过
...
- **估时**：1.2d
- **前置依赖**：HANDOFF-31 ✅
- **修复项**：原缺陷 #1 / #3 / #4 / #5 / #6 / #8（P0/P1 交互层）
- **UI 规格基准**：`docs/handoff_mini_player/miniPlayer_ui_contract_20260425.md`（§13 v1.1，**开工前必读**）

**文件范围**：

- `apps/web-next/src/app/[locale]/_lib/player/MiniPlayer.tsx`（视频与交互核心）：
  - **真实 `<video>` element**（UI Contract §4）：
    - `<video ref={videoRef} position: absolute; inset: 0; object-fit: contain>`
    - mount 时从 `playerStore.currentTime` 设 `video.currentTime`（精确续播起点）
  - **activeSrc 解析**（UI Contract §13.1）：
    - 独立调 `GET /videos/:shortId/sources?episode=currentEpisode` 获取 sources
    - 读取 `playerStore.activeSourceIndex`，取 `sources[idx]?.url`；越界 fallback `sources[0]?.url`；空时 src=null
  - **Autoplay 语义**（UI Contract §13.2）：
    - `playerStore.isPlaying === true` → 尝试 `video.play()`；`NotAllowedError` → `autoplayBlocked = true`（play 图标 + 点击播放，不重试）
    - `playerStore.isPlaying === false` → 不调用 play()，显示 pause overlay
  - **`onTimeUpdate`**（250ms 节流，UI Contract §13.3）：
    - `playerStore.setCurrentTime(t)` + `saveProgress(shortId, episode, t)`（两者都写）
  - **Overlay 状态机**（UI Contract §4.2，6 态）：loading / error / 无 src / autoplay-blocked / playing+hover / paused
  - **Header hover reveal**（UI Contract §3）：
    - 默认 `opacity: 0; pointer-events: none`；hover 主体 → `opacity: 1; pointer-events: all`（150ms，受 `--motion-scale`）
    - 离开主体 200ms 延迟消失（防移到 header 时闪烁）
    - 所有 header 按钮 `onPointerDown={e => e.stopPropagation()}`；`tabIndex` 随 opacity 切换
  ...
  - **折叠/展开**（UI Contract §6 + §13.5）：
    - 切换 `isExpanded` → 更新容器 height → **立即按当前 corner 重新计算 top**（防 bl/br 超出 margin）
    - `transition: height 200ms ease`（受 `--motion-scale`）；`isExpanded` 不持久化
  - **点击防误触**（UI Contract §4）：pointerdown 记录坐标，移动 < 5px 且 duration < 300ms → toggle play/pause
  - **m 键静音**：`keydown` 监听，toggle `video.muted`
  - **ARIA 完整实现**（UI Contract §9）：所有按钮 aria-label；进度条完整 ARIA slider 属性

- `apps/web-next/src/components/player/PlayerShell.tsx`（只读确认，无需修改）：
  - `activeSourceIndex` 在 sources 加载后已保留（lines 85-86）
  - `saveProgress` 导入已存在（mini 复用同一函数）
  - 续播路径：mini 调 `saveProgress` → 返回 /watch → `ResumePrompt` 弹出 → 用户点继续（**无需改动**）
...
- [ ] 视频区有真实 `<video>`，activeSrc 非空时可播放；mount 时从 `playerStore.currentTime` 续播
...
- [ ] `onTimeUpdate` 同时写 `playerStore.setCurrentTime` 和 `saveProgress`（节流 250ms）
- [ ] 返回 /watch 后 ResumePrompt 出现，用户点"继续"从断点恢复（误差 ≤ 3s）
- [ ] header 默认透明不可交互，hover 主体后可见；按钮 pointerdown stopPropagation
...
#### HANDOFF-34 — MiniPlayer 体验增强（P2，可延后）（状态：✅ 已完成）

- **创建时间**：2026-04-25（从原 HANDOFF-33 内容迁移）
- **实际开始时间**：2026-04-25 10:00
- **完成时间**：2026-04-25 13:30
- **建议模型**：sonnet
- **估时**：0.6d
- **前置依赖**：HANDOFF-33 ✅
...
- **审核要点**（11 项）：
  ...
  4. 事件隔离：`onPointerDown stopPropagation` 是否覆盖所有 header 按钮；点击/拖拽区分逻辑（5px/300ms）是否在视频区实现
  5. dock recompute：viewport resize 时 corner 重新计算正确（四角各一场景）；展开/折叠后 top 值重新计算（防 bl/br 超出）
...
#### HANDOFF-36 — MiniPlayer UI 交互微调（header 文案 + 展开改 hover + controls 75% 透明）（状态：✅ 已完成 2026-04-25）

- **创建时间**：2026-04-25 16:09
- **实际开始时间**：2026-04-25 16:09
...
1. **header 文案**：`shortId ? '正在播放' : '迷你播放器'` → `${title}${episodeCount > 1 ? ' · 第N集' : ''}`；title 区 `flex:1 + min-width:0 + ellipsis`，集数 `flex-shrink:0` 永远可见
2. **展开 → hover 显示**：删除 `isExpanded` state / `handleToggleExpand` / header toggle 按钮；容器高度永远 `videoH`；`MiniPlayerControls` 改为 `position:absolute; bottom:0`，基于 `headerVisible` 切 opacity；高度 44px → 32px（同 header），按钮 32×32 → 24×24，时间字号 11px → 10px
3. **75% 不透明度**：controls 背景 `color-mix(in srgb, var(--player-mini-ctrl-bg) 75%, transparent)`

**数据源**：useMiniPlayerVideo 新增 fetch `/videos/:shortId` 拿 title + episodeCount

**文件范围**：
- `apps/web-next/src/app/[locale]/_lib/player/MiniPlayerHeader.tsx`
- `apps/web-next/src/app/[locale]/_lib/player/MiniPlayerControls.tsx`
- `apps/web-next/src/app/[locale]/_lib/player/MiniPlayer.tsx`
- `apps/web-next/src/app/[locale]/_lib/player/useMiniPlayerVideo.ts`
- `tests/unit/web-next/MiniPlayer.test.tsx`

---

## [SEQ-20260425-LOG-V1] 日志系统建设（提案 docs/logging_system_proposal_20260425.md）

- **状态**：🟡 规划中
- **创建时间**：2026-04-25 17:16
- **最后更新时间**：2026-04-26 02:25（INFRA-18 ✅ 完成，1 P2 + 2 P3 全闭环；INFRA-17 自 CONDITIONAL PASS 升级为 PASS；SEQ-20260425-LOG-V1 主线全 ✅，仅剩 INFRA-12 实现下沉为隐式后续，触发窗口 2026-05-10）
- **目标**：从混合终端流升级为分类、结构化、可回溯的日志体系；为浏览器端引入轻量上报通道
- **范围**：`scripts/dev.mjs` / `apps/api/{lib,server,workers,routes/internal}` / `apps/web-next/src/lib` / `packages/logger`（新建）/ `docs/rules` / `CLAUDE.md`（仅索引表）
- **依赖**：无（提案 docs/logging_system_proposal_20260425.md 第 9 节 5 个决策点 + 复核新增 8/9/10 三点全部由用户拍板，commit dc51d27）
- **方案对应**：本序列 INFRA-07..12 ↔ 提案 LOG-03/02/01/04/05/06，文件范围与决策严格按提案落地
- **编号修订（2026-04-25 18:25）**：原编号 INFRA-01..06 与历史项目初始化系列（changelog 第 54/126/137/148/159/172 行）撞号，违反 task-queue "禁止复用已存在编号" 规则，已统一改 INFRA-07..12；commit 8756a11 message 仍写 INFRA-01 是历史已 commit 不可改，changelog 条目同步改 INFRA-07 并备注
- **不做（划线防越界）**：不引入 ELK / Loki / Sentry / pino-pretty 任何新外部依赖；不做日志查询 UI；不做线上日志聚合；不合并 `CrawlerTaskLogService` 等业务日志（决策 5）

### 任务列表（按执行顺序）

1. INFRA-07（LOG-03）— `scripts/dev.mjs` 改造：pipe + 落盘 + rotate + errors 聚合（状态：✅ 已完成）
   - 创建时间：2026-04-25 17:16
   - 计划开始：序列首发（建议下一会话立即启动）
   - 实际开始：2026-04-25 18:00
   - 完成时间：2026-04-25 18:15
   - 前置依赖：无
   - 建议模型：sonnet
   - 子代理调用：否
   - 估时：0.5d
   - 改动点：
     - `scripts/dev.mjs`：`stdio:'inherit'` → `['ignore','pipe','pipe']`；按 service 分流到 `logs/dev/<service>.log` + `<service>.ndjson`；rotate（10MB / 5 份）
     - **errors 聚合规则（决策 4 选 B + 复核 #2 收紧）**：① JSON 行解析成功 → 兼容 pino numeric level（`>=40` warn / `>=50` error / `>=60` fatal）与字符串 level（`warn`/`error`/`fatal`）；② JSON 解析失败 fallback 到非 JSON 路径；③ 非 JSON 行**仅**当来源 stream 是 stderr 且匹配保守前缀（`Error:` / `Warning:` / `[ERR]` / `[WARN]`）时复制到 `logs/errors/errors-<date>.ndjson`，避免误把普通 stderr 都当 error
     - **client 流前向兼容（复核 #1 收紧）**：dev.mjs 见到 `service:'client'` 字段就分流到 `logs/client/<date>.ndjson`；本卡**不创建**任何 client 端点 / 前端代码（端点在 INFRA-10 落地）
     - 启动打印 logs/ 绝对路径；SIGINT flush + 关闭 fd
     - `package.json`：新增 `npm run logs` / `logs:errors` 脚本
     - `.gitignore`：追加 `logs/`
   - 文件范围：`scripts/dev.mjs`、`package.json`、`.gitignore`
   - 验收要点：
     - 4 子进程的 stdout/stderr 实时出现在 `logs/dev/<service>.{log,ndjson}` 中，主终端仍可读且带 [service] 前缀
     - errors 聚合三路径全覆盖：① pino numeric level 50 行落入 errors；② 字符串 `level:"error"` 行落入 errors；③ 普通 stderr 文本（如 Next.js compile 信息）**不**落入 errors
     - 单文件超 10MB 触发 rotate，最旧 `.5` 被删
     - SIGINT 后 fd 关闭无丢尾行
     - `logs/` 已被 .gitignore 排除
     - **守门 A（人工验证）**：人工启动 `npm run dev` ≥ 60 秒，肉眼确认四个 service 文件实时增长、Ctrl+C 后无丢尾行；制造 11MB 输入强制触发 rotate 验证 `.5` 删除
     - `npm run typecheck` / `npm run lint` / `npm run test:run` 全绿（dev.mjs 不入 typecheck，但需保持 mjs ESM 语义正确）
   - 完成备注要求：changelog 条目附**至少 3 行真实日志格式样例**（脱敏后），覆盖 info / warn / error 各一条，作为未来格式漂移对比基线（守门 D）
   - 完成备注：`stdio:'inherit'` → `['ignore','pipe','pipe']` 全量改造完成；readline 行缓冲按 service 落 .log + .ndjson；errors 双路径聚合（JSON pino numeric/string level + 非 JSON stderr 保守前缀）；client 流前向兼容；rotate 10MB/5份；SIGINT flush；.gitignore 排除 logs/；`npm run logs` / `npm run logs:errors` 脚本新增。typecheck PASS / lint PASS / 1723 unit tests PASS。守门 A 需人工运行 60s 验证。守门 D 3 条样例已写入 changelog。执行模型: claude-sonnet-4-6

2. INFRA-08（LOG-02）— API + worker 接入统一 logger（状态：✅ 已完成）
   - 创建时间：2026-04-25 17:16
   - 计划开始：INFRA-07 完成后
   - 实际开始：2026-04-25 19:10
   - 完成时间：2026-04-25 19:35
   - 前置依赖：INFRA-07 ✅
   - 建议模型：sonnet
   - 执行模型：claude-sonnet-4-6
   - 子代理调用：否（Spike 验证 Fastify v4 类型限制，按预定降级方案落地）
   - 估时：1.0d
   - 开发前输出（CLAUDE.md 质量门禁）：
     - **Spike：Fastify v4 `loggerInstance` 类型可行性**（复核第 4 点）—— 用 5 行最小复现验证 `Fastify({ loggerInstance: pinoLogger })` 在 `fastify-type-provider-zod` 上下文下能否通过 typecheck；不通过则回退到 `logger: { level, serializers, redact }` 选项形式（仅"复用同一 pino 实例"目标降级，结构化与脱敏不受影响）
   - **范围收敛（复核 #3）**：本卡**只承诺** ① workers 9 个；② API 启动路径（`server.ts`）；③ 队列基础设施（`apps/api/src/lib/queue.ts`）。其他 service / route / lib（如 `redis.ts` / `postgres.ts` / `services/*`）中的散点 stderr 替换**不进本卡口径**，由后续轻量任务处理（暂不立卡）
   - 改动点：
     - `apps/api/src/lib/logger.ts`：新增 `createLogger({ service: 'api' })`（实现暂留本文件，决策 7）；导出 `withRequest(logger, req)` / `withJob(logger, job)` 派生器；类型 / 序列化器 / redact 表先用本地 stub 占位，标注 INFRA-09 完成后切到 `@resovo/logger`
     - `apps/api/src/lib/queue.ts`：替换裸 stderr / console 为本 logger（队列基础设施在范围内）
     - `server.ts`：替换 `logger:{ level }` 为 `loggerInstance: logger`（或回退方案）；加 `genReqId`（`crypto.randomUUID()`）；加 `onResponse` hook 输出 access log（含 `duration_ms` / `status` / `request_id` / `method` / `url`）
     - 9 个 `apps/api/src/workers/*.ts`：`register*` 顶部派生 `child({ worker: '<name>' })`；job handler 进入时 `withJob(log, job)`；删除/替换裸 `process.stderr.write` 与 `console.*`；按 Bull v4（`Bull.Job<T>`）类型签名（复核第 1 点，**不是 BullMQ**）
   - 文件范围：`apps/api/src/{lib/logger.ts,lib/queue.ts,server.ts,workers/*.ts}`
   - 验收要点：
     - `request.log.error/warn/info` 全部带 `request_id`，跨日志可串联
     - 每条 worker 日志带 `worker` / `job_id`
     - access log 出现在 stdout 且 `duration_ms` / `status` 字段齐全
     - PII 字段（authorization / cookie / set-cookie / password / token / refreshToken / accessToken / email / phone / ip / url.query）在序列化后被 redact；**禁止把原始 req/res/headers 直接放入 ctx**（复核第 7 点硬规则）
     - `npm run typecheck` / `npm run lint` / `npm run test:run` 全绿
     - **守门 B（关键路径回归 5 路径）**：登录 / 视频列表 / 视频详情 / crawler 创建一次 / crawler 失败一次，日志中 `request_id` / `worker` / `job_id` / `stage` 串联可见，且 API 行为无回归
   - 完成备注要求：changelog 附 **5 路径** 各 1 条脱敏日志样例（access + worker + error 各类），作为格式漂移基线（守门 D）
   - 完成备注：新建 `lib/logger.ts`（createFastifyLoggerOptions / createLogger / withRequest / withJob / baseLogger）；Fastify v4 `logger: pinoInstance` 导致 Logger 泛型过于具体，按任务预定降级改为 `logger: options` 形式；`server.ts` 新增 genReqId(crypto.randomUUID) + onResponse access log hook；`queue.ts` 替换 attachQueueLogger 为结构化 qlog；9 个 workers 全部替换裸 stderr/console，顶部派生 child({ worker }) + withJob 派生 job 上下文。redact 18 条路径（11 PII 字段含嵌套）；url.query 通过 serializeReq split('?')[0] 截断；禁止原始对象透传。typecheck PASS / lint PASS / 1723 unit tests PASS。守门 B 需人工验证 5 路径串联。守门 D 5 条样例已写入 changelog

3. INFRA-09（LOG-01）— 新建 `packages/logger` workspace 包（首期仅类型 + 序列化器 + redact 表）（状态：✅ 已完成）
   - 创建时间：2026-04-25 17:16
   - 实际开始：2026-04-25 23:30
   - 完成时间：2026-04-25 23:45
   - 前置依赖：INFRA-08 ✅
   - 建议模型：sonnet
   - 执行模型：claude-sonnet-4-6
   - 子代理调用：arch-reviewer（claude-opus-4-7）— 契约评审 NEEDS_REVISION；6 MUST + 3 SHOULD 全采纳（M1/formatPretty 基于决策 6 保留）
   - 估时：0.5d
   - 开发前输出：
     - 公共 API 契约草案（types.ts / serializers.ts / redact.ts / levels.ts 导出签名）
     - spawn arch-reviewer 子代理评审，**通过后**才下笔；评审结论与子代理模型 ID 写入卡片"子代理调用"字段
     - **跨 app 解析前置验证（复核 #5）**：① admin (`apps/server`) / web-next 是否需要在 `next.config.js` 加 `transpilePackages: ['@resovo/logger']`；② 现有 `@resovo/types` / `@resovo/player-core` 的 export / path 配置作为参考模板；③ 若 source 直接 import（无 dist），确认 ts/tsconfig 的 `composite` / `references` 配置是否需调整。该验证不通过则回退到"包内出 dist 再被消费"模式
   - 改动点：
     - 根 `package.json`：**显式化 pino 依赖**（复核 #4 选定路径）—— 把 pino 从 fastify transitive 提升为 direct dependency（版本对齐当前 lockfile 已存在的 fastify-bundled 版本）；理由：pino 已在 lockfile 中，显式化只是从间接转直接，**不引入新技术栈**，与 CLAUDE.md "不引入技术栈外新依赖" 精神一致
     - `packages/logger/package.json`：`peerDependencies: pino`（避免重复声明）；`devDependencies: pino`（本地 typecheck）
     - `packages/logger/src/types.ts`：`ServiceName`、`LogContext`、`LoggerOptions`（直接 import `pino` 类型，不自定义最小接口避免漂移）
     - `packages/logger/src/serializers.ts`：`err / req / res` 序列化器；**禁止透传原始对象**（硬规则）
     - `packages/logger/src/redact.ts`：完整 redact paths 表（authorization / cookie / set-cookie / password / token / refreshToken / accessToken / email / phone / ip / url.query → `<redacted>`，query 整体替换不做白名单透出）
     - `packages/logger/src/levels.ts`：`computeLevel(env)` + `formatPretty(record): string`（30–50 行 ANSI 着色，决策 6 自实现替代 pino-pretty）
     - `packages/logger/src/index.ts`：仅 re-export 上述类型与函数，**首期不导出 `createLogger` 实现**（决策 7）
     - 根 `package.json`：workspace 已通配 `packages/*`，无需改 workspaces 字段；新建包 `name: '@resovo/logger'`
     - `apps/api/src/lib/logger.ts`：从 `@resovo/logger` import 类型与序列化器，替换 INFRA-08 的本地 stub
     - 如解析验证显示需要：`apps/server/next.config.js` / `apps/web-next/next.config.js` 的 `transpilePackages` 增加 `@resovo/logger`
   - 文件范围：`packages/logger/**`、`apps/api/src/lib/logger.ts`（仅 import 切换）、`package.json`、（按需）`apps/server/next.config.js`、`apps/web-next/next.config.js`
   - 验收要点：
     - arch-reviewer 子代理评审 PASS 且结论入卡的"子代理调用"字段（含模型 ID `claude-opus-4-6`）
     - `@resovo/logger` 在 api / web-next / admin 三处都能 typecheck 通过
     - redact 在单测中覆盖 11 个 PII 字段
     - `formatPretty` 在 dev 模式输出可读单行（按 level 着色）；ndjson 始终原样
     - `npm run typecheck` / `npm run lint` / `npm run test:run` 全绿

4. INFRA-10（LOG-04）— 浏览器 client logger + `/v1/internal/client-log` 端点（状态：✅ 已完成）
   - 创建时间：2026-04-25 17:16
   - 计划开始：INFRA-09 完成后
   - 实际开始：2026-04-26
   - 完成时间：2026-04-26
   - 前置依赖：INFRA-09 ✅ + INFRA-15 ✅
   - 建议模型：sonnet
   - 子代理调用：否
   - 估时：1.0d
   - 改动点：
     - `apps/web-next/src/lib/logger.client.ts`：同步 API（info / warn / error）；buffer（max=50, flush=2s, error 立即 flush）；`navigator.sendBeacon` 优先，回退 `fetch keepalive`；失败重试 1 次后丢弃；全局 hook（`window.onerror` / `unhandledrejection`，dev 自动 hook `console.error`，prod 不 hook）
     - `apps/web-next/src/lib/logger.server.ts`：`createLogger({ service: 'web-next' })`（实现，仅 import @resovo/logger 类型与序列化器）
     - `apps/api/src/routes/internal/client-log.ts`：`POST /v1/internal/client-log`；export 命名固定 **`internalClientLogRoutes`**（对齐现有 `internalImageBrokenRoutes`，复核 #6）；zod 校验（≤ 100 条 / ≤ 64KB body）；origin 白名单（dev: 全 localhost；prod: NEXT_PUBLIC_APP_URL）；IP 限流（60 rpm，内存桶）；**prod 仅登录用户，dev 全开**（决策 2 + LOG-04 前置约束）；落地方式：用 `logger.child({ service: 'client' })` 写 stdout，由 dev.mjs 分流到 `logs/client/`（复核第 5 点）
     - `apps/api/src/server.ts`：`import { internalClientLogRoutes } from '@/api/routes/internal/client-log'` + `await fastify.register(internalClientLogRoutes, { prefix: '/v1' })`
     - **替换 4 处 web-next 实际 console 调用**（重新核对：提案"6 处"中 `SafeImageNext.tsx:18,79` 是注释，实际是 4 处）：`BrandProvider.tsx:131,140`、`playerStore.ts:143`、`SafeImageNext.tsx:83` → 改为 `logger.client.warn/error`
   - 文件范围：`apps/web-next/src/lib/{logger.client.ts,logger.server.ts}`、`apps/api/src/routes/internal/client-log.ts`、`apps/api/src/server.ts`、`apps/web-next/src/contexts/BrandProvider.tsx`、`apps/web-next/src/stores/playerStore.ts`、`apps/web-next/src/components/media/SafeImageNext.tsx`
   - 验收要点：
     - 浏览器抛 `console.error` → 端点收到 → stdout 有 `service:'client'` → `logs/client/<date>.ndjson` 有记录
     - 单批 > 100 条或 > 64KB 被 zod 拒绝（400）
     - 同一 IP 1 分钟内第 61 次请求被 429
     - prod 模式下未登录请求被 401（dev 模式下匿名仍可）
     - origin 不在白名单内被 403
     - 4 处 web-next console.* 已迁移
     - **守门 C（浏览器三输入）**：在真实 Chrome devtools 制造 ① `throw new Error()` 同步异常 ② `Promise.reject()` 不 catch ③ 显式 `console.error()`，三种输入都能被 client logger 捕获并送达端点；同时跨用 5 分支验证（200 正常 / 400 超量 / 401 prod 未登录 / 403 origin 非法 / 429 限流）
     - `npm run typecheck` / `npm run lint` / `npm run test:run` 全绿
   - 完成备注要求：changelog 附 client-log endpoint **5 分支** 各 1 条响应样例（200/400/401/403/429）以及 1 条 client 来源的 ndjson 日志样例（守门 D）

5. INFRA-11（LOG-05）— 新增 `docs/rules/logging-rules.md` + 更新 CLAUDE.md 索引（状态：✅ 已完成）
   - 创建时间：2026-04-25 17:16
   - 计划开始：INFRA-10 完成后
   - 实际开始：2026-04-26 01:05
   - 完成时间：2026-04-26 01:20
   - 前置依赖：INFRA-10 ✅ + INFRA-16 ✅
   - 建议模型：haiku（机械文档归档，CLAUDE.md "强制降 Haiku 第 5 条 / 第 1 条"）
   - 子代理调用：可选 `Task(subagent_type: 'doc-janitor', model: 'claude-haiku-4-5-20251001')` 执行
   - 估时：0.3d
   - 改动点：
     - `docs/rules/logging-rules.md`（新建）：
       1. **最终决策表（复核 #8）**：列号统一为 1..8（用户最终拍板编号），表内括注每条对应的提案原文位置（"原第 9 节决策 X" / "复核新增决策 X"）；规则文档**只引用最终编号 1..8**，不再写"决策 6 自实现 pretty"等中间编号
       2. level 选择指南（trace / debug / info / warn / error / fatal 各自语义）
       3. PII 红线硬规则（11 字段 + **"禁止原始 req/res/headers 直接入 ctx"**）
       4. `logger.child` 派生范式（request / worker / job 三种场景）
       5. ServiceName 命名规范（含 `worker:<name>` / `client` / `script` 子格式）
       6. **changelog 日志格式样例守门规则（守门 D 通用化）**：所有涉及日志格式或新 logger 接入的任务，changelog 条目必须附**至少 3 行真实日志格式样例**（脱敏后），覆盖 info / warn / error 各一条
       7. 验收清单（来自 INFRA-07..10 验收要点合并）
     - `CLAUDE.md`：第"规范文件索引"表新增一行 `日志相关任务 | docs/rules/logging-rules.md | logger / log / 日志 / pino / request_id / worker job`
   - 文件范围：`docs/rules/logging-rules.md`（新建）、`CLAUDE.md`（仅索引表追加一行）
   - 验收要点：
     - logging-rules.md 包含最终决策 1..8 完整表
     - CLAUDE.md 索引表语义清晰且不破坏其他行
...
6. INFRA-12（LOG-06）— 实现下沉到 `packages/logger`（隐式后续，无 deadline）（状态：⬜ 未开始）
   - 创建时间：2026-04-25 17:16
   - 计划开始：触发条件达成后由人工排程
   - **触发条件**：INFRA-08 ✅ + INFRA-10 ✅，且监测期 ≥ 14 天无回归
   - 前置依赖：INFRA-08 ✅ + INFRA-10 ✅ + 监测期满
   - 建议模型：sonnet
   - 子代理调用：可能需要 arch-reviewer（合并签名变更若涉及公共 API 调整）
   - 估时：0.5d
   - 改动点：
     - 把 `apps/api/src/lib/logger.ts` 与 `apps/web-next/src/lib/logger.client.ts` 中的 `createLogger / withRequest / withJob` 实现迁入 `packages/logger/src/`
     - 各 app 的 `lib/logger*.ts` 改为 thin re-export
   - 文件范围：`packages/logger/src/**`、`apps/api/src/lib/logger.ts`、`apps/web-next/src/lib/logger.{client,server}.ts`
   - 验收要点：
     - 无消费方代码改动（thin re-export 保持签名）
     - `npm run typecheck` / `npm run lint` / `npm run test:run` 全绿
     - 日志输出格式无差别（同样 ts / level / service / ctx 字段，ndjson 与 pretty 双流均无字段缺失）

7. INFRA-13 — INFRA-07 复审修补：dev.mjs JSON 字段补全 + shutdown drain（状态：✅ 已完成）
   - 创建时间：2026-04-25 18:55
   - 计划开始：2026-04-25 18:55
   - 实际开始：2026-04-25 18:55
   - 完成时间：2026-04-25 18:58
   - 前置依赖：INFRA-07 ✅
   - 建议模型：opus（已在主循环 claude-opus-4-7）
   - 子代理调用：无
   - 估时：0.3d
   - 触发原因：Codex 复审（proposal 文件第 495–528 行）发现 INFRA-07 实现层 2 个 P1 + 1 个 P2 问题；本卡为修补卡，不重新执行 INFRA-07
   - 改动点：
     - **Finding 1 修复（P1）**：`scripts/dev.mjs:171-183` JSON 分支增加字段 enrich——`{ ts, level, service: label, stream, ...parsed }`，pino numeric level 经 `pinoLevelStr` 转字符串、time 转 ISO ts；原行已含 `service:'client'` 等字段时由 `...parsed` 后置覆盖默认值保留原意
     - **Finding 2 修复（P1）**：`scripts/dev.mjs:298-329` shutdown 路径重构——挂 `child.once('close')` 监听器统计完成数，所有 child close 后才 `closeAllStreams() + process.exit`；3s SIGKILL 兜底 + 5s 强退 finalExit 兜底
     - **Finding 3 修复（P2）**：`docs/changelog.md` INFRA-07 条目文案——errors 阈值 numeric ≥ 50 → ≥ 40，字符串 error/fatal → warn/error/fatal；ndjson 代码块去 ndjson 标识改普通 ``` 块（保留 // 注释，不强求合法 NDJSON）
     - 顺手清理：删 `scripts/dev.mjs:4` unused import `createReadStream`（Codex 残余风险清理项）
   - 文件范围：`scripts/dev.mjs`、`docs/changelog.md`
   - 验收要点：
     - `node --check scripts/dev.mjs` 通过
     - 启 dev 30s + 80×curl 后，`logs/dev/api.ndjson` 每行均含 `ts`、`level`、`service`、`stream` 四字段（含 pino 透传行）
     - SIGINT 后子进程 close 事件触发 closeAllStreams，最后 3 行均完整 JSON
     - rotate 行为不变（沿用 INFRA-07 同样 LOG_MAX_BYTES=2048 + 80×curl 端到端验证）
     - changelog 文案与 dev.mjs `isWarnOrAbove` 一致（≥ 40 / warn|error|fatal）
     - `npm run typecheck` / `npm run lint` 全绿
   - 完成备注要求：changelog 条目附 enrich 后真实样例 ≥ 3 行（pino 透传行 + wrap 行 + client 流前向兼容行各代表）

8. INFRA-14 — INFRA-08 复审修补：脱敏表 / requestIdLogLabel / maintenance+backfill withJob / 守门 B+D 真实化（状态：✅ 已完成）
   - 创建时间：2026-04-25 19:50
   - 计划开始：2026-04-25 19:50
   - 实际开始：2026-04-25 19:50
   - 完成时间：2026-04-25 23:05
   - 前置依赖：INFRA-08 ✅
   - 建议模型：opus（保留主循环 claude-opus-4-7）
   - 子代理调用：无
   - 估时：1.0d
   - 草案 commit：`949aff9`（详见 `docs/logging_system_proposal_20260425.md:599+`）
   - 触发：自审 P1-3 + Codex 4 finding（proposal:561-596）整合共 6 P1 + 4 P2，纳入 F1-F8 修补，延期 F9（services 25 处 stderr）/ F10（11 PII 字段单测随 INFRA-09 抽包）
   - 改动点：
     ...
     - `apps/api/src/workers/maintenanceWorker.ts`：`processMaintenanceJob(data, jobLog)` 增 jobLog 参数；register 内 `withJob(workerLog, job)` 传入；4 处 `workerLog.info('job done')` → `jobLog.info('job done')`（F6）
     - `apps/api/src/workers/imageBackfillWorker.ts`：`process('backfill', 1, async (job) => runImageBackfill(withJob(workerLog, job)))`；`enqueueBackfillJob` 用 `bullJob.id` 写 enqueue 日志含 job_id（F7）
     - `docs/changelog.md`：① INFRA-08 段真实样例替换编造样例 + "需人工验证" → 实测 PASS 记录 + P1-3 重犯备注；② 追加 INFRA-14 完成条目
   - 文件范围：`apps/api/src/{lib/logger.ts,server.ts,workers/maintenanceWorker.ts,workers/imageBackfillWorker.ts}`、`docs/changelog.md`
   - 验收要点：
     - logger.ts REDACT_PATHS 含 set-cookie + url.query（含 *. 嵌套）
     - server.ts 配 requestIdLogLabel:'request_id'
     - request.log.\* 顶层字段 = `request_id`（grep ndjson `"reqId":` 计数 = 0）
     - maintenanceWorker 4 处 job done 日志含 job_id（实测触发 maintenance）
     - imageBackfillWorker process + enqueue 日志含 job_id（实测触发 backfill）
     - PII 实测：Set-Cookie / Authorization / `?password=SECRET` 任一注入，ndjson 0 次出现 secret 原文
     - `npm run typecheck` / `npm run lint` / `npm run test:run` 全绿
     - changelog 附 ≥ 5 行真实样例（5 路径 access + redact 对比 + maintenance/backfill withJob）
     - **流程纠正（F3）**：本卡先写 tasks.md 工作台卡片再开工 ✅，自纠 INFRA-07/08 连续 2 次 P1-3 违规
   - 完成备注要求：changelog INFRA-14 条目必附 ① F4 redact 修复后 cookie + url.query redact 对比 ② F5 request_id rename 后 access log ③ F6 maintenance job done 含 job_id ④ F7 backfill enqueue + done ⑤ P1-3 流程瑕疵第二次记录 + INFRA-09 起强制门禁

9. INFRA-15 — INFRA-09 复审修补：package-lock 同步 + pino 行为级 redact 集成测试（状态：✅ 已完成）
   - 创建时间：2026-04-25 23:42
   - 计划开始：草案评审通过后
   - 实际开始：2026-04-25 23:45
   - 完成时间：2026-04-25 23:52
   - 前置依赖：INFRA-09 ✅（CONDITIONAL PASS）
   - 建议模型：sonnet（机械性 lockfile 修补 + 单测增量，无新架构决策）
   - 子代理调用：无
   - 估时：0.3d
   - 草案 commit：（待提交，详见 `docs/logging_system_proposal_20260425.md` 文件尾部追加块）
   - 触发：Codex INFRA-09 审核 2 finding（proposal:720-753）+ 主循环独立审核（proposal 同段，确认 F1 是 P1 阻断——`node_modules/@resovo/logger` symlink 缺失 + lockfile 无 packages/logger 条目；当前所有 PASS 都靠 tsconfig paths 旁路 npm 解析）
   - 改动点：
     - `package-lock.json`：用项目 `engines.node >=22` + `packageManager: npm@10.8.2` 跑一次 `npm install`，让 npm 注册 `packages/logger` workspace + 创建 `node_modules/@resovo/logger` symlink + 同步 lockfile（F1）
     - `tests/unit/lib/logger.test.ts`：新增 1 个 describe 块「pino integration redact behavior」，构造含 11 PII 字段顶层 + `*.field` 嵌套 + `headers.set-cookie` + `req.url.query` + `url.query` 的对象，用 `pino({ redact: { paths: REDACT_PATHS, censor: '<redacted>' } }, writableStream)` 写入内存 stream，断言 secret 原文 0 出现且对应字段 === `'<redacted>'`（F2）
     - `docs/changelog.md`：① INFRA-09 段追加 P1-Finding-1 "完成态不一致" 备注 + 链接至 INFRA-15；② F10 关闭描述细化为"路径表层 + pino 行为层"双层覆盖；③ 追加 INFRA-15 完成条目
     - `docs/tasks.md`：开工时写工作台卡片，完成后清空（延续 INFRA-14 范本）
   - 文件范围：`package-lock.json`、`tests/unit/lib/logger.test.ts`、`docs/changelog.md`、`docs/tasks.md`、`docs/task-queue.md`
   - 验收要点：
     - `package-lock.json` 含 `packages/logger` workspace 与 `node_modules/@resovo/logger` 节点
     - `node_modules/@resovo/logger` 物理 symlink 存在指向 `../../packages/logger`
     - `npm ls @resovo/logger` 解析无错
     - `npm run lint` 无 turbo "Workspace 'packages/logger' not found in lockfile" 警告
     - 新增 pino 集成行为测试覆盖 11 字段 + `headers.set-cookie` + `req.url.query` 容器路径，注入 secret 字符串后 pino 输出 0 命中且字段值 === `'<redacted>'`
     - `npm run typecheck` / `npm run lint` / `npm run test:run` 全绿（test 数 ≥ 1762）
     - `git diff package-lock.json` 仅含本卡相关变更，无无关 lockfile drift
     - **流程纠正**：本卡先写 tasks.md 工作台卡片再开工，延续 INFRA-14 范本
   - 完成备注要求：changelog INFRA-15 条目必附 ① F1 修复证据（lockfile 修复前后 grep 计数对比 + `ls -la node_modules/@resovo/logger` 输出）② F2 修复证据（新增测试块代码片段 + 测试计数 26 → 26+N 对比）③ INFRA-09 段回写指针 ④ F10 双层覆盖说明

10. INFRA-16 — INFRA-10 复审修补：浏览器→API 链路打通 + CORS 403 契约 + source_ip PII（状态：✅ 已完成）
    - 创建时间：2026-04-26 00:36
    - 计划开始：草案评审通过后
    - 实际开始：2026-04-26 00:40
    - 完成时间：2026-04-26 01:00
    - 前置依赖：INFRA-10 ✅（CONDITIONAL PASS）
    - 建议模型：sonnet（机械性接线修复 + 单测增量，无新架构决策）
    - 子代理调用：无
    - 估时：0.4d
    - 草案 commit：（待提交，详见 `docs/logging_system_proposal_20260425.md` 文件尾部追加块）
    - 触发：Codex INFRA-10 审核 3 finding（proposal:917-955）+ 主循环独立确认 F1 是 P1 真实 bug（curl 直连 :4000 误判通过，浏览器→API 真实链路 0 工作）
    - 改动点：
      - `apps/web-next/src/lib/logger.client.ts`：① 新增 `const BASE_URL = process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:4000/v1'` ② `ENDPOINT = \`${BASE_URL}/internal/client-log\`` 拼绝对 URL（对齐现有 `api-client.ts:3` / `report-broken-image.ts:14` 模式）③ fetch fallback `credentials:'same-origin'` → `'include'`（跨 origin cookie 必需）（F1）
      - `apps/api/src/server.ts`：在 cors register 之后追加全局 `fastify.setErrorHandler`，把 CORS error 映射为 HTTP 403 + `code:'FORBIDDEN_ORIGIN'`；其他错误流委托默认（F2）
      - `apps/api/src/routes/internal/client-log.ts:107-118`：删除 `source_ip: request.ip` 参数（首选）；保留 `request_id` 作请求关联（F3）
      - `tests/unit/web-next/logger-client.test.ts`：新增测试断言默认 ENDPOINT 形如 `http://localhost:4000/v1/internal/client-log` + 覆盖 NEXT_PUBLIC_API_URL 环境变量场景
      - `docs/changelog.md`：① INFRA-10 段追加 CONDITIONAL PASS 备注（F1 真实 bug 显式记录 + 链接 INFRA-16）② 追加 INFRA-16 完成条目
    - 文件范围：`apps/web-next/src/lib/logger.client.ts`、`apps/api/src/server.ts`、`apps/api/src/routes/internal/client-log.ts`、`tests/unit/web-next/logger-client.test.ts`、`docs/changelog.md`、`docs/tasks.md`、`docs/task-queue.md`
    - 验收要点：
      - logger.client.ts 用 NEXT_PUBLIC_API_URL BASE 拼绝对 URL；fetch fallback `credentials:'include'`
      - **真实浏览器实测**（curl 不能替代）：dev 启动 → 打开 http://localhost:3000 → devtools 跑 3 输入 → Network 面板看 POST 目标是 :4000 → logs/client/ 出现 3 条带 source 区分的记录
      - server.ts setErrorHandler 把 CORS error 映射为 403
      - curl `-H "Origin: http://evil.com"` 返回 HTTP 403 + `code:'FORBIDDEN_ORIGIN'`（5 分支契约真实达成）
      - client-log.ts 删除 `source_ip`；grep `"source_ip"` / `"127.0.0.1"` 在 client.ndjson == 0
      - 单测新增 BASE_URL 默认值断言
      - 无回归：原 5xx/4xx 路径继续按现有 errorResponse 格式响应（curl POST /v1/auth/login 错密码 422 / GET /nonexistent 404 / 其他 5xx 仍正常）
      - `npm run typecheck` / `npm run lint` / `npm run test:run` 全绿
      - **流程纠正**：本卡先写 tasks.md 工作台卡片再开工，延续 INFRA-14/15/10 范本
    - 完成备注要求：changelog INFRA-16 条目必附 ① F1 浏览器实测证据（Network 面板说明 + 3 输入 ndjson 真实样例带 source 区分）② F2 curl evil.com origin → 403 真实响应 ③ F3 grep 验证 source_ip / IP 字面量 = 0 ④ INFRA-10 段回写指针 ⑤ 5 分支真实达成度

11. INFRA-17 — INFRA-11 复审修补：worker service 命名 / 浏览器验收口径 / docs-format 验收措辞（状态：✅ 已完成）
    - 创建时间：2026-04-26 01:51
    - 计划开始：草案评审通过后
    - 实际开始：2026-04-26 01:55
    - 完成时间：2026-04-26 02:05
    - 前置依赖：INFRA-11 ✅（CONDITIONAL PASS）
    - 建议模型：sonnet（机械文档措辞修正，无新决策）
    - 子代理调用：无
    - 估时：0.2d
    - 草案 commit：（待提交，详见 `docs/logging_system_proposal_20260425.md` 文件尾部追加块）
    - 触发：Codex INFRA-11 审核 3 finding（proposal:1126-1156）+ 主循环独立确认 3 P2 全成立（无 P1）
    - 改动点：
      - `docs/rules/logging-rules.md` § 5（ServiceName 命名规范）：① 不再把 `worker:crawler-worker` 等列为 ServiceName 实例 ② 5.1 子节明确"当前 API 内 Bull workers 用 `service:'api'` + `worker:'<kebab-case-name>'`"，ndjson 顶层 service 永远是 'api' ③ 5.2 子节明确 `worker:${string}` 类型保留给未来独立 worker 进程，当前未实例化 ④ 9 个 worker 表格字段名改为 worker（非 service）（F1）
      - `docs/rules/logging-rules.md` § 7（浏览器→API 链路）+ § 8.4（验收清单）：① 删除"二选一等价"措辞 ② 增分级表（sendBeacon/cookie/page lifecycle 必须真实浏览器；CORS/端点 origin/业务逻辑 curl+Origin 充分）③ 明列 curl 4 项局限（preflight / cookie SameSite-Secure / page lifecycle / Network 端口）（F2）
      - `docs/task-queue.md` INFRA-11 验收项措辞：「`npm run verify:docs-format` 通过」改为「新增 logging-rules.md 通过元信息检查；全局 verify:docs-format 仍因 12 项 pre-existing 历史问题失败，另立独立维护卡」（F3）
      - `docs/changelog.md`：① INFRA-11 段追加 CONDITIONAL PASS 备注 + 链接 INFRA-17 ② 追加 INFRA-17 完成条目
    - 文件范围：`docs/rules/logging-rules.md`、`docs/task-queue.md`（INFRA-11 验收项 + INFRA-17 状态）、`docs/changelog.md`、`docs/tasks.md`（本卡）
    - 验收要点：
      - § 5 ServiceName 不再误导：worker:${string} 改为"类型保留"；9 worker 表字段名改为 worker
      - § 7 浏览器验收口径：删除"等价二选一"，明确分级 + curl 4 项局限
      - § 8.4 验收清单同步分级要求
      - task-queue INFRA-11 验收项与 changelog 历史遗留说明措辞一致
      - 与 apps/api/src/workers/maintenanceWorker.ts:18 等代码实际惯例一致（grep 验证）
      - `npm run typecheck` / `npm run lint` / `npm run test:run` 全绿（本卡纯文档无代码改动）
      - `npm run verify:docs-format`：logging-rules.md 仍通过元信息检查，不引入新错误
      - **流程纠正**：本卡先写 tasks.md 工作台卡片再开工，延续 INFRA-14/15/16 范本第 6 次应用
    - 完成备注要求：changelog INFRA-17 条目必附 ① F1 § 5 修订前后 diff（worker:${string} 从"实例"改为"类型保留"）② F2 分级表完整（4 改动类型 × 验收要求矩阵 + curl 4 项局限）③ F3 task-queue 措辞 diff ④ INFRA-11 段回写指针 ⑤ ndjson 真实样例 1 行（确认 service:'api' + worker:'<name>' 现状）

12. INFRA-18 — INFRA-17 残留修补：§ 7 编号去重 / 鉴权行 cookie 限定 / test:run flaky caveat（状态：✅ 已完成）
    - 创建时间：2026-04-26 02:15
    - 计划开始：草案评审通过后
    - 实际开始：2026-04-26 02:18
    - 完成时间：2026-04-26 02:25
    - 前置依赖：INFRA-17 ✅（CONDITIONAL PASS）
    - 建议模型：sonnet（机械文档去重 + 措辞收紧 + changelog caveat，无新决策）
    - 子代理调用：无
    - 估时：0.1d
    - 草案 commit：（待提交，详见 `docs/logging_system_proposal_20260425.md` 文件尾部追加块）
    - 触发：Codex INFRA-17 残留复审 3 finding（proposal:1383-1418）+ 主循环独立确认 1 P2 + 2 P3 全成立
    - 改动点：
      - `docs/rules/logging-rules.md:311-319`：删除重复的旧 § 7.1（跨 origin 配置基线）+ § 7.2（CORS 拒绝映射为 403）共 9 行——INFRA-17 修订时新增 7.0/7.1/7.2/7.3 但忘了删除原 7.1/7.2，导致编号歧义 + 内容重复（F1）
      - `docs/rules/logging-rules.md:281-286`（§ 7.0 分级表第 4 行）：把笼统的"业务逻辑（端点内部 zod / 限流 / 鉴权）"拆为 3 行——「zod / 限流」+「header/token 鉴权（Authorization Bearer）」+「cookie/session 鉴权（SameSite/Secure/credentials:'include'）」；最后一行明确"必须真实浏览器"，header/token 仍 curl+Origin 充分（F2）
      - `docs/changelog.md` INFRA-17 段守门栏：追加 flaky caveat——主循环本机 Node v20.19.6 跑 1760 全绿；Codex bundled Node 复跑 1 个 VideoImageSection 既有 flaky，单跑 PASS，与本卡无关（F3）
      - `docs/changelog.md`：追加 INFRA-18 完成条目
    - 文件范围：`docs/rules/logging-rules.md`、`docs/changelog.md`、`docs/task-queue.md`（INFRA-18 状态）、`docs/tasks.md`（本卡）
    - 验收要点：
      - F1: `grep -c "^### 7\." docs/rules/logging-rules.md` 修复前 6 / 修复后 4
      - F1: `grep -c "### 7\.1" docs/rules/logging-rules.md` == 1（不再是 2）
      - F1: § 7 结构连续 7.0 → 7.1 → 7.2 → 7.3
      - F2: 分级表第 4 行拆为 header/token vs cookie/session 鉴权两行；cookie/session 明确"必须真实浏览器"
      - F3: changelog INFRA-17 守门段含 flaky caveat（VideoImageSection 与本卡无关）
      - **防回归**：commit 前 `grep -c "^### N\." docs/rules/logging-rules.md` 验证 § 5 / § 7 / § 8 编号唯一（避免 F1 类疏忽再发）
      - `npm run typecheck` / `npm run lint` / `npm run test:run` 全绿（本卡纯文档无代码改动）
      - `npm run verify:docs-format`：logging-rules.md 仍通过元信息检查；不引入新错误（12 项 pre-existing 历史问题维持）
      - **流程纠正**：本卡先写 tasks.md 工作台卡片再开工，延续 INFRA-14/15/16/17 范本第 7 次应用
    - 完成备注要求：changelog INFRA-18 条目必附 ① F1 grep 计数对比（修复前 6 / 修复后 4）② F2 分级表完整 3 行（业务逻辑 / header-token / cookie-session）+ 拆分理由 ③ F3 flaky caveat 完整内容 ④ INFRA-17 段回写指针 ⑤ 防回归自检：commit 前 grep 编号唯一的输出



---

## [SEQ-20260426-01] 后台治理调研（执行序列）

> 创建时间：2026-04-26 04:55
> 最后更新时间：2026-04-26 05:25
> 触发：用户列出后台 9 大痛点（采集合并/线路展示/审核台验证不一致/标签兼容/前台 404/筛选重置/首页运营位/视频源管理/表格统一），需先输出现状报告再拆修复卡
> 范围：apps/server (Next.js admin)、apps/api/src/routes/admin、DB schema、apps/web/ 旧目录残留
> 产出：docs/admin_audit_20260426.md（只汇总现状 + 痛点定位 + P0/P1/P2 优先级，不含修复方案）
> 状态：✅ 已完成（ADMIN-AUDIT-01 ✅ / ADMIN-DESIGN-BRIEF-01 ✅ / ADMIN-DESIGN-BRIEF-02 ✅，3 张卡 2026-04-26 闭环；brief 升 v2 状态 approved-for-design 可发给 Claude Design）

1. ADMIN-AUDIT-01 — 后台现状全面梳理与 9 大痛点定位（状态：✅ 已完成）
   - 创建时间：2026-04-26 04:55
   - 实际开始：2026-04-26 04:55
   - 完成时间：2026-04-26 05:25
   - 完成备注：
     - 执行模型：claude-sonnet-4-6
     - 子代理：5 × Explore (claude-sonnet-4-6) 分别覆盖痛点 1+5 / 2+8 / 3+6 / 4+7 / 9
     - 产出 `docs/admin_audit_20260426.md` 共 340 行，含 8 章 + 附录
     - §2 覆盖 14 个 admin 模块（videos/staging/moderation/submissions/sources/subtitles/banners/home-modules/users/image-health/crawler/system×5/design-tokens）
     - §4 列出全部 16 个 admin 路由文件，共 122 端点
     - §7 9 痛点分布：P0 × 3（痛点 1/3/6）+ P1 × 6（痛点 2/4/5/7/8/9）+ P2 × 0
     - §8 额外发现 8 条隐性问题（H-1 至 H-8）
     - 附录 A 提出 7 个 CHG 拆分建议（CHG-A 至 CHG-G），按用户要求**未创建任何修复任务**
     - 共享层沉淀评估：本卡无代码改动，无沉淀需求
     - 偏离原因：无（建议模型 sonnet = 执行模型 sonnet）
   - 建议模型：sonnet（调研汇总，非架构决策；并行 spawn Explore 子代理）
   - 估时：0.6–1d
   - 触发：用户提出 9 大后台痛点，需先全景报告再拆修复任务
   - 改动点：
     - 新建 docs/admin_audit_20260426.md（§1 总览 / §2 模块清单 / §3 链路与数据流 / §4 接口面 / §5 UI 与表格 / §6 数据管理 / §7 9 痛点对照 + P0/P1/P2 / §8 风险与盲区）
   - 文件范围：`docs/admin_audit_20260426.md`、`docs/changelog.md`、`docs/tasks.md`、`docs/task-queue.md`
   - 验收要点：
     - 报告含 8 个章节且每节非空
     - §2 模块清单覆盖：videos / staging / moderation / sources / crawler / banners / home-modules / users / system-settings / image-health（≥10 模块），每模块给出 (路由 / API / DB 表 / 主组件) 四元组
     - §4 列出 apps/api/src/routes/admin 全部 16 个路由文件的 method+path+鉴权摘要
     - §7 9 痛点逐条定位到 file:line 或表名/列名，附 P0/P1/P2 标记与一句话现状判断（缺失 / bug / 设计漂移）
     - 旧 apps/web/ 残留情况说明（是否有源码、是否仅构建产物）
     - 不得在报告内给出修复方案（只描述现状），修复方案留待后续 CHG 卡
     - 不得修改 apps/server/、apps/api/、apps/web-next/ 任何代码（只读调研）
   - 完成备注要求：changelog ADMIN-AUDIT-01 条目必附 ① 报告文件行数 ② §2 实际覆盖模块清单 ③ §7 9 痛点 P0/P1/P2 分布统计 ④ 子代理调用清单（5 个 Explore，逐个列 model + 主题）

2. ADMIN-DESIGN-BRIEF-01 — Claude Design 后台改进设计 brief（轻量版初稿）（状态：✅ 已完成）
   - 创建时间：2026-04-26 05:35
   - 实际开始：2026-04-26 05:35
   - 完成时间：2026-04-26 05:55
   - 完成备注：
     - 执行模型：claude-opus-4-7（偏离说明：建议 sonnet 但本卡延续上一卡同一会话产出，opus 主循环不切换；纯文档撰写无架构决策风险）
     - 子代理：无（主循环直接产出，所有领域知识已在 ADMIN-AUDIT-01 上下文中）
     - 产出 `docs/admin_design_brief_20260426.md` 共 268 行（≤350 轻量版门限达标），含 7 节 + 附录，27 个二三级标题
     - §3 引用共享组件 6 个含 file:line（ModernDataTable / TableSettingsPanel / AdminDropdown / SelectionActionBar / PaginationV2 / shared 子目录）
     - §5 痛点归属分类：design 主导 7 项（痛点 2/3/5/6/7/8/9）+ design 辅助 4 项（痛点 1/4 + H-5/H-7）+ 非 design 范围 4 项（H-1/H-2/H-3/H-6）
     - §6 5 项推荐：①审核台重构（双信号+状态保留）②视频源/线路管理 ③首页运营位统一编辑器 ④共享表格 v2 + useTableQuery ⑤采集合并/拆分 UX
     - §7 6 类交付物（Figma 6-page 结构 / token diff / Props 变更 / a11y checklist / ux-copy 规范 / handoff md）+ 6 阶段协作流程
     - 共享层沉淀评估：本卡为 brief 文档，无代码沉淀；后续若 design 实施引发共享组件 Props 变更需另立 ADR（§4.3 已写明）
     - 与 ADMIN-AUDIT-01 关系：互相引用、不重复内容；audit 是现状真源，brief 是 design 启动包
   - 建议模型：sonnet（基于 ADMIN-AUDIT-01 现状报告衍生的设计 brief，无新架构决策）
   - 估时：0.3d
   - 触发：用户要求轻量版初稿，先让 Claude Design 起步，全量版（含 task flows + 页面 anatomy）后续再扩
   - 改动点：
     - 新建 docs/admin_design_brief_20260426.md（轻量版 7 节：§1 角色 / §2 业务概念词典 / §3 现有设计资产 / §4 设计约束与边界 / §5 9 痛点 + 8 隐性问题 design 归属 / §6 优先改进矩阵（5 项 ROI 推荐）/ §7 交付物期望）
     - 与 docs/admin_audit_20260426.md 互相引用（brief 不重复抄现状，只引用 §章节号）
   - 文件范围：`docs/admin_design_brief_20260426.md`、`docs/changelog.md`、`docs/tasks.md`、`docs/task-queue.md`
   - 验收要点：
     - brief 全文 ≤350 行（轻量版定义）
     - 7 节齐全，每节非空
     - §3 设计资产引用至少 5 个共享组件实际路径（file:line）
     - §5 痛点归属表逐条标注 design 主导 / design 辅助 / 非 design 范围，覆盖 9 痛点 + ≥4 条隐性问题
     - §6 给出 5 个推荐项，每项含：现状定位 + design 主导内容 + 成功度量
     - §7 给出可执行的交付物清单（Figma 结构 / token diff / Props 变更 / a11y checklist / handoff 格式）
     - 不得包含 ANY 修复方案的具体 UI 设计（只是 brief，不是 spec）
     - 不得修改 apps/* 任何代码（纯文档）
   - 完成备注要求：changelog 必附 ① brief 文件行数 ② §5 痛点归属分类统计（design 主导 N / design 辅助 N / 非 design N）③ §6 5 项推荐清单（仅标题）④ 子代理调用清单（预期"无"，主循环直接产出）

3. ADMIN-DESIGN-BRIEF-02 — Brief R1 审核 4 MUST + 3 高 ROI SHOULD 修补（状态：✅ 已完成）
   - 创建时间：2026-04-26 06:05
   - 实际开始：2026-04-26 06:05
   - 完成时间：2026-04-26 06:30
   - 完成备注：
     - 执行模型：claude-opus-4-7（偏离说明：建议 sonnet 但延续 ADMIN-DESIGN-BRIEF-01 同一会话产出，opus 主循环不切换）
     - 子代理：无（机械性文档修订，主循环直接产出）
     - brief 文件行数：268 → 415（+147 行扩容，主要来自 §0.5 对齐矩阵 + §0.6 Non-Goals + KPI 基线表 + 5 推荐前置依赖字段 + 推荐 4 ADR 警告块）
     - **MUST-1 grep 验证**：主体（行 1-340）`grep -nE '\]\(docs/'` 0 命中（修复前 5 命中：companion + §5 + §7.3 必读 4 行）；R1 review section 中"docs/"是引用错误示例字符串，不修
     - **MUST-2 重算**：design 主导 7 + 辅助 4 + 非范围 4 + 合并 2 = 17 = 9 痛点 + 8 隐性 ✓
     - **MUST-3 §0.5 对齐声明**：含 ① 性质澄清（非新业务需求）② 5×3 对齐矩阵（推荐 1-5 × 三份方案）③ 强约束（不得新增 admin 专属 token / 不污染 base / 不进 M7 主轨）④ ADR-037 + ADR-039 关联
     - **MUST-4 KPI 基线**：§1 KPI 表全列含基线 / 目标 / TBD 标注（已可观测 4 项：筛选保留率 0%、采纳率 58%、修复时延"无救济"、运营手工 SQL）；§6 5 项推荐成功度量同步带基线
     - **SHOULD-3 前置依赖矩阵**：推荐 1 依赖推荐 4 useTableQuery + 后端双信号 / 推荐 2 可独立 / 推荐 3 依赖后端 home_modules CRUD API / 推荐 4 ADR 评审强制 / 推荐 5 强依赖后端 candidate API（最后启动）
     - **SHOULD-4 推荐 4 ADR 候选**：标题加「(ADR 候选)」+ ⚠️ 评审强制警告块（必须 spawn Opus arch-reviewer，§7.2 阶段 4→5 之间硬前置）
     - **SHOULD-7 §0.6 Non-Goals**：8 条边界宣言（不重设计前台 / 不改 schema / 不增 i18n / 不做 mobile / 不引新依赖 / 不改 API 契约 / 不扩张权限模型 / 不重设计 design-tokens 浏览页）
     - **额外完成 SHOULD-8**（顺手做，零成本）：§7.3 必读 ADR 直名（ADR-037 + ADR-039 已显式列出，不再"模糊后续追加"）
     - **R1.5 验收推荐采纳分布**：4 MUST 全部采纳 / 8 SHOULD 采纳 4（3/4/7/8）+ 留 v2 backlog 4（1/2/5/6）/ 4 DISCUSS 全部留 v2 backlog
     - 头部 status: draft-v1 → approved-for-design；review_round 标 R1 conditional accept → R2 修补完毕
     - 共享层沉淀评估：本卡为 brief 修补，无代码沉淀
   - 建议模型：sonnet（机械性文档修订，无新决策；按 INFRA-14~18 范本"修补卡"模式）
   - 估时：0.3d
   - 触发：主循环 sonnet R1 审核 brief，verdict conditional accept；用户选 B 方案（修 4 MUST + SHOULD-3/4/7）
   - 改动点：
     - **MUST-1 内链相对路径**：brief 元数据 + §5 + §7.3 全部 `docs/xxx.md` 链接修正为相对 `docs/` 的真实路径（`./xxx.md` / `../CLAUDE.md` / `rules/xxx.md`），显示文本可保留 `docs/xxx` 直观形式
     - **MUST-2 §5.2 统计行重算**：旧错（主导 6/辅助 4/非范围 5/合并 2 = 17 不自洽）→ 新（主导 7/辅助 4/非范围 4/合并 2 = 17 ✓）
     - **MUST-3 三份方案对齐声明**：在 §0 元数据后新增 §0.5「与三份方案的对齐声明」——含性质澄清（本 brief 非新业务需求）+ 5×3 对齐矩阵 + 强约束（不得新增 admin 专属 token）+ ADR-037/039 关联
     - **MUST-4 KPI 基线**：§1 KPI 表补可观测基线值（采纳率 58% / 筛选保留率 0% / 错误合并修复时延"无救济"等已有数据），不可测的标 TBD（"上线后采集"）；§6 5 项推荐成功度量同步带基线
     - **SHOULD-3 前置依赖**：§6 5 项推荐每项加「前置依赖」字段（推荐 1 依赖推荐 4 的 useTableQuery / 推荐 5 依赖后端 candidate API / 推荐 3 依赖 home_modules CRUD API / 推荐 2/4 可独立）
     - **SHOULD-4 推荐 4 ADR 候选标注**：推荐 4 标题旁加 「(ADR 候选)」标记 + 一段说明（ModernDataTable Props 变更需 spawn Opus arch-reviewer，§7.2 阶段 4 之前必须完成评审）
     - **SHOULD-7 Non-Goals 段**：在 §1 前新增 §0.6「Non-Goals」≤8 条边界宣言（不重设计前台 / 不改 schema / 不增 i18n / 不做 mobile / 不引新依赖 / 不改 API 契约 / 不扩张权限模型 / 不重设计 design-tokens 浏览页）
     - brief 头部 `status: draft-v1` → `status: approved-for-design`，新增 `review_round: R1 (conditional accept) → R2 (4 MUST + 3 SHOULD 修补完毕)`
   - 文件范围：`docs/admin_design_brief_20260426.md`、`docs/changelog.md`、`docs/tasks.md`、`docs/task-queue.md`
   - 验收要点：
     - MUST-1: 全文 `grep -nE 'docs/[^)]+\.md\)'` 在 brief 内 0 命中（仅显示文本可含 `docs/`，link target 必须相对路径）
     - MUST-2: §5.2 统计行算式 7+4+4+2=17 与 9+8 一致
     - MUST-3: §0.5 含性质澄清 + 5×3 矩阵 + 强约束 + ADR 关联 4 部分
     - MUST-4: §1 KPI 表全列含「当前值/TBD」+ §6 5 项推荐成功度量含基线
     - SHOULD-3: §6 5 项推荐均含「前置依赖」字段
     - SHOULD-4: 推荐 4 标题含「(ADR 候选)」+ 评审步骤说明
     - SHOULD-7: 新增 §0.6 Non-Goals 段含 ≥6 条
     - brief 头部 status 升级为 approved-for-design
     - brief 全文不超过 400 行（修补后预计 350-400 行，仍属轻量版扩容）
     - 不得修改 apps/* 任何代码（纯文档）
     - **流程**：本卡先写 tasks.md 工作台卡片再开工，延续 INFRA-14~18 修补范本第 8 次应用
   - 完成备注要求：changelog 必附 ① 修补后 brief 文件行数（268 → ?） ② MUST-1 grep 验证输出（修复前后命中数对比） ③ MUST-2 重算公式验证 ④ MUST-3 §0.5 节摘要 ⑤ MUST-4 KPI 基线表 ⑥ SHOULD-3 前置依赖矩阵 ⑦ R1.5 验收推荐采纳的 7 条（4 MUST + 3 SHOULD）+ 未采纳的 5 条（5 SHOULD + 4 DISCUSS 全部留 v2 backlog）
