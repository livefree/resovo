...
1. **CHG-SN-9-DT-HEADER-REDESIGN-ADR** · 表格头重设计 ADR-149 起草 — 状态：✅ 已完成（2026-05-23）
   ...
   **ADR-149 AMENDMENT 1（2026-05-24）— D-149-13/14/15 + EP 序列重写 4→7 段**：
   ...
   - **CHG-SN-9-DT-HEADER-REDESIGN-EP-5-SOURCES-SORT-FULLSTACK** · 播放线路 (/admin/sources) 排序全栈 5 层修：types/api.ts/route zod/DB query/SourcesClient useEffect deps + 白名单字段（line_count / source_count / updated_at）+ 单测 — 状态：⬜ 待启动（~0.5-0.7w / 用户反馈"点击列名不切换排序"=全栈断链 / 共享 DataTable 已正确实装）
