...
- **状态**：⏸ 部分暂停（CHG-UX2-03d 根因未定位 → 暂停 -04/-05/-06；详见 video-table-cell-compression-debug.md）
- **创建时间**：2026-05-04
...
#### CHG-UX2-03d · 删 wrapper + cover 列宽贴合 + grid fixed track 不压缩（⏸ 暂停 — 根因未定位）
- **状态**：⏸ 暂停（2026-05-04，5 次修法均未生效）
- **调试记录**：`docs/designs/backend_design_v2.1/video-table-cell-compression-debug.md`
- **保留改动**：删 wrapper + cover 72px（CHG-UX2-03d 第一版有效部分）；后续 hotfix 修法（minmax / row max-content / cell minWidth）已 commit 但未生效
- **重启触发**：user 决定恢复调研时按 §7 P0/P1 调研路径继续
- **执行模型**：claude-opus-4-7
