...
- **状态**：🔄 执行中（CW2-A ✅ CW2-B-ADR ✅ CW2-B-EP ✅ CW2-C-ADR ✅；CW2-C-EP-A/-B ⬜）
...
5a. **CHG-SN-9-CW2-C-EP-A** — Fix-D5 后端契约 + 调度（ADR-154 落地后端）
   ...
   - 验收要点：
     - checkInterval 边界单测：60min 到期 → 触发；未到期 → 不触发；首次（null）→ 立即触发
     - R-154-1 单测：createRun 抛错时锚点不前进
     - deserialize 向后兼容（无 intervalMinutes 键 → 默认 60）
   - 估时：0.25w
   - 状态：⬜
...
5b. **CHG-SN-9-CW2-C-EP-B** — Fix-D5 前端 UI（SchedulerConfigDrawer）
   ...
   - 依赖：CW2-C-EP-A（类型已合入）
   - 文件范围：
     - `apps/server-next/src/app/admin/crawler/_client/SchedulerConfigDrawer.tsx`：scheduleType AdminSelect（每日定时/间隔触发）+ 条件渲染（daily → dailyTime / interval → intervalMinutes）
   - 验收要点：
     - scheduleType 切换 → 条件字段显示/隐藏单测
     - intervalMinutes 输入校验（min=5, max=1440）
   - 估时：0.15w
   - 状态：⬜
