...
- **META-54-A2-PROD（⬜ 待开始 · 🔒 部署后 gated，SEQ-20260616-04，建议 sonnet）— 生产库非电影季级重富集**：META-54-A2 仅在 dev（resovo_dev）跑（9→170 season exact）。**生产库待 META-54-D（commit `6b7b7a5c` 搜索词剥季号）部署上线后**，跑同款 `node --env-file=<prod env> --import tsx scripts/reenrich-backfill.ts --mode tmdb-season`，兑现生产存量非电影脱离待确认。**前置硬约束**：① META-54-D 必须已部署生产（否则搜索词仍带季号、93% no_candidate，白跑）；② 生产 Redis + enrichmentWorker 在线（reenrich 仅入队，worker 消费全量 enrich）；③ run-unique jobId（脚本已保证 `backfill-${runTs}-${id}`，防 Bull 残留漏跑）。**可选前置**：先 `cleanup-tmdb-season-refs.ts --dry-run` 复核生产 stale show-id-as-season 行数（dev=0，生产大概率亦 0）。**验收**：生产非电影 tmdb season exact catalog 数较跑前显著上升（dev 实测升级率 series/variety/documentary 85-87% / anime 58%）。**预期长尾**（非缺陷）：2026 未来季待 TMDB 上架 + 冷门国产动态漫画/网络番 TMDB 无数据。建议随 META-54-D 部署批次一并执行。
