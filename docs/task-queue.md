...
- **文件范围**：
  ...
  - `packages/design-tokens/src/brands/default.ts`（如需声明 gray.925 override）
  - 生成产物：`packages/design-tokens/dist/tokens.css` + `packages/design-tokens/src/css/tokens.css`
- **变更内容**：见方案 §4.1 + §4.2
- **完成判据**：
  - 视频库 dark 模式 row 分割线肉眼可见
  - light 模式 canvas 与 surface 反差比当前明显
  - validate-tokens / verify-token-references 全绿
