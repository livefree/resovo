...
3. **SEARCH-03** — Phase 2：统一 admin_search ES 索引（状态：⬜ 后排，依 Phase 1 埋点**数据**）
   ...
   - **SEARCH-03-PRE-IMPL** — 埋点实施（route emit admin_search_query + 新 POST /admin/search/telemetry 端点 + searchTelemetry.ts hashQuery + **admin-ui onCommandAction/CommandItem.telemetry Props** + server-next 点击接线 + PII 守门单测 + changelog 真实日志样例）（状态：⬜ 待开始，依 SEARCH-03-PRE 设计 ✅〔含审核修订〕）
     - 文件范围：`apps/api/src/routes/admin/search.ts` + 新 `apps/api/src/lib/searchTelemetry.ts` + **`packages/admin-ui/src/shell/admin-shell.tsx`（AdminShellProps +onCommandAction）+ `packages/admin-ui/src/shell/types.ts`（CommandItem +telemetry）** + `apps/server-next/src/lib/admin-global-search.ts`（mapAdminSearchToCommandGroups 预存 rank、hook 无需改导出）+ `apps/server-next/src/app/admin/admin-shell-client.tsx`（注入 onCommandAction 同步 fire-and-forget POST）+ 单测。新 env `SEARCH_TELEMETRY_SALT` 进部署/env 文档。**建议模型：opus**（**改 admin-ui 公开 Props〔onCommandAction + CommandItem.telemetry〕→ 强制 arch-reviewer Opus + commit `Subagents: arch-reviewer` trailer**；原 sonnet 结论经审核修订作废）。二阶要点见 ADR-200 D-200-10.4：先 onNavigate 再 onCommandAction / 同步发起防 router.push 卸载丢请求 / onCommandAction undefined 回归测试 / PII 守门双覆盖（emit ctx + 请求体不进 access log）。
