...
| **META-58-B-1-DOUBAN-MULTISTATE** | 🔄 进行中 | **后端 douban 4 态谓词 + 多值过滤迁移（arch-reviewer 裁定，ADR-216 D-216-10/12）**：扩展 `video-ref-applied.ts` video 级 4 态谓词（matched=applied / candidate=`NOT matched AND EXISTS(candidate ref，**非 primary**)` / unmatched=`…AND douban_match_status='unmatched'` / pending=`…AND meta_quality IS NULL`）+ JS↔SQL 对拍。迁 **douban** 多值过滤：moderation.ts:219、videos.ts:434、videos.status.ts:180+`:207 投影`、videos.ts:644（douban 半）。清理 `metadata-status.derive.ts:471-473` douban_status 兜底分支。**bangumi 不动**（D-216-11）。 | queries/谓词/derive/tests ≤5 | **opus**（共享谓词契约扩展） | META-58-A + arch-reviewer 裁定 ✅ | 4 态互斥性单测；candidate 非 primary 守护；pending=meta_quality IS NULL；douban 多值口径等价旧列；JS↔SQL 守护；bangumi 过滤不破坏。 |
...
| **META-60-FREEZE-GUARD** | ⬜ 待开始 | **完全停写 + 列冻结守卫（D-216-1/D-216-3/D-216-5）**：读路径全迁后 enrich 完全停写列；CI/lint 守卫禁**运行时代码/新测试**读写 `{douban,bangumi}_status`（**非字面全仓 grep**，C-2：历史 migration/类型注释天然保留）；登记独立 **DROP ADR**（软泡期 ≥1 完整 enrich 周期 + 观察后 Opus ADR：DROP 列 + architecture.md 同步 + 同 commit 删 `statusColumnState`/SQL status 分支/`MetadataStatusSourceRow.{douban,bangumi}Status`）。 | lint/CI/service/docs ≤4 | sonnet（DROP ADR 另起 opus 卡） | 守卫命中运行时新增列读写即失败；完全停写后无 stale 暴露；DROP ADR 起草卡登记。 |
