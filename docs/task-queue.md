...
2b. **CHG-E2E-GATE-AUDIT-B2** — v1 类 3 全链路 6 失败处置（状态：⬜ 待开始；依赖 -B ✅）
   - 建议模型：sonnet
   - 范围：① publish-flow 3 项——spec 在 ADMIN_SPECS 但测前台页（:3000 web server 未起）→ 裁定移域或补 server 依赖 ② admin-source-and-video-flows 2 项断言漂移核查（reject reason 载荷 / dropdown 端点） ③ video-governance reject path 1 项 ④ known flaky moderation reject 顺带核因。
