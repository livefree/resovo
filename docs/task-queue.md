...
- **状态**：🔄 进行中（Phase 0 ✅ + Phase 1 ✅ + Phase 2 ✅ + **Phase 3 进行中**：3A ✅ ImageGovernanceDrawer〔server-next 编排：矩阵=useVideoImages 4 类省 stills、替换聚焦 poster/coverUrl §C 持 Map 取 sourceRef、1D DTO 补 eventId resolve 展示事件、行点击+flash+refresh；8 视图测试〕；下一卡 3B DataTable 治理工作台〔依赖 1A-1D+3A〕→ 3C 文档〔haiku〕）
