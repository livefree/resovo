...
- **变更内容**：见方案 §4.3
- **完成判据**：
  - dark 视频库正文/副标对比度与设计稿一致（截图比对差距 < 5%）
...
- **影响的已完成任务**：DecisionCard / RejectModal / VisChip / Pill / 视频库 chip 列等所有引用 `--state-*-bg/fg/border` 的位置
- **文件范围**：
  - `packages/design-tokens/src/semantic/state.ts`（重写为双主题统一软底）
  - 生成产物
  - 走查清单文档（新建：`docs/designs/backend_design_v2.1/state-pill-soft-walkthrough_20260503.md`）
- **变更内容**：见方案 §4.4
- **完成判据**：
  - dark + light 两主题下 success/warning/error/info 四色 pill 视觉与 dual-signal soft 风格一致
  - arch-reviewer (claude-opus-4-7) 评级 ≥ B+
  - 走查清单覆盖 ≥ 8 个消费组件
- **强制子代理**：spawn arch-reviewer 在 PR 合并前评级

#### CHG-UI-05 · DataTable row 分割线显式落地
- **状态**：⬜ 待开始
- **创建时间**：2026-05-03
- **建议模型**：sonnet
- **变更原因**：CHG-UI-02 token 层把 border 收紧后，仍需在 DataTable 行级显式声明 `border-bottom` 和 `tr:hover { background: var(--bg-surface-row) }`
- **影响的已完成任务**：所有用 admin-ui DataTable 的列表页（视频库、节目库、审核台、播放线路、图片健康、用户管理）
- **文件范围**：
  - `packages/admin-ui/src/components/data-table/*.tsx` 或对应 styles 文件（路径以实际为准）
...
- **第二批**：行密度 / 封面尺寸 / 间距 token 对齐
- **第三批**：tag-chip 11 色饱和度回收
...
| 欠账 ID | 来源 | 描述 | 截止节点 |
|---------|------|------|---------|
| DEBT-UI-BG-INSET | CHG-UI-02 verify-token-references 校验 | `--bg-inset` 8 处引用未定义（VideoEditDrawer + TabImages + TabDouban + TabLines），main HEAD 已存在；既不在 packages/design-tokens 中声明，也不在消费方 fallback；推断为 SEQ-20260429-02 期间漂移残留 | CHG-UI-06 视觉走查时确认是否需补 token 或替换为已有 surface 角色 |
