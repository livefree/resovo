...
13. **SRCHEALTH-P3-3-B** — `host_health` 表 + 熔断双存储 + 软降权（**已拆 -B1/-B2**：arch-reviewer claude-opus-4-8 裁决 G——范围 5 项踩线 + schema/worker/api-service 跨 3 层双触发强制拆）
    ...
    13b. **SRCHEALTH-P3-3-B2** — listSources JOIN host_health 排序分桶 + hostTripped 透出（读侧）（状态：🟡 规划）
    - 范围：sources queries LEFT JOIN + SourceService 分桶排序 + VideoSource.hostTripped 可选字段（**packages/types 契约改动，commit 须带 arch-reviewer trailer**）+ 排序校准单测 + PLAYER/VIDEO 域 e2e。依赖：-B1。建议模型：sonnet。
