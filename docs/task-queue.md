...
| **META-58-B-MULTISTATE-MIGRATE** | ⬜ 待开始（**前置 arch-reviewer 契约扩展**） | **多值过滤消费方（实现期发现 DC-216-1 契约 gap）**：moderation.ts:216、videos.ts:434/437、videos.status.ts:180 的 `douban_status`/`bangumi_status` **4 态下拉过滤**（pending/matched/candidate/unmatched）超 `videoRefAppliedSql`（只 boolean applied）。**需 arch-reviewer 裁定 video 级 per-state 谓词契约扩展**（DC-216-1 加 candidate/missing/pending video 级谓词，与旧列 4 态映射对齐、非 catalog-first overall）。+ C-1 扩展 UI 消费方（distinct-whitelist/route schema/server-next URL serialization/隐藏列/moderation 过滤 UI）。 | 契约扩展 + queries/UI/tests（拆后定） | **opus**（共享谓词契约扩展，强制 Opus 情形 1） | META-58-A + arch-reviewer 契约裁定 | 4 态过滤口径与旧列等价（video 级非 catalog overall）；多值 facet/URL/隐藏列迁移；JS↔SQL 守护。 |
