...
13. **NTLG-P2-b** — 多渠道通知统一订阅（webhook 已有 / 邮件实装 / `submission.created` 补触发点）+ 通知偏好（状态：⏸️ **暂缓**，用户裁定 2026-06-10）
    ...
    - **拆卡蓝图（恢复时用）——P2-b ≠ email，建议拆 -1/-2**：
      - **P2-b-1（无门控，可随时做，性价比高）**：`submission.created` 补触发点（webhook 已定义事件但无触发点）+ 清理 `NotificationsTab.tsx` "后端不发 webhook" 陈旧错误注释（WebhookDispatcher 实际已实装，governance plan §问题4）+ 通知偏好。纯代码内、零新依赖、无需 ADR。
