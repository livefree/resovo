...
- **状态**：🔄 进行中（1/9 卡完成：CHG-SN-8-01 ✅ 2026-05-21；下一卡 CHG-SN-8-02 自动续推）
- **创建时间**：2026-05-21
...
2. **CHG-SN-8-02** · Crawler 站点列表列补全（reference §6.8）
   - **状态**：⬜ 待开始（CHG-SN-8-01 PASS 后）
   - **范围**：crawler-site-columns-v2.tsx 加列 — 行尾「增量 / 全量」inline btn（替代行级 dropdown）+ 调度 mode pill + 上次任务状态 dot + 本批量 / P-crawler.md §2/§3.2 更新
   - **关联问题**：用户问题 #11
   - **工时估算**：0.2w

3. **CHG-SN-8-03** · 采集完成 toast → /admin/moderation?run_id 深链（W1 金票 ②）
   - **状态**：⬜ 待开始
   - **范围**：handleRunAll(All|Site) 完成后 toast 含「查看本次新增视频」action button → 跳 /admin/moderation?run_id=<runId> / Moderation 接 run_id 查询参数定位左队列首条 / P-crawler.md + P-moderation.md + W1 ②/③ 更新
   - **e2e 黄金路径**：触发增量 → toast「采集完成 N 条」+「查看」→ 点击跳 /admin/moderation?run_id → 左队列首条命中
   - **关联问题**：链路打通（H3）
   - **工时估算**：0.15w

4. **M-SN-SHARED-04-A** · VideoPicker 业务原语沉淀（消灭 UUID 输入的钥匙）
   - **状态**：⬜ 待开始（独立于 CHG-SN-8-04/05/06 但作为 -08 的 hard prereq）
   - **建议模型**：opus（共享组件 API 契约 — CLAUDE.md §模型路由强制 Opus 子代理评审）
   - **范围**：packages/admin-ui/src/components/pickers/VideoPicker.tsx 含 PickerTrigger + PickerDialog + cmd+K 风格搜索 / 调 GET /admin/videos?q=&limit=20 / 30-pickers/VideoPicker.md §1-§6 定稿
   - **子代理**：arch-reviewer (Opus) — 共享组件 API 契约必走
   - **工时估算**：0.3w
...
6. **CHG-SN-8-05** · 审核台 RightPane 探/播 per-line「重测」xs btn
   - **状态**：⬜ 待开始
   - **范围**：LinesPanel 每行加「重测」xs btn → 调 /admin/sources/:id/probe（已有？需核）/ DualSignal 实时刷新 / P-moderation.md §3 + W2 更新
   - **关联问题**：用户问题 #5
   - **工时估算**：0.2w

7. **CHG-SN-8-06** · 审核台「通过即上架」开关 + staging 衔接
   - **状态**：⬜ 待开始
   - **范围**：RightPane footer 加 toggle / approve API 加 `publishOnApprove?: boolean` / 通过后若开关 on 自动入 publish state（跳过 staging tab 中转）
   - **关联问题**：链路简化
   - **工时估算**：0.25w（含端点扩展）

8. **CHG-SN-8-07** · /admin/staging → /admin/moderation?tab=staging 单一真源
   - **状态**：⬜ 待开始
   - **范围**：apps/server-next/src/app/admin/staging/page.tsx 改为 308 permanentRedirect 至 /admin/moderation?tab=staging / 同步 admin-nav / verify:manual-coverage KNOWN_NO_MANUAL 已包含 staging 无需改
   - **关联问题**：用户问题 #5 IA 收敛
   - **工时估算**：0.1w

9. **CHG-SN-8-08** · 视频库行级 inline「发起合并」深链（依赖 SHARED-04-A VideoPicker）
   - **状态**：⬜ 待开始（依赖 #4 M-SN-SHARED-04-A PASS）
   - **范围**：InlineRowActions 加「合并」xs btn → 触发 VideoPicker 选 candidate_b → 深链 /admin/merge?candidate_a=<row_id>&candidate_b=<picked_id>&from=videos / P-videos.md §3 + W4 更新
   - **关联问题**：用户问题 #7（合并入口）+ #8 (反 UUID)
   - **工时估算**：0.2w
