...
- **状态**：🔄 进行中（2/11 — **Phase 0 ADR 全收口**：0A ✅ ADR-208 + 0B ✅ ADR-209 均 Accepted；Phase 1 后端 1A→1B→1C→1D 硬串行解锁，建议 sonnet 会话）
...
| **IMGH-P2-1A** ⬜ | **后端：candidates 端点（零 migration）**：① `imageHealth` queries 读 `metadata_field_proposals` 候选（按 field 过滤 + source + confidence + 缩略 URL；语义=每来源每字段一条最优候选，§17.2.1，文案「跨源候选」）② candidates route + service（按 ADR-0A 命名空间）③ DTO 入 `lib/image-health/api.ts` ④ 端点单测（含 SQL/参数校验）。**审计 TS 枚举扩展归 1B**。 | 4 | sonnet | 0A Accepted | route/service/db 分层 + `verify:endpoint-adr` + 端点单测 |
