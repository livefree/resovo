...
| 5-B | ⬜ **PRE-DEAD-LINE-AUTO-RETIRE-WORKER-B** 待开始（依赖 #5-A ship）| Wave 4 #5-B / worker 层 4 文件 / apps/worker/src/jobs/auto-retire-line.ts NEW + cron.autoRetireLine 配置 + index.ts 注册 + worker integration test ≥ 5 case / 不再需 Opus 评审（评审已 PASS） | 否（依赖 -A ship + ADR-164 D-164-8）| sonnet-4-6 |
