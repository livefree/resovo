...
- **状态**：🔄 进行中（10/11 — Phase 0 ✅（0A/0B）+ Phase 1 ✅（1A/1B/1C/1D）+ **Phase 2 ✅ 全收口**：2A ✅ ImageCompare + 2B ✅ ImageCandidatePicker〔admin-ui 共享组件，Opus arch-reviewer agentId a9732b79ad7128d4d 一次定双契约〕。**下一阶段 Phase 3**：3A ImageGovernanceDrawer〔server-next 编排，依赖 1A/1B/1C+2A/2B〕→ 3B DataTable 治理工作台〔依赖 1A-1D+3A〕→ 3C 文档〔haiku〕）
...
| **IMGH-P2-3A** ⬜ | **UI 编排：`ImageGovernanceDrawer`**（server-next `_client/`，680px/fullscreen，§6.3/§17.3.3）：① 图片矩阵（poster/backdrop/logo/banner/stills，点击 → ImageLightbox）② 破损详情区（event_type + domain + occurrence + 首现/最近）③ 替换区（ImageCandidatePicker 选候选 → ImageCompare 对比 → apply-candidate；或手填 URL → PUT images）④「标记已解决」→ resolve-event ⑤ 行点击打开 + toast + 行 flash + 抽屉刷新。模块编排非共享契约。 | 5 | sonnet | 1A/1B/1C + 2A/2B | 模块内 Drawer 范式 + 视图测试（候选补图 + 手填 + 标记已解决流程） |
