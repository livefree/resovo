...
| **META-58-B-2-FACET-UI** | ⬜ 待开始 | **前端 facet 静态枚举（ADR-216 D-216-12 / C-1）**：`distinct-whitelist.ts:60-61` 移除 douban_status 键（ADR-150 AMENDMENT 登记）→ facet 改静态枚举（4 态稳定闭集）；验证 server-next URL serialization / 隐藏列 / moderation 过滤 UI **零回归**（前端契约面不变，仅后端谓词换）。 | distinct/route/UI/tests ≤4 | sonnet | META-58-B-1 | facet 4 态枚举不变；URL/隐藏列零回归；distinct-whitelist AMENDMENT 登记。 |
