...
7. **CHG-DESIGN-07** — Dashboard 8 卡片浏览态 🔄 7A+B+C+D-1 已完成；7D-2 留新 session（拆 7A/7B/7C/7D 4 阶段，2026-04-30 修订）

   **当前进度**：
   - 7A ✅ 共享组件契约（Opus arch-reviewer CONDITIONAL → 必修闭环 → PASS；Codex stop-time fix 4 处契约矛盾闭环）
   - 7B ✅ 实装 + 单测（Opus arch-reviewer **PASS 直接通过**；Codex stop-time fix 2 处契约-实装一致性闭环）
   - 7C ✅ 业务集成 + 数据契约对齐 + regression gate（Codex stop-time fix 3 处假数据/文档同步/stale refs 闭环；2691 单测 + typecheck + lint + verify 全绿）
   - 7D-1 ✅ 9 项视觉规格 desk review（代码字面对齐 reference §5.1 全部规格）
   - 7D-2 🔄 待启动（新 session）：起 dev server + Playwright MCP 截图 + e2e smoke 实跑 + visual baseline 入库
