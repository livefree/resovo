...
- **依赖**：M-SN-4 milestone B+ PASS（CHG-SN-4-10-D 已闭环 2026-05-05）/ `docs/M-SN-4-milestone-audit-2026-05-05.md` Y1 触发的 cutover-blocker 子序列母卡建议
...
- **真源**：`docs/server_next_plan_20260427.md` §6 M-SN-5.5（v2.6 新增）+ `docs/M-SN-4-milestone-audit-2026-05-05.md` §6
...
> 真源：`docs/M-SN-4-milestone-audit-2026-05-05.md` §6 严格核对 4🔴+2🟠+1🟡（🟡 不计入本期）

1. **CHG-SN-5-PRE-01-A** · DEBT-SN-3-B（staging cookie + nginx e2e 演练，需人工，🔴）— 状态：✅ 已完成（2026-05-12，本地 Caddy 替代演练，5 步金票路径全绿）
   - 创建时间：2026-05-06
   - 完成时间：2026-05-12
   - 文件范围（实际）：docs/server_next_PRE-01-A-drill-2026-05-12.md（演练记录文档）+ docs/changelog.md + docs/task-queue.md
...
2. **CHG-SN-5-PRE-01-B** · DEBT-SN-3-C（M-SN-3 milestone 阶段审计，🔴）— 状态：✅ 已完成（2026-05-12，arch-reviewer Opus B+ PASS 无条件）
   ...
   - 文件范围（实际）：`docs/M-SN-3-milestone-audit-2026-05-12.md`（新建）+ docs/task-queue.md + docs/changelog.md
...
- **范围**：
  - `docs/M-SN-5.5-milestone-audit-2026-05-12.md`（新建 milestone audit 文档）
...
1. **CHG-SN-5.5-AUDIT** — M-SN-5.5 milestone 阶段审计（Opus arch-reviewer）— 状态：✅ 已完成（2026-05-12，A- PASS 无条件）
   ...
   - **审计输出**（实际产出 `docs/M-SN-5.5-milestone-audit-2026-05-12.md`）：
   ...
   - **文件范围**（实际）：`docs/M-SN-5.5-milestone-audit-2026-05-12.md`（新建） + docs/task-queue.md + docs/changelog.md + docs/tasks.md
...
- **真源**：`docs/server_next_plan_20260427.md` §6 M-SN-5（行 519-536） + `docs/M-SN-5.5-milestone-audit-2026-05-12.md` ⑤ 人工 checklist + ⑨ 附加观察
...
> **2026-05-18 范围重大扩展**：用户复核发现 server-next 后台**架构性偏离设计稿**（多页面 v1 风格 DataTable + Tab + 外置 SelectionActionBar，未对照 `docs/designs/backend_design_v2.1/reference.md` §5.x + screens-2.jsx 真源），M-SN-7 主线由"清债务"转为"**设计稿对齐重做**"。规划文档：`docs/M-SN-7-design-realign-plan.md`（5 用户决策 + 11 修订意见全部合并落地）。新增专项段「设计稿对齐重做」见下方。

### 高优先（M-SN-7 前 3 卡推荐顺序）

| ID | 标题 | 范围 | 建议模型 | 工时 | 优先级 |
|---|---|---|---|---|---|
| **CHG-SN-7-PRE-01** | 文件大小守卫 verify:file-size-budget（**复核扩范围**） | 新脚本 `scripts/verify-file-size-budget.mjs`：扫描 `apps/**/*.tsx`、`apps/**/*.ts`、`packages/**/*.tsx` 超 500 行 → exit 1；preflight 集成；**baseline 豁免清单 5 文件**（M-SN-6 复核 2026-05-17 实测）：MergeClient 756 / VideoListClient 734 / ModerationConsole 583 / sidebar 696 / data-table 608 — 标 baseline 不阻断 CI 但纳入"主动拆分预案"；**新增文件零容忍**（首次提交超 500 即 FAIL）；GENERIC_WHITELIST 支持 `.types.ts` / `index.ts` 等结构性大文件豁免 | sonnet | 0.12w | 🔴 P0（H1 系统性盲点根因 + M-SN-6 复核漏检揭示） |
| **CHG-SN-7-PRE-02** | ADR-121 起草 R-MID-1 RETRO 协议正式化 | 写 ADR-121「R-MID-1 audit RETRO 协议正式化」：4 真源同步范式（union + ACTION_TYPES + EXPECTED + REQUIRED/PAYLOAD）+ 6 文件固定框架 + PATCH ≤ 5 上限豁免依据；引用 CHG-SN-6-14/16-A/20-A/25-RETRO/26-RETRO 5 卡先例；签约 sonnet 主循环可独立完成不需 Opus | opus（主循环）+ arch-reviewer (Opus) 评审 ADR 起草质量 | 0.15w | 🔴 P0（5 卡先例无规范来源） |
| **CHG-SN-7-MISC-PERSITE** | perSiteOverrides UI 实装（M-SN-6 deferred 债务） | SchedulerConfigDrawer 加 perSiteOverrides 编辑区（每站点 enabled 切换 + mode 选择 inherit/incremental/full）；可考虑独立子 Drawer 嵌套 / 表格化编辑；CHG-SN-6-27 中标记 advisory 引用此卡 | sonnet | 0.15-0.25w | 🟡 P2 |

### 中风险（M-SN-7 中段评估）

| ID | 标题 | 范围 | 建议模型 | 工时 | 优先级 |
|---|---|---|---|---|---|
| **CHG-SN-7-MISC-FILE-SIZE** | **5 文件主动拆分预案（M-SN-6 复核扩范围）**：原 CRAWLER-FILE-SIZE 改名 + 范围扩 2 漏检 | 评估 5 文件按 PATCH-1 范式拆分：crawler 3 接近上限（CrawlerRunDetailView 445 / TaskLogsDrawer 491 / CrawlerRunsView 429）+ **AuditClient 558**（M-SN-6 新增 / CHG-SN-6-01）+ **ImageHealthClient 501**（M-SN-6 新增 / CHG-SN-6-02）；若 PRE-01 守卫触发警告则启动拆分；范式参考 CrawlerSitesTab + ControlsCard + FormDrawer + columns 拆分 | sonnet | 0.3-0.6w（5 文件） | 🟡 P2 |
...
| ID | 标题 | 范围 | 工时 | 优先级 |
|---|---|---|---|---|
| **CHG-SN-7-LOW-1** | 双子卡范式 -A audit + -B UI 文档化 | 补 `docs/rules/admin-module-template.md` 一节"写端点 + UI 拆卡决策树"，引用 CHG-SN-6-16/20/25/26 先例 | 0.05w | 🟢 P3 |
| **CHG-SN-7-LOW-2** | NEGATED ADR 占位 / 重启路径集中说明 | `docs/decisions.md` 头部加一节"NEGATED ADR 占位语义"，引用 ADR-114/119/120-NEGATED 3 例，明确 plan §4.7 候选保留 + 重启 ADR 编号路径（如 ADR-119a） | 0.05w | 🟢 P3 |
| **CHG-SN-7-LOW-3** | ModerationConsole csv 豁免追溯 | ADR-106（admin-ui 下沉）或对应 plan 段落补一句话："toolbar-less 视图豁免 csv-export"，明确 ModerationConsole + 后续无 DataTable 的视图不强制接入共享导出工具 | 0.03w | 🟢 P3 |
...
> 真源：`docs/M-SN-7-design-realign-plan.md`（§0–§8 全文）。本段为 task-queue 索引；详细 spec ↔ 现状 ↔ 重做归属对照见计划文档 §1（16 路由审计矩阵）+ §2（Crawler 完整对照）。
>
...
| ID | 标题 | 范围 | 建议模型 | 工时 | 优先级 |
|---|---|---|---|---|---|
| **CHG-SN-7-PRE-04** | 全量审计 16 admin 路由 vs 设计稿 §5.1–§5.16 | 拆 16 子卡（每路由 0.05w）逐路由 Sonnet 扫描产出 ✅/⚠️/❌ + 偏离分级 → 末尾**单次 spawn Opus** 对全 ❌ 项归类 + REDO-NN 优先级排序（0.1w）；输出 `docs/M-SN-7-design-realign-audit-FULL.md`；**首张子卡：/admin/dashboard §5.1 审计** | sonnet（主循环逐路由）+ Opus（收尾排序） | 0.9w | 🔴 P0（主线入口） |
| **CHG-SN-7-PRE-05** | ADR-123 分类映射 schema 起草 | Crawler 站点行展开"分类映射 collapsible"依赖；通过 → REDO-01-F 按 spec 实施；不通过 → REDO-01-F 自动降级为"占位 + 跳 settings" | arch-reviewer Opus | 0.1w | 🔴 P0 |
...
| ID | 标题 | 范围 | 建议模型 | 工时 | 优先级 |
|---|---|---|---|---|---|
| **CHG-SN-SHARED-01** | **KpiCard `progress?` prop 扩展**（**PRE-04 子卡 #1 dashboard 实测重大发现 2026-05-18**：admin-ui 已入库 KpiCard；本卡改为对现有原语 props 增量扩展） | 扩展 props（含 `progress?`）+ 现有消费方零破坏验证（MetricKpiCardRow + WorkflowCard 双签）+ 单测 + 视觉 baseline 更新；向后兼容硬约束 | Opus（契约）+ Sonnet（实施） | **0.1w**（原 0.35w，下调） | 🔴 P0 |
| **CHG-SN-SHARED-02** | `<ExpandableTable rows columns expandRow rowKey? pagination?>` | API 契约（**含 `pagination?: { pageSize, pageSizeOptions }` 显式入口** 承载站点 100+ / sources 矩阵分页）+ 实施 + admin-ui 入库 + demo + 单测；**Opus 必须裁决组件能力**：是否原生支持 selection 列（能力契约，与"是否启用"分离） | Opus（契约）+ Sonnet（实施） | 0.4w | 🔴 P0 |
...
| ID | 标题 | 范围 | 建议模型 | 工时 | 优先级 |
|---|---|---|---|---|---|
| **CHG-SN-7-REDO-01-A** | Opus 子代理：消费 SHARED 产出 + 设计本页 props/state/事件契约 + 裁决 Crawler 页是否启用 selection | 依赖 SHARED-01/02/03 全部完成 | Opus 必须 | 0.15w | 🔴 P0 |
| **CHG-SN-7-REDO-01-B** | 后端：ADR-122（KPI + Timeline + per-site run + run-all）+ 实施 4 个新端点 + DB queries + 单测 | Opus 子代理评估与 analytics/dashboard 端点重叠 → 写入 ADR-122 §"与现有端点关系" | Opus（ADR）+ Sonnet（实施） | 0.6w | 🔴 P0 |
| **CHG-SN-7-REDO-01-C** | 前端骨架：新 CrawlerClient（3 actions + KPI row + 时间轴 card 框架 + 站点列表骨架，**不含展开行**） | 依赖 A + B | Sonnet | 0.3w | 🔴 P0 |
| **CHG-SN-7-REDO-01-D** | 前端站点行 + `{more}` 菜单 | 迁移 8 字段表单 + CRUD + 启停 + 标记 + 删除 + 行级 `+ 增量 / 全量` | Sonnet | 0.3w | 🔴 P0 |
| **CHG-SN-7-REDO-01-E** | 前端行展开：线路 sub-table | 跨调 sources 现有 API（`GET /admin/sources/sites/:key/routes` + `PATCH /admin/sources/routes/:id`） | Sonnet | 0.4w | 🔴 P0 |
| **CHG-SN-7-REDO-01-F** | 前端行展开：分类映射 collapsible | ADR-123 通过 → 按 spec；不通过 → 占位 + 跳 settings | Sonnet | 0.2w | 🔴 P0 |
| **CHG-SN-7-REDO-01-G** | 高级菜单（调度配置 / 重建索引 / 全局止血 / 冻结切换 4 项 + DropdownMenu） | 复用 SchedulerConfigDrawer + stopAllCrawler + triggerReindex + setCrawlerFreeze 现有 API | Sonnet | 0.2w | 🔴 P0 |
| **CHG-SN-7-REDO-01-H** | runs 列表迁独立路由 `/admin/crawler/runs` + sidebar 二级菜单 | 移动 CrawlerRunsView.tsx → `crawler/runs/_client/` + 新建 `crawler/runs/page.tsx` | Sonnet | 0.15w | 🔴 P0 |
| **CHG-SN-7-REDO-01-I** | 删除旧文件（`CrawlerSitesTab.tsx / CrawlerControlsCard.tsx / crawler-site-columns.tsx`） | **前置**：`git tag pre-redo-crawler-<YYYYMMDD>`；Rollback 区记 revert 命令 | Sonnet | 0.05w | 🔴 P0 |
| **CHG-SN-7-REDO-01-J** | 视觉回归测试 + e2e + Opus 验收 | §2.4 全 22 行 checklist 必须 100%；像素 diff 容差 2%；评级 < A− → 自动 revert + RECHECK 子卡 | Opus 验收 | 0.2w | 🔴 P0 |

### CHG-SN-7-REDO-02 Submissions §5.13 Card list 重做（**~1w**，PRE-04 已闭环锁定）

| ID | 标题 | 状态 |
|---|---|---|
| **CHG-SN-7-REDO-02-A..F** | Submissions DataTable → Card list（32px 状态 icon box / 引用块 / 重验·查看·处理按钮 / Segment 4 类） | PRE-04 子卡 #9 闭环（❌ 架构错位）→ 复用 REDO-01 子卡模板拆解 |
...
| ID | 标题 | 估时 | 模型 |
|---|---|---|---|
| **CHG-SN-7-REDO-03-A** | sidebar IA 重构（5 子路由 → 1 入口 `/admin/settings`）+ 旧路由 redirect + ADR | 0.4w | Opus（IA 决策）+ Sonnet |
| **CHG-SN-7-REDO-03-B** | SettingsContainer 5 Tab → 8 Tab 扩展（补图片 / 通知 / API·Webhook / 登录会话） | 0.6w | Sonnet |
| **CHG-SN-7-REDO-03-C** | 后端 settings 端点字段扩展 + ADR | 0.3w | Opus（ADR）+ Sonnet |
| **CHG-SN-7-REDO-03-D** | 验收 | 0.2w | Opus 验收 |

**注**：吸收原 `CHG-SN-7-MISC-SETTINGS-TABS`（M-SN-6 FOLLOWUP 卡）为 REDO-03-B 子任务。

### CHG-SN-7-REDO-04 Staging 路由处置（**0.1w 或 ~1.5w**，待 Opus 裁决）

> 触发：reference §5.5 spec 完整定义，但当前路由 `/admin/staging` **不存在**。部分功能在 `/admin/moderation?tab=staging`。需 Opus 裁决：

- **方案 A**：staging 是独立路由 → ~1.5w 新页 + 后端 + 验收
- **方案 B**：IA 修订 staging 合并 moderation tab（同 analytics → dashboard tab 模式）→ 0.1w 仅补 redirect

裁决前置：PRE-04 收尾 Opus 排序时一并裁决。
...
| M-SN-SHARED milestone | **~0.5w**（PRE-04 子卡 #1 dashboard 实测下调 0.4w） |
| REDO-01（采集） | 2.55w |
| REDO-02（Submissions） | ~1w |
| REDO-03+（剩余 14 路由） | ~6–10w |
| **M-SN-7 全 milestone** | **~11.5–15.5w**（PRE-04 实测下调） |

### MISC 跟踪卡（PRE-04 子卡审计产出）

| ID | 标题 | 严重度 | 估时 | 触发子卡 |
|---|---|---|---|---|
| **CHG-SN-7-MISC-DASHBOARD-1** | dashboard page__head 2 按钮 onClick 绑定（全站全量采集 / 进入审核台） | 🟡 P2 | 0.05w | #1 |
| **CHG-SN-7-MISC-DASHBOARD-2** | dashboard 4 类卡片数据真实化 + 后端 6 endpoints + ADR（与 STATS-EXTEND-ANALYTICS 合并） | 🟡 P2 | 0.5–0.8w | #1 + #12 |
...
| **CHG-SN-7-MISC-MERGE-1** | merge Segment 3 类（待审候选 / 已合并 / 已拆分）补全 | 🟡 P2 | 0.15w | #6 |
...
详见上方「设计稿对齐重做」段 + `docs/M-SN-7-design-realign-plan.md` 全文。
