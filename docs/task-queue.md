...
## SEQ-20260619-02 — image-health 页面重构 P2（治理闭环 · 新增端点 + 共享组件 + 治理抽屉）🔄

- **状态**：✅ **全交付 2026-06-20**（Phase 0 ✅〔ADR-208 + ADR-209〕+ Phase 1 ✅〔1A-1D 后端〕+ Phase 2 ✅〔2A ImageCompare + 2B ImageCandidatePicker〕+ Phase 3 ✅〔3A 治理抽屉 + 3B 工作台增强 + 3C 文档收尾〕）。**收口待办（合并 main 前）**：PHASE COMPLETE 全量审计节点——`npm run test -- --run`（单测全量）+ `npm run test:e2e`（4 projects）尚未在本 SEQ 收口处跑（3C 为 docs-only，code 阶段均增量门禁过）；建议合并 dev→main 前补跑。历史详情：3A ImageGovernanceDrawer〔§C 持 Map 取 sourceRef + 1D DTO 补 eventId〕；3B 工作台〔缩略列 + eventType/候选数列 + 服务端 filter + bulk + distinctFetcher 复用 /broken-domains，+27 新测，含 Codex 复审修复缩略列裁切〕；3C 手册 P-image-health/W3 + decisions ADR 实现标记。
- **创建时间**：2026-06-19｜**最后更新**：2026-06-20
- **来源**：用户「开始启动 P2」（承接 SEQ-20260619-01 P1 收口；P1 已交付双 Tab IA + KpiCard/Spark + ImageLightbox + 行内切此域 + 文档纠错）。
- **设计真源**：`docs/designs/image-health-ux-handoff_20260618.md` §5.2 / §6 / §7 / §11.2 / §12.2 + **§17.4 收敛分期（权威）**；`docs/research/image-health-codebase-survey_20260619.md` 事实底座。
- **范围裁定（P2 = 人工治理闭环；新增端点 + schema + 2 共享组件 + 治理抽屉编排）**：
  - **新增端点**（每个 → 先起 ADR + `verify:endpoint-adr`，CLAUDE.md MUST-8）：candidates（读 proposals 跨源候选）/ apply-candidate（safeUpdate + status=pending_review + 入队 + 审计）/ resolve-event 薄端点（暴露已存在 `resolveImageEvents`）/ ids 精确重扫端点（**禁复用全局 rescan 伪装选中批量**，§17.3.1）/ `/missing-videos` 服务端筛选 query 扩展（search/status/event_type/source/brokenDomain，§17.3.2）。
  - **审计扩展（非 schema 变更，Codex BLOCK-1 纠正）**：`admin_audit_log.action_type` 为 free-text `TEXT` **无 DB CHECK**（052 migration `:27-32` 仅 `target_kind` 有 CHECK）；`target_kind='image_health'` **已存在**（`admin-moderation.types.ts:293` + architecture.md §审计）→ **零 migration**，仅扩 `AdminAuditActionType` TS 枚举（+`image_health.apply_candidate` +`image_health.resolve_event`）+ 审计标签/docs（随 ADR-0A 决策，1B 落地）。
  - **新共享组件**（每个 → 强制 Opus 子代理 + arch-reviewer PASS + commit `Subagents:` trailer，§11.2）：`ImageCompare`（当前 vs 候选并排）/ `ImageCandidatePicker`（多源候选缩略选图，消费 candidates DTO）。`ImageLightbox` 已于 P1-3 落地复用。
  - **模块编排**（非共享契约，按现有 Drawer 范式）：`ImageGovernanceDrawer`（server-next `_client/`，680px/fullscreen，§17.3.3）。
- **明确推迟 P3（不在本序列）**：分级自愈 worker step（§8）/ `image_governance_status` 推导 + `degraded` 枚举扩展（§9，schema 变更另起 ADR）/ Dashboard 真实 attentions 端点（§10.2 站内①）/ admin-shell 待办 drawer（站内②）/ 阈值告警渠道（邮件/Slack/webhook）。
- **本序列内推迟（标 optional follow-up）**：candidates 端点的「无候选时实时 TMDB 多图拉取」（§7.2，涉外部 API + 缓存 + 限流，复杂度高 → P2-1A 仅读 proposals；实时拉取如确需另起独立 ADR）。
- **开卡前事实核验（本会话 Opus 亲验）**：
  - ✅ `/admin/images/*` 命名空间**不存在**（现有图片端点全在 `/admin/image-health/*`；`routes/admin/identity-candidates` 是视频合并候选，**不冲突**）→ **ADR-0A 须裁决新端点命名空间**：`/admin/images/*`（按方案 §7.2）vs 并入 `/admin/image-health/*`（命名空间一致、与现有 6 端点同 route 文件）。倾向后者（一致性 > 方案字面）。
  - ✅ `resolveImageEvents` 确在 `imageHealth.scan.ts:161`（已 re-export 自 `imageHealth.ts:495`，写 `resolved_at` + `resolution_note`）→ resolve-event 仅缺路由暴露，薄端点成立（§6.3 / §12.1）。
  - ✅ `metadata_field_proposals` 含 `coverUrl`/`backdropUrl`/`logoUrl` 图片字段（§7.1）→ candidates 读 proposals 底座成立；但语义是「每来源每字段一条最优候选」非「同源多图列表」（§17.2.1），UI 文案用「跨源候选」。
  - ⚠️ `PUT /admin/videos/:id/images` 当前**无 `insertAuditLog`**（§17.2.2）→ 人工替换审计须经 apply-candidate 端点承担（ADR-0A 决策审计归属），或 PUT 路径补审计（评估后定）。
- **依赖序**：Phase 0（0A→0B，ADR 全 Opus + arch-reviewer PASS，**任一红线未闭环 → BLOCKER**）→ Phase 1（**1A→1B→1C→1D 硬串行**，Codex BLOCK-5 纠正：四卡均改 `lib/image-health/api.ts` DTO/client + `routes/admin/image-health.ts` 同路由文件〔命名空间并入后〕+ `db/queries/imageHealth*.ts`，"可并行不同文件"是错误声明）→ Phase 2（**2A→2B 硬串行**，Codex CONCERN-4：两卡均改 admin-ui barrel/types 导出；依赖 0A 定型 candidates DTO）→ Phase 3（3A 抽屉依赖 1A/1B/1C+2A/2B；3B DataTable 依赖 1A/1B/1C/1D+3A；3C 文档依赖 3A/3B）。
- **模型策略**：Phase 0 ADR → **opus 主循环 + arch-reviewer (Opus)**；Phase 1 后端端点 → sonnet（标准 route/service/query，零 migration）；Phase 2 共享组件契约 → **opus + arch-reviewer 强制**；Phase 3-A/3-B UI 编排 → sonnet；3-C 文档 → haiku（doc-janitor）。

### 任务列表（11 卡，每卡范围 ≤ 5 项 / 验收口径唯一）

| 卡 | 内容 | 范围项 | 建议模型 | 依赖 | 门禁 |
|---|---|---|---|---|---|
| **IMGH-P2-0A** ✅ | **ADR：补图闭环端点契约 + 审计扩展**：起草 **ADR-208**（Accepted）裁决：① 命名空间**并入 `/admin/image-health/*`**（否决 `/admin/images/*`）② `GET /admin/image-health/candidates?catalogId&field` 读 proposals 跨源候选 ③ `POST /admin/image-health/apply-candidate` **复用 safeUpdate** 得 §7.3 优先级闸门 + manual hard lock（`field∈skippedFields→409`）+ status=pending_review + 入队 + 审计 ④ 审计扩 `AdminAuditActionType` TS 枚举（零 migration）⑤ batch 不做伪批量 + PUT images 审计归属。**已完成 2026-06-19**：arch-reviewer (claude-opus-4-8) CONDITIONAL-PASS〔6 事实断言逐条为真，H1 全域 admin-only + M1/M2/M3 + L1/L2 全吸收〕+ Codex (gpt-5.5) BLOCK→消解〔端点表 verifier 范式 + source guard 422 + apply 补 videoId + stale sourceRef 校验 409；6 OK〕；verify:endpoint-adr ✅ 130 ADR 端点（含本 ADR 新 2）；verify:docs-format 零新增违规。执行模型: claude-opus-4-8；子代理: arch-reviewer (claude-opus-4-8)。详见 changelog [IMGH-P2-0A] + decisions.md ADR-208 | 5 | **opus** | 无 | **Opus 子代理 + arch-reviewer PASS + Subagents trailer** + Codex 对抗性审核（ADR 非代码产物） + `verify:endpoint-adr` 端点登记 |
| **IMGH-P2-0B** ✅ | **ADR：治理表增强端点契约**：起草 **ADR-209**（Accepted）裁决：① `/missing-videos` 服务端筛选 query（search〔含 short_id〕/posterStatus/event_type/posterSource/brokenDomain；**total 一致 = page/count 共用 buildMissingVideosFilter + evt 谓词置外层 WHERE**）+ brokenDomain distinct **复用 broken-domains 端点**（零 ADR-150 扩展）② resolve-event 薄端点 `{eventIds, note?}`（不支持 videoId；`resolveImageEvents` 改返 rowCount；经 ImageHealthService）+ 审计 `image_health.resolve_event` ③ ids 精确重扫 `{videoIds}` **scoped 入队**（扩 `listPendingImageUrls(catalogIds?)`，禁裸调全局 enqueueBackfillJob）④ DTO 补 catalogId + event_type + candidateCount/hasHighConfidenceCandidate（bool_or(is_winner)，page CTE 聚合）。**已完成 2026-06-20**：arch-reviewer (claude-opus-4-8) CONDITIONAL-PASS〔7 事实断言为真，HIGH-1 入队两段式 + MEDIUM-1/2 + LOW-1/2/3 全吸收〕+ Codex (gpt-5.5) BLOCK→消解〔D-209-3 全局副作用→scoped 入队 + search short_id + resolvedCount/分层 + 审计清单 + page CTE〕；verify:endpoint-adr ✅ 132 ADR 端点（含本 ADR 新 2）；docs-format 零新增违规。执行模型: claude-opus-4-8；子代理: arch-reviewer (claude-opus-4-8)。详见 changelog [IMGH-P2-0B] + decisions.md ADR-209 | 4 | **opus** | 0A（命名空间裁定先行） | **Opus 子代理 + arch-reviewer PASS + Subagents trailer** + Codex 对抗性审核 + `verify:endpoint-adr` |
| **IMGH-P2-1A** ✅ | **后端：candidates 端点（零 migration）**：① `getFieldProposalsByCatalogIdAndField` 读 `metadata_field_proposals`（`WHERE catalog_id=$1 AND field_name=$2`，复用 mapProposal）② `GET /admin/image-health/candidates` route（admin-only；trust 用 canonical `CATALOG_SOURCE_PRIORITY` 在 route 层派生+排序，禁 SQL/前端硬编码；非串候选剔除；空→[]）③ DTO `ImageCandidate` + client `listImageCandidates` 入 `lib/image-health/api.ts` ④ 单测（route 7 + query 2）。**已完成 2026-06-20**：读端点跟域内 5 兄弟读端点范式走 route→query（ImageHealthService 偏 worker 侧，引入 service 仅为单读端点反破坏一致性）；typecheck/lint EXIT=0 / verify:endpoint-adr ✅ 244 路由（candidates 匹配 ADR-208）/ test:changed 364 全过 + 新增 9 测全绿。执行模型: claude-opus-4-8（会话用户裁定 Opus 续跑 Phase 1）；子代理: 无。详见 changelog [IMGH-P2-1A] | 4 | sonnet | 0A Accepted | route/service/db 分层 + `verify:endpoint-adr` + 端点单测 |
| **IMGH-P2-1B** ✅ | **后端：apply-candidate 端点**：① service（§7.3 trust 优先级取候选 + manual hard lock 守卫 + safeUpdate 写 URL + status=pending_review + updated_at）② 入队 health-check + blurhash-extract ③ 扩 `AdminAuditActionType` TS 枚举（+`image_health.apply_candidate`，target_kind=image_health 已存在 → **零 migration**）+ 写审计 ④ route + DTO ⑤ 端点单测（优先级闸门 / hard lock 跳过 / 审计载荷断言）。**已完成 2026-06-20**：**复用 safeUpdate 闸门禁自建**（field∈skippedFields→409 不静默成功）；source 运行时守卫→422 INVALID_SOURCE；sourceRef 一致校验→409 CANDIDATE_STALE；审计 6 真源 + 3 测试镜像全同步零 migration；typecheck/lint EXIT=0 / verify:endpoint-adr ✅ 245 路由 / test:changed 升全量 7992 全过 + 新增 10 测全绿。执行模型: claude-opus-4-8；子代理: 无。详见 changelog [IMGH-P2-1B] | 5 | sonnet | 0A Accepted + 1A | route/service/db 分层 + `verify:endpoint-adr` + 端点单测（闸门 + 审计） |
| **IMGH-P2-1C** ✅ | **后端：resolve-event + ids 精确重扫端点**：① resolve-event route（薄封装 `resolveImageEvents(ids:string[],note?)`，入参 `{eventIds: string[], note?}` ← BLOCK-2；扩 `AdminAuditActionType` +`image_health.resolve_event` + 写审计，零 migration）② ids 精确重扫 route/service（按 videoIds/catalogIds 重置 poster_status=pending_review + 入队，与全局 rescan scope 切开，**禁伪装**）③ DTO 入 api.ts ④ 端点单测。**已完成 2026-06-20**：**route→ImageHealthService→query 守分层**（resolve-event 经 `ImageHealthService.resolveEvents`，rescan-selected 经 `rescanSelectedVideos`）；`resolveImageEvents` void→rowCount（零调用方安全）+ resolvedCount=0 幂等不报 404；**rescan-selected scoped 闭环禁全局副作用**（`getCatalogIdsByVideoIds`→`rescanPostersByCatalogIds`〔cover_url 守卫〕→扩 `listPendingImageUrls(...catalogIds?)` 仅入选中行 dedup jobId；**单测断言 `enqueueBackfillJob` 永不调用**＝Codex BLOCK 守卫）；resolve_event 审计 6 真源 + 3 测试镜像同步零 migration，rescan-selected 复用 `image_health.rescan`；typecheck/lint EXIT=0 / **verify:endpoint-adr 247 路由**（+2）/ test:changed 升全量 8006 全过 + 新增 12 测全绿。执行模型: claude-opus-4-8；子代理: 无。详见 changelog [IMGH-P2-1C] | 4 | sonnet | 0B Accepted + 1B（硬串行） | route/service/db 分层 + `verify:endpoint-adr` + 端点单测 |
| **IMGH-P2-1D** ✅ | **后端：missing-videos 服务端筛选 query + 行级数据契约**：① query 扩展 WHERE（search ILIKE / posterStatus / event_type / posterSource / brokenDomain）保持分页 total 一致 ② SELECT 补 `v.catalog_id`（BLOCK-3，供抽屉调 candidates）+ `candidateCount`/`hasHighConfidenceCandidate` 聚合 `metadata_field_proposals`（BLOCK-4，避 3B 候选列 N+1/死列）③ distinct（brokenDomain 受控表达式 or image-health 专用，按 0B 裁定）④ DTO 补筛选入参 + `catalogId` + `event_type`（**不加尺寸列**，CONCERN-1：沿用客户端 naturalWidth）⑤ 单测（筛选组合 + total 一致 + catalogId/candidateCount 断言）。**已完成 2026-06-20**：抽共享 `buildMissingVideosFilter(filters,startIndex)` + `MISSING_VIDEOS_FROM` 常量 → **page 与 count 逐字共用 FROM+LATERAL+WHERE**（total 一致硬约束）；evt 谓词置外层 WHERE 使 LEFT JOIN LATERAL 等价 INNER（MEDIUM-1 不变量，单测断言谓词在 `) evt ON TRUE` 之后）；候选聚合在 **page CTE〔已 LIMIT ≤20 行〕** 上 LATERAL（Codex CONCERN 避全量 N+1）；brokenDomain 输出/筛选改 SQL regexp_replace 统一口径；search 含 short_id exact；新 `countMissingPosterVideos` 取代 route 内联 count；DTO 补 catalogId/eventType/candidateCount/hasHighConfidenceCandidate（同步 BrokenSamplesGrid.test mock）。distinct 复用 `/broken-domains`〔零新端点〕。typecheck/lint EXIT=0 / verify:endpoint-adr 247〔非新 route〕/ test:changed 增量 129 全过 + 新增 8 测 + image-health 全域 35 测回归绿。执行模型: claude-opus-4-8；子代理: 无。详见 changelog [IMGH-P2-1D] | 5 | sonnet | 0B Accepted + 1C（硬串行） | route/service/db 分层 + 单测（筛选 + total 一致 + 新列） |
| **IMGH-P2-2A** ✅ | **共享组件：`ImageCompare`**（admin-ui 新）：当前图 vs 候选新图并排，标注两侧尺寸/比例/可达性（候选客户端探活 + 尺寸校验后才允许确认，§6.2）。Opus 子代理定 Props 契约（current/candidate/onConfirm/校验态/降级）+ 窄屏纵向堆叠 + 颜色零硬编码。**已完成 2026-06-20**：Opus arch-reviewer（claude-opus-4-8, agentId a9732b79ad7128d4d）CONDITIONAL PASS 定 ImageCompare+ImageCandidatePicker 双契约；落 `feedback/image-compare.{tsx,types.ts}`（循 P1 lightbox 范式：受控 open + onLoad 自测尺寸 + slot 逃生口 + ImageStatus 复用）；探活+尺寸校验下沉组件（A-1）+ minWidth/minHeight 默认 200 可覆盖（A-2）+ onConfirm 哑回传尺寸不调 API（A-3）+ aspect 仅标注不入闸门（A-4）+ 窄屏 flex-wrap 堆叠 + 颜色全 token；feedback barrel 导出。typecheck/lint EXIT=0 + 11 组件测试全过 + test:changed 升 admin-ui 全量 1055 全过。执行模型: claude-opus-4-8；子代理: arch-reviewer (claude-opus-4-8)。详见 changelog [IMGH-P2-2A] | 2（admin-ui 契约 + 组件实装） | **opus** | 0A（候选 DTO 定型） | **Opus 子代理 + arch-reviewer PASS + Subagents trailer** + admin-ui 组件测试 |
| **IMGH-P2-2B** ✅ | **共享组件：`ImageCandidatePicker`**（admin-ui 新）：多源候选缩略选图（消费 candidates DTO：缩略 + source pill + confidence 标记 🟢/🟡 + 选中态 + 加载更多入口槽，§5.2/§17.3.5）。Opus 子代理定 Props 契约 + EmptyState/LoadingState 复用 + 颜色零硬编码。**已完成 2026-06-20**：复用 2A 子代理（arch-reviewer claude-opus-4-8 agentId a9732b79ad7128d4d）双契约；落 `feedback/image-candidate-picker.{tsx,types.ts}`；**自有 `ImageCandidateOption` 禁反向 import server-next**（B-1 依赖方向）+ 候选键由消费方计算填 option.key（B-2）+ **confidence 视觉仅由 isWinner 决定 🟢/🟡 不硬编码阈值**（B-3）+ loadMoreSlot 逃生口非 onLoadMore（B-4）+ 复用 EmptyState/LoadingState/ErrorState（B-5）+ renderSourcePill 逃生口（图标消费方注入）+ 颜色全 token；§C 协同记入 3A（消费方持 Map<key,candidate> 取 sourceRef）。typecheck/lint EXIT=0 + 12 组件测试全过 + test:changed admin-ui 全量 1056 全过。执行模型: claude-opus-4-8；子代理: arch-reviewer (claude-opus-4-8)。详见 changelog [IMGH-P2-2B] | 2 | **opus** | 0A + 1A（candidates DTO）+ 2A（硬串行，admin-ui barrel/types） | **Opus 子代理 + arch-reviewer PASS + Subagents trailer** + admin-ui 组件测试 |
| **IMGH-P2-3A** ✅ | **UI 编排：`ImageGovernanceDrawer`**（server-next `_client/`，680px，§6.3/§17.3.3）：① 图片矩阵 ② 破损详情区 ③ 替换区（候选→对比→apply-candidate；或手填→PUT images）④「标记已解决」→ resolve-event ⑤ 行点击 + toast + flash + 刷新。**已完成 2026-06-20**：落 `ImageGovernanceDrawer.tsx`（Drawer right 680px/90vw 编排 ImageCandidatePicker+ImageCompare+ImageLightbox+AdminInput）。数据可用性核查裁定（§17.5）：①矩阵=`useVideoImages` 4 类〔poster/backdrop/logo/banner_backdrop，stills 无 hook 省略〕；③替换聚焦 poster/coverUrl，**§C 持 `Map<optionKey,ImageCandidate>` 取回 sourceRef** 调 applyImageCandidate / 手填复用 `useVideoImages.update('poster')`；④**1D DTO 补 `eventId`**〔1D LATERAL 加 select evt.id，resolve 展示中的单事件，不违反 BLOCK-2〕→ resolveImageEvents；⑤ DataTable `onRowClick` 开抽屉 + `flashRowKeys` 行 flash〔1.5s 自动清〕+ refresh。typecheck/lint EXIT=0 + 8 视图测试〔候选补图/手填/标记已解决/矩阵/破损详情/禁用态〕+ test:changed 145 全过 + image-health client 回归 45 全过。执行模型: claude-opus-4-8；子代理: 无〔模块编排非共享契约〕。详见 changelog [IMGH-P2-3A] | 5 | sonnet | 1A/1B/1C + 2A/2B | 模块内 Drawer 范式 + 视图测试（候选补图 + 手填 + 标记已解决流程） |
| **IMGH-P2-3B** ✅ | **UI：Tab B DataTable 治理工作台增强**（§5.2，全复用 admin-ui 一体化 props）：① 缩略列（Thumb；破损/缺失走 FallbackCover 占位语义）② filter chips（posterStatus/event_type/posterSource via enum + brokenDomain via distinctFetcher，消费 1D）③ bulkActions（批量重扫选中〔1C ids 端点〕/ 批量去编辑深链；**「批量从候选补图」按 0A CONCERN-3 裁定** — ADR 不支持 batch apply 则仅渲染「批量打开候选队列」，**不渲染伪批量**〔无死按钮〕）④ flashRowKeys（替换/重扫成功）⑤「跨源候选数」列（消费 1D `candidateCount`/`hasHighConfidenceCandidate`，🟢高置信/🟡待确认，**不逐行请求避 N+1**）+ 行点击打开 3A 抽屉。**已完成 2026-06-20**：纯 server-next 消费层，全复用 admin-ui 一体化 DataTable props（不改 admin-ui 公开 Props，M8「强制 Opus」前提不成立）。新增 `imageHealthFilters.ts`（buildMissingFilters 翻译 filters Map→1D 入参 + imageHealthDistinctFetcher 哨兵 table 复用 GET /broken-domains）+ `ImageHealthBulkActions.tsx`（批量重扫选中=1C rescanSelectedVideos + 打开候选队列=开首个选中行抽屉，CONCERN-3 无伪批量）；ImageHealthColumns 增缩略列（Thumb，missing 走 fallback / broken·pending 直显 posterUrl）+ eventType 列 + 跨源候选数列（🟢/🟡/—，消费 1D 聚合不逐行请求）+ 4 列 filter 声明（title→search text / posterStatus·posterSource·eventType enum+options / brokenDomain distinctTable）；ImageHealthClient 接 filters/selection state + onQueryChange 捕获 filters（回 page 1）+ distinctFetcher + bulkActions + 行点击开 3A 抽屉（已接）。**偏离**：① enum auto-filter 多选→1D 单值 `z.enum`，翻译取 value[0]（UI-only 卡不改 1D schema）② Thumb 无 onError（admin-ui 无 FallbackCover），缺失走 fallback、破损直显 posterUrl（broken-img 即运维信号）③ brokenDomain distinct 经哨兵 table 复用 /broken-domains 绕开通用 _dt/distinct 白名单。门禁全绿：typecheck/lint/verify:adr-contracts EXIT=0 / verify:endpoint-adr 247（无新 route）/ test:changed 54 + image-health 全域 76 测过；+27 新测（filters 11 + columns 11 + bulk 5）+ ImageHealthClient 更新 fixture+4 用例。e2e N/A（UI 消费卡，无新 route/页面）。执行模型: claude-opus-4-8；子代理: 无。详见 changelog [IMGH-P2-3B]。**解锁 3C 文档收尾。** | 5 | sonnet | 1A/1B/1C/1D + 3A | DataTable v2 一体化（禁 v1 三件套）+ 视图测试（筛选 + bulk + 候选列） |
| **IMGH-P2-3C** ✅ | **文档收尾**：① 手册 `P-image-health.md` Tab B 工作台形态（缩略列/筛选/bulk/候选列/治理抽屉/补图闭环/标记已解决）② `W3-image-fallback.md` 补图与标记已解决步骤 ③ `docs/decisions.md` index + ADR cross-refs ④ frontmatter last_reviewed 刷新。**已完成 2026-06-20**：P-image-health §0 元信息（端点扩 candidates/apply-candidate/resolve-event/rescan-selected + 主任务卡 SEQ-20260619-02）+ §1/§2 布局图补 Tab B 工作台 + 抽屉 + §3.6 工作台（缩略/筛选/候选数/bulk）+ §3.7 治理抽屉补图闭环 + §3.8 标记已解决 + 本次更新条；W3 frontmatter scope 扩 + §2b 单视频精细补图/标记已解决工作流 + §5 ADR 链接；decisions.md ADR-208/209 状态加「已实现」+ changelog/手册 cross-ref。frontmatter last_reviewed → 2026-06-20（两手册）。verify:docs-format 我改 3 文件全在「143 ✅」内（报错均既有遗留 archive/README，EXIT=0 不阻断）；docs-only test:changed 自动跳过。执行模型: claude-opus-4-8（主循环直接做，用户选 A）；子代理: 无。Codex 非代码审核不适用（操作手册+索引，无新设计决策）。详见 changelog [IMGH-P2-3C]。**→ SEQ-20260619-02 全交付。** | 4 | haiku | 3A/3B | docs-only（doc-janitor） + 主循环复核 |

- **P2 范围红线（不得越界）**：每个新 admin route **先 ADR 后实施**（`verify:endpoint-adr` 必过）；新共享组件契约 **强制 Opus 子代理 + arch-reviewer + Subagents trailer**；schema 变更 **同步 architecture.md**；**无死按钮**（无可执行端点不渲染）；颜色零硬编码（design-tokens）；DataTable v2 一体化（禁 v1 三件套）；后端严守 Route→Service→DB 分层；**禁复用全局 rescan 伪装选中批量**（§17.3.1）；筛选必与服务端分页同源（§17.3.2）。
- **P2 不做（明确推迟 P3）**：分级自愈 worker（§8）/ image_governance_status 推导 + degraded 枚举（§9）/ Dashboard 真实端点（§10.2①）/ admin-shell 待办 drawer（②）/ 阈值告警（③）。candidates 实时 TMDB 多图拉取本序列内推迟（标 optional follow-up，§7.2）。
- **Codex 对抗性审核（2026-06-19，序列登记落盘后/commit 前，范围 11 卡 ≥ 3 项必须）**：codex exec read-only / gpt-5.5 / 5136 行日志。裁决 **BLOCK → 已逐条修订消解**（主循环 Opus 亲验 BLOCK-1/2/3 事实全部确认成立）：
  - **BLOCK-1 已消解**：误判 audit `action_type`/`target_kind` CHECK 扩展 migration。实证 `admin_audit_log.action_type` 是 `TEXT` **无 CHECK**（052 `:27-32` 仅 target_kind 有 CHECK）、`target_kind='image_health'` 已存在（`admin-moderation.types.ts:293`）→ **零 migration**，改扩 `AdminAuditActionType` TS 枚举（1B/1C）。1A 去 migration（5→4 项）。
  - **BLOCK-2 已消解**：resolve-event 薄端点入参 `{eventId\|videoId}` 越界。实证 `resolveImageEvents(db, ids:string[], note?)`（`imageHealth.scan.ts:161`）只收事件 id 数组 → 改 `{eventIds: string[], note?}`；videoId 维度须另拆业务端点（0B/1C）。
  - **BLOCK-3 已消解**：治理抽屉缺 `catalogId` 契约。实证 `MissingVideoRow`（`api.ts:31`）只有 `videoId` 无 `catalogId`、query（`imageHealth.ts:313`）未 SELECT `v.catalog_id` → 3A 行→抽屉无法调 candidates。1D 补 `catalogId` DTO + SELECT（0B/1D/3A）。
  - **BLOCK-4 已消解**：3B 候选数列无后端数据源（逐行请求 N+1 / 否则死列）。1D 补 `candidateCount`/`hasHighConfidenceCandidate` 聚合（0B/1D/3B）。
  - **BLOCK-5 已消解**：1C/1D 与 1A/1B「可并行不同文件」错误。四卡均改 `api.ts` + image-health route + imageHealth queries → 改 **1A→1B→1C→1D 硬串行**。
...
| 卡 | 内容 | 范围项 | 建议模型 | 依赖 | 门禁 |
|---|---|---|---|---|---|
| **IMGH-P3-1A** ✅（f804de79 / claude-opus-4-8 / ADR-210 arch-reviewer+Codex done） | **后端：recent-broken-samples 端点 + ADR-210**：① 起 **ADR-210**（recent-broken-samples 端点契约：D-210-1 破损样本口径切到 `broken_image_events` 事件流〔废弃 poster_status='broken' client 过滤〕/ D-210-2 限定 `image_kind='poster'`〔2:3 海报位〕/ D-210-3 `DISTINCT ON (video_id)` 去重）② `getRecentBrokenSamples(db,limit)` query 落 `imageHealth.scan.ts`（避恶化 imageHealth.ts 639 行已超限；基于 broken_image_events `resolved_at IS NULL AND image_kind='poster'`，DISTINCT ON video_id 取最近 last_seen_at，JOIN videos+media_catalog，broken_domain SQL `regexp_replace` 统一口径）③ ImageHealthService 方法（守分层）④ `GET /admin/image-health/recent-broken-samples` route（admin-only，limit clamp 1-50 默认 24，只读无审计，对齐 stats/broken-domains 范式）⑤ 端点单测 + imageHealth.ts re-export | 5 | **opus** | 无 | **ADR-210 先行 + Opus arch-reviewer PASS + Subagents trailer** + Codex 对抗性审核（ADR） + `verify:endpoint-adr` + 端点单测 |
| **IMGH-P3-1B** ✅（f804de79 / claude-opus-4-8） | **前端：破损样本区改独立数据源**：① `lib/image-health/api.ts` 新增 `BrokenSampleRow` 类型 + `getRecentBrokenSamples(limit)` fetcher ② `BrokenSamplesGrid` props 改 `BrokenSampleRow[]`，**去客户端 `posterStatus==='broken'` 过滤**（行即破损样本，保留 MAX_SAMPLES slice + Lightbox meta）③ `ImageHealthClient` overview 并行加载独立 `brokenSamples` state（不再从 missingRows 借数据传入）④ 组件测试更新（BrokenSamplesGrid + ImageHealthClient fixture） | 4 | sonnet | 1A（端点 + DTO 先行） | 模块内消费范式 + 组件测试 + `test:e2e:admin` |
| **IMGH-P3-2** ✅（f804de79 / claude-opus-4-8） | **健康概览 KPI 卡片信息密度增强（用户会话内插入，补登记）**：① 卡①「视频总数（已发布）」→「图片正常视频」：已发布+全部双口径「封面 ok / 视频数 + 覆盖率」②「Poster/Backdrop 覆盖率」两卡合并「图片覆盖率」：封面/背景/台标/Banner 4 类各 已发布%/全部% ③ KPI 网格 4→3 卡（留近 7 日破损）。**口径**：卡①健康=封面 `poster_status='ok'`（用户 AskUserQuestion 选定）。**实现**：`getImageHealthStats` 改单次扫描 `COUNT(*) FILTER(...)` 双口径×4 类；`ImageHealthStats` 重构 `{published,all,brokenLast7Days}`（保留顶层 brokenLast7Days/NavCountsService 依赖）；改现有 `/stats`〔非新 route〕；新建 `ImageHealthKpiCards.tsx` 复用共享 `KpiCard`〔`value:ReactNode` 槽，**不改 admin-ui Props**〕；ImageHealthClient 591→541 收敛；复合 value 根用 `display:grid` `<span>` 避 `<p>` 内 `<div>` 非法嵌套。**已完成 2026-06-20**（代码+单测，待 commit）：typecheck/lint EXIT=0 / test:changed 22 文件 192 测全过 / 集成测试新契约（preflight 跑）。执行模型: claude-opus-4-8（用户交互直驱，建议 sonnet）；子代理: 无。 | 5 | sonnet | 无（同分支，逻辑独立于 1A/1B） | typecheck/lint/test:changed；无新 route（`verify:endpoint-adr` 不触发）/ 无 admin-ui Props 改（M8 不触发）/ 无新 ADR |
| **IMGH-P3-4A** ✅（2026-06-20 / claude-opus-4-8 / arch-reviewer PASS） | **后端：problem-images 端点（ADR-211 Accepted）**：① `getProblemImages(db, kind, scope, offset, limit)` query 落 `imageHealth.scan.ts`——口径 D-211-2 `<kind>_url IS NOT NULL AND btrim(url)<>'' AND (status<>'ok' OR EXISTS 未解决真坏事件 image_kind=$kind ∧ event_type∈白名单)`；**kind→列名经白名单 Record/CASE 禁裸插值**；LATERAL 取最近真坏事件；派生 `problemReason`（broken_event>broken>low_quality>pending）默认排序真坏在前 ② `getProblemImageCounts(db, scope)` 4 类 COUNT ③ `ImageHealthService` 方法（守分层）④ `GET /admin/image-health/problem-images` route（admin-only / 只读无审计 / kind·scope·offset·limit zod 收口，默认 kind=poster / limit clamp 1-100 默认 48）⑤ `ProblemImageRow` DTO + 端点单测（口径/url 守卫/注入防护/reason 排序/counts） | 5 | **opus** | ADR-211 Accepted（✅） | **Opus arch-reviewer PASS（新 route）+ Subagents trailer + `verify:endpoint-adr` + 端点单测** |
| **IMGH-P3-4B** ✅（2026-06-20 / claude-opus-4-8 / 门禁全绿 typecheck·lint·test:changed 199·verify:endpoint-adr 133·e2e:admin 84） | **前端：问题图片可视化治理板 + recent-broken-samples 退役**：① `ImageHealthProblemBoard`（4 类 Segment tab 默认封面 + 已发布/全部 Segment + 网格 + 加载更多）② `ProblemImageCard`（缩略=真实URL / `<img onError>`→`--state-error-border` 失败态〔不复用 Thumb〕/ hover 详情浮层〔secondary source 空字段隐藏〕/ 标题取代域名 / 点击开 `ImageGovernanceDrawer` 定位 kind）③ 全宽布局 + TOP 破损域名下移 ④ offset 漂移三缓解（videoId+kind 去重追加 / 治理后刷新 counts / 切 tab·scope·筛选重置）⑤ problemReason 分色 + 状态子筛选 ⑥ **退役 checklist 同 commit**：删 recent-broken-samples route + `getRecentBrokenSamples`(scan/re-export/service) + `api.ts` fetcher + `BrokenSamplesGrid`+测试 + ADR-210 端点表行 ⑦ 组件测试 | 5 | sonnet | 4A（端点 + DTO） | 模块内消费范式 + 组件测试 + `test:e2e:admin`；不改 admin-ui Props；颜色零硬编码 |
| **IMGH-P3-4C** ✅（2026-06-20 / claude-opus-4-8 / 报告 `docs/research/imgh-p3-4c-safeimage-coverage_20260620.md`；核出唯一漏网点 DailyAnimeRow:97 → 登记 4D 修复卡） | **核查：前台 SafeImage 零裂图覆盖面（Codex LOW-2/L-4）**：核查 `apps/web-next` **所有**公开封面/图片入口是否都走 `SafeImage`（含 `SafeImageNext` onError→FallbackCover 行为）；列出裸 `<img>`/直挂 URL 的漏网点 → 这是目标「用户端零裂图」真正闭环点。仅核查 + 出清单（修复另起卡） | 3 | sonnet | 无（与 4A/4B 解耦） | 核查报告（落 docs/research 或 changelog）+ 漏网点清单 |
| **IMGH-P3-4D** ✅（2026-06-20 / claude-opus-4-8 / 4C 派生 / typecheck·lint·test:changed 5；前台零裂图全闭环） | **修复：DailyAnimeRow 裸 img → SafeImage（4C 核出唯一前台裂图漏网点）**：`apps/web-next/src/components/home/DailyAnimeRow.tsx:97` 裸 `<img src={item.coverUrl}>` 改 `SafeImage`（mode lazy / `fallback={{ title, type:'anime', seed }}`），对齐 BrowseCard 范式 → 前台用户端零裂图全闭环（失败/空封面走 FallbackCover）。**不接 reportBrokenImage**（DailyAnime 为 Bangumi calendar 外部源、非站内 media_catalog，broken_image_events 需 video_id 语义不符）+ 组件测试断言 coverUrl=null→FallbackCover(role=img) | 2 | sonnet | 无（4C 报告已定位） | 模块内消费范式 + 组件测试 + typecheck/lint；颜色零硬编码 |
...
- **状态**：🔄 进行中（**方向转向 dissolve**：ADR-212 patch 路线〔event_class 分行 + migration 049，经 3 轮 Codex + 2 轮 arch-reviewer〕**复杂且脆弱** → 用户批准 **方案 C（dissolve）** → spawn **arch-reviewer (claude-opus-4-8, a06695fa2c0aa033c)** 独立设计 **CONDITIONAL-PASS**〔正确性 #1 + 边界复用 #2 显著优于 patch；7 修订全吸收〕→ **ADR-213 落盘** `docs/decisions.md`〔D-213-1~9，决策真源〕+ **ADR-212 转 Rejected/Superseded by ADR-213**〔保留 D-212-9 吸收 + patch 论证审计〕；**Codex 对抗审核 round-1 (a1d0700349d19909a) needs-attention → 3 HIGH+2 MEDIUM+1 LOW 全吸收**〔URL 同源守卫 / stale-ok 升 P4-S 前置 + 可观测面 / 信号列仅 4 kind / 双写 warn / recall 取舍 / total 回归测〕→ **Draft v2** → **Codex re-review round-2 needs-attention：2 HIGH+1 MEDIUM**〔均 round-1 修正的传播/一致性缺口：回填缺 URL 守卫 / 瞬态刷 checked_at 拆 stale 网 / stale-ok 未并入单一谓词〕**全吸收** → **Draft v3** → **Codex re-review round-3：1 HIGH（同类传播缺口第 3 次，6→3→1 收敛）全吸收**〔checked_at 回填改「不用通用 updated_at、留 NULL→unknown + 一次性真实扫描排空」〕→ **Draft v4** → **Codex re-review round-4：1 HIGH = rollout 时序缺口**〔一次性扫描放 0M 早于写 checked_at 的 A〕**全吸收**：时序改 `0M→A→A-SCAN门→C`、C 硬依赖扫描完成 → **Draft v5 → ADR-213 Accepted（2026-06-22，用户裁定设计定稿：数据模型 rounds 1–3 封板，剩余 rollout 细节交实施卡用代码+测试落实）**；**P4-0 gate ✅ + P4-0M 收口 ✅（migration 121 真库回填演练 PASS：poster seeded=1=expected/misseed=0/missed=0/checked_at 全 NULL，见 changelog [IMGH-P4-0M]）+ P4-A 代码交付 ✅（commit 968d4efb·worker 写 checked_at + 判别式 + A-SCAN 能力；**A-SCAN 待部署期跑** run-imgh-ascan.ts 落真值——C 硬前置门）+ P4-B 交付 ✅（beacon 信号列双写 + URL 守卫，779ba438）+ P4-S 交付 ✅（图片健康周期巡检 scheduler，e7ecd8bd + jobId 修复 49eea719）+ **P4-C 交付 ✅**（读端单真源 `problemFilterSqlV2` + events 退出读路径 + `client_error` 窗口 + `unknown` DTO + 前端分色，本提交）**，时序 0M→A→A-SCAN门→C **代码全交付**。**A-SCAN 部署门用 flag 落地**：依赖 `checked_at` 的 stale-ok→unknown 道由 `IMAGE_HEALTH_STALE_OK_ENABLED`（默认 OFF）门控——核心 dissolve 修复（7 误报上线即消失）flag-OFF 即生效；A-SCAN 排空后运维置 `=true` 开 unknown 面。**SEQ 代码层全收口**（含 Codex stop-gate 修订 D-213-10：migration 122+123 触发器——URL 替换即重置全 url 派生列〔`<kind>_client_error_at`+`<kind>_checked_at`+`<kind>_status`〔尊重显式写〕+blurhash/primary_color/dimensions〕，消「信号/status/占位残留旧 URL → 已替换图被误判」）。剩纯运维：① 部署 → migrate **121+122+123** → 跑 `run-imgh-ascan.ts` → `verify-imgh-121` 确认 unknown 收敛 → 置 `IMAGE_HEALTH_STALE_OK_ENABLED=true`；② prod 置 `IMAGE_HEALTH_SCHEDULER_ENABLED=true` 起周期巡检。两 flag 开后健康板宣称「零漏检/完备」。**建议合并前补跑 `test:e2e:admin` + 集成测 `image-health-url-change-trigger`（需 DB）**。）
- **创建时间**：2026-06-21｜**最后更新**：2026-06-21
...
- **状态**：🔄 Phase 0 ✅（commit `4f748b47`）→ Phase 1 进行中：**CARD-SIZE-DB ✅**（2026-06-22 migration 124 建表 + seed 3 行 + audit 17→18 + architecture.md §5.19 + DB 级倒置行集成测 12/12；门禁 typecheck/lint/test:changed/verify:adr-contracts=0 + migrate 冷启动 + 集成测全绿 / commit 见 changelog [CARD-SIZE-DB]）→ **CARD-SIZE-TYPES-QUERIES ✅**（2026-06-22 `@resovo/types` card-size.types〔CardSizeClass/CardSizeSettings/UpdateCardSizeSettingsInput/CARD_SIZE_DEFAULTS〕+ index value re-export + `db/queries/card-size-settings.ts`〔list/find/update 仿 home-section〕+ seed 一致性单测 4 测〔SQL seed == CARD_SIZE_DEFAULTS，D-214-5〕；门禁 typecheck/lint=0 + test:changed 597/8167〔types 基础包升全量〕 + verify:adr-contracts=0 / commit 见 changelog [CARD-SIZE-TYPES-QUERIES]）→ **CARD-SIZE-SERVICE-ADMIN ✅**（2026-06-22 CardSizeService〔list 枚举序 + update before/after audit〕 + admin `GET /admin/card-sizes` + `PUT /:sizeClass`〔ADR-215〕 + server 注册 + audit `card_size.update` targetId=row.id + AuditLogService TARGET_KINDS/ACTION_TYPES 4 镜像扩展 + zod 倒置 body 守卫〔grid/scroll 双 schema `.strict()`〕 + 路由测 13〔倒置 body/范围/枚举/404/audit payload 断言〕；含 ADR-215 端点表补 `#` 列令 verify:endpoint-adr 可解析；门禁 typecheck/lint=0 + test:changed 598/8182 + verify:adr-contracts=0〔endpoint-adr 250 路由对齐〕 / commit 见 changelog [CARD-SIZE-SERVICE-ADMIN]）→ **CARD-SIZE-PUBLIC-CACHE ✅**（2026-06-22 公开 route `GET /card-sizes`〔无鉴权 Redis 读穿缓存 key card-sizes:v1 + TTL 60s〕 + CardSizeService 构造接 redis + `getPublicCardSizes` 读穿 + `invalidatePublicCache` PUT 后 `redis.unlink` best-effort 不上抛〔D-215-6 / Codex-R3〕 + server 注册 + 测 public 3〔hit/miss+setex/无鉴权〕 + admin +2〔unlink 调用/失败仍 200〕；门禁 typecheck/lint=0 + test:changed 18 + verify:adr-contracts=0 / commit 见 changelog [CARD-SIZE-PUBLIC-CACHE]）→ **Phase 1 全 4 卡交付 ✅**（DB → TYPES-QUERIES → SERVICE-ADMIN → PUBLIC-CACHE）→ **Phase 2 解锁**（下一卡 CARD-SIZE-SSR：`lib/server/card-size-fetch.ts` server-only 取数〔revalidate ≤60s〕 + `[locale]/layout.tsx` 注入 `:root` `<style>` + 失败降级 CARD_SIZE_DEFAULTS，建议模型 sonnet）。Phase 1 门禁较原登记 +3 测（DB 倒置行〔CARD-SIZE-DB ✅〕/ 网格溢出视觉回归〔CARDGRID〕/ SSR 渲染页新鲜度 e2e〔E2E〕）。｜ **创建时间**：2026-06-22 ｜ **最后更新时间**：2026-06-23｜ **CARD-SIZE-SSR ✅**（2026-06-23 新建 `apps/web-next/src/lib/server/card-size-fetch.ts`〔server-only `GET /card-sizes` + `next.revalidate=60`〔D-214-9 新鲜度有界〕/ 失败 catch→`CARD_SIZE_DEFAULTS` 降级 + `serverLogger.warn` 非空 catch〔D-214-6〕/ 纯函数 `buildCardSizeRootCss` 按档位单位派生 `:root` 变量〔网格 `--card-cols-{class}-desktop`+`--card-gap-{class}` / scroll `--card-w-scroll`+`--card-gap-scroll`〕+ `Number()` 防注入〕 + `[locale]/layout.tsx` children 前注入 `<style data-card-size-vars>`〔消 FOUC〕 + 单测 6〔成功 URL+revalidate60 / 非2xx·抛错·空data 三降级 / 网格出 cols+gap·scroll 出 w+gap·无倒置变量 / 降级==CARD_SIZE_DEFAULTS〕；门禁 typecheck=0/lint=0/test:changed〔vitest --changed 6 passed〕/verify:adr-contracts=0 + **web-next build `Compiled successfully` 验证 pino 入 Next bundle 零 warning**〔build 静态生成阶段 /500 `<Html>` 报错经 stash 差分确认为既有/环境问题、与本卡无关〕 / commit 见 changelog [CARD-SIZE-SSR]）→ **CARD-SIZE-CARDGRID ✅**（2026-06-23 新建共享 `apps/web-next/src/components/shared/card-grid/CardGrid.tsx`〔`sizeClass: Exclude<CardSizeClass,'scroll'>` 封闭枚举、禁自由 gridCols；纯 className 透传，无 inline style〕 + globals.css `.card-grid` 响应式类〔`--cg-cols` 计数级联 mobile=2/≥640=3/≥1024=`var(--card-cols-{class}-desktop)`；`repeat(var(--cg-cols,2),minmax(0,1fr))` 缺值兜底退 2 列不坍塌；`> *{min-width:0}` 防溢出 D-214-4/Codex-R2；gap `var(--card-gap-{class})`〕 + 单测 9〔组件 class/className/data-testid 透传/children 5 + globals.css 契约 minmax/min-width:0/桌面 var/gap 4〕；**arch-reviewer (claude-opus-4-8) CONDITIONAL PASS**——吸收 BLOCKER〔inline→CSS 类 Scheme A〕+ 2 HIGH〔缺值兜底 + 后继真源声明〕+ 2 MEDIUM〔`--cg-cols` 命名 + 删伪断言〕；偏离登记 D-214-7〔`sizeClass` 收紧 Exclude scroll + 引入中间变量 `--cg-cols`〕；门禁 typecheck=0/lint=0/test:changed〔vitest --changed 9 passed〕/verify:adr-contracts=0 / commit 见 changelog [CARD-SIZE-CARDGRID]）→ **CARD-SIZE-SCROLL ✅**（2026-06-23 横滚行 Shelf〔PosterTrack/Top10Track/HorizontalTrackSkeleton〕/TopTenRow〔Top10Track/TrackSkeleton〕/DailyAnimeRow 卡宽 `--shelf-card-w-portrait`·`--shelf-card-w-top10`→`--card-w-scroll`、横滚 track gap `--shelf-gap`→`--card-gap-scroll`；FeaturedGrid 网格路径 gap 保留 `--shelf-gap`〔属 FEATURED-NORMALIZE〕；默认值相同〔170px/16px〕故视觉中性、变 DB 可配；无兜底依赖 SSR 恒注入〔scroll 行专属 [locale] 首页〕；门禁 typecheck=0/lint=0/test:changed〔vitest --changed 13 passed：ShelfRow/DailyAnimeRow/HomeBrandFiltering 横滚回归无破〕/verify:adr-contracts=0 / commit 见 changelog [CARD-SIZE-SCROLL]）→ **CARD-SIZE-VIDEOCARD-VARIANT ✅**（2026-06-23 VideoCard 加 `interaction?:'takeover'|'navigate'`〔默认 takeover → 现有消费方〔TopTenRow/FeaturedRow/Shelf/VideoGrid〕零行为变更〕外壳分流两**独立**组件：takeover 保留 usePlayerStore/useRouter/FloatingPlayButton 海报 Fast Takeover 直达播放器〔DOM 逐字保留防回归〕、**navigate 整卡 `<Link>` 纯跳转详情、严禁 usePlayerStore/useRouter/不渲染 FloatingPlayButton**〔D-214-7 P2〕；提取共享子件 VideoCardCover/VideoCardMeta/PosterHoverDim 消重；两分支根 `data-testid=video-card` + `data-interaction` 判别；VideoCard.test 扩 8 测〔默认=takeover / 显式 takeover 行为一致 / navigate 根 `<a>` href 详情 / 恰 1 link〔HIGH-1〕/ 无 button+FloatingPlayButton〔P2〕/ 点击不调 enter·push / text-decoration:none〔HIGH-2〕/ 内容对等〕；**arch-reviewer (claude-opus-4-8) CONDITIONAL PASS**——吸收 3 HIGH〔navigate 单 link 守卫 + textDecoration:none 对齐 BrowseCard + 拆独立组件断 store 订阅〕+ MEDIUM〔Cover 留 group/poster 作用域不断裂、`bg-black/40` 技术债备注〕；偏离登记 D-214-7〔interaction 设可选默认 takeover + 导出 type 别名 VideoCardInteraction + data-interaction 属性〕；保留 BrowseCard/browse-card testid〔归 BROWSE-MIGRATE〕；门禁 typecheck=0/lint=0/test:changed〔vitest --changed 43 passed，VideoCard 19〕/verify:adr-contracts=0 / commit 见 changelog [CARD-SIZE-VIDEOCARD-VARIANT]）→ **CARD-SIZE-BROWSE-MIGRATE ✅**（2026-06-23 BrowseGrid 主网格 + skeleton `grid-cols-2 sm:3 lg:5` → `<CardGrid sizeClass="standard">`〔列数/gap 默认 5/16 与原口径一致 → 视觉中性、转 DB 可配〕 + 卡 `<BrowseCard>` → `<VideoCard interaction="navigate">`〔href 经 getVideoDetailHref 不变〕 + **删 BrowseCard.tsx**〔零消费方〕 + e2e spec testid `browse-card`→`video-card`〔browse-category-routes / browse-tvshow，含注释〕；**修 vitest.config.ts** `@/components/shared/*` 别名 importer-aware〔CARDGRID 起 web-next 有真实 shared 层、原 CUTOVER 无条件硬路由 server-next 已不成立且零消费方；server-next importer→server-next、默认→web-next〕；BrowseGrid.test 补 fixture `subtitleLangs`/`posterStatus`/`posterBlurhash` + `matchMedia` stub〔改渲 VideoCard→TagLayer/StackedPosterFrame 需要〕；门禁 typecheck=0/lint=0/**全量单测 601 文件 8210 测全过**〔config 改动按 ADR-180 升全量〕/verify:adr-contracts=0；e2e〔分类路由可达 + tvshow href 映射〕testid 已对齐、playwright 实跑随 CARD-SIZE-E2E〔Phase 4，同 CARDGRID/SCROLL 口径〕 / commit 见 changelog [CARD-SIZE-BROWSE-MIGRATE]）→ **CARD-SIZE-FEATURED-NORMALIZE ✅**（2026-06-23 ① FeaturedRow FeaturedGrid + skeleton `1.6fr 1fr 1fr 1fr` 首列大卡异宽 → `<CardGrid sizeClass="standard">` 等宽〔默认 5 列/16px〕；删 MIN_SLOTS sparse-fill 空占位〔等宽 minmax(0,1fr)+CardGrid `>*{min-width:0}` 结构消除挤垮、占位与 DB 可配列数不兼容〕；fetch `limit=4`→`limit=5`〔FEATURED_SLOTS 对齐 standard 默认列〕。② Shelf 删 `Top10Track`/`FeaturedGrid`/`EmptyPlaceholderCardGrid` 死函数 + 死分支〔3 消费方全 poster-row、TOP10 用独立 TopTenRow〕；`ShelfTemplate` 收窄 'poster-row'、dispatch 直 PosterTrack；template prop 保留兼容 8 调用点。③ RelatedVideos 删 `grid` 死分支 + `RelatedVideosSkeleton`〔零消费〕+ VideoGrid import；移除 `variant` prop 恒 sidebar；VideoDetailClient 去 `variant="sidebar"`。净删 -161 行；featured-row-sparse.spec 注释更新〔断言对等宽 1/5 宽仍成立〕；门禁 typecheck=0/lint=0/test:changed〔ShelfRow/DailyAnimeRow/HomeBrandFiltering 13 测过〕/verify:adr-contracts=0；首页/详情 playwright e2e 随 CARD-SIZE-E2E / commit 见 changelog [CARD-SIZE-FEATURED-NORMALIZE]）→ **🎉 Phase 2 全 6 卡交付 ✅**（SSR → CARDGRID → SCROLL → VIDEOCARD-VARIANT → BROWSE-MIGRATE → FEATURED-NORMALIZE：全站卡片接入 DB 注入 CSS 变量 + 统一 CardGrid〔standard/compact〕/横滚变量 + VideoCard takeover/navigate 双变体，FeaturedRow/BrowseGrid 归一等宽、死路径〔Shelf top10/featured 模板 · BrowseCard · RelatedVideos grid〕清退）→ **剩 Phase 3/4**：① **CARD-SIZE-ADMIN-UI**〔server-next `_tabs/CardSizeTab.tsx` 3 档表单〔网格档列数+gap / scroll 档卡宽+gap，校验镜像 zod〕 + SettingsContainer「前台展示」Tab 注册，sonnet〕；② **CARD-SIZE-E2E**〔SSR 注入值→视觉链路 + admin PUT→公开读新鲜度 e2e〔D-214-9 R3〕 + 网格窄容器/长标题视觉回归〔D-214-4/7〕 + 全栈回归 test 全量 + test:e2e 4 projects，sonnet〕。→ **CARD-SIZE-ADMIN-UI ✅**（2026-06-23 Phase 3：server-next「前台展示」Tab。新建 `lib/card-size/api.ts`〔`listCardSizes`→`GET /admin/card-sizes` / `updateCardSize`→`PUT /:sizeClass`，导出 Grid/ScrollCardSizeBody 可编辑投影 D-215-2，仿 home-curation/api〕 + `lib/card-size/validation.ts`〔`CARD_SIZE_BOUNDS` 列数[2,8]/卡宽[120,280]/gap[0,64] + `validateCardSizeField` —— **plain 常量镜像服务端 zod**（CardSizeService Grid/Scroll BodySchema）+ DB CHECK，**不引入 zod 依赖**〔server-next 无 zod、对齐 sibling tab inline 校验约定、避免技术栈外依赖；服务端 zod `.strict()`+DB CHECK 仍权威〕〕 + `_tabs/CardSizeTab.tsx`〔取数渲 3 档：standard/compact 网格档=桌面列数+gap、scroll 档=卡宽+gap；**每档独立 save**对齐 PUT/:sizeClass 粒度+单档 audit；越界禁 save + AdminInput error 态 + hint 红字；dirty/reset/reload + toast 仿 LoginSessionsTab；复用 AdminCard/AdminButton/AdminInput/ErrorState/LoadingState/useToast 零新组件；NumberField/CardSizeClassCard page-local 消重〔仅本 tab 用、未达 3 处提取信号〕〕 + `SettingsContainer` 注册〔TabId `card-size` + TABS「前台展示」+ import + tabpanel 分支，additive 不动既有 8 tab〕 + 单测 6〔渲染+testid / 三档渲染+字段初值 / 改列数→dirty→updateCardSize 网格 body / 越界 10>8 禁 save+「范围 2–8」文案+不调 update / scroll 渲卡宽 aria-label「卡片宽度」非列数 / 加载失败 ErrorState〕。**偏离登记**：原 task-queue 登记「校验镜像 zod」——实现以 plain 常量镜像服务端 zod 边界而非引入 zod（实现手段选择，契约未变；理由见上）。门禁 typecheck=0/lint=0〔4 successful，本卡零告警〕/test:changed〔vitest --changed CardSizeTab 6 passed〕/verify:adr-contracts=0〔endpoint-adr 对齐，无新 admin route〕；ADMIN e2e playwright 实跑随 Phase 4 CARD-SIZE-E2E〔纯增量 tab 不动既有路由〕 / commit 见 changelog [CARD-SIZE-ADMIN-UI]）→ **剩 Phase 4**：**CARD-SIZE-E2E**〔SSR 注入值→视觉链路 + admin PUT→公开读新鲜度 e2e〔D-214-9 R3〕 + 网格窄容器/长标题视觉回归〔D-214-4/7〕 + 全栈回归 test 全量 + test:e2e 4 projects，sonnet〕。→ **CARD-SIZE-E2E ⏸️ spec 交付 + 可跑门禁全绿**（2026-06-23 Phase 4：新建 `tests/e2e-next/card-size-grid.spec.ts`〔4 测：① SSR 注入 `<style data-card-size-vars>` + `:root` 6 变量〔standard 5/16·compact 3/12·scroll 170/16〕+ 无倒置变量 D-214-6/9/10；② featured-grid 桌面 5 列 + gap 16px 消费 DB 变量 D-214-7；③ 长标题不撑破轨道·grid 无水平溢出 D-214-4 min-width:0；④ 窄视口降 2 列响应式〕；仿 typography-layout/featured-row-sparse 既证范式〔同 _fixtures + route mock〕，仅依赖 web server、SSR 降级值==seed 故断言稳定。门禁 typecheck=0/lint=0/**全量单测 602 文件 8216 测全过**〔PHASE COMPLETE 兜底节点〕/verify:adr-contracts=0〔250 路由对齐、无新 route〕。**⚠️ e2e 实跑环境阻塞**：worktree 缺 `.env.local`〔gitignore 本地文件不随 worktree 复制〕→ apps/api/web-next/server-next dev server 均起不来 → playwright 不可跑；`npm run test:e2e`〔4 projects〕+ admin PUT→公开读新鲜度端到端〔D-214-9 R3 mutation 侧，契约层已由 card-size-admin/public 单测覆盖〕**须在主 checkout/CI 跑 = 合并 main 前 e2e gate 节点**〔CLAUDE.md「合并 main 前必跑 test:e2e」〕。spec 不占单 🔄 槽 / commit 见 changelog [CARD-SIZE-E2E]）→ **🎉 SEQ-20260622-03 代码全交付**（Phase 0–4：ADR×2 / DB / types / service / public-cache / SSR / CardGrid / scroll / VideoCard / browse / featured / admin-ui / e2e-spec —— DB 驱动可配卡片尺寸体系 + 后台编辑 UI + 前台统一 CardGrid/VideoCard 全栈闭环）。**剩唯一前置门**：合并 main 前 `npm run test:e2e` 全量 + 视觉回归实跑（具完整环境处）。→ **CARD-SIZE-ADMIN-PREVIEW ✅**（2026-06-23 Phase 3 增强 / 用户反馈：「前台展示」Tab 仅表单缺实时预览 → CardSizeTab 每档卡内嵌 `CardSizePreview` WYSIWYG 占位网格，消费 **draft 值**实时重渲〔网格档 `repeat(列数,minmax(0,1fr))`+gap 渲列数个 2:3 占位、scroll 档 flex 横排定宽占位×6〕；**后台自包含不跨 app 引前台 CardGrid/VideoCard**；占位主题 CSS 变量零硬编码色；越界照实渲染+NaN 防御；单测扩 2〔标准档 grid-template-columns 反映 draft 列数+改值更新 / scroll 横滚预览占位卡宽反映 draft〕共 8 测。门禁 typecheck=0/lint=0/test:changed〔CardSizeTab 8 passed〕/verify=0 / commit 见 changelog [CARD-SIZE-ADMIN-PREVIEW]）。当前无活跃卡。
...
- **状态**：✅ 全序列交付 + 合并 main `01b32abf`（2026-06-24；详见本行末尾 + changelog [CARD-SIZE-A1A2-GATE]）｜ 执行过程：（**Phase 0 ✅** ADR Amendment A1 Accepted + Codex 审 → **卡片尺寸线批次 ✅** #1A SCHEMA + #1B API + #3 GRID-CSS + #4 TAB〔类型耦合批次：types 删 compact + 单位翻转破坏全栈测试，作一个批次统一 commit；typecheck=0/lint=0/全量单测 602 文件 8221 测/verify=0 全绿，commit 见 changelog [CARD-SIZE-A1-SCHEMA/API/GRID-CSS/TAB]〕 → **横滚线** #2 ScrollRow ✅〔arch-reviewer (claude-opus-4-8) CONDITIONAL PASS：2 HIGH〔a11y role/tabindex/aria-label + Fragment 边界〕+ 2 MEDIUM + 1 LOW 全吸收，commit 见 changelog [CARD-SIZE-A1-SCROLLROW]〕 → #5 详情拆侧栏 ✅〔拆 320px 双栏→全宽横滚 + RelatedVideos 退役 SidebarList→ScrollRow+VideoCard(navigate) + 删 detail-sidebar 死 CSS，typecheck/lint=0 + related-videos 4 测，commit 见 changelog [CARD-SIZE-A1-DETAIL]〕 → #6 播放页横滚 ✅〔watch page.tsx PlayerShell 下方复用 RelatedVideos 横滚，typecheck/lint=0 + 相关测 20 绿(npx vitest)，commit 见 changelog [CARD-SIZE-A1-WATCH]〕 → #7 e2e spec ✅〔card-size-grid.spec size-driven 重写：standard --card-w-standard + compact 无残留 + auto-fill 卡宽断言，typecheck/lint=0；实跑环境阻塞登记合并 gate，commit 见 changelog [CARD-SIZE-A1-E2E]〕。**🎉 SEQ-20260623-01 代码全交付（#0–#7）** → **✅ 合并 main `01b32abf`（2026-06-24）**：合并 gate `chore/card-size-a1a2-gate-20260623` 全栈实跑 typecheck/lint/verify=0 · 全量单测 604/8235 · migration 125+126 冷启动 · test:e2e 180 passed/0 failed；gate 修复 `7e02d0e4`〔card-size-grid ⑤ 改测「窄视口卡宽恒=W」〕。⚠️ 遗留：A2「手机 2 列」受 `app-shell min-width:1200`〔globals.css §7.2 桌面优先〕未兑现，须另起架构任务；视觉回归 baseline 未入库待补。详见 changelog [CARD-SIZE-A1A2-GATE]。）
- **创建时间**：2026-06-23
- **最后更新时间**：2026-06-23
...
- **状态**：✅ 全序列交付 + 合并 main `01b32abf`（2026-06-24）。A2-1~A2-4 代码 + A2-5 e2e spec 经合并 gate `chore/card-size-a1a2-gate-20260623` 全栈实跑：全量单测 604/8235 · migration 126 冷启动〔card_size_settings → 单行 global/160/16，删 desktop_columns〕· test:e2e 180 passed/0 failed。gate 修复 `7e02d0e4`〔仅测试层、生产代码未动〕：card-size-grid ⑤ 改测「窄视口卡宽恒=W」、browse-tvshow `MOCK_TVSHOW` 补 subtitleLangs/posterStatus/posterBlurhash。**⚠️ 遗留**：A2 用户裁定「手机 W=160→2 列」受 `app-shell min-width:1200`〔globals.css §7.2 桌面优先 / 低于此值整站横向滚动〕架构约束**未兑现**——如需移动端响应式 2 列须另起架构任务〔推翻 §7.2，超出 A1/A2 范围〕；视觉回归 baseline 未入库待补。详见 changelog [CARD-SIZE-A1A2-GATE]。
...
- **用户锁定决策（硬约束）**：① 单一全局卡宽 `--card-w` + `--card-gap`（废 standard/scroll/compact 分档）；② 网格 `repeat(auto-fit, min(var(--card-w),100%))` 精确定宽 + `justify-content:center` 居中留白（**实施期 auto-fill→auto-fit 自纠**，Codex-A2-R7 复核：固定宽轨道下 auto-fill 保留空轨道致卡少不居中、auto-fit 折叠空轨道才真居中）；③ 横滚 `width:var(--card-w)`；④ 手机端列数由 W 决定（接受 W 大则 1 列）；⑤ 后台单一「卡片宽度 + 间距」表单。
...
| **A2-3** | CARD-SIZE-A2-CSS | CardGrid `repeat(auto-fit,min(var(--card-w),100%))` 精确定宽 + `justify-content:center`（**auto-fit 自纠 Codex-A2-R7**）/ ScrollRow `width:var(--card-w)` / **Shelf·TopTenRow·DailyAnimeRow 内联 `--card-w-scroll`→`--card-w`**（Codex-A2-R4 实测内联消费）/ card-size-fetch 遍历→单一 `--card-w`+`--card-gap` / globals.css；CardGrid/ScrollRow/fetch 测重写 | **opus**（CSS 原语翻转） | 视觉回归、单测 |
...
4B. HANDOFF-40B — 分类/搜索页接入 + 删旧 FilterArea + E2E（状态：✅ 2026-06-24）
...
| **STATS-02-SCHEMA** | ✅ 已完成（2026-06-25；migration **128** 六表 + 双唯一防线〔idempotency_key 显式命名 + session/video/episode null-safe〕 + visitor_is_ephemeral + 索引；packages/types 6 域类型；videoPlayStats query 骨架；architecture §5.20；schema 集成测〔强化精确断言〕。Codex 对抗审 CONDITIONAL PASS〔1M 约束命名 + 2H/1M/1L 集成测精确化全吸收〕。门禁：typecheck=0 / 改动文件 lint=0 / verify:adr-contracts=0 / test:changed 零回归〔121 既有 worktree 环境失败，moderationBatch 已证 pristine 同失败〕。**冷启动 + 集成测执行延后合并期**〔worktree 无 .env.local、不动 live dev 库〕。stats-01-adr 分支） | migration 127/128：创建 `video_play_events` / `video_play_hourly` / `video_play_daily` / `video_play_daily_visitors` / `video_play_totals` / `video_hot_scores`；补 pending/time/date/hot 索引；`video_play_daily_visitors(bucket_date)` 清理索引；新增 DB query 模块骨架与类型；同步 `docs/architecture.md`。 | DB/types/docs ≤5 | **opus**（schema + hot-score 真源） | STATS-01 Accepted | migration 冷启动；schema 集成测；`verify:adr-contracts`；`architecture.md` §schema 同步；测试覆盖两个唯一约束存在与 `occurred_at` trusted 字段约束。 |
| **STATS-03-A-WRITE-ENDPOINT** | 🔄 拆分执行（原子化红线：原 A 跨 middleware+route+service+query+限流 >5 项）→ **A1 ✅ 已完成**（2026-06-25：visitor-cookie 全局中间件 D-216-7，commit 待提交）｜ **A2 ✅ 已完成**（2026-06-25：写端点 route+service+queries；commit 待提交。Codex 对抗审 3 轮 BLOCK→PASS：原子 Redis SET NX marker 关闭并发 TOCTOU〔重试/并发不烧限流〕+ 双防线 23505 + occurredAt 非对称 clamp〔单一 ingestedAt 基准〕+ 双维限流 fail-closed + ip/ua HMAC + zod 数值上限。门禁 typecheck/lint/verify/test:changed 全绿）。原 STATS-03-A 拆分 A1+A2 **全部完成**。〔流程偏离登记：A1 直接进入实现、未先写 tasks.md 卡，已门禁全绿补记，A2 起恢复先写卡〕 | `POST /videos/:id/play-events`：route 只校验并返回 `202`；service 解析 short_id、校验公开可见与 source、消费 middleware visitor_hash、hash IP/UA、clamp `occurredAt`、visitor+IP 双维限流、幂等插入。命中 `idempotency_key` 或 session/video/episode 任一唯一约束均视为幂等成功，不得 500。 | API route/service/queries ≤5（跨 2 层，契约闭环） | **opus** | STATS-02 | route/service/query 单测；重复 `idempotency_key` 返回 202 且不二插；同 session/video/episode 但不同 key 返回 202 且不 500；恶意未来/远古 `occurredAt` 被 clamp/回退；匿名路径不访问 `users`；visitor+IP rate-limit 命中；`verify:endpoint-adr` 公共端点对齐。 |
| **STATS-03-B-PLAYER-REPORTING** | ✅ 已完成（2026-06-25：`lib/play-stats.ts` helper〔常量 + buildPlaySessionId + isQualifiedPlay〔watchSeconds 语义〕+ createQualifiedPlayDetector〔真实观看时长累加排除 seek〕+ async sha256 idempotencyKey + reportVideoPlayEvent〕+ `playerStore.playSessionId/ensurePlaySessionId`〔跨 full↔mini 共享会话〕+ PlayerShell〔full〕+ useMiniPlayerVideo〔mini，HIGH-3 正确接入点〕双接入。Codex 对抗审 2 BLOCK/3 HIGH/3 MEDIUM/1 LOW 全吸收〔preview 沿用现网范式 / watchSeconds 改 detector / session 集中收敛 / 同步去重防竞态 / mini 接入点改 hook / crypto 能力检测 / episode 口径统一 / 字段边界〕。门禁：typecheck=0 / 改动 4 文件 lint clean〔worktree 全量 lint 为 eslintrc cascade 环境冲突〕/ test:changed=150 全过含 MiniPlayer(41)+player-shell 回归 / verify:adr-contracts=0。**PLAYER e2e 延后合并期 gate**〔worktree 无 .env.local + cascade，同 STATS-02 先例〕。执行模型 claude-opus-4-8；子代理 Codex 对抗审。commit 待提交） | 前端上报 helper：`buildPlaySessionId`、确定性 `buildPlayEventIdempotencyKey`、`isQualifiedPlay`、`reportVideoPlayEvent(apiClient)`；`PlayerShell` 主接入，复用 `previewMode` 写保护；mini-player 仅在能独立跨 qualified threshold 时接入；失败不影响播放。 | web-next player/helper/tests ≤5 | **opus**（PLAYER 关键路径） | STATS-03-A 契约冻结 | helper 单测；`PlayerShell` preview 不上报；同 play_session/video/episode 只上报一次；source 切换不单独计数；PLAYER e2e 回归；不得绕过 `apiClient`。 |
| ~~**STATS-04-AGGREGATE**~~ | 🔄 拆分执行（原子化红线：聚合 + retention 两独立 job 跨 worker job×2 + queries + config/index + tests >5 项）→ 拆 **-A**（聚合）/**-B**（retention） | — 见 -A/-B — | — | — | — | — |
| **STATS-04-A-AGGREGATE** | ✅ 已完成（2026-06-25：`play-stats-aggregate.ts` 聚合 job——drain ≤10 独立事务 + 单批 8 步〔`FOR UPDATE SKIP LOCKED` 取批 → per-video advisory xact lock 串行化 → hourly/daily+UV/totals 增量 upsert → hot_scores 全量重算〔=EXCLUDED 覆盖〕→ mark aggregated〕 + config cron 1min + index 注册。**Codex 对抗审 3 轮 BLOCK→PASS**：① BLOCK hot_score 并发 lost update → per-video advisory lock〔按实际 hashtext lock_key 排序防碰撞死锁〕；② HIGH ROLLBACK 失败吞错污染连接 → BatchRollbackError + release(err) 销毁；③ HIGH node-cron 同进程重入 → isRunning guard；④ HIGH integration TRUNCATE 无防护 → current_database token 守卫；⑤ MEDIUM UV/hot 覆盖下降测试增强。门禁：typecheck=0 / 改动文件 eslint clean / test:changed=43〔含 14 编排单测〕零回归 / verify:adr-contracts=0。**integration 数值测〔8 例〕延后合并期**〔worktree 无专用 test DB，同 STATS-02 先例〕。执行 claude-opus-4-8；子代理 Codex 3 轮对抗审。commit 待提交） | `apps/worker` 批量聚合 job：每批 LIMIT=500、单批单事务、`FOR UPDATE SKIP LOCKED` 取 `aggregated_at IS NULL ORDER BY ingested_at ASC`；upsert hourly/daily/daily_visitors/totals + hot_scores 全量重算；commit 后标记 aggregated；cron 1min、单 tick ≤10 独立事务。SQL worker 内联（ADR-107 §4）。 | worker job/config/index/tests ≤5 | **opus** | STATS-02（可与 03B 部分并行） | worker 重跑不 double count；rollback 留 pending；commit 标 aggregated；daily UV 同 visitor/day/video 只加 1；batch LIMIT 生效；hot_scores 物化 + 滑窗下降；mock 编排单测 + integration 数值测（延后合并期）。 |
| **STATS-04-B-RETENTION** | ✅ 已完成（2026-06-25：`play-stats-retention.ts`——job-level advisory lock〔多实例 skip + unlock 失败 release(err) 销毁〕+ 三表批量删〔`ctid IN (SELECT ... LIMIT)` 循环至不满批〕：events 双谓词 `aggregated_at IS NOT NULL AND < NOW()-90d`〔**未聚合永不删**〕/ visitors `bucket_date<CURRENT_DATE-400` / hourly `bucket_hour<NOW()-90d`；daily/totals/hot_scores 永久不动 + config cron `'15 4 * * *'` + index 注册。**Codex 对抗审 1 轮 PASS**〔8 对抗点全 verified：未聚合不删/批量收敛/advisory lock 三路径/边界保守/永久表不触碰/无活锁/并发安全/integration 真实覆盖硬不变量；2 LOW 测试硬化吸收：单测负向断言无 OR 旁路 + integration 89/91d 近边界〕。门禁：typecheck=0 / 改动文件 eslint clean / test:changed=38〔含 9 编排单测〕零回归 / verify:adr-contracts=0。**integration 数值测〔5 例〕延后合并期**〔worktree 无专用 test DB〕。执行 claude-opus-4-8；子代理 Codex 1 轮对抗审 PASS。commit 待提交） | `apps/worker` retention maintenance job：`video_play_events` aggregated 后 90d / `video_play_daily_visitors` 400d（按 `bucket_date` 索引）/ `video_play_hourly` 90d 清理；**绝不删 `aggregated_at IS NULL`**；daily/totals/hot_scores 永久。独立 cron（非 1min，参 auto-retire 日级）。 | worker job/config/index/tests ≤5 | **opus** | STATS-04-A | retention 不删未聚合事件；按 `bucket_date` 索引清理 visitor 表；hourly/events 90d 边界；daily/totals/hot_scores 不动。 |
| **STATS-05-A-PUBLIC-TYPES-READ** | ✅ 已完成（2026-06-25：`Video.playCount`〔必填〕+ `VideoCard.playCount`〔可选，ES 卡片暂无〕；videos.internal `VIDEO_JOIN` 左连 `video_play_totals` + `VIDEO_FULL_SELECT COALESCE(0) AS play_count` + mapVideoRow/mapVideoCard 映射〔单点收敛全前台读路径〕；3 surface 无条件展示〔VideoCard cards+related / DetailHero detail / PlayerShell watch，含 0〕；`formatPlayCount` util DRY 提取〔3 处共用〕。**Codex 对抗审 2 轮 BLOCK→PASS**：轮1 拦 2 BLOCK〔改错组件 VideoMeta 死代码→实际 DetailHero；漏 watch→PlayerShell meta 行〕+ 2 HIGH〔`>0` 隐藏 0 违反"显示 0"→无条件渲染；搜索卡片→ES SearchResultRow 延后 STATS-06〕+ MEDIUM 组件测试；轮2 PASS 全闭环。门禁：typecheck=0 / 改动文件 eslint=0 / verify=0 / 全量 test:changed 8379 passed〔唯一失败 CrawlerRunsView 为预存并发 flaky，单独 33 passed、零 playCount 引用、与本卡无关〕。测试：mapper projection 7 + util 4 + VideoCard 渲染 3 + DetailHero/PlayerShell 断言。**VIDEO/PLAYER e2e 延后合并期**〔worktree；PlayerShell 仅 meta 行加展示 span、未碰播放逻辑〕。执行 claude-opus-4-8〔卡建议 sonnet，opus 会话全自动推进执行，成本偏离已记录〕；子代理 Codex 2 轮对抗审。commit 待提交） | 公共读模型：`packages/types` `Video.playCount`；API video select/projection 左连 `video_play_totals` 并 `COALESCE(0)`；前台 cards/detail/watch/related 元信息展示播放次数；不改变排序语义。 | types/API read/web UI ≤5（跨 API/UI 两层，字段透传闭环；无 schema migration） | sonnet（实际 opus 会话执行） | STATS-04（至少 totals 可产出） | typecheck；projection 单测；无统计行显示 0；卡片/详情/播放页展示一致；VIDEO e2e；确认此次展示逻辑不需沉淀新共享组件或说明理由。 |
| **STATS-05-B-PUBLIC-HOT** | ✅ 已完成（2026-06-25：`listVideos` sort=hot 改用 `video_hot_scores` 物化热度真源——条件 `LEFT JOIN video_hot_scores vhs`〔additive，非 hot 排序不引入无谓 join〕+ 统一排序 `vhs.hot_score DESC NULLS LAST, vhs.play_count_7d DESC NULLS LAST, vpt.total_play_count DESC NULLS LAST, v.updated_at DESC`〔vpt 复用 STATS-05-A JOIN〕，**删活跃源计数占位**；`listTrendingVideos` 改用 period 窗口聚合桶——`TRENDING_WINDOW` 常量 map：today→`video_play_hourly`〔`bucket_hour ≥ NOW()−24h AND ≤ NOW()`，D-216-9 防未来桶〕/ week/month→`video_play_daily`〔`bucket_date ≥ CURRENT_DATE−6/29 AND ≤ CURRENT_DATE`〕，`LEFT JOIN (SUM(play_count) AS window_plays GROUP BY video_id) w` + `ORDER BY w.window_plays DESC NULLS LAST, v.updated_at DESC`，**删 updated_at recency 占位过滤**。**稳定 fallback**：聚合空窗→window_plays 全 NULL→退化为原 updated_at DESC，对 home-curation/home-autofill 消费方零回归〔签名/返回不变〕。**Codex 对抗审 1 轮 PASS**〔0 BLOCK/HIGH/MEDIUM；设计直落冻结 ADR-216 决策〕。门禁：typecheck=0〔8 workspace〕/ 改动文件 eslint=0 / verify:adr-contracts EXIT=0〔endpoint-adr + sql-schema-alignment 等 3 类 ✅，新 SQL 引用 hot_scores/hourly/daily 列全对齐 migration〕/ test:changed=1126 passed〔82 文件，含本卡新增 15 + home-autofill-core 33 + home-queries 15 消费方回归全过〕。测试：videos-list-query hot sort 9 + videos-trending-query 6〔三 period 窗口源 + 排序 + fallback + 防未来桶〕。**VIDEO e2e 延后合并期**〔worktree 无 .env.local，同 STATS-02/05-A 先例〕。执行 claude-opus-4-8〔卡建议 opus·模型对齐〕；子代理 Codex 1 轮对抗审 PASS。commit 待提交） | `/videos?sort=hot` 与 `/videos/trending` 改用 `video_hot_scores` / hourly/daily 聚合：排序 `hot_score DESC NULLS LAST, play_count_7d DESC, total_play_count DESC, updated_at DESC`；`period=today|week|month` 明确按 ADR today 语义；保留公开可见过滤。 | API queries/service/tests ≤5 | **opus**（排序语义变更） | STATS-04 + STATS-05-A | `/videos?sort=hot` 根据聚合改变顺序；trending today/week/month 使用聚合桶；无 hot_score 时稳定 fallback；VIDEO e2e；不再使用 active source count 作为 hot 真源。 |
| **STATS-06-A-SEARCH-HOT-READ-BACKFILL**〔原 STATS-06-SEARCH-HOT〕 | ✅ 已完成（2026-06-25 / commit 待提交：ES 读对齐层。es_mapping additive 3 字段 + VideoIndexSyncService doc 投影 LEFT JOIN play 表〔**保留 null 非 `?? 0`** + **updated_at 改 v.updated_at 同源** + syncVideoStrict〕 + SearchService hot sort 改 D-216-3 4-key 链 + reindex 脚本〔syncVideoStrict 真实计数 + 覆盖断言 + 3 形态数据收敛抽样 + --allow-drift〕 + tests。**Codex 任务卡审 1 轮 FAIL→3 BLOCK+4 HIGH 全吸收**〔降格 -A / null 语义 / tiebreak 同源 / 假收敛 / 行为测试 / 可见集〕 + **实现审 CONDITIONAL PASS→2 HIGH+2 MEDIUM 补强**〔null 收敛抽样 / 覆盖断言 / 可见集断言 / 独立 ES comparator〕。门禁 typecheck=0/eslint=0/verify=0/test:changed=692。SEARCH e2e + reindex 实跑 + PG↔ES 对拍延后合并期。**实时 ≤2min = STATS-06-B**。） | Elasticsearch 文档增加 `play_count_total` / `play_count_7d` / `hot_score`；索引同步或批量 updater 按 ADR 路径落地；`/search?sort=hot` 改用 ES play fields，与 `/videos?sort=hot` 语义对齐；提供 reindex/回填脚本。 | ES mapping/index sync/search service/script/tests ≤5 | **opus** | STATS-04 + STATS-05-B | mapping additive；SearchService hot 不再只看 `rating_votes`；reindex 脚本幂等且**数据收敛**断言；SEARCH e2e；**reindex/reconcile 后同一快照** `/videos?sort=hot` 与 `/search?sort=hot` 排序一致（生产 ≤2min 实时归 -B）。 |
| **STATS-06-B-REALTIME-SYNC** | ✅ 已完成（2026-06-25 / commit 待提交：方案①——worker play-stats-aggregate **两阶段**：阶段一 DB drain 收集 affected video_id、释放 PG client；阶段二 best-effort `es.bulk` partial-update ES play 字段〔worker 自包含 `@elastic` client，可选，URL 缺失→null no-op 不阻断聚合；ADR-107 §4 零 import apps/api〕。aggregateOneBatch 复用 AFFECTED query 返 videoIds〔不新增 txn query〕。**Codex 任务卡审 FAIL→1 BLOCK+4 HIGH+3 MEDIUM 全吸收**〔两阶段隔离 / 返 videoIds / ES 可观测 / 404 收紧 / stale-overwrite 记录 / 默认 null 改既有单测〕 + **实现审 0 BLOCK→3 HIGH+1 MEDIUM 全吸收**〔isDocumentMissing 仅认 document_missing_exception / drain 失败仍同步已 commit 批后 rethrow / maxRetries:0+30s deadline 防钉死 isRunning / shutdown 关 esClient〕 + **确认轮 5 项全 RESOLVED**〔3 文案修正〕。门禁：typecheck=0〔8 workspace〕/ 改动文件 eslint=0 / verify:adr-contracts=0〔sql-schema-alignment ✅〕/ test:changed 升全量 **8417 passed 零回归**〔worker 聚合套件 24：T1-T15+S1-S8+S4b〕。SEARCH e2e + reindex 实跑 + PG↔ES ≤2min 实时窗口对拍延后合并期。执行 claude-opus-4-8；子代理 Codex 3 轮对抗审。stats-01-adr 分支） | ES play fields ≤2min 实时同步（D-216-4「聚合 commit 后增量更新变更视频文档」）：方案二选一——① apps/worker play-stats-aggregate commit 后用 worker 自包含 ES client partial-update 变更视频（ADR-107 §4：worker 不 import apps/api，自带 @elastic client + 内联 index/字段）；② apps/api maintenanceScheduler 增 ~1min play-stats tick + maintenanceWorker 新 job type 调 VideoIndexSyncService.syncPlayStats（读 video_hot_scores by computed_at partial-update）。`computed_at` 作变更游标。 | worker job/lib OR maintenanceWorker+scheduler+service+tests ≤5 | **opus** | STATS-06 | 聚合后 ≤2min ES play fields 收敛；partial update 幂等不清其余 doc 字段；跨 surface 排序一致性在实时窗口内成立；不破坏 worker→apps/api 零耦合边界。 |
| **STATS-07-ADMIN-ANALYTICS** | ✅ **全交付 2026-06-26**（ADR + A + B 三卡闭环）（**STATS-07-ADR ✅ 2026-06-25**：ADR-217 **Accepted**〔Codex 任务卡审 CONDITIONAL PASS 4H/4M/2L + arch-reviewer Opus CONDITIONAL PASS 2H/5M/2L + Codex ADR 终审 1 假阳性 BLOCK/3H/3M 三轮真问题全吸收〕——3 admin GET 端点契约〔overview/trend/top-videos〕+ period 7d/30d/90d 口径 + 唯一数据源 `video_play_daily` + 分层/隐私/时区同源/BIGINT/zero-fill/可见性/不对账全冻结；verify:adr-contracts EXIT=0〔endpoint-adr 139 ADR 端点含本表〕；admin 端点门禁达标。**STATS-07-A ✅ 2026-06-25**〔3 admin GET 端点〔overview/trend/top-videos〕落地：packages/types 3 DTO + videoPlayStats 3 analytics query〔daily-only 唯一源〕+ VideoPlayAnalyticsService + analytics.video-plays route〔per-endpoint 严格 zod + adminOnly〕+ server.ts 注册；Codex 代码审无 BLOCK，3 条测试增强〔HIGH-1 时区真池相等门 + MEDIUM-2 top-videos SQL 子句静态门 + LOW-3 zod 边界〕全吸收；typecheck=0 / verify:endpoint-adr 253 路由对齐〔139 ADR 端点含新 3 端点〕/ verify:adr-contracts=0 / test:changed 619 文件 8451 测全过；集成测 DB 层延后合并期〕**→ STATS-07-B ✅ 2026-06-26**〔server-next analytics UI：新 dashboard「视频播放」Tab 落地——`lib/video-plays/api.ts` 3 fetch + `VideoPlaysAnalyticsView`〔period 选择器 + overview 5 KpiCard + trend inline SVG〔零图表库，镜像 AnalyticsView〕 + top-videos admin-ui DataTable〔server mode 直渲全行 + toolbar/列 ⋯ 抑制〕 + stale guard〕 + DashboardClient tab 接线；Codex 卡审〔1 BLOCK @resovo/types + 2 HIGH e2e 路径/DataTable 切片 + 3 MEDIUM/LOW〕全吸收 + 代码审无 BLOCK〔HIGH-1 列 ⋯ 死交互抑制 + 测试增强 stale/25 行/avg/period 回显，5 条全吸收〕；typecheck=0 / 单测 7 测 + test:changed 23 测全过 / playwright admin project 收集 3 e2e / verify:adr-contracts=0〔无新 route〕；ADMIN e2e 实跑延后合并期〕，**A 先于 B**〔packages/types 共享契约〕。**🎉 STATS-07 全交付：admin endpoint ADR-217 + API〔07-A〕+ UI〔07-B〕闭环**） | 独立 admin endpoint ADR 后落地 `/admin/analytics/video-plays/overview|trend|top-videos` 与后台 UI：period 7d/30d/90d、total plays、watch seconds、avg watch seconds、匿名/登录拆分、daily trend、top videos。只读 aggregate 表，不扫 raw events。ADR 定稿后若 API/service/UI 范围仍超 5 项，必须继续拆 `STATS-07-A/B`，不得单卡硬做。 | admin ADR gate + analytics 实施切分 | **opus** | STATS-04 + STATS-05-A + 独立 admin endpoint ADR PASS | 先起独立 ADR 并过 arch-reviewer；`verify:endpoint-adr` 必须识别新 admin routes；admin API 测；ModernDataTable 等后台规范组件；ADMIN e2e；确认 analytics 口径与 `watch_history` 不对账。 |
...
## [SEQ-20260627-01] META — per-video 状态列与 video_external_refs 口径漂移根治

- **状态**：🔄 执行中（META-55-ADR ✅ ADR-219 Accepted·待用户裁可解锁 DC-219-1；用户 2026-06-27「通过方案，开始执行」）
- **创建时间**：2026-06-27 21:30 ｜ **最后更新时间**：2026-06-27 22:11
- **来源**：用户报「`videos.douban_status=matched` 137 条 vs refs `douban auto_matched` 198；`bangumi` 241 vs 277；且 TMDB 无 `tmdb_status` 列」。本卡为根因诊断后的立案（诊断证据见下，已实库只读核验，未改任何数据）。
- **目标**：消除 per-video 状态列（`videos.{douban,bangumi}_status`，migration 032/082）与 `video_external_refs` / `catalog_external_refs`（ADR-177/ADR-201 canonical）之间的**双真源漂移**；即时矫正存量错列；从源头阻止新漂移；对齐四源（douban/bangumi/tmdb/imdb）状态口径。

### 根因诊断（实库只读核验，2026-06-27）

口径漂移分三类、性质不同，**不可一刀切**：

1. **douban 真漂移（数据正确性，需矫正）**：douban 列分布 matched=137 / candidate=180 / pending=29 / unmatched=6347；douban applied ref（`auto_matched`+`manual_confirmed`）共 200 条、**live video 199 条**。交叉核验：
   - status=matched 137 条 ⇒ 100% 有 applied ref + catalog `douban_id`（**这部分一致**）；
   - **60 条 live video 持有 `is_primary=true` 的 applied douban ref（`meta_score` 80–100、`enriched_at` 非空、catalog_id 存在）却 `douban_status='unmatched'`**（另 2 条 candidate）。这 60 条即「198 vs 137」差额主体。
   - 写入来源：漂移行 ref `linked_by=auto/method=network`（58）+ `moderator/manual`（2）；且 **60/60 均 `ref.linked_at ≤ enriched_at`** —— ref 先写好，之后 MetadataEnrichService 又跑了一遍、按**自身独立判据**判为「未匹配」把列覆写成 `unmatched`。
   - **根因**：identity/refs 子系统（ADR-177：blockingRecall/offlineRescore/rollup）与 MetadataEnrichService/DoubanService 列写入是**两套非事务耦合、判据不一致**的状态机；ADR-201 已把 refs 定为派生 canonical、把状态列降为 fallback，但**未回填存量、未冻结列的直接消费**，列遂滞后说谎。
2. **bangumi 假漂移（无真漂移，仅卫生）**：bangumi(anime) 列 matched=241 / candidate=45 / unmatched=490；`bangumi auto_matched` ref 277 条 = **live 241（全部 `bangumi_status=matched`，100% 一致）+ 36 条挂在软删除 anime 视频上**。`video_external_refs` 对 `videos` 是 `ON DELETE CASCADE`，但**软删（`deleted_at` 置位）不触发 CASCADE**，故残留。「277 vs 241」差额=36 软删残留，非 live 漂移。douban 同样有 1 条软删残留。
3. **tmdb 无状态列（结构正解，反证）**：tmdb 无 `tmdb_status` 列，却有 `auto_matched` ref 1260 + catalog exact 564。tmdb/imdb **单一 refs 真源 → 结构上不可漂移**，恰是目标范式；douban/bangumi 的「列 + refs 双真源」才是漂移病灶。（注：ADR-201 derive 注释「tmdb/imdb 无 refs 写入路径」已与现状不符——实有 1260 tmdb ref，注释待校。）

**受影响的直接列消费方**（绕过 derive、直接 `WHERE douban_status/bangumi_status`，会因列滞后而漏算 ~60 douban 视频）：`moderation.ts:111`(`ENRICH_COMPLETE_SQL`)/`:216`、`staging.ts:167`/`:317`、`videos.ts:434`/`:644`/`:660`、`videos.status.ts:180`（待审列表筛选）。`metadata-status.derive.ts` 路径因 refs 优先已自动规避漂移（但 derive 与直接列消费并存=口径不一致）。

### 范围 / 不做

- **做**：①「单一真源」方向决策（ADR-219）；② 存量矫正（列←refs/catalog 回填，止血）；③ 源头防复发（enrich 不得在已有 applied ref 时把列写 `unmatched` / 判据统一 / 事务耦合）；④ 直接列消费方迁到 refs 派生谓词、冻结列作为可查询真源；⑤ 软删 ref 卫生（purge/tombstone）。
- **不做**：本序列不新增任何外部抓取/匹配能力；不改 derive 的 refs-canonical 既定优先级（ADR-201）；不在 ADR 定向前物理 DROP 列。

### 依赖与原子化

- ADR-219 **先行硬前置**。跨 schema·script·service·多消费方查询 + **两谓词语义契约** → **强制拆卡 + 拆 2 张设计卡**（DC-219-1/DC-219-2，CLAUDE.md 强制升 Opus 情形 1「定义共享谓词契约」）。**串行主链（两轮对抗审定稿，详见 ADR-219 实施序列）**：`55-ADR → DC-219-1 谓词契约 → 57-ENRICH 守卫 → 58-CONSUMER 迁移〔迁 videoRefAppliedSql〕→ DC-219-2 软删处置表 → 59-REF 软删读期过滤 → 56-DROP-prep → 60-完全停写+冻结守卫 → 独立 DROP ADR`。**关键裁定（D-219-2，Codex B-1 推翻初裁后改裁）**：derive overall 是 **catalog-first**（catalog candidate 拦截 video auto_matched，实测 60 漂移行仅 34 applied/28 candidate），故消费方**不能迁 overall**、须迁 **video-ref-applied 谓词**（含 auto_matched，等价旧 matched）→ 60 行真止血、无 14 行降级。

### 任务列表（按执行顺序）

| 卡 | 状态 | 内容 | 范围项 | 建议模型 | 依赖 | 门禁 |
|---|---|---|---|---|---|---|
| **META-55-ADR** | ✅ 完成（待用户裁可解锁实现） | 起草定稿 ADR-219。arch-reviewer 两轮裁定（初裁 + 复议收回承重错误）+ Codex **两轮**对抗审（round-1 4B+3C / round-2 1B+3C+1NIT）全数吸收；方向 A 变体 Accepted、两谓词分工收口契约定稿、强制拆 DC-219-1/DC-219-2。 | docs | **opus**（主循环）+ `arch-reviewer`(opus)×2 + codex-rescue×2 | 无 | ✅ ADR Accepted；arch-reviewer 两轮 PASS；Codex 两轮 BLOCK 全处理；不写实现代码。**待用户裁可解锁 DC-219-1。** |
| **DC-219-1-PREDICATE** | ✅ 完成 | **谓词语义契约（Opus·57+58 共同前置，D-219-9）**：定义 `videoRefAppliedSql` 导出常量（`EXISTS … is_primary AND match_status IN('auto_matched','manual_confirmed')`，阈值含 auto 写死=等价旧 matched）；明确两谓词边界（video-ref-applied=是否匹配/发布门禁/富集完成；catalog-overall=元数据状态列排序/工作队列，**不混用**）；JS↔SQL 对拍单测义务登记。**`is_primary` invariant（Codex r2 C-1）**：实测 0 边角但 041 无 schema 约束，须裁定谓词是否强制 is_primary + 约束测试守护。 | 契约 + 导出常量 + 测试 ≤3 | **opus**（共享谓词契约） | META-55-ADR Accepted | 谓词常量单一真源；两谓词边界文档化；对拍单测；is_primary invariant 约束测试；无第三套判据。 |
| **META-57-ENRICH-GUARD** | ✅ 完成 | **根因守卫·非完全停写（D-219-3，B-2 吸收）**：enrich 写列前一致性守卫——已存在 applied video ref（同 `videoRefAppliedSql` 判据）时**不得覆写列为 unmatched**；列仍随 refs 演进（不冻结、无 stale 暴露）。MetadataEnrichService（:94/:183）/DoubanService（:263/:468）/BangumiService 同构。**完全停写并入 META-60**。 | service/queries/tests ≤4 | **opus**（enrich 匹配决策逻辑） | DC-219-1 | 单测：已有 applied ref 时 enrich 不覆写 unmatched；真未命中仍写；守卫判据=DC-219-1 同源；关键路径回归。 |
| **META-58-A-APPLIED-MIGRATE** | ✅ 完成 | **A 核心止血·boolean applied 消费方（D-219-2，实现期拆 A/B）**：迁「是否匹配/发布门禁/富集完成」boolean 用途到 `videoRefAppliedSql`——moderation.ts:111 `ENRICH_COMPLETE_SQL`〔`douban_status='matched'`→`videoRefAppliedSql('douban')`，bangumi 口径一并〕、staging.ts:167/317〔requireDoubanMatched〕、videos.ts:644/660〔backfill `unmatched`→`NOT videoRefAppliedSql`〕。**并入**：补 `idx_video_external_refs_video_provider_status` 索引（D-219-7）+ derive 头注校正（D-219-8）+ 改受影响测试口径。 | queries/index/tests ≤5（boolean 用途逐一对拍） | **opus**（查询语义变更） | META-57 | 60 douban 欠计消除（boolean 路径真止血、无 14 行降级）；moderation/staging 计数回归；不破坏 API；EXPLAIN 无热路径回归。 |
| **META-58-B-1-DOUBAN-MULTISTATE** | ✅ 完成 | **后端 douban 4 态过滤谓词 + 多值过滤迁移（ADR-219 D-219-10/13）**：`video-ref-applied.ts` 扩展过滤侧 4 导出（`DoubanRefStateInput`/`matchesDoubanRefState`/`doubanRefStateSql`/`doubanRefStateFilterSql`）+ JS↔SQL 穷尽四分对拍。迁 **douban** 过滤侧：moderation.ts:219、videos.ts:434、videos.status.ts:180、videos.ts:644+660（douban 半）；bangumi 半全保留（D-219-11）。**Codex 对抗审 2 处 BLOCK 修正**：① pending 改兜底穷尽 `IS DISTINCT FROM 'unmatched'`（D-219-10 原定义非穷尽→漏行 bug）；② **投影迁移整体延 META-60**（D-219-12 漏列 moderation:344+VIDEO_FULL_SELECT，单独迁分裂+耦合 derive）。derive 列兜底清理延 META-60（cache-only 兜底误升 applied）。 | 谓词/queries/tests=6 文件 | **opus**（共享谓词契约扩展） | META-58-A + arch-reviewer 裁定 ✅ | ✅ 穷尽四分恰好命中 1 态；candidate 非 primary 守护；pending IS DISTINCT FROM；douban 多值口径等价旧列；JS↔SQL 守护；bangumi 过滤不破坏。门禁 typecheck=0/lint=0/test:changed 1202/verify=0。 |
| **META-58-B-2-FACET-UI** | ✅ 完成 | **distinct 白名单移除 douban_status 死配置（ADR-219 D-219-12 / ADR-150 AMENDMENT 4）**：事实修正——前端 facet 早用静态 `DOUBAN_STATUS_OPTIONS`（VideoColumns.tsx:504，列未声明 `filterDistinctTable`→从不查 distinct 端点），白名单键为死配置；移除该键 + `datatable-shared` toBeUndefined 守护 + ADR-150 AMENDMENT 4 登记。前端零改动；bangumi 保留（D-219-11）。Codex 对抗审 2 处措辞修正吸收。 | distinct/test/ADR=3 文件 | **opus**（含 ADR AMENDMENT） | META-58-B-1 ✅ | ✅ douban 键移除；bangumi 保留；toBeUndefined 守护；门禁 typecheck=0/lint=0/test:changed 293/verify=0。commit 291244b4。 |
| **META-61-BANGUMI-STATUS-RETIRE** | ⬜ 待开始（独立后续，**前置补 bangumi 信号**） | **bangumi_status 退役（ADR-219 D-219-11 排除范围外，独立卡）**：bangumi 无 `meta_quality.bangumi_match_status` 信号、无法表达 unmatched/pending。前置：补 `meta_quality.bangumi_match_status`（对齐 douban_match_status 结构，BangumiService 写入路径同步）= schema/信号契约变更，强制 Opus。之后迁 videos.ts:438 + bangumi 4 态谓词 + 退役 bangumi_status 列。 | 信号 schema + service + queries（拆后定） | **opus** | META-58-B-1 + bangumi 信号补齐 | bangumi 4 态信号真源；迁移后退役 bangumi_status 列。 |
| **META-62-REFS-INDEX** | ⬜ 待开始（D-219-7 推迟项，性能触发或 Phase complete 前） | **video_external_refs 覆盖索引 + EXPLAIN 基线（D-219-7 ↔ D-219-12 reconcile）**：补 `idx_video_external_refs_video_provider_status (video_id, provider, is_primary DESC, match_status)`（041 现无 match_status 索引）加速 `videoRefAppliedSql`/`doubanRefState*` EXISTS + derive 8 子查询；moderation 列表分页 + videos COUNT 两路径 `EXPLAIN ANALYZE` 基线。D-219-12 已裁功能正确可推迟（041 现有索引足），本卡 = 性能优化兜底（D-219-6 触发：>50k 行或 P95>200ms）。 | migration + EXPLAIN doc ≤2 | sonnet | META-58 全 ✅（已满足） | 索引建成；EXPLAIN 基线记录；architecture.md 索引同步；无功能回归。 |
| **DC-219-2-SOFTDEL-TABLE** | ✅ 完成 | **refs 读路径软删处置表（D-219-9，META-59 清单）**：穷举全仓 `grep video_external_refs` = 13 文件,三类逐路径裁定落表。**类A**（必过滤）metadata-status.derive.ts;**类B**（外层 gate）externalData/externalIdLoader/split-suggestions/video-merge-candidates/video-ref-applied(谓词 invariant)/moderation;**类C**（聚合/gap）home-autofill-douban/-bangumi/external-resources-stats;**N/A** 写路径 3 Service。**事实修正**：video-merge-candidates 原误判 `:69 JOIN deleted_at`（该结构不存在）,实为 `external_ids` per-video 投影 + 外层 `v.id=ANY($1)` 调用方 gate → 类B。 | docs/checklist=2（decisions 表 + tasks 卡） | **opus**（ADR 处置表设计）+ codex-rescue（对抗审,后台 a971042a） | META-55-ADR Accepted | ✅ 13 文件穷举;类A/B/C/N-A 逐条 ground-truth 核实;video-ref-applied invariant 登记;事实错误已修;META-59 据表实施。 |
| **META-59-REF-HYGIENE** | ✅ 完成 | **软删纯读期过滤（D-219-4，只改类C；arch-reviewer 改判 derive 类A→类B）**：按 DC-219-2 处置表 + arch-reviewer 第二轮裁定,只 gate **类C**——`external-resources-stats.ts` 3 条 COUNT(:85/:89-91/:95-97) + `home-autofill-douban.ts:168` gap + `home-autofill-bangumi.ts:166` gap 加 `JOIN videos v ON v.id=ver.video_id AND v.deleted_at IS NULL`。**derive 不动**（类B,全消费方已 gate;DC-219-2 处置表同步修正 derive 类A→类B + 移假漂移标注）。277→241 统计转 live 口径。 | 3 代码 + 3 测试 + 处置表修正 | **opus** + arch-reviewer(opus, ab76495e) | DC-219-2 ✅ | ✅ 类C 3 路径加 gate(软删 ref 不计入);derive 类B 不动;gate 断言守护;门禁 typecheck=0/lint=0/test:changed 227/verify=0。 |
| **META-56-DROP-PREP** | ✅ 完成 | **DROP-prep 对齐脚本（D-219-2，DROP 卫生）**：`scripts/align-douban-status-to-refs.ts` — 圈定有 applied douban ref（`videoRefAppliedSql` DC-219-1 单一真源）但 `douban_status != matched` 的 live video（欠计漂移，立案 60 行）→ 对齐 matched。**方向与 fix-douban-status-consistency.ts 相反**（那修虚高降级，本卡修欠计升 matched）。幂等（UPDATE 自带谓词 + IS DISTINCT FROM matched）+ --dry-run + 参数化批写 + 事务。仅 douban、不触软删/已一致行。 | script + test=2 | sonnet（本会话 opus 连续推进）| META-57 + META-58 ✅ | ✅ 圈定判据=videoRefAppliedSql 同口径；幂等；dry-run；6 单测（parseArgs + SQL builder）；门禁 typecheck=0/lint=0/test:changed/verify=0。**实跑（dry-run→real）属运维步、部署期执行**。 |
| **META-60-FREEZE-GUARD** | ⬜ **deploy-blocked**（前置硬依赖：① META-56 对齐脚本 `align-douban-status-to-refs` deploy 实跑〔运维步〕消除存量漂移 + 对齐 cache ② 软泡期 ≥1 完整 enrich 周期观察——D-219-13:24214「须列完全停写 + DROP-prep 对齐 cache 后才安全」；纳入 B-1 延后项 D-219-13） | **完全停写 + 列冻结守卫 + 投影/derive 收尾（D-219-1/D-219-3/D-219-5/D-219-13）**：读路径全迁后 enrich 完全停写列；CI/lint 守卫禁**运行时代码/新测试**读写 `{douban,bangumi}_status`（**非字面全仓 grep**，C-2：历史 migration/类型注释天然保留）；**并入 B-1 延后项（D-219-13）**：① douban **投影迁移**（`moderation.ts:344` / `VIDEO_FULL_SELECT` / `videos.status.ts:207` → 派生，需先解 derive `statusColumnState` 耦合）；② `metadata-status.derive.ts:471-473` douban 列兜底清理 + `statusColumnState` douban 分支 + `PROVIDER_SQL_SPECS` douban statusCol（保留 bangumi）——须列停写 + META-56 DROP-prep 对齐 cache 后才安全；登记独立 **DROP ADR**（软泡期 ≥1 完整 enrich 周期 + 观察后 Opus ADR：DROP 列 + architecture.md 同步 + 同 commit 删 `MetadataStatusSourceRow.{douban,bangumi}Status`）。 | lint/CI/service/docs（拆后定） | sonnet（DROP ADR 另起 opus 卡） | 守卫命中运行时新增列读写即失败；完全停写后无 stale 暴露；投影迁移消除过滤/投影不一致；DROP ADR 起草卡登记。 |
| **META-63-DROP-ADR** | ⬜ 待开始（**软泡期后 Opus**，独立 DROP ADR） | **douban_status 列物理 DROP ADR 起草（D-219-5/D-219-13）**：META-60 完全停写 + 软泡期 ≥1 完整 enrich 周期观察后，起草独立 Opus ADR——DROP `videos.douban_status` 列 + 幂等 migration + architecture.md 同步 + 同 commit 删 `MetadataStatusSourceRow.doubanStatus` / statusColumnState douban 分支 / 投影残留。bangumi 不在范围（META-61 独立）。 | ADR + migration + service（拆后定） | **opus**（ADR 决策） | META-60 全收尾 + 软泡期 | DROP 前运行时零残留；architecture.md 同步；幂等 migration；ADR Codex 对抗审。 |

### 关键约束与红线

- **方向先行 + 契约先行**：ADR-219 未 Accepted 前不得动消费方/列/enrich；DC-219-1 谓词契约未定不得起 META-57/58（二者共用 `videoRefAppliedSql`）。
- **两谓词不混用（D-219-2 核心）**：`videoRefAppliedSql`（video 级 applied）服务「是否匹配/发布门禁」；`METADATA_STATUS_JOIN_SQL` overall（catalog-first）服务「元数据状态列排序/工作队列」。**禁用 overall 回答「是否匹配」**（B-1 暴露的范畴错误：catalog candidate 会误降级 auto 匹配行）。
- **口径同源**：DROP-prep 映射、ENRICH 守卫判据、CONSUMER 迁移谓词三者必须 = DC-219-1 的 `videoRefAppliedSql`，禁止再造判据。
- **迁移即止血（D-219-2 修正后）**：迁 **video-ref-applied 谓词**（非 overall）才止血；META-56 回填仅服务 DROP 卫生，不阻塞 58。
- **软删纯读期（D-219-4，收回 tombstone）**：零 schema 变更，refs 读路径按处置表 join `videos.deleted_at`；恢复无损；**禁物理 purge / 禁加 refs.deleted_at**。
- **DROP 不在 ADR-219 拍板**：走 META-60 登记的独立 Opus ADR，前置软泡期 + architecture.md 同步 + 运行时代码零残留 + JS↔SQL status 分支同 commit 删。
- **非代码产物门禁**：本任务卡 + ADR-219 修订稿 commit 前须过 Codex round-2 对抗审（workflow-rules「Codex 对抗性独立审核条款」；round-1 4 BLOCKER 已吸收）。

### 合并 Gate

- 每张实现卡跑 `npm run typecheck` / `lint` / 相关单测或 `test:changed` / `verify:adr-contracts`；矫正脚本卡补「实跑前 dry-run 对账 + 实跑后诊断复核漂移归零」。
- ADMIN/VIDEO 触达卡（58）补对应 e2e；Phase complete 前全量单测 + `test:e2e`。
- **质量缺口闭合（2026-06-28 外部 review，3 项）**：
  - **P1 覆盖索引推迟登记（D-219-7 ↔ D-219-12 reconcile）**：D-219-7 要求 `idx_video_external_refs_video_provider_status (video_id, provider, is_primary DESC, match_status)` + moderation/videos COUNT `EXPLAIN ANALYZE` 基线「并入 META-58」；D-219-12（arch-reviewer 第三轮）**改裁推迟**——041 现有索引功能正确（applied 谓词命中 partial `(video_id,provider) WHERE is_primary`；candidate 谓词走 `(video_id)`，每 video refs 少 + EXISTS 短路，无瓶颈），性能瓶颈（D-219-6：>50k 行或 P95>200ms）再起独立卡。**登记后续卡 META-62-REFS-INDEX**（覆盖索引 + EXPLAIN 基线，sonnet，D-219-6 触发或 Phase complete 前补）——此前推迟仅在 D-219-12 散文，本登记使其可追溯。
  - **P2a META-58 per-card e2e 推迟归属**：gate「ADMIN/VIDEO 触达卡(58) 补对应 e2e」per-card e2e **未随 58-A/B-1/B-2 跑**（本会话工具 env 不稳 + 改动为 douban filter 谓词迁移/死配置移除，行为面被既有 `test:e2e:admin`/`:video` 覆盖）。**归属 Phase complete 前 `test:e2e` 全量 gate**（本节已列），非静默缺失；Phase complete 时须实跑确认 ADMIN doubanStatus 筛选 + moderation pending 队列无回归。
  - **P2b commit trailer**：d4681d5d（B-2 收尾）原缺 `Subagents` trailer，已 amend 补 `Subagents: none`（→ a5c640e0）。
- **SEQ-20260627-01 deploy 收尾序列（编码工作已完成，剩余 deploy/soak/prerequisite-gated）**：可编码代码卡全交付（META-57 守卫 / DC-219-1·2 契约+处置表 / META-58-A·B-1·B-2 过滤迁移 / META-59 软删 gate / META-56 对齐脚本）。剩余收尾**硬依赖 deploy-time 运维 + 时序**，序列：① **deploy 实跑 META-56** `align-douban-status-to-refs`（`--dry-run` 对账 → real，消除存量漂移 + 对齐 cache）→ ② **软泡期 ≥1 完整 enrich 周期**观察 → ③ **META-60**（停写 + 投影迁移 + derive 清理 + 冻结守卫，届时 D-219-13:24214 安全前提满足）→ ④ **META-63-DROP-ADR**（Opus 起草 DROP ADR + 物理 DROP 列）。**META-61**（bangumi 退役）独立，前置补 `meta_quality.bangumi_match_status` 信号。**META-62**（覆盖索引）D-219-6 性能触发或 Phase complete 前。
...
- **HANDOFF-47（卡片 hover 浮起）— 已拆 -A/-B（原子化 >5 项）**：母卡含 token 镜像 + VideoCard 浮起机制 + 4 横滚容器留白（`.scroll-row`/Shelf/VideoGrid-scroll/TopTenRow）= 6-7 单元，超阈值故拆。**源核实修正**：`--shadow-card-hover` 真源已存在（`primitives/shadow.ts` `cardHover` → `tokens.css` line 74 = `0 8px 24px -8px rgb(0 0 0 / 0.28), 0 2px 6px rgb(0 0 0 / 0.08)`）→ 与 HANDOFF-46 同理**逐字镜像 rgb**（早先「按 --shadow-drawer 改 oklch」设想基于「真源没有」假设，已被源核实取代）。shadow 值取 Token Audit §3.2 canonical（≠ Motion Spec demo 示意值 `0 16px 40px`）。
  - **HANDOFF-47-A（✅ 完成，2026-06-29；详见 changelog [HANDOFF-47-A-20260629]）**：`globals.css` 镜像 `--shadow-card-hover`（逐字 tokens.css line 74 rgb）+ `.video-card-lift`/`.video-card-poster` hover 浮起规则（`translateY(-6px) scale(1.03)` + shadow，门控 `(hover:hover)+(pointer:fine)` / `prefers-reduced-motion`〔禁 transform 保 shadow〕 / `--motion-scale` + hover `z-index:1`）+ `VideoCard.tsx` 两分支加类（无 Props 变更）+ `.scroll-row` 补 `padding-top: var(--space-3)`。覆盖 grid 上下文（安全）+ 详情/播放 ScrollRow。**门禁**：typecheck=0/lint=0/test:changed 54 测全过；真实 dev server Playwright：grid hover→`matrix(1.03,0,0,1.03,0,-6)` + shadow 解析为 `--shadow-card-hover` + `z-index:1`；`.scroll-row` paddingTop=12px 浮起不裁顶（headroom 2px）；`prefers-reduced-motion`→transform=none 保 shadow；`--ease-page` 等无回归 + PostCSS 干净。**[HANDOFF-47-A-FIX]**（Codex 复审）：`--motion-scale` 真值为「动效强度」0/1/1.5（非 0.25 减弱），改为按既有惯例（globals.css:522/532）缩放 transition **时长**（1→0.2s/1.5→0.3s/0→即时）、位移幅度固定 `translateY(-6px) scale(1.03)`（详见 changelog [HANDOFF-47-A-FIX]）。
  - **HANDOFF-47-B（✅ 完成，2026-06-29；详见 changelog [HANDOFF-47-B-20260629]）**：内联样式横滚容器留白——Shelf PosterTrack+skeleton / VideoGrid scrollContainerStyle / TopTenRow Top10Track+skeleton 补 `paddingTop: 'var(--space-3)'`（与 `.scroll-row` 同量 12px）。**DailyAnimeRow 排除**（用 SafeImage 非 VideoCard、无 `.video-card-lift`）。**门禁**：typecheck=0/lint=0/test:changed 8 测过（ShelfRow+HomeBrandFiltering）；**跨上下文 Playwright 全量回归**：首页 4 横滚容器（top10-track + movie/series/anime-grid）paddingTop=12px + hover 浮起不裁顶（headroom 2px）；分类 grid + 搜索 20 卡 grid hover `matrix(1.03,0,0,1.03,0,-6)` + shadow 解析；navigate 卡点击→详情页、takeover 卡海报点击→`/watch` Fast Takeover **均无回归**。
...
- **HANDOFF-48 ADR-044/048 motion token 收敛核验 ✅ 完成**（2026-06-30；详见 changelog [HANDOFF-48-20260630] + decisions.md ADR-044/048 AMENDMENT 2026-06-30）：arch-reviewer（claude-opus-4-8）+ Codex 双轮对抗审。**核心原则「数值相等 ≠ 语义等价」**——13 个待收敛 token 中 10 个 orphaned（全仓零消费）。**前提修正**（Codex 纠正）：这批非「M5 待实装契约」，而是「SharedElement/RouteStack 已实装但内联硬编码旁路 token」（useFLIP 硬编码 320/cubic-bezier(0.2,0,0,1)、useEdgeSwipeBack 硬编码 240，与 token 定义值还不一致）。处置：**delete 2 globals.css 死 CSS var**（`--transition-shared` + `--tabbar-underline-transition-duration`，仅 globals.css 手写 var）+ **preserve 其余孤儿定义** + **repoint 0**（240ms 家族同 push 值但语义各异、错耦风险）+ 修正 globals.css 4 处注释错配 + ADR-044/048 AMENDMENT。零运行时行为变更，Playwright 实测 live token 解析不变。
  - **HANDOFF-48-FU-A（候选，需前置设计决策 / Opus·ADR-gated）**：SharedElement/RouteStack 实装与 design token 归属，二选一：① **接入**（useFLIP/useEdgeSwipeBack 改读 CSS var/props 令实装消费 token；实装值 320/cubic-bezier(0.2,0,0,1)/240 与 token 定义值 360/cubic-bezier(0.4,0,0.2,1)/240 不一致，接入**改变实际动画行为**＝行为变更、须关键路径回归）；② **确认弃用**（实装内联值为准、token 永久弃用）。
  - **HANDOFF-48-FU-B（候选）**：清理 design-tokens 包级孤儿 semantic token + `alias-coverage.test.ts` 断言——**tabbar 部分独立可清**（`tabbar.underlineTransitionDuration` 随 49-E-C SlidingUnderline 替换已彻底死）；**shared-element/route-stack/takeover 部分依赖 FU-A 选②弃用**。删包级 token + 单测**触发 ADR-180 全量**。
...
| **PLAYER-12-A**（types+API+ADR） | ✅ 完成（2026-07-01；详见 changelog [PLAYER-12-A-20260701]） | `VideoLineMatrix` DTO（focusEpisode + 每线 focusEpisodeSource〔完整可播放〕+ representative〔纯 label/health 投影，Codex Finding 2 去 sourceUrl/type〕，BLOCKER-1）+ `GET /videos/:id/sources?view=matrix`（zod refine：view+episode 必需、与 preview 正交，HIGH-2）+ `SourceService.listLineMatrix` 复用 listSources JS reduce 聚合（禁 SQL GROUP BY，BLOCKER-2）+ `buildLineKey`/`groupSourcesIntoLineMatrix` 沉淀 @resovo/types 跨端真源（HIGH-1 方案 A，web-next re-export）+ ADR-218 + 单测。arch-reviewer 三轮 + Codex 对抗审吸收。门禁全绿。 | packages/types + apps/api（sources 路由/SourceService）+ ADR-218 + web-next line-display-name re-export | **opus**（arch-reviewer claude-opus-4-8 ×3 + Codex 对抗审） | 无 | ✅ typecheck=0/lint=0/test:changed 8354/verify:adr-contracts=0/verify:endpoint-adr=0 |
| **PLAYER-12-B**（web-next SSR·双供给） | ✅ 完成（2026-07-01；详见 changelog [PLAYER-12-B-20260701]） | `fetchLineMatrix`（ISR 可缓存小载荷）+ **详情页** SSR 双供给（detail-page-factory 加 initialMatrix 不删 initialSources，MEDIUM-2）+ DetailHero 切消费矩阵 representative 派生线路名（回退 sources 韧性）+ Finding-2 遗留（RawSourceForTheme/ThemedSource.src/type 降可选 + coalesce '' + buildThemedLinesFromMatrix）。**watch 页 + PlayerShell initialMatrix 注入推迟 -C**（Codex 纠正：-B PlayerShell 不消费，接线 watch 页会在播放器路径多一次全量 listSources 却无用）。关键路径 parity 单测证线路名逐字一致；web detail e2e 10 passed。 | apps/web-next（video-detail/line-display-name/line-matrix + detail-page-factory + VideoDetailClient/DetailHero） | opus/sonnet | PLAYER-12-A | ✅ typecheck=0/lint=0/test:changed 188/e2e:video(web) 10 passed |
| **PLAYER-12-C**（PlayerShell 重构+回归） | ⬜ 待开始 | **watch 页新增 initialMatrix 注入**（-B 已推迟至此）+ PlayerShell 内存模型「全集常驻」→「骨架 + 当前集全线路切片」（切集重取 `?view=matrix&episode=N` 刷全线路切片，MEDIUM-1）；activeSrc/切线覆盖/兜底环扫/看门狗改按 focusEpisodeSource 取；删旧 initialSources prop（双供给收口）。**回归全部播放器关键路径**（断点续播/线路切换/影院模式/字幕/兜底切线/看门狗）。 | apps/web-next（watch page + PlayerShell + line-matrix.ts + useMiniPlayerVideo 视需要） | **opus**（播放器 shell 接口重构，CLAUDE.md 强制） | PLAYER-12-A（契约冻结） | typecheck/lint/test:changed/test:e2e:player（关键路径回归） |
| **PLAYER-12-D**（详情页双供给收口） | ⬜ 待开始（-B soak 后） | 详情页 RSC 瘦身收口：删 detail-page-factory 的 `fetchVideoSources`/initialSources（-B 后 DetailHero 已切 matrix，initialSources 仅剩回退）+ VideoDetailClient 去 sources 全集 client-fetch 兜底（避免删 SSR 后转 client 拉 9MB）。真正兑现**详情页**载荷瘦身（-C 兑现 watch 页）。 | apps/web-next（detail-page-factory + VideoDetailClient + DetailHero 去 sources 回退） | opus/sonnet | PLAYER-12-B（矩阵消费 soak） | typecheck/lint/test:changed/test:e2e:video |
...
依据：ADR-218「已知限制」节 + Codex Finding 1（HIGH）arch-reviewer UPHOLD 裁决。触发条件：非硬阻塞（现口径继承现网、无新增回归），按优先级排期；若线上出现同 display_name 站合并投诉则升优先级。
