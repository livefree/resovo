...
- **状态**：🚧 进行中（CHG-UX2-03f 已结案 cover bug 真因 → 解锁 -04/-05/-06）
- **创建时间**：2026-05-04
...
- **依赖**：CHG-UX2-01..05
- **建议模型**：opus
- **子代理（强制）**：arch-reviewer (claude-opus-4-7) 全序列评级；A 或 B+ → 收口；C → BLOCKER
...
- **CHG-UX2-EXT-F**：spacing token 真源补缺 — 触发：CHG-UX2-05 还原的 5 处弱语义裸值需要新 token 槽位承接（或 -06 收口阶段 arch-reviewer 视觉走查发现新缺口）
  - **背景**：CHG-UX2-05 复核发现 5 处 padding 借用 layout token 时语义错位，已还原为裸值（commit `507a28b6` + `7ceb6015`），等真源补缺槽位后再迁移
  - **候选新槽位**：
    · `panel-padding-x/y` — PendingCenter SECTION / RejectedTabContent actions section 等 panel-in-page 的内 padding（区别于 section-gap 的"段间间距"语义）
    · `button-padding-x` — VideoListClient BATCH_BTN/HEAD_BTN 等 component scope 按钮水平 padding（应在 components/button.ts 真源；本槽属暂时占位）
    · 评估 `card-padding` shorthand 用法 — RejectedTabContent card body 4 边等值 14 的场景（card-padding-y / card-padding-x = 14/18 不一致，需新 `card-padding`/`card-padding-uniform` 槽位或重新审视 card-padding-x 取值）
  - **关联文件**（迁移目标）：
    · `apps/server-next/src/app/admin/moderation/_client/PendingCenter.tsx` SECTION padding
    · `apps/server-next/src/app/admin/moderation/_client/RejectedTabContent.tsx` actions / card body / rejection info 三处 padding
    · `apps/server-next/src/app/admin/videos/_client/VideoListClient.tsx` BATCH_BTN_BASE_STYLE / HEAD_BTN_STYLE padding
  - **真源**：`packages/design-tokens/src/admin-layout/spacing.ts`（如新增 panel-* 槽位）+ `packages/design-tokens/src/components/button.ts`（如新增 button-padding-* 槽位）
