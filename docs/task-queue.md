...
1. **CHG-SN-9-DT-HEADER-REDESIGN-ADR** · 表格头重设计 ADR-149 起草 — 状态：✅ 已完成（2026-05-23）
   ...
   **ADR-149 AMENDMENT 1（2026-05-24）— D-149-13/14/15 + EP 序列重写 4→7 段**：
   ...
   - **CHG-SN-9-DT-AUTOFILTER-EP-3-A** · 12 消费方批量迁移 子卡 A（拆 2 sub-commit + HOTFIX）：
     ...
     - **sub 2 PATCH** arch-reviewer 评审消解（2 红线 + 1 黄线）— 状态：🔄 进行中（2026-05-24 / arch-reviewer Opus EP-3-A 整体评级 **B** / R-EP3A-1 D-150-4 业务 key 桥接系统性断链（共享层 3 处）+ R-EP3A-2 sort 非白名单后端静默 fallback（M-SN-8 反模式）+ Y-EP3A-1 SORT_IDENT_REGEX 启动期断言 / 4 实施 + 3 测试 / 0 Props API 变更 / 0 ADR / 待二次评审目标 A-）
