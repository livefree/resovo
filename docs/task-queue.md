...
7. **CHG-DESIGN-07** — Dashboard 8 卡片浏览态（拆 7A/7B/7C/7D 4 阶段，2026-04-30 修订）
   ...
   **数据源策略**（reference.md §5.1.4 「Stats API 字段仍需核对，不应把接口成功渲染成 —」直接落地）：
   - WorkflowCard 4 段进度：复用 `getModerationStats.pendingReview / published`（已对接）；`待审 / 暂存 / 已上架` 缺字段用 deterministic mock + 标注 follow-up `STATS-EXTEND-DASHBOARD`
   - 4× MetricKpiCard：复用现有 stats 字段；`源可达率 / 失效源 / 7 天 spark 趋势` 暂用 deterministic mock + 标注 follow-up
   - AttentionCard / RecentActivityCard / SiteHealthCard：**全 deterministic mock**（reference.md §5.1.2 已给 mock 蓝图）；mock 数据集中在 `apps/server-next/src/lib/dashboard-data.ts`，便于 M-SN-4+ 替换为 SWR hook
   ...
   **7C — 5 类业务 Card + DashboardClient 重构 + 数据 mock 集中**
   ...
   - **自动化 regression gate**（缺一项 = 7C 未完成；防 CHG-SN-3-08 假完成模式重演）：
     - **unit smoke**（vitest / jsdom）：`tests/unit/components/server-next/admin/dashboard/DashboardClient.test.tsx` 三组断言（mock next/navigation + getModerationStats）
       - case A 接口完整成功：4 行布局存在（`[data-dashboard-row="1|2|3"]` + page__head 各 1 个）；9 类卡片选择器全部命中（`[data-card="attention|workflow|metric-kpi|recent-activity|site-health"]`）；`'—'` **不出现**在任意 cell 文本节点（断言 `expect(container.textContent).not.toMatch(/—/)`）
       - case B 接口字段缺失（如 stats 返回 `{ pendingReview: 5 }` 仅一个字段）：缺失字段卡片走 mock fallback；卡片节点带 `data-source="mock"`；同样 `'—'` 不出现
     ...
     - **e2e smoke**（playwright，`tests/e2e/admin/dashboard.spec.ts`）：
       - 模拟 moderator 登录 cookie；`page.route` 拦截 `/v1/admin/videos/moderation-stats` 三种返回（200 完整 / 200 部分 / 500）
       - 断言三态下 `getByTestId('dashboard-page')` 可见 + 9 类卡片 `[data-card]` 全部 visible + 不可见任意 `'—'` 字符（`page.getByText('—').first()` 为 null）
       - **stats 200 路径**断言所有 4 张 MetricKpiCard 数值非破折号（防止 reference.md §5.1.4 教训复发）
   ...
   **整卡完成标准**：7A→7B→7C→7D 全部 ☑ + 7C 三组自动化 regression gate 全绿 + 9 项视觉对照 ☑ + 8+ 张 visual baseline 截图入库 + verify:token-references / 单测 / e2e / typecheck / lint 全绿；任务卡 commit trailer 含 4 阶段子代理 model ID
   ...
   - **回归防线总结**（防 CHG-SN-3-08 假完成模式）：
     1. 7C unit smoke 三 case 守门（接口完整 / 字段缺失 / 接口失败）
     2. 7C e2e smoke 三 stats 路径守门 + 断言无破折号
     3. 7D visual baseline 截图入库（≥8 张），后续 visual diff CI 消费
     4. 删 StatCard 前全仓 grep 确认无外部引用
     5. 任务设计审升级：S 级模块改造卡列表显式涵盖 07 → 必须 spawn arch-reviewer (Opus) ≥ 2 次（7A + 7B）
...
12. **CHG-DESIGN-12** — Cell 共享组件沉淀（DualSignal / VisChip / thumb / pill / inline xs actions）（2026-04-30 新增；CHG-DESIGN-08 视频库接入时一起做）
