...
6. **META-36** — Phase 3C：视频库元数据列排序过滤改造（状态：🔄 -A ✅ / -B 待开始，依 META-32 ✅ + META-33 ✅）
   - 建议模型：sonnet
   ...
   - 6b. **META-36-B** — 四来源图标簇消费迁移（状态：⬜ 待开始，依 36-A ✅）
     - 建议模型：sonnet
     ...
     - 文件：`VideoColumns.tsx`（meta 列 cell 换 `MetadataSourceIconCluster` 消费 `row.metadataStatus`）；`VideoEditDrawer.tsx`（QUICK_HEAD 头部簇换 density=header + showScore + enrichedAtLabel）；退役旧簇消费点 + 测试迁移（`enrichment-cluster-faces.test.tsx` → 图标簇消费断言）。
