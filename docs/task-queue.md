...
1. **CHG-SN-9-DT-HEADER-REDESIGN-ADR** · 表格头重设计 ADR-149 起草 — 状态：✅ 已完成（2026-05-23）
   ...
   **ADR-149 AMENDMENT 1（2026-05-24）— D-149-13/14/15 + EP 序列重写 4→7 段**：
   ...
   - **CHG-SN-9-DT-HEADER-REDESIGN-EP-4.5** · 矩阵触发器接入 DataTable 主组件：toolbar-right + thead-right fallback + ColumnMatrixMenu wiring 6 callback（含 BLOCKER 修订：业务 key 桥接 clearAllColumnFilters + 合并式 resetColumnVisibility 不丢 width）+ column-visibility.ts 2 工具函数沉淀 + dt-styles `[data-table-matrix-trigger]` 样式块 + 14-16 单测 — 状态：⬜ 待启动（~0.3w / 依赖 AMENDMENT 2 PASS）
   - **CHG-SN-9-DT-HEADER-REDESIGN-EP-5-shared** · DataTableEnumFilter/TextFilter/DateRangeFilter 共享原语 + 50 单测 — 状态：⬜ 待启动（~0.3w）
   - **CHG-SN-9-DT-HEADER-REDESIGN-EP-5-crawler-runs** · 2 select 迁移到列级 ⋯ filterContent + 桥接合约 — 状态：⬜ 待启动（~0.25w）
   - **CHG-SN-9-DT-HEADER-REDESIGN-EP-5-submissions** · 2 select 迁移 — 状态：⬜ 待启动（~0.25w）
   - **CHG-SN-9-DT-HEADER-REDESIGN-EP-5-users** · 1 search 留 + 2 select 迁移 — 状态：⬜ 待启动（~0.3w）
   - **CHG-SN-9-DT-HEADER-REDESIGN-EP-5-audit** · 2 select + 2 input + 2 datetime → date-range 迁移 — 状态：⬜ 待启动（~0.4w）
   - **CHG-SN-9-DT-HEADER-REDESIGN-EP-5-videos** · 5 select 迁移（业务 key 桥接最复杂场景）+ search 迁移 + FilterChipBar 保留 — 状态：⬜ 待启动（~0.4w）
   - **CHG-SN-9-DT-HEADER-REDESIGN-EP-6** · types.ts 删 4 @deprecated prop + 注释清理 + 视觉回归 — 状态：⬜ 待启动（~0.2w）
   - **CHG-SN-9-DT-HEADER-REDESIGN-EP-7** · @livefree 走读 10 代表页 + #UR-B1/B2/B3/B4 闭合验证 + 5 消费方一致性核查 + e2e smoke — 状态：⬜ 待启动（~0.3w）
   - **总工时**：原 2.5w → **5.2w**（+2.7w / AMENDMENT 1 增量）
