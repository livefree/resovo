...
| **CHG-SN-7-REDO-01-E2** | 行级 3 mutations 后端 + 前端 3 actions 接入 | 3 端点（POST routes/:id/test / reprobe / DELETE routes/:id）+ ADR（Opus advisory A 建议合并 actionType `sources.route_action` 走 ADR-121 D-121-5 4 文件框架）+ CrawlerSiteExpand 3 actions 接入（占位 → 真实 onClick + confirm + freeze 守卫） | Opus (ADR) + Sonnet | 0.35w (Y2 重估) | 🔴 P0 |
