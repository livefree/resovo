...
| 卡 | 内容 | 范围项 | 建议模型 | 依赖 | 门禁 |
|---|---|---|---|---|---|
| **IMGH-P2-0A** ⬜ | **ADR：补图闭环端点契约 + 审计扩展**：起草 ADR-NNN（Opus + arch-reviewer PASS）裁决：① 端点命名空间（`/admin/images/*` vs 并入 `/admin/image-health/*`，倾向后者一致）② `GET .../:catalogId/candidates?field=coverUrl\|backdropUrl\|logoUrl` 读 proposals 候选 DTO（缩略/source/confidence；实时 TMDB 拉取推迟）③ `POST .../apply-candidate` 入参 `{catalogId,field,source,sourceRef}` + §7.3 trust 优先级闸门（manual hard lock 不覆盖）+ status=pending_review + 入队 health-check/blurhash + 审计 `image_health.apply_candidate` ④ **审计扩展裁定（BLOCK-1）**：扩 `AdminAuditActionType` TS 枚举（**零 migration**，action_type 无 CHECK / target_kind=image_health 已存在）+ **batch apply 策略裁定（CONCERN-3）**：是否支持「批量从候选补图」（每行 field/source/sourceRef 不一 → 失败汇总语义）或仅「批量打开候选队列」 ⑤ `PUT images` 审计归属裁定（§17.2.2，PUT 当前无 insertAuditLog）。 | 5 | **opus** | 无 | **Opus 子代理 + arch-reviewer PASS + Subagents trailer** + Codex 对抗性审核（ADR 非代码产物） + `verify:endpoint-adr` 端点登记 |
