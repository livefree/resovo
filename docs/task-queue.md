...
- **依赖链**：`PRE-1`（任意时点）；`PRE-2 → CHG-VIR-4`；`CHG-VIR-1/2/3/4`（Phase 0，可并行）→ `CHG-VIR-5/6`（Phase 1）→ **`CHG-VIR-6.5`（Phase 2 前置门禁 / ADR-105a AMENDMENT）** → `CHG-VIR-7/8/9`（Phase 2）→ **`CHG-VIR-OBS-BACKFILL` ✅（Phase 3 硬前置已解除 2026-06-03 / 单环境定档回填完毕）** → **`CHG-VIR-10` ✅（Phase 3 已完成 2026-06-03）**；`CHG-VIR-9-D`（merge 翻默认 + 连通分量折叠，依赖 OBS-BACKFILL + merge shadow 稳定）；`CHG-VIR-11`（Phase 4，依赖 PRE-1+Phase1）；`CHG-VIR-12`（Phase 5，依赖 ADR-176/177 + Phase1-4 稳定）。
