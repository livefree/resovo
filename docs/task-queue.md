...
- **状态**：🔄 进行中（9/11 — Phase 0 ✅（0A/0B）+ Phase 1 ✅（1A/1B/1C/1D）+ **Phase 2 进行中**：2A ✅ ImageCompare〔admin-ui 共享组件，Opus arch-reviewer agentId a9732b79ad7128d4d 定双契约〕；下一卡 2B ImageCandidatePicker〔硬串行，同设计〕）
...
| **IMGH-P2-2B** ⬜ | **共享组件：`ImageCandidatePicker`**（admin-ui 新）：多源候选缩略选图（消费 candidates DTO：缩略 + source pill + confidence 标记 🟢/🟡 + 选中态 + 加载更多入口槽，§5.2/§17.3.5）。Opus 子代理定 Props 契约 + EmptyState/LoadingState 复用 + 颜色零硬编码。 | 2 | **opus** | 0A + 1A（candidates DTO）+ 2A（硬串行，admin-ui barrel/types） | **Opus 子代理 + arch-reviewer PASS + Subagents trailer** + admin-ui 组件测试 |
