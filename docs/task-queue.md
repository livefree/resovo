...
- **状态**：🔄 进行中（1/11 — IMGH-P2-0A ✅ ADR-208 Accepted；下一卡 0B 治理表增强 ADR）
...
| **IMGH-P2-0B** ⬜ | **ADR：治理表增强端点契约**：起草 ADR-NNN（Opus + arch-reviewer PASS）裁决：① `/missing-videos` 服务端筛选 query 扩展（search/posterStatus/event_type/posterSource/brokenDomain；分页 total 一致）+ **`brokenDomain` distinct 裁定（CONCERN-2）**：派生自 `broken_image_events.url` 非简单列 → 扩 ADR-150 通用 distinct 受控表达式 or 新增 image-health 专用 distinct ② resolve-event **薄端点契约（BLOCK-2 纠正）**：暴露 `resolveImageEvents(ids:string[],note?)`，入参 `{eventIds: string[], note?}`（**不支持 videoId**——函数实签名只收事件 id 数组；videoId 维度=查未解决事件，须另拆业务端点）+ 审计 `image_health.resolve_event` ③ ids 精确重扫端点契约（`{videoIds\|catalogIds}`，与全局 rescan scope 切开，**禁伪装**）④ **`/missing-videos` DTO 补 `catalogId`（BLOCK-3，供 3A 行→抽屉调 candidates）+ `event_type`（Lightbox P2 精确元信息，承接 P1-3）+ `candidateCount`/`hasHighConfidenceCandidate` 聚合（BLOCK-4，供 3B 候选数列避 N+1）；尺寸字段不入后端（CONCERN-1，沿用客户端 naturalWidth）**。 | 4 | **opus** | 0A（命名空间裁定先行） | **Opus 子代理 + arch-reviewer PASS + Subagents trailer** + Codex 对抗性审核 + `verify:endpoint-adr` |
