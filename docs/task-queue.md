...
| 16 | **CHG-SN-6-RETRO-3-B** | ultrareview P1-1 + P2-6/7/8：CSS shorthand+longhand stylelint guard（verify:style-shorthand-conflict.mjs）+ AuditClient 521 行拆 cell（CHG-DESIGN-12 沉淀）+ ImageHealthClient 缺图列扩展 + SettingsContainer 顶部按钮原生 → AdminButton | sonnet（拆分 cell 跨包改 admin-ui 触发 Opus 评审）| 0.15-0.25w | 🟡 P1 |
