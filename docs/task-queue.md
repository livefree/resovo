...
| 46 | **CHG-SN-6-29-PATCH-2** ✅ 已完成（2026-05-17）| M-SN-6 债务可见性兜底 — task-queue.md 起 7 跟踪卡（3 高优先 PRE/MISC + 4 低风险 LOW）+ M-SN-6 milestone 关闭声明 / 1 文件 / 0 测试改动 / 4018 unit PASS 保持 | opus（延续会话）| 0.05w | 🔴 P0（债务可见性）|
...
|---|---|---|---|---|---|
| **CHG-SN-7-PRE-01** | 文件大小守卫 verify:file-size-budget | 新脚本 `scripts/verify-file-size-budget.mjs`：扫描 `apps/**/*.tsx`、`apps/**/*.ts`、`packages/**/*.tsx` 超 500 行 → exit 1；preflight 集成；GENERIC_WHITELIST 支持 `.types.ts` 类型聚合文件 / `index.ts` re-export 文件等结构性大文件豁免；首次扫描应零命中（PATCH-1 后所有 crawler 文件 ≤ 500） | sonnet | 0.1w | 🔴 P0（H1 系统性盲点根因） |
...
| ID | 标题 | 范围 | 建议模型 | 工时 | 优先级 |
|---|---|---|---|---|---|
| **CHG-SN-7-MISC-CRAWLER-FILE-SIZE** | crawler 3 大文件主动拆分预案（M1 / M-SN-6 中风险） | 评估 CrawlerRunDetailView 445 / TaskLogsDrawer 491 / CrawlerRunsView 429 是否拆分：若 PRE-01 守卫触发警告则按 PATCH-1 范式拆分 | sonnet | 0.2-0.4w（实际触发时） | 🟡 P2 |
...
- 新视图/Drawer：/admin/audit + image-health + crawler（4 视图 + 3 Drawer + 4 控制按钮）+ SettingsContainer 5/5 Tab
...
- 文件大小硬上限：合规（最大 CrawlerRunsView 429 < 500）

**M-SN-7 入口**：CHG-SN-7-PRE-01（文件大小守卫）→ CHG-SN-7-PRE-02（ADR-121 R-MID-1 协议化）→ 选择性承接 PERSITE / 通知 Hub MVP（需 ADR） / DAG 视图（需 ADR）/ M-SN-6 中低风险债务。
