...
1. **CHG-SN-5-PRE-01-A** · DEBT-SN-3-B（staging cookie + nginx e2e 演练，需人工，🔴）— 状态：⬜ 未启动
   - 创建时间：2026-05-06
   - 文件范围：staging 环境配置 + 演练记录文档（不动 plan 不动 code）
   - 验收要点：cookie 跨服务（server ↔ server-next）切换透明 + 5 步金票路径全绿
   - **staging-waiver 协议**：如人工演练受阻可走 staging-waiver（用户 sign-off 触发 + 登记到 task-queue 欠账段）
   - 工时估算：0.3w
2. **CHG-SN-5-PRE-01-B** · DEBT-SN-3-C（M-SN-3 milestone 阶段审计，🔴）— 状态：⬜ 未启动
   - 创建时间：2026-05-06
   - 依赖：PRE-01-A 完成或 staging-waiver
   - 文件范围：`docs/M-SN-3-milestone-audit-2026-MM-DD.md`（新建）
   - 子代理：arch-reviewer (Opus) — A/B/C 评级
   - 工时估算：0.1w
3. **CHG-SN-5-PRE-01-C** · DEBT-SN-4-05-A（toggleSource 乐观锁缺失，🔴 并发安全）— 状态：⬜ 未启动
   - 创建时间：2026-05-06
   - 文件范围：apps/api/src/services/SourceService.ts toggleSource + apps/api 端点 + tests/unit
   - 验收要点：加 ETag/version 列 + expectedUpdatedAt 乐观锁 + 并发测试
   - 工时估算：0.1w
4. **CHG-SN-5-PRE-01-D** · DEBT-SN-4-05-B（feedback.ts XFF trustProxy 白名单，🔴 IP 欺骗绕过 rate-limit）— 状态：⬜ 未启动
   - 创建时间：2026-05-06
   - 文件范围：apps/api/src/server.ts trustProxy + nginx 配置 + apps/api/src/routes/feedback.ts getClientIp
   - 验收要点：XFF 仅信任白名单 IP + rate-limit 不可绕过
   - 工时估算：0.1w
5. **CHG-SN-5-PRE-01-E** · DEBT-SN-4-A（5 件下沉组件 ~12 张 Playwright `toHaveScreenshot()` baseline + 回溯 M-SN-4 改动校验，🟠）— 状态：⬜ 未启动
   - 创建时间：2026-05-06
   - 文件范围：tests/visual/playwright.config + 12 张 baseline PNG（BarSignal × 5 状态 / StaffNoteBar display+edit / LineHealthDrawer / RejectModal / DecisionCard 三态）
   - 前置：建立 Playwright visual harness 基础设施
   - 工时估算：0.4w
6. **CHG-SN-5-PRE-01-F** · DEBT-SN-4-07-A（visual baseline 7 张占位 PNG（69-byte 单像素）替换为真截图，🟠）— 状态：⬜ 未启动
   - 创建时间：2026-05-06
   - 文件范围：tests/visual/admin-moderation/* 7 张 PNG 替换
   - 工时估算：0.2w

### B 段：DEBT-LINE-KEY-01 决策（1 单卡）

7. **CHG-SN-5-PRE-02** · DEBT-LINE-KEY-01 决策（line_key 一级建模 vs 维持复合键，🟡 决策性）— 状态：⬜ 未启动
   - 创建时间：2026-05-06
   - 主循环模型：claude-opus-4-7
   - 子代理：arch-reviewer (Opus) — 强制（决策性 schema 设计）
   - 关联 plan §：§6 M-SN-5.5 B 段 + §10.9 R-M-SN-5-01 + §9 ADR-114 候选
   - 文件范围：`docs/decisions.md`（方案 A 否定 ADR / 方案 B → ADR-114 候选占位）+ `docs/server_next_plan_20260427.md` 修订（按选定方案更新）
   - **不在范围（明列防扩张）**：migration 文件 / 端点 schema 修订 / ADR-114 实施
   - 验收要点：
     - 方案二选一裁决（A 维持复合键 / B 一级建模）
     - 若方案 B：ADR-114 候选登记 + Non-Goals 第 3 条豁免 sign-off 待办登记 + ADR-端点先后协议引用
     - **不允许本卡内含 migration / 端点 schema 修订**
   - 工时估算：0.3w
   - **后续触发**（不在本卡）：方案 B 选定后另起 SEQ：(1) ADR-114 起草卡（Opus PASS 前置）/ (2) Non-Goals 豁免 sign-off + plan v2.7 修订 / (3) migration 卡 / (4) 端点修订卡

### C 段：admin-ui 通用原语/Popover 6 子卡（每件独立 Opus 评审）

> 强约束：本段卡只下沉原语到 packages/admin-ui，**不接业务视图**。

8. **CHG-SN-5-PRE-03-A** · `PageHeader`（reference §5 各页 page__head 统一壳）— 状态：⬜ 未启动
   - 文件范围：packages/admin-ui/src/components/page-header/* + tests/unit
   - 验收要点：title / subtitle / actions 三 slot + 零硬编码颜色 + Storybook demo
   - 工时估算：0.1w
9. **CHG-SN-5-PRE-03-B** · `AdminButton`（reference §4.2 Button 规范）— 状态：⬜ 未启动
   - 文件范围：packages/admin-ui/src/components/admin-button/*
   - 验收要点：variant（primary / secondary / ghost / danger）+ size（sm/md/lg）+ loading + icon
   - 工时估算：0.1w
10. **CHG-SN-5-PRE-03-C** · `AdminInput`（reference §4.2 Input 规范）— 状态：⬜ 未启动
    - 文件范围：packages/admin-ui/src/components/admin-input/*
    - 验收要点：type（text/email/password/number）+ size + prefix/suffix + error 态
    - 工时估算：0.1w
11. **CHG-SN-5-PRE-03-D** · `AdminSelect`（reference §4.2 Select 规范）— 状态：⬜ 未启动
    - 文件范围：packages/admin-ui/src/components/admin-select/*
    - 验收要点：单选 / 多选 / 搜索 / 异步加载 + 键盘导航
    - 工时估算：0.15w
12. **CHG-SN-5-PRE-03-E** · `AdminCard`（reference §4.3 Card 规范）— 状态：⬜ 未启动
    - 文件范围：packages/admin-ui/src/components/admin-card/*
    - 验收要点：surface 层级 + padding 槽位 + header / body / footer 三段
    - 工时估算：0.1w
13. **CHG-SN-5-PRE-03-F** · `Popover`（reference §4.5 弹层规范）— 状态：⬜ 未启动
    - 文件范围：packages/admin-ui/src/components/popover/*
    - **强约束**：API 契约复杂度（Portal / focus-trap / dismiss 协议）若超 Drawer，**必须先升独立 sub-ADR + Opus arch-reviewer PASS 才能起实施卡**
    - 工时估算：0.15-0.25w（含 sub-ADR 起草缓冲）
...
- M-SN-5.5 milestone 阶段审计（plan §5.3）由 SEQ 完成时另起独立 milestone-audit 卡承担
