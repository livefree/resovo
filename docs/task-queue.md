...
8. **CHG-DESIGN-08** — Video library 视觉对齐 ✅ **8A 已完成**（2026-04-30）；8B/8C 留新 session
   - **8A 已落地**：
     - page__head（标题/副标题 + 「导出 CSV」「手动添加视频」disabled buttons + Codex防 inert fix#1）
     - 封面 64×36 横图 → 32×48 竖版（消费 Thumb size="poster-sm"）
     - 列对齐 reference §6.1 标杆：thumb / title+meta / type Pill / sources dot+文案 / probe DualSignal /
       image P0 Pill / visibility VisChip / review Pill / actions（保留 AdminDropdown）
     - 删除 VideoStatusIndicator / VideoTypeChip / 对应单测（被 5 cell 取代）
   - **8B（留新 session）**：saved views（个人/团队） + 表头菜单 + flash row 接入（DataTable Step 7A API 已就位）
   - **8C（留新 session）**：unit smoke + e2e 守门 + Playwright MCP visual baseline 入库 `tests/visual/videos/`
   - **follow-up `VIDEO-INLINE-ROW-ACTIONS-MIGRATE`**：actions 列 inline xs btn ×5 重构（VideoRowActions
     AdminDropdown → InlineRowActions）。当前 dropdown 含 visibility multi-state / review state-transition /
     豆瓣同步 等业务 actions；强行迁 inline 5 个会丢业务能力（VideoEditDrawer 暂不含这些 actions）。
     触发条件：先在 CHG-DESIGN-10 VideoEditDrawer 扩张里把 visibility/review/publish 工作流
     纳入 Drawer 内部，再启动 inline 重构卡，避免业务退化
   - **验收（已达成）**：reference.md §6.1 列规范 9/10 项对齐（actions 形态留 follow-up）；视觉签收 8C 阶段做
