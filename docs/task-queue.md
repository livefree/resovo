...
13. **CHG-SN-5-PRE-03-F** · `Popover`（reference §4.5 弹层规范）— 状态：⏸ 触发 sub-ADR 前置约束（2026-05-06，需起 PRE-03-F-ADR 独立卡）
    - 创建时间：2026-05-06
    - 文件范围（原计划）：packages/admin-ui/src/components/popover/*
    - **强约束**：API 契约复杂度（Portal / focus-trap / dismiss 协议）若超 Drawer，**必须先升独立 sub-ADR + Opus arch-reviewer PASS 才能起实施卡**
    - **复杂度评估结论**（2026-05-06 主循环 Opus 4.7）：Popover **明确超过 Drawer**：
      - Drawer 154 行 + 8 props，固定 placement（left/right/top/bottom），无 placement 算法
      - Popover 必须含 placement 算法（auto-flip / shift 防 viewport overflow，~6-12 placement 方位）+ trigger-content 关联模式（Drawer 无 trigger 概念）+ arrow 子组件（可选）+ portal target/appendTo prop + 受控/非受控 open + ARIA combobox/menu 不同模式
      - API props 估算 11+ 个（>Drawer 8 个）+ 手写 placement 算法 vs 引入 floating-ui 依赖（packages/admin-ui 不引入新依赖）的实现复杂度倍增点
    - **触发结论**：必须先起 **PRE-03-F-ADR** 独立 sub-ADR 卡（API 契约 + placement 策略 + portal/focus-trap/dismiss 协议 + 与既有 AdminDropdown / AdminSelect inline popover 模式对比 + Opus arch-reviewer PASS）→ ADR PASS 后才能起实施卡
    - PRE-03-F 实施卡**不在本 SEQ-20260506-02 范围**；本 SEQ 8/13（PRE-01-C/D + PRE-02 + PRE-03-A..E 五件）已闭环 + 4 张不可执行（PRE-01-A/B 人工 + PRE-01-E/F Playwright harness）+ PRE-03-F 转 sub-ADR 前置 → SEQ 实际工作量已最大化
    - 工时估算（原）：0.15-0.25w（含 sub-ADR 起草缓冲）— 实际超出本 SEQ 范畴
