...
2. **CHG-DESIGN-02** — DataTable frame 扩展（建议 opus + arch-reviewer）✅ Step 1–6 + 7A 完成（Step 7B 推迟到 CHG-DESIGN-08）
   - 进度（2026-04-30）：Step 1–6 ✅ + Step 7A ✅ 全部完成；arch-reviewer (claude-opus-4-7) CONDITIONAL PASS，5 项必修全部落地
   - Step 7A 已实现：`.dt__body` 独立滚动 + `.dt__foot` 内置 pagination(`pagination?: PaginationConfig`) + 隐藏列 chip + `HiddenColumnsMenu` popover + filter chips slot + 6 种 FilterValue 默认 formatter + `column-visibility.ts` 共享工具
   - Step 7B 视频库接入 / Step 7C cell 沉淀（CHG-DESIGN-12）作为后续推进，不在 CHG-DESIGN-02 范围内
   - **不引入** `TableFrame` 新抽象层（reference.md §0-7）
   - 阈值：DataTable props ≈ 17（< 20，安全余量充足）
   - **验收结果**：35 单测全绿（step-7a-pagination-foot / step-7a-hidden-cols / step-7a-filter-chips / step-7a-body-scroll）；2603 全套 typecheck / lint / verify:token-references / verify:admin-guardrails 通过；视频库消费切换路径已转入 CHG-DESIGN-08 + Step 7B
