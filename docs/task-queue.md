...
2. **CHG-PLAN-03** — M-SN-5 主体 SEQ 起草 + Opus 评审 + 用户 sign-off
   - **状态**：🟢 待用户 sign-off（2026-05-12，arch-reviewer Opus 2 轮闭环 PASS：第 1 轮 CONDITIONAL → 修订 3 红线 + 4 黄线 + 3 advisory → 第 2 轮 PASS 无条件）
   - **建议模型**：opus
   - **执行模型**：claude-opus-4-7
...
- **状态**：🟡 规划中（等待 SEQ-20260512-01 CHG-PLAN-03 Opus 评审 + 用户 sign-off）
- **创建时间**：2026-05-12
...
1. **CHG-SN-5-01** · `/admin/submissions`（用户投稿视图，无新端点）
   - **状态**：⬜ 待开始 · **建议模型**：sonnet
...
3. **CHG-SN-5-03** · `/admin/users`（用户管理视图，无新端点）
   ...
   - **范围**：列表 + 角色变更 + 封禁 + 解封（消费现有 users 端点）+ 6 原语消费
   - **依赖端点核验**（R1 修正）：CHG-SN-5-03 启动前主循环必须独立 grep `apps/api/src/routes/admin/users.ts` 端点全集 + 在卡内登记证据；如发现端点缺位（如角色变更端点）→ 该卡升级为"含 ADR 前置 + 端点新增"模式（类比 Phase B/C），整体 SEQ 工时上调
...
4. **CHG-SN-5-04** · ADR-104 起草（home_modules admin API 协议 sub-ADR）
   - **状态**：⬜ 待开始 · **建议模型**：opus
   - **关联 plan §**：§4.5 ADR-端点先后协议 + §9 ADR 索引 ADR-104
   - **范围**：6 端点契约（list / create / update / delete / reorder / publish）+ 鉴权 + 缓存失效 + 错误码 + audit log
   - **子代理调用**：arch-reviewer (Opus) — ADR sub-ADR Opus PASS 强制（CLAUDE.md 模型路由）
   - **完成判据**：ADR-104 落 `docs/decisions.md` + Opus PASS ≤ 3 轮 + plan §9 ADR 索引推进
   - **不在范围**：端点实施代码（独立 -05/-06/-07 卡承担）
   - **工时估算**：0.15w

5. **CHG-SN-5-05** · home_modules 端点实施第 1 批（list + create + update）
   - **状态**：⬜ 待开始 · **依赖**：CHG-SN-5-04 ADR-104 PASS
   - **建议模型**：sonnet
...
7. **CHG-SN-5-07** · `/admin/home`（首页运营位编辑器视图）
   - **状态**：⬜ 待开始 · **依赖**：CHG-SN-5-05 + CHG-SN-5-06 完成
...
8. **CHG-SN-5-08** · ADR-105 起草（merge candidate / split / unmerge API 协议 sub-ADR）
   - **状态**：⬜ 待开始 · **依赖**：CHG-SN-5-04 ADR-104 PASS（Y4 修正：ADR 起草串行，不并行）
   - **建议模型**：opus
   - **关联 plan §**：§4.5 ADR-端点先后协议 + §9 ADR 索引 ADR-105 + ADR-114-NEGATED（line_key 否定 PRE-02 决策约束 schema 维持复合键 `(source_site_key, source_name)`）
   - **范围**：3-4 端点契约（candidate-preview / split / unmerge / merge-audit-log）+ audit log schema + 鉴权 + 错误码 + 跨视频合并候选算法基线
   - **完成判据**（A2 修正）：ADR-105 落 `docs/decisions.md` + Opus PASS ≤ 3 轮 + plan §9 ADR 索引推进 + 含 candidate 评分函数 v1 接口签名 + 性能基线（≤100 候选 + p95 ≤200ms）+ R-M-SN-5-B 性能 retest 入口
   - **子代理调用**：arch-reviewer (Opus) — ADR sub-ADR Opus PASS 强制
   - **工时估算**：0.15w（**Y4 修正：ADR-104/105 起草串行，先 -04 PASS 再 -08；ADR 起草占用 Opus 子代理 + 用户 sign-off 注意力，且 ADR-105 涉及 ADR-114-NEGATED 二次表态，不并行**）

9. **CHG-SN-5-09** · candidate-preview 端点实施
   - **状态**：⬜ 待开始 · **依赖**：CHG-SN-5-08 ADR-105 PASS
   - **建议模型**：sonnet
   - **范围**：跨视频合并候选预览端点（GET 列表 + 评分算法）+ zod + unit test
   - **工时估算**：0.25w

10. **CHG-SN-5-10** · split + unmerge 端点实施
    - **状态**：⬜ 待开始 · **依赖**：CHG-SN-5-08 ADR-105 PASS
    - **建议模型**：sonnet
    - **范围**：拆分端点（POST 拆视频组）+ 取消合并端点（POST 撤销合并）+ audit log + unit test
    - **工时估算**：0.25w

11. **CHG-SN-5-11** · `/admin/sources`（线路矩阵 + 视频维度分组 + 全局别名表）
    - **状态**：⬜ 待开始 · **依赖**：CHG-SN-5-09 + CHG-SN-5-10 完成（如需 merge audit log）
...
12. **CHG-SN-5-12** · `/admin/merge`（合并 candidate 预览 + 拆分工作台视图）
    - **状态**：⬜ 待开始 · **依赖**：CHG-SN-5-09 + CHG-SN-5-10 完成
    - **建议模型**：sonnet；如启动后发现合并算法 UI 表达复杂度高于预期 → 写 BLOCKER 暂停会话，用户 sign-off 后另启 opus 会话续推
    - **关联 plan §**：§6 M-SN-5 行 528 + ADR-114-NEGATED 复合键约束
    - **范围**：合并 candidate 预览 + 拆分工作台 + audit timeline
    - **e2e 黄金路径**：登录 → 进入 /admin/merge → candidate 预览列表 → 选中一组 → 确认合并 → 拆分工作台展示新合并 → audit timeline 写入条目
    - **工时估算**：0.5w

**Phase D · milestone 收尾审计（1 卡）**

13. **CHG-SN-5-13** · M-SN-5 milestone 阶段审计（Opus arch-reviewer）
    - **状态**：⬜ 待开始 · **依赖**：CHG-SN-5-01 ~ CHG-SN-5-12 全部 ✅
    - **建议模型**：opus
