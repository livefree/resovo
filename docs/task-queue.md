...
4. **CHG-DESIGN-04** — Sidebar 过渡动效 + 分区标题等高占位
   ...
   - active 配色已在 CHG-DESIGN-01 fix 中改为 `--accent-active`（满足 WCAG AA），本卡只需保留
   - 顺手补语义化 token 别名 `--accent-on-soft`（指向 `--accent-active`，作为"fg-on-accent-soft 背景"的语义入口），把 sidebar / pagination 现有 `--accent-active` 引用改为 `--accent-on-soft`，让消费方语义自洽
