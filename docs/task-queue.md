...
| ID | 标题 | 范围 | 建议模型 | 工时 | 优先级 |
|---|---|---|---|---|---|
| ✅ **CHG-SN-7-PRE-04** 已完成（2026-05-20）| 全量审计 16 admin 路由 vs 设计稿 §5.1–§5.16 | 16/16 路由逐路由审计产出 ✅/⚠️/❌；arch-reviewer Opus 收尾裁决 REDO-NN 优先级（REDO-01/02/03/04 全识别 + 全完成）；评级 **A−**；MISC 跟踪 16 项（9 ✅ / 1 🟡P2 / 6 🟢P3）；REDO-04 IA 分歧正式裁决「独立路由」；`docs/M-SN-7-design-realign-audit-FULL.md` 收尾节填充 | sonnet 主循环 + arch-reviewer (claude-opus-4-7) | 0.9w 实际（收尾 0.05w）| ✅ |
...
- **状态**：🔄 进行中（1/3 卡完成：CHG-SN-7-CLEANUP-01-A ✅ 2026-05-21）
- **创建时间**：2026-05-21
...
2. **CHG-SN-7-CLEANUP-01-B** · 引用改写 + docs/README + CLAUDE.md §规范文件索引修订
   - **状态**：⬜ 待开始（C1-A 完成 + verify 红线评估后启动）
   - **建议模型**：sonnet
   - **范围**：
     - sed 批量改 CLAUDE.md / docs/README.md / docs/decisions.md / docs/changelog.md / docs/task-queue.md / docs/server_next_plan_20260427.md / docs/architecture.md / docs/rules/*.md 内的 27 项归档文件路径
     - 重写 docs/README.md §1 权威文档 + §3 保留参考清单
     - 重写 docs/README.md §6 新增「M-SN-8 + manual 入口」段
     - CLAUDE.md「规范文件索引」节去除已归档项的旁支引用（admin-module-template.md 决策 3 保持现状不拆）
   - **风险**：CLAUDE.md 是硬约束文件；改前必须 grep 验证全部引用
   - **依赖**：CHG-SN-7-CLEANUP-01-A PASS
   - **工时估算**：0.2w

3. **CHG-SN-7-CLEANUP-01-C** · docs/manual 骨架 + verify:manual-coverage 守卫
   - **状态**：⬜ 待开始（C1-B 完成后启动）
   - **建议模型**：sonnet
   - **范围**：
     - 新建 docs/manual/ 完整目录结构（README + _template/ + 00/01 序章 + 5 workflow 空架 + 15 page 空架 + 5 picker 空架 + glossary）共约 30 文件
     - PAGE_TEMPLATE.md / WORKFLOW_TEMPLATE.md 模板（强制 8 章节字段）
     - 新建 scripts/verify-manual-coverage.mjs（CI 守卫：admin/* page.tsx ↔ manual/20-pages/ 1:1）
     - package.json 加 "verify:manual-coverage"
     - preflight.sh 集成新 verify 步骤
   - **依赖**：CHG-SN-7-CLEANUP-01-B PASS
   - **工时估算**：0.3w
