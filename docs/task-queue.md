...
- **状态**：🔄 执行中（META-55-ADR ✅ ADR-216 Accepted·待用户裁可解锁 DC-216-1；用户 2026-06-27「通过方案，开始执行」）
...
- **做**：①「单一真源」方向决策（ADR-216）；② 存量矫正（列←refs/catalog 回填，止血）；③ 源头防复发（enrich 不得在已有 applied ref 时把列写 `unmatched` / 判据统一 / 事务耦合）；④ 直接列消费方迁到 refs 派生谓词、冻结列作为可查询真源；⑤ 软删 ref 卫生（purge/tombstone）。
- **不做**：本序列不新增任何外部抓取/匹配能力；不改 derive 的 refs-canonical 既定优先级（ADR-201）；不在 ADR 定向前物理 DROP 列。

### 依赖与原子化

- ADR-216 **先行硬前置**。跨 schema·script·service·多消费方查询 + **两谓词语义契约** → **强制拆卡 + 拆 2 张设计卡**（DC-216-1/DC-216-2，CLAUDE.md 强制升 Opus 情形 1「定义共享谓词契约」）。**串行主链（两轮对抗审定稿，详见 ADR-216 实施序列）**：`55-ADR → DC-216-1 谓词契约 → 57-ENRICH 守卫 → 58-CONSUMER 迁移〔迁 videoRefAppliedSql〕→ DC-216-2 软删处置表 → 59-REF 软删读期过滤 → 56-DROP-prep → 60-完全停写+冻结守卫 → 独立 DROP ADR`。**关键裁定（D-216-2，Codex B-1 推翻初裁后改裁）**：derive overall 是 **catalog-first**（catalog candidate 拦截 video auto_matched，实测 60 漂移行仅 34 applied/28 candidate），故消费方**不能迁 overall**、须迁 **video-ref-applied 谓词**（含 auto_matched，等价旧 matched）→ 60 行真止血、无 14 行降级。

### 任务列表（按执行顺序）

| 卡 | 状态 | 内容 | 范围项 | 建议模型 | 依赖 | 门禁 |
|---|---|---|---|---|---|---|
| **META-55-ADR** | ✅ 完成（待用户裁可解锁实现） | 起草定稿 ADR-216。arch-reviewer 两轮裁定（初裁 + 复议收回承重错误）+ Codex **两轮**对抗审（round-1 4B+3C / round-2 1B+3C+1NIT）全数吸收；方向 A 变体 Accepted、两谓词分工收口契约定稿、强制拆 DC-216-1/DC-216-2。 | docs | **opus**（主循环）+ `arch-reviewer`(opus)×2 + codex-rescue×2 | 无 | ✅ ADR Accepted；arch-reviewer 两轮 PASS；Codex 两轮 BLOCK 全处理；不写实现代码。**待用户裁可解锁 DC-216-1。** |
| **DC-216-1-PREDICATE** | ✅ 完成 | **谓词语义契约（Opus·57+58 共同前置，D-216-9）**：定义 `videoRefAppliedSql` 导出常量（`EXISTS … is_primary AND match_status IN('auto_matched','manual_confirmed')`，阈值含 auto 写死=等价旧 matched）；明确两谓词边界（video-ref-applied=是否匹配/发布门禁/富集完成；catalog-overall=元数据状态列排序/工作队列，**不混用**）；JS↔SQL 对拍单测义务登记。**`is_primary` invariant（Codex r2 C-1）**：实测 0 边角但 041 无 schema 约束，须裁定谓词是否强制 is_primary + 约束测试守护。 | 契约 + 导出常量 + 测试 ≤3 | **opus**（共享谓词契约） | META-55-ADR Accepted | 谓词常量单一真源；两谓词边界文档化；对拍单测；is_primary invariant 约束测试；无第三套判据。 |
| **META-57-ENRICH-GUARD** | ✅ 完成 | **根因守卫·非完全停写（D-216-3，B-2 吸收）**：enrich 写列前一致性守卫——已存在 applied video ref（同 `videoRefAppliedSql` 判据）时**不得覆写列为 unmatched**；列仍随 refs 演进（不冻结、无 stale 暴露）。MetadataEnrichService（:94/:183）/DoubanService（:263/:468）/BangumiService 同构。**完全停写并入 META-60**。 | service/queries/tests ≤4 | **opus**（enrich 匹配决策逻辑） | DC-216-1 | 单测：已有 applied ref 时 enrich 不覆写 unmatched；真未命中仍写；守卫判据=DC-216-1 同源；关键路径回归。 |
| **META-58-A-APPLIED-MIGRATE** | ✅ 完成 | **A 核心止血·boolean applied 消费方（D-216-2，实现期拆 A/B）**：迁「是否匹配/发布门禁/富集完成」boolean 用途到 `videoRefAppliedSql`——moderation.ts:111 `ENRICH_COMPLETE_SQL`〔`douban_status='matched'`→`videoRefAppliedSql('douban')`，bangumi 口径一并〕、staging.ts:167/317〔requireDoubanMatched〕、videos.ts:644/660〔backfill `unmatched`→`NOT videoRefAppliedSql`〕。**并入**：补 `idx_video_external_refs_video_provider_status` 索引（D-216-7）+ derive 头注校正（D-216-8）+ 改受影响测试口径。 | queries/index/tests ≤5（boolean 用途逐一对拍） | **opus**（查询语义变更） | META-57 | 60 douban 欠计消除（boolean 路径真止血、无 14 行降级）；moderation/staging 计数回归；不破坏 API；EXPLAIN 无热路径回归。 |
| **META-58-B-1-DOUBAN-MULTISTATE** | ✅ 完成 | **后端 douban 4 态过滤谓词 + 多值过滤迁移（ADR-216 D-216-10/13）**：`video-ref-applied.ts` 扩展过滤侧 4 导出（`DoubanRefStateInput`/`matchesDoubanRefState`/`doubanRefStateSql`/`doubanRefStateFilterSql`）+ JS↔SQL 穷尽四分对拍。迁 **douban** 过滤侧：moderation.ts:219、videos.ts:434、videos.status.ts:180、videos.ts:644+660（douban 半）；bangumi 半全保留（D-216-11）。**Codex 对抗审 2 处 BLOCK 修正**：① pending 改兜底穷尽 `IS DISTINCT FROM 'unmatched'`（D-216-10 原定义非穷尽→漏行 bug）；② **投影迁移整体延 META-60**（D-216-12 漏列 moderation:344+VIDEO_FULL_SELECT，单独迁分裂+耦合 derive）。derive 列兜底清理延 META-60（cache-only 兜底误升 applied）。 | 谓词/queries/tests=6 文件 | **opus**（共享谓词契约扩展） | META-58-A + arch-reviewer 裁定 ✅ | ✅ 穷尽四分恰好命中 1 态；candidate 非 primary 守护；pending IS DISTINCT FROM；douban 多值口径等价旧列；JS↔SQL 守护；bangumi 过滤不破坏。门禁 typecheck=0/lint=0/test:changed 1202/verify=0。 |
| **META-58-B-2-FACET-UI** | ✅ 完成 | **distinct 白名单移除 douban_status 死配置（ADR-216 D-216-12 / ADR-150 AMENDMENT 4）**：事实修正——前端 facet 早用静态 `DOUBAN_STATUS_OPTIONS`（VideoColumns.tsx:504，列未声明 `filterDistinctTable`→从不查 distinct 端点），白名单键为死配置；移除该键 + `datatable-shared` toBeUndefined 守护 + ADR-150 AMENDMENT 4 登记。前端零改动；bangumi 保留（D-216-11）。Codex 对抗审 2 处措辞修正吸收。 | distinct/test/ADR=3 文件 | **opus**（含 ADR AMENDMENT） | META-58-B-1 ✅ | ✅ douban 键移除；bangumi 保留；toBeUndefined 守护；门禁 typecheck=0/lint=0/test:changed 293/verify=0。commit 291244b4。 |
| **META-61-BANGUMI-STATUS-RETIRE** | ⬜ 待开始（独立后续，**前置补 bangumi 信号**） | **bangumi_status 退役（ADR-216 D-216-11 排除范围外，独立卡）**：bangumi 无 `meta_quality.bangumi_match_status` 信号、无法表达 unmatched/pending。前置：补 `meta_quality.bangumi_match_status`（对齐 douban_match_status 结构，BangumiService 写入路径同步）= schema/信号契约变更，强制 Opus。之后迁 videos.ts:438 + bangumi 4 态谓词 + 退役 bangumi_status 列。 | 信号 schema + service + queries（拆后定） | **opus** | META-58-B-1 + bangumi 信号补齐 | bangumi 4 态信号真源；迁移后退役 bangumi_status 列。 |
| **META-62-REFS-INDEX** | ⬜ 待开始（D-216-7 推迟项，性能触发或 Phase complete 前） | **video_external_refs 覆盖索引 + EXPLAIN 基线（D-216-7 ↔ D-216-12 reconcile）**：补 `idx_video_external_refs_video_provider_status (video_id, provider, is_primary DESC, match_status)`（041 现无 match_status 索引）加速 `videoRefAppliedSql`/`doubanRefState*` EXISTS + derive 8 子查询；moderation 列表分页 + videos COUNT 两路径 `EXPLAIN ANALYZE` 基线。D-216-12 已裁功能正确可推迟（041 现有索引足），本卡 = 性能优化兜底（D-216-6 触发：>50k 行或 P95>200ms）。 | migration + EXPLAIN doc ≤2 | sonnet | META-58 全 ✅（已满足） | 索引建成；EXPLAIN 基线记录；architecture.md 索引同步；无功能回归。 |
| **DC-216-2-SOFTDEL-TABLE** | ✅ 完成 | **refs 读路径软删处置表（D-216-9，META-59 清单）**：穷举全仓 `grep video_external_refs` = 13 文件,三类逐路径裁定落表。**类A**（必过滤）metadata-status.derive.ts;**类B**（外层 gate）externalData/externalIdLoader/split-suggestions/video-merge-candidates/video-ref-applied(谓词 invariant)/moderation;**类C**（聚合/gap）home-autofill-douban/-bangumi/external-resources-stats;**N/A** 写路径 3 Service。**事实修正**：video-merge-candidates 原误判 `:69 JOIN deleted_at`（该结构不存在）,实为 `external_ids` per-video 投影 + 外层 `v.id=ANY($1)` 调用方 gate → 类B。 | docs/checklist=2（decisions 表 + tasks 卡） | **opus**（ADR 处置表设计）+ codex-rescue（对抗审,后台 a971042a） | META-55-ADR Accepted | ✅ 13 文件穷举;类A/B/C/N-A 逐条 ground-truth 核实;video-ref-applied invariant 登记;事实错误已修;META-59 据表实施。 |
| **META-59-REF-HYGIENE** | ✅ 完成 | **软删纯读期过滤（D-216-4，只改类C；arch-reviewer 改判 derive 类A→类B）**：按 DC-216-2 处置表 + arch-reviewer 第二轮裁定,只 gate **类C**——`external-resources-stats.ts` 3 条 COUNT(:85/:89-91/:95-97) + `home-autofill-douban.ts:168` gap + `home-autofill-bangumi.ts:166` gap 加 `JOIN videos v ON v.id=ver.video_id AND v.deleted_at IS NULL`。**derive 不动**（类B,全消费方已 gate;DC-216-2 处置表同步修正 derive 类A→类B + 移假漂移标注）。277→241 统计转 live 口径。 | 3 代码 + 3 测试 + 处置表修正 | **opus** + arch-reviewer(opus, ab76495e) | DC-216-2 ✅ | ✅ 类C 3 路径加 gate(软删 ref 不计入);derive 类B 不动;gate 断言守护;门禁 typecheck=0/lint=0/test:changed 227/verify=0。 |
| **META-56-DROP-PREP** | ✅ 完成 | **DROP-prep 对齐脚本（D-216-2，DROP 卫生）**：`scripts/align-douban-status-to-refs.ts` — 圈定有 applied douban ref（`videoRefAppliedSql` DC-216-1 单一真源）但 `douban_status != matched` 的 live video（欠计漂移，立案 60 行）→ 对齐 matched。**方向与 fix-douban-status-consistency.ts 相反**（那修虚高降级，本卡修欠计升 matched）。幂等（UPDATE 自带谓词 + IS DISTINCT FROM matched）+ --dry-run + 参数化批写 + 事务。仅 douban、不触软删/已一致行。 | script + test=2 | sonnet（本会话 opus 连续推进）| META-57 + META-58 ✅ | ✅ 圈定判据=videoRefAppliedSql 同口径；幂等；dry-run；6 单测（parseArgs + SQL builder）；门禁 typecheck=0/lint=0/test:changed/verify=0。**实跑（dry-run→real）属运维步、部署期执行**。 |
| **META-60-FREEZE-GUARD** | ⬜ **deploy-blocked**（前置硬依赖：① META-56 对齐脚本 `align-douban-status-to-refs` deploy 实跑〔运维步〕消除存量漂移 + 对齐 cache ② 软泡期 ≥1 完整 enrich 周期观察——D-216-13:24214「须列完全停写 + DROP-prep 对齐 cache 后才安全」；纳入 B-1 延后项 D-216-13） | **完全停写 + 列冻结守卫 + 投影/derive 收尾（D-216-1/D-216-3/D-216-5/D-216-13）**：读路径全迁后 enrich 完全停写列；CI/lint 守卫禁**运行时代码/新测试**读写 `{douban,bangumi}_status`（**非字面全仓 grep**，C-2：历史 migration/类型注释天然保留）；**并入 B-1 延后项（D-216-13）**：① douban **投影迁移**（`moderation.ts:344` / `VIDEO_FULL_SELECT` / `videos.status.ts:207` → 派生，需先解 derive `statusColumnState` 耦合）；② `metadata-status.derive.ts:471-473` douban 列兜底清理 + `statusColumnState` douban 分支 + `PROVIDER_SQL_SPECS` douban statusCol（保留 bangumi）——须列停写 + META-56 DROP-prep 对齐 cache 后才安全；登记独立 **DROP ADR**（软泡期 ≥1 完整 enrich 周期 + 观察后 Opus ADR：DROP 列 + architecture.md 同步 + 同 commit 删 `MetadataStatusSourceRow.{douban,bangumi}Status`）。 | lint/CI/service/docs（拆后定） | sonnet（DROP ADR 另起 opus 卡） | 守卫命中运行时新增列读写即失败；完全停写后无 stale 暴露；投影迁移消除过滤/投影不一致；DROP ADR 起草卡登记。 |
| **META-63-DROP-ADR** | ⬜ 待开始（**软泡期后 Opus**，独立 DROP ADR） | **douban_status 列物理 DROP ADR 起草（D-216-5/D-216-13）**：META-60 完全停写 + 软泡期 ≥1 完整 enrich 周期观察后，起草独立 Opus ADR——DROP `videos.douban_status` 列 + 幂等 migration + architecture.md 同步 + 同 commit 删 `MetadataStatusSourceRow.doubanStatus` / statusColumnState douban 分支 / 投影残留。bangumi 不在范围（META-61 独立）。 | ADR + migration + service（拆后定） | **opus**（ADR 决策） | META-60 全收尾 + 软泡期 | DROP 前运行时零残留；architecture.md 同步；幂等 migration；ADR Codex 对抗审。 |

### 关键约束与红线

- **方向先行 + 契约先行**：ADR-216 未 Accepted 前不得动消费方/列/enrich；DC-216-1 谓词契约未定不得起 META-57/58（二者共用 `videoRefAppliedSql`）。
- **两谓词不混用（D-216-2 核心）**：`videoRefAppliedSql`（video 级 applied）服务「是否匹配/发布门禁」；`METADATA_STATUS_JOIN_SQL` overall（catalog-first）服务「元数据状态列排序/工作队列」。**禁用 overall 回答「是否匹配」**（B-1 暴露的范畴错误：catalog candidate 会误降级 auto 匹配行）。
- **口径同源**：DROP-prep 映射、ENRICH 守卫判据、CONSUMER 迁移谓词三者必须 = DC-216-1 的 `videoRefAppliedSql`，禁止再造判据。
- **迁移即止血（D-216-2 修正后）**：迁 **video-ref-applied 谓词**（非 overall）才止血；META-56 回填仅服务 DROP 卫生，不阻塞 58。
- **软删纯读期（D-216-4，收回 tombstone）**：零 schema 变更，refs 读路径按处置表 join `videos.deleted_at`；恢复无损；**禁物理 purge / 禁加 refs.deleted_at**。
- **DROP 不在 ADR-216 拍板**：走 META-60 登记的独立 Opus ADR，前置软泡期 + architecture.md 同步 + 运行时代码零残留 + JS↔SQL status 分支同 commit 删。
- **非代码产物门禁**：本任务卡 + ADR-216 修订稿 commit 前须过 Codex round-2 对抗审（workflow-rules「Codex 对抗性独立审核条款」；round-1 4 BLOCKER 已吸收）。
...
- **质量缺口闭合（2026-06-28 外部 review，3 项）**：
  - **P1 覆盖索引推迟登记（D-216-7 ↔ D-216-12 reconcile）**：D-216-7 要求 `idx_video_external_refs_video_provider_status (video_id, provider, is_primary DESC, match_status)` + moderation/videos COUNT `EXPLAIN ANALYZE` 基线「并入 META-58」；D-216-12（arch-reviewer 第三轮）**改裁推迟**——041 现有索引功能正确（applied 谓词命中 partial `(video_id,provider) WHERE is_primary`；candidate 谓词走 `(video_id)`，每 video refs 少 + EXISTS 短路，无瓶颈），性能瓶颈（D-216-6：>50k 行或 P95>200ms）再起独立卡。**登记后续卡 META-62-REFS-INDEX**（覆盖索引 + EXPLAIN 基线，sonnet，D-216-6 触发或 Phase complete 前补）——此前推迟仅在 D-216-12 散文，本登记使其可追溯。
...
> 创建时间：2026-07-10 ｜ 最后更新时间：2026-07-10（FE-1 ✅）
...
| **SEARCH-FE-2**（i18n 补全 + 删死代码） | ⬜ 待开始 | SearchOverlay + SearchEmptyState 硬编码中文文案迁 next-intl（热门搜索/输入关键词/搜索中/内容/相关搜索/未找到/查看全部；TYPE_LABELS）+ en.json/zh-CN.json 补 key + 删死代码 `SearchSuggestions.tsx`（全库零引用，与 SearchOverlay 重复）。 | `SearchOverlay.tsx` + `SearchEmptyState.tsx` + `messages/*.json` + 删 `SearchSuggestions.tsx` | sonnet | 无 | typecheck/lint/test:changed |
| **SEARCH-FE-3**（骨架一致 + reveal 卫生） | ⬜ 待开始 | loading / Suspense 骨架从 VideoGrid 网格改为列表行骨架（匹配 SearchResultRow 真实布局，消加载→呈现布局跳变）+ 收敛 SearchCircularReveal 死联动（无人写 `resovo:search-reveal-origin`→origin 永远默认，Nav 补写坐标或简化注释兑现承诺）。 | `SearchEmptyState.tsx` + `SearchPage.tsx` + `SearchCircularReveal.tsx`（+ 视需 `Nav.tsx`） | sonnet | 无 | typecheck/lint/test:changed |
...
- **未纳入本序列的 UX 增强**（属新功能、需产品定义，另行登记候选）：搜索历史 / 最近搜索（localStorage）、浮层键盘导航（↑↓/Enter/aria-activedescendant）、无结果拼写纠错（did-you-mean）、debounce 三处统一（Nav 200ms / SearchSuggestions 120ms / SearchPage 300ms）、后端 highlight.description 前端未渲染（白算或补展示）。
