...
| 13 | **CHG-SN-6-05** ✅ 已完成（2026-05-15）| SettingsContainer ConfigTab 实施 — GET/POST /admin/system/config 消费 / JSON textarea + 订阅 URL input + dirty 标识 + 4 错误码差异化 toast（INVALID_JSON / INVALID_SUBSCRIPTION_URL / VALIDATION_ERROR / 通用） / 3 文件 / 13 测试 / 共享原语 ~85% / 零新 ADR；audit 留 RETRO-3；commit 待落地 | opus（延续会话；建议 sonnet）| 0.1w | 🟡 P1 |
