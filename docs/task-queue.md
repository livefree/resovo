...
| # | 卡 ID | 范围 | 模型 | 工时 | 优先级 |
|---|---|---|---|---|---|
| 1 | **CHG-SN-6-CHECKLIST-AUDIT-3** | `verify:sql-schema-alignment` 静态扫描守卫（grep queries `<alias>.<column>` vs migration 全集应用后 schema 核验）+ preflight 集成 + arch-reviewer Opus 评审 | opus + arch-reviewer | 0.15w | 🔴 P0 |
| 2 | **CHG-SN-6-INTEGRATION-TEST** | `tests/integration/api/admin-*.test.ts` 集成测试套件（跑真实 PG 子集）覆盖每个 admin route happy path SQL 执行；CI 流水线集成 | sonnet | 0.3w | 🔴 P0 |
| 3 | **CHG-SN-6-CI-MIGRATE-DRY-RUN** | CI 加 `npm run migrate --dry-run` 干跑核验 dev DB / 生产 / staging 三环境 migration 一致性 | sonnet | 0.05w | 🟡 P1 |
| 4 | **CHG-SN-6-AUDIT-TIMELINE** | ADR-118 起草（GET /admin/video-merges/audit 协议）+ Opus PASS + 端点实施 + /admin/merge audit timeline section 扩展（三段式） | opus + arch-reviewer + sonnet | 0.4w | 🟡 P1 |
| 5 | **CHG-SN-6-RETRO-1** | M-SN-3/-4 视图卡前台测试覆盖率 < 9 用例批量补齐（CHG-SN-5-13-PATCH 沉淀硬指标 RETROACTIVE） | sonnet | 0.3w | 🟡 P1 |
| 6 | **CHG-SN-6-RETRO-2** | plan §5.3 阶段审计协议修订列入"视图测试 ≥ 9 / 共享原语 ≥ 80% 占比"硬清单 | sonnet | 0.1w | 🟡 P1 |
| 7 | **CHG-SN-6-DATATABLE-STICKY-SCROLL** | DataTable body 独立滚动 UX 增强（sticky header + 父级 height 约束规范）；ADR-103 AMENDMENT 续 | sonnet | 0.15w | 🟢 P2 |

**自动化循环**：延续 -13 模式（执行 → 评估 → PATCH → 隐式重审 → 通过 → 下一卡）；不可绕过节点 = 每卡完成后用户验证 + ADR 起草前确认。

**工时合计**：1.45w（RETRO 批次）+ plan §6 M-SN-6 主体卡（暂未拆解，沿用现描述）。
