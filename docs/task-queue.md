...
1. **CHG-SN-9-DT-HEADER-REDESIGN-ADR** · 表格头重设计 ADR-149 起草 — 状态：✅ 已完成（2026-05-23）
   ...
   - **后续 EP**（ADR PASS 后启动，5 段渐进 / typecheck 不破裂）：
     - **CHG-SN-9-DT-HEADER-REDESIGN-EP-1** · types.ts deprecate + column-matrix-menu.tsx + dt-styles 矩阵样式 + 35 单测 — 状态：🔄 进行中（2026-05-23 启动 / ~0.6w）
     - CHG-SN-9-DT-HEADER-REDESIGN-EP-2 · header-menu anchor 切换 + 列名 toggle 排序 + ⋯ stopPropagation + 10 单测（~0.5w）
     - CHG-SN-9-DT-HEADER-REDESIGN-EP-3 · 删 hidden-columns-menu / filter-chips / filter-chip 三文件 + 10 单测（~0.3w）
