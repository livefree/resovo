...
- **状态**：🔄 进行中（Phase 0 ✅ + Phase 1 ✅ + Phase 2 ✅ + **Phase 3 进行中**：3A ✅ ImageGovernanceDrawer〔server-next 编排：矩阵=useVideoImages 4 类省 stills、替换聚焦 poster/coverUrl §C 持 Map 取 sourceRef、1D DTO 补 eventId resolve 展示事件、行点击+flash+refresh；8 视图测试〕；3B ✅ DataTable 治理工作台〔缩略列 + eventType/跨源候选数列 + 4 列服务端 filter〔消费 1D〕+ selection/bulkActions〔批量重扫=1C / 打开候选队列 CONCERN-3 无伪批量〕+ distinctFetcher 复用 /broken-domains；+27 新测〕；**下一卡 3C 文档收尾〔haiku〕**）
...
| **IMGH-P2-3C** ⬜ | **文档收尾**：① 手册 `P-image-health.md` Tab B 工作台形态（缩略列/筛选/bulk/候选列/治理抽屉/补图闭环/标记已解决）② `W3-image-fallback.md` 补图与标记已解决步骤 ③ `docs/decisions.md` index + ADR cross-refs ④ frontmatter last_reviewed 刷新。 | 4 | haiku | 3A/3B | docs-only（doc-janitor） + 主循环复核 |
