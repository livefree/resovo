...
| **IMGH-P3-4D** ✅（2026-06-20 / claude-opus-4-8 / 4C 派生 / typecheck·lint·test:changed 5；前台零裂图全闭环） | **修复：DailyAnimeRow 裸 img → SafeImage（4C 核出唯一前台裂图漏网点）**：`apps/web-next/src/components/home/DailyAnimeRow.tsx:97` 裸 `<img src={item.coverUrl}>` 改 `SafeImage`（mode lazy / `aspect="2:3"` / `fallback={{ title, type, seed }}` / `onLoadFail`→`reportBrokenImage`），对齐 VideoCard/BrowseCard 范式 → 前台用户端零裂图全闭环 + 每日新番失败封面进 broken_image_events（被 ADR-211 治理板覆盖）+ 组件测试断言 onError→FallbackCover | 2 | sonnet | 无（4C 报告已定位） | 模块内消费范式 + 组件测试 + typecheck/lint；颜色零硬编码 |
