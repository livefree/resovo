...
13. **NTLG-P2-b** — 多渠道通知统一订阅（webhook 已有 / 邮件实装 / `submission.created` 补触发点）+ 通知偏好（状态：⏸️ **暂缓**，用户裁定 2026-06-10）
    ...
    - **拆卡蓝图（恢复时用）——P2-b ≠ email，建议拆 -1/-2**：
      - **P2-b-1（无门控，可随时做）**：核心剩余 = `submission.created` **webhook 补触发点**——event 已在枚举定义（`packages/types/src/system.types.ts:239`），但 `webhookDispatcher.enqueue` 无该 event 调用（现仅 crawler.run.failed / moderation.pending.threshold / storage.r2.alert / video.batch.complete；UserSubmissionService 仅 emit `user_submission.action` 到通知表、未接 webhook）。纯代码内、零新依赖、无需 ADR。**已核销不再列入（Codex stop-review 2026-06-10 修正真源污染，治理方案 2026-06-08 描述已过时）**：~~清理 NotificationsTab "后端不发 webhook" 陈旧注释~~（grep 零命中、注释已不存在）；~~通知偏好~~ 的 webhook 事件订阅维度已存在（`notification_webhook_events` opt-in subscribedEvents + NotificationsTab UI）——若指 email/per-user 渠道偏好则归 -2 或另评估，非现成待做项。
