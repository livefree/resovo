...
14. **NTLG-P2-c ✅** — 「消息中心」页（全量历史 + 检索）+ 未读 SSE 实时推送（替代 60s 轮询）+ 收口 P1-c-C 3 项（状态：✅ 已完成 2026-06-09 — -ADR〔ADR-196〕/-A〔消息中心〕/-B〔SSE 双模式〕/-C〔crawler 并入 list + 红点 unread-count + F6③ deferred〕全收口；归档 F5 v1 deferred 留独立 follow-up）
    ...
    - **🔄 进行中 NTLG-NTF-DISMISS（重档，dismiss 软移除）**：ADR-197 子代理设计中（arch-reviewer Opus / agentId a2edc8aa4e6cfa1a9，后台）——抽屉级软移除（移除 ≠ 物理删除，消息中心仍 TTL 留存）。难点：抽屉项多源（notifications 表 + audit 派生 + task 派生）需统一 item_key + per-user dismissals 表 + 抽屉三处排除 vs 消息中心不排除 + 派生项 dismiss 范围（general/finished 可 / upcoming·active 不可）。设计完落 ADR-197 + 拆 -A schema / -B api / -C UI。
