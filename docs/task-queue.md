...
3. **META-33** — Phase 2：admin-ui `MetadataSourceIconCluster` + `MetadataStatusPanel` 原语（状态：🔄 进行中 2026-06-14，依 META-32 ✅）→ 拆 -A（图标簇+tooltip+退役标记）/ -B（StatusPanel）
   ...
   - 3b. **META-33-B** — `MetadataStatusPanel`（variant detail/drawer/compact：整体态+完整度+最近增强+四来源卡+问题列表+下一步动作；onAction 上层注入 `(action, provider?)`，DEV-33-3 仅承载 MetadataNextAction）（状态：🔄 进行中 2026-06-14，复用 -A arch-reviewer gate agentId a910e6bb5fa5df2a7，commit 带 Subagents trailer）
     - 复用 -A：内嵌 `<MetadataSourceIconCluster density="panel">` + `buildMetadataTooltip` headline + 文案常量；问题列表复用 cell `Pill`（issueLevel→variant：danger/warn/info）；来源卡复用 `MetadataProviderIcon` + 外链 `SOURCE_HREF_BUILDERS`；`sourceEvidence?: ReactNode` slot（消费方注入原始 ref，守单向依赖）。文件接近 500 行须拆子组件（source-card / issue-list）。
