...
1. **CHG-SN-9-DT-HEADER-REDESIGN-ADR** · 表格头重设计 ADR-149 起草 — 状态：✅ 已完成（2026-05-23）
   ...
   **ADR-149 AMENDMENT 1（2026-05-24）— D-149-13/14/15 + EP 序列重写 4→7 段**：
   ...
   - **CHG-SN-9-DT-AUTOFILTER-EP-1** · 共享 DataTableAutoFilter（替代 header-menu filterContent slot 部分）+ 数据类型推导 hook + Google Sheets 三段布局 popover + types.ts AutoFilterColumnFields union + 35 单测 — 状态：🚫 BLOCKED on ADR-150 PASS（~0.6w）
   - **CHG-SN-9-DT-AUTOFILTER-EP-2** · 后端通用 distinct 端点 `/admin/_dt/distinct` + distinct-whitelist.ts + filter-schema.ts DtFiltersSchema 共享 zod + 6 表白名单注册 + 20 单测（SQL 注入 4 case）+ ErrorCode COLUMN_NOT_WHITELISTED — 状态：🚫 BLOCKED on EP-1（~0.5w）
   - **CHG-SN-9-DT-AUTOFILTER-EP-3-A** · 12 消费方批量迁移 子卡 A：CrawlerRunsView（已迁 D-149-15 → D-150） + AuditClient（toolbar 4 → 列内 filterable） — 状态：🚫 BLOCKED on EP-2（~0.3w）
   - **CHG-SN-9-DT-AUTOFILTER-EP-3-B** · 子卡 B：SubmissionsListClient + UsersListClient — 状态：🚫 BLOCKED on EP-3-A（~0.3w）
   - **CHG-SN-9-DT-AUTOFILTER-EP-3-C** · 子卡 C：VideoListClient + StagingPageClient — 状态：🚫 BLOCKED on EP-3-B（~0.3w）
