...
5. **CHG-SN-5-PRE-01-E** · DEBT-SN-4-A（5 件下沉组件 ~12 张 Playwright `toHaveScreenshot()` baseline + 回溯 M-SN-4 改动校验，🟠）— 状态：拆分为 -E-1（基础设施）+ -E-2（真截图）
   ...
   - **CHG-SN-5-PRE-01-E-2**（用户卡 / 真截图入库）：⏸ 待启动（依赖 -E-1 完成 ✅ + 用户本地启 server-next dev 跑 `--update-snapshots`）
     - 文件范围：12 张 baseline PNG 入库 `tests/visual/admin-ui/*-snapshots/`
     - 工时估算：~0.2w（含跑 update-snapshots + git add + 视觉抽检）
