...
7. **CHG-DESIGN-07** — Dashboard 8 卡片浏览态（拆 7A/7B/7C/7D 4 阶段，2026-04-30 修订）
   ...
   **7D — Browser/Visual 验收 + reference §5.1 逐行对照**
   - 7C 实装完成后启动
   - 输出：
     - 用 Playwright MCP 启 dev server-next + navigate `/admin` + browser_snapshot + take_screenshot 三行 + 单卡片 close-up
     - 在 changelog 任务条目内列截图占位 + 与 reference.md §5.1 逐行对照表（以上 9 项 ☑/☒）
     - 任意 fail 项回到 7C 修复后再走 7D
   - 不在范围：写代码

   **整卡完成标准**：7A→7B→7C→7D 全部 ☑ + 所有 9 项视觉对照 ☑ + verify:token-references / 单测 / typecheck / lint 全绿；任务卡 commit trailer 含 4 阶段子代理 model ID
