...
14. **NTLG-P2-c** — 「消息中心」页（全量历史 + 检索 + 归档）+ 未读 SSE 实时推送（替代 60s 轮询）（状态：🔄 进行中 2026-06-09 — 拆 -ADR/-A/-B/-C，原子化判据 Q2 跨 schema/api/service/BFF/UI 多层 + SSE 新传输架构需 ADR）
    ...
    - **拆卡蓝图**：
      ...
      - **NTLG-P2-c-C** — 收口 P1-c-C 3 项（归档 F5 已 v1 deferred / D-196-DEV-1，本卡不做）。状态：🔄 进行中 2026-06-09 — **拆 -C-1/-C-2**（跨 api-service〔F6①〕+ admin-ui 共享 Props〔F6②〕+ server-next UI〔F6②〕**3 层** + F6② 触 `admin-shell.tsx:175` 红点逻辑 → 改 `packages/admin-ui/src/shell/types.ts` 公开 Props **强制 arch-reviewer Opus 子代理** → 原子化红线必拆）。依赖 -ADR + -A。
        ...
        - **NTLG-P2-c-C-2** — F6② 红点改读 unread-count（替 list-derived BLOCKER-1 回填）。状态：⬜ 待开始。**需 arch-reviewer Opus 子代理**（设计 admin-ui shell 红点 Props 契约扩展：`admin-shell.tsx:175` `notificationDotVisible = notifications.some(!read)` 改 unread-count 数字驱动 + 向后兼容 + commit `Subagents:` trailer）+ server-next `useAdminNotifications` 新增 `unreadCount`（unread-count 端点初始/轮询 + SSE `onUnread(count)` 实时）+ admin-shell-client wire。依赖 **-C-1**（crawler 进新表后 unread-count 才含 crawler，bell↔drawer 一致）。建议模型：sonnet 主循环 + Opus 子代理设计 Props。
