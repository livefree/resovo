...
| 4b | ⬜ **CHG-SN-9-PLAYER-ERROR-RETRY-CONTROL-EP** 待开始（依赖 #4 -ADR ship）| Wave 4 #4-EP / 评审后拆卡 / 消费方接入：AdminPlayer key-bump remount + PlayerShell retry watchdog 3s 后切线 + retryAttemptRef per-sourceIdx + 测试更新（admin-player + player-shell-on-error）/ 吸收 Y-166-3 + Y-166-6 / 估 2 业务 + 2 测试 + 1 docs = 5 文件 ≤ 5 / 不再需 Opus 评审（评审已 PASS） | 否（依赖 ADR-166 Accepted）| sonnet-4-6 |
