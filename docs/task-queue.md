...
12. **NTLG-P2-a** — `task_runs` 统一抽象层（§2.2 路径 B），UI 收敛单一投影（状态：🔄 进行中 2026-06-09 — 拆 -A/-B/-C，原子化判据 Q1 范围 >5 + Q2 跨 schema/api-service/worker/route 多层 + 含控制路径破坏性切换须分阶段验证）
    ...
    - **拆卡蓝图（ADR-194 落地，沿 P1-a/P1-c「schema 地基 → 写接入 → 控制/破坏性切换」先例）**：
      ...
      - **NTLG-P2-a-C** — ADR-191 `:id` re-point + bull 协作式取消（控制路径）（状态：⬜ 待开始）
