...
- **被取代说明（2026-04-30 / CHG-DESIGN-11）**：本卡完成时设计为 FilterToolbar **外置** + Pagination **外置** + ColumnSettingsPanel **外置** + DataTable 仅承载列。**当前真源已被 CHG-DESIGN-02（SEQ-20260429-02）取代**：toolbar / search / filter chips / saved views / 表头集成菜单 / bulk action bar / pagination 全部纳入 DataTable 一体化契约。落地分两阶段：(1) ✅ Step 1–6 已实现 toolbar / bulkActions / flashRowKeys / enableHeaderMenu / saved views 内置 props；(2) 🔄 Step 7A 计划实现 pagination / .dt__body 独立滚动 / 隐藏列 chip / filter chips slot —— 落地前消费方仍可走外置 PaginationV2 等过渡形态，落地后必须切换。视频库 v2 接入路径走 CHG-DESIGN-08（视觉对齐）+ CHG-DESIGN-02 Step 7B（接入新 API）。本卡完成的代码不动，但**不作为新模块的实现模板**。
- **实际开始**：2026-04-29
- **实际完成**：2026-04-29
...
- **状态**：⏸ 暂停（2026-04-29 — 用户裁定优先推进 SEQ-20260429-02 设计稿对齐改造，本任务待对齐序列收敛后再恢复）
...
2. **CHG-DESIGN-02** — DataTable frame 扩展（建议 opus + arch-reviewer）⏸ Step 7 暂停
   - 进度（2026-04-30）：Step 1–6 已完成（types/framed surface / 表头菜单 / toolbar+views / bulkActions+flashRowKeys / saved views 持久化）
   - Step 7 拆分：7A DataTable 骨架完整化（独立 body 滚动 + 内置 .dt__foot pagination + 隐藏列 chip + filter chips slot）/ 7B 视频库接入 / 7C cell 沉淀（独立卡 CHG-DESIGN-12）
   - **暂停于 Step 7**，等待 CHG-DESIGN-11 文档真源切换治理完成后恢复
   - **不引入** `TableFrame` 新抽象层（reference.md §0-7）
   - 阈值：DataTable props > 20 才退回 frame 路线
   - **验收**：视频库消费新 props 一体化（不再外置 Toolbar/Pagination/SelectionActionBar）；arch-reviewer PASS
...
5. **CHG-DESIGN-05** — Shell 视觉对齐
   - footer role 文案改回「管理员 · admin」形态
   - 实装自定义 NavTip（折叠态 hover 显示 label + shortcut kbd），替换原生 `title`
   - admin-shell-client 接入 notifications + tasks 数据 prop（mock 或真实，由产品决定）
   - **验收**：与 reference.md §4.1 当前实装差异表逐行对照，缺项归零

6. **CHG-DESIGN-06** — Settings 入口收敛
   - sidebar NAV 移除 `system/monitor / system/cache / system/config / system/migration` 4 个 system 子项；保留单一「站点设置」入口
   - 设置页内部按 reference.md §5.11 用 180/1fr 双栏布局 + 8 类 tab
   - **验收**：sidebar 系统管理组从 8 项收敛到 ≤4 项；设置页内部 tab 切换工作正常

7. **CHG-DESIGN-07** — Dashboard 8 卡片浏览态
   - 按 reference.md §5.1 实装：`AttentionCard` / `WorkflowCard` / 4× `MetricKpiCard` / `RecentActivityCard` / `SiteHealthCard`
   - 新增共享组件：`<KpiCard>` + `<Spark>` 抽到 `packages/admin-ui`
   - 编辑态（`CardLibraryDrawer` / `FullscreenCard`）后置到 §A4 决议后再做
   - **验收**：DashboardClient 4 行布局完整；3+ 处使用的 KpiCard/Spark 已抽 shared

8. **CHG-DESIGN-08** — Video library 视觉对齐（依赖 CHG-DESIGN-02）
   - 补 `page__head`（标题/副标题/导出 CSV/手动添加视频 actions）
   - 封面 64×36 横图改 32×48 竖版
   - 列对齐：DualSignal / VisChip / dot+文案 源活跃 / P0 图片 pill / inline xs row actions
   - 接入 saved views（个人/团队）+ 表头菜单 + flash row
   - **验收**：与 reference.md §6.1 列规范完全一致；视觉签收

9. **CHG-DESIGN-09** — Analytics 内容迁入
   - 按 reference.md §5.15 把 `/admin?tab=analytics` 内容补全：4 KPI + 采集任务量折线 + 源类型分布 + 爬虫最近任务表
   - 复用 §6.9 列定义
   - **验收**：tab 内容完整，不再是占位

10. **CHG-DESIGN-10** — VideoEditDrawer 扩张
    ...
    - footer：最后编辑信息 + 取消 + 保存
    - **验收**：与 reference.md §4.5 设计稿特征逐项对齐
...
12. **CHG-DESIGN-12** — Cell 共享组件沉淀（DualSignal / VisChip / Spark / KpiCard / thumb / pill / inline xs actions）（2026-04-30 新增；CHG-DESIGN-08 视频库接入时一起做）
    - 抽 reference.md §10 业务复合组件清单到 packages/admin-ui
...
- CHG-DESIGN-02 视为 S 级模块改造，必须 spawn `arch-reviewer` (Opus) 通过
