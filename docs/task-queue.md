...
| **IMGH-P3-4C** ⬜ | **核查：前台 SafeImage 零裂图覆盖面（Codex LOW-2/L-4）**：核查 `apps/web-next` **所有**公开封面/图片入口是否都走 `SafeImage`（含 `SafeImageNext` onError→FallbackCover 行为）；列出裸 `<img>`/直挂 URL 的漏网点 → 这是目标「用户端零裂图」真正闭环点。仅核查 + 出清单（修复另起卡） | 3 | sonnet | 无（与 4A/4B 解耦） | 核查报告（落 docs/research 或 changelog）+ 漏网点清单 |
