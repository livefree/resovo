...
2. **CHG-SN-9-CW1-B-ADR** — Bug-A 任务级 cancel 端点 ADR 起草（状态：⬜ 待启动）
   - 创建时间：2026-05-25
   - 建议模型：opus + arch-reviewer (opus)
   - 范围：新增 ADR-NNN（task-level cancel endpoints / D-N-1..6 决策）
   - 文件范围：docs/decisions.md（追加 ADR）
   - 验收要点：6 决策点 PASS + Opus PR review A−；触发 plan §4.5 R7 MUST-8（新增 admin route 须 ADR）
   - 估时：0.15w

3. **CHG-SN-9-CW1-B-EP** — Bug-A 实施（状态：⬜ 待启动，依赖 -ADR PASS）
   - 创建时间：2026-05-25
   - 建议模型：sonnet
...
4. **CHG-SN-9-CW1-C** — 关键词采集 Drawer（状态：⬜ 待启动）
   - 创建时间：2026-05-25
   - 建议模型：sonnet
   - 范围：纯 next UI 新组件 + API client 2 函数；后端 0 改动
   - 文件范围：apps/server-next/src/app/admin/crawler/_client/KeywordCrawlDrawer.tsx（新约 250 行）+ CrawlerAdvancedMenu.tsx（加菜单项）+ CrawlerClient.tsx（state + render）+ apps/server-next/src/lib/crawler/api.ts（previewKeyword + runCrawlerKeyword）
   - 验收要点：单 keyword + 类型筛选 + siteKeys 默认全选 enabled 站 + 预览表格 + 立即采集 + 单测 5+ case
   - 估时：0.3w

5. **CHG-SN-9-CW1-D** — Dashboard 自动采集卡（状态：⬜ 待启动，依赖 CW1-A 的 autoCrawlNext 字段）
   - 创建时间：2026-05-25
   - 建议模型：sonnet
   - 范围：next /admin DashboardClient 新加卡 + CrawlerClient query param 支持
   - 文件范围：apps/server-next/src/app/admin/_client/AutoCrawlScheduleCard.tsx（新约 150 行）+ DashboardClient.tsx + apps/server-next/src/app/admin/crawler/_client/CrawlerClient.tsx（?openDrawer=scheduler 自动打开）
   - 验收要点：3 状态渲染（未启用 / 倒计时 / 失败）+ 编辑链接跳 /admin/crawler?openDrawer=scheduler 自动打开 + 单测
   - 估时：0.2w

6. **CHG-SN-9-CW1-E-ADR** — Topbar 铃铛端点 ADR 起草（状态：⬜ 待启动）
   - 创建时间：2026-05-25
   - 建议模型：opus + arch-reviewer (opus)
   - 范围：新增 ADR-NNN（admin system background-events endpoint / D-N-1..5 决策：event schema / 聚合源 audit_logs + active runs + auto-crawl next 三源 / 轮询 vs SSE / 60s 频率 / role）
   - 文件范围：docs/decisions.md
   - 验收要点：5 决策点 PASS + Opus PR review A−
   - 估时：0.15w

7. **CHG-SN-9-CW1-E-EP** — Topbar 铃铛实施（状态：⬜ 待启动，依赖 -ADR PASS）
   - 创建时间：2026-05-25
   - 建议模型：sonnet
