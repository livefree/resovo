...
| 45 | **CHG-SN-6-29-PATCH-1** ⏳ 待开始 | CrawlerClient.tsx 拆分（H1 必修）— 拆 CrawlerSitesTab（列表 + drawer 编辑 + batch action + 表单）+ CrawlerControlsCard（4 按钮组 + handlers）+ 主 orchestrator ≤ 300 行 / 守卫 CrawlerClient.test 25 测试 + SchedulerConfigDrawer.test 8 测试零回归 / 视觉不变 | opus（延续；可考虑 arch-reviewer Opus 评审拆分边界）| 0.4w | 🔴 P0（绝对禁止项违反必修）|
| 46 | **CHG-SN-6-29-PATCH-2** ⏳ 待开始 | M-SN-6 债务可见性兜底 — task-queue.md 起 3 跟踪卡：CHG-SN-7-PRE-01 文件大小守卫 / CHG-SN-7-PRE-02 ADR-121 R-MID-1 协议正式化 / CHG-SN-7-MISC perSiteOverrides UI 实装 + M-SN-7 低风险债务（双子卡范式文档化 / NEGATED ADR 占位说明 / Moderation csv 豁免追溯 / useDoubleConfirm 复用阈值）| haiku（机械文档追加）| 0.05w | 🔴 P0（债务可见性）|
