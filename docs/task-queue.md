...
- **依赖链**：`PRE-1`（任意时点）；`PRE-2 → CHG-VIR-4`；`CHG-VIR-1/2/3/4`（Phase 0，可并行）→ `CHG-VIR-5/6`（Phase 1）→ **`CHG-VIR-6.5`（Phase 2 前置门禁 / ADR-105a AMENDMENT）** → `CHG-VIR-7/8/9`（Phase 2）→ **`CHG-VIR-OBS-BACKFILL`（Phase 3 硬前置 / 生产 title_observations 回填）** → `CHG-VIR-10`（Phase 3）；`CHG-VIR-9-D`（merge 翻默认 + 连通分量折叠，依赖 OBS-BACKFILL + merge shadow 稳定）；`CHG-VIR-11`（Phase 4，依赖 PRE-1+Phase1）；`CHG-VIR-12`（Phase 5，依赖 ADR-176/177 + Phase1-4 稳定）。
...
11-E. **CHG-VIR-9-D** — merge 默认源翻转 identity + N-video 连通分量折叠（状态：⬜ 待开始 / 待 merge shadow 稳定后用户裁定排期）
    - 创建时间：2026-06-03 12:50
    - 建议模型：sonnet（折叠算法 + 默认值翻转，契约已在 9-A/9-B 定档）
    ...
    - 依赖：**CHG-VIR-OBS-BACKFILL（硬前置：生产回填）** + merge shadow 对比稳定（用户裁定时点）。

**Phase 3 — ingest-time shadow scoring**

12. **CHG-VIR-10** — Phase 3：findOrCreate 旁路 shadow scoring（不改 catalog_id 绑定）（状态：⬜ 未开始 / 2026-06-03 复核补全启动配置）
    - 创建时间：2026-06-02 19:41
    ...
    - **硬前置**：CHG-VIR-OBS-BACKFILL（生产 title_observations 全量回填）——blocking 召回覆盖 = title_observations 覆盖度；生产未回填则 ingest shadow 召回不全、报表失真（dev 已回填验证）。
    ...
    - 依赖：CHG-VIR-8 ✅（identity_candidate + 评分模块）+ **CHG-VIR-OBS-BACKFILL（硬前置）**。
