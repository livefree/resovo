...
- **方案对应**：本序列 INFRA-01..06 ↔ 提案 LOG-03/02/01/04/05/06，文件范围与决策严格按提案落地
- **不做（划线防越界）**：不引入 ELK / Loki / Sentry / pino-pretty 任何新外部依赖；不做日志查询 UI；不做线上日志聚合；不合并 `CrawlerTaskLogService` 等业务日志（决策 5）

### 任务列表（按执行顺序）

1. INFRA-01（LOG-03）— `scripts/dev.mjs` 改造：pipe + 落盘 + rotate + errors 聚合（状态：✅ 已完成）
   - 创建时间：2026-04-25 17:16
   ...
   - 改动点：
     ...
     - **client 流前向兼容（复核 #1 收紧）**：dev.mjs 见到 `service:'client'` 字段就分流到 `logs/client/<date>.ndjson`；本卡**不创建**任何 client 端点 / 前端代码（端点在 INFRA-04 落地）
...
2. INFRA-02（LOG-02）— API + worker 接入统一 logger（状态：⬜ 未开始）
   - 创建时间：2026-04-25 17:16
   - 计划开始：INFRA-01 完成后
   - 前置依赖：INFRA-01 ✅
   - 建议模型：sonnet
   ...
   - 改动点：
     - `apps/api/src/lib/logger.ts`：新增 `createLogger({ service: 'api' })`（实现暂留本文件，决策 7）；导出 `withRequest(logger, req)` / `withJob(logger, job)` 派生器；类型 / 序列化器 / redact 表先用本地 stub 占位，标注 INFRA-03 完成后切到 `@resovo/logger`
...
3. INFRA-03（LOG-01）— 新建 `packages/logger` workspace 包（首期仅类型 + 序列化器 + redact 表）（状态：⬜ 未开始）
   - 创建时间：2026-04-25 17:16
   - 计划开始：INFRA-02 完成后
   - 前置依赖：INFRA-02 ✅
   - 建议模型：sonnet
   ...
   - 改动点：
     ...
     - `apps/api/src/lib/logger.ts`：从 `@resovo/logger` import 类型与序列化器，替换 INFRA-02 的本地 stub
...
4. INFRA-04（LOG-04）— 浏览器 client logger + `/v1/internal/client-log` 端点（状态：⬜ 未开始）
   - 创建时间：2026-04-25 17:16
   - 计划开始：INFRA-03 完成后
   - 前置依赖：INFRA-03 ✅
   - 建议模型：sonnet
...
5. INFRA-05（LOG-05）— 新增 `docs/rules/logging-rules.md` + 更新 CLAUDE.md 索引（状态：⬜ 未开始）
   - 创建时间：2026-04-25 17:16
   - 计划开始：INFRA-04 完成后
   - 前置依赖：INFRA-04 ✅
...
6. INFRA-06（LOG-06）— 实现下沉到 `packages/logger`（隐式后续，无 deadline）（状态：⬜ 未开始）
   - 创建时间：2026-04-25 17:16
   - 计划开始：触发条件达成后由人工排程
   - **触发条件**：INFRA-02 ✅ + INFRA-04 ✅，且监测期 ≥ 14 天无回归
   - 前置依赖：INFRA-02 ✅ + INFRA-04 ✅ + 监测期满
   - 建议模型：sonnet
