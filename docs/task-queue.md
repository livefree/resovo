...
1. **CHG-DESIGN-01** — Token completeness 修复
   - 清理已知未定义引用：`--accent-subtle / --bg-subtle / --bg-surface-hover / --state-error / --accent-on`
   - 补建设性 token：`--accent-soft / --accent-border / --bg-surface-popover / --scrollbar-size`、`.pulse` keyframe + class
   - 新增 `scripts/verify-token-references.mjs` token 引用校验脚本（未定义 → 报错）
   - **验收**：grep 全仓库 0 命中已知未定义；typecheck/lint/test 全绿；视觉无回归

2. **CHG-DESIGN-02** — DataTable frame 扩展（建议 opus + arch-reviewer）
   - 扩展 `packages/admin-ui` 的 `DataTable`：内置 search 280px、saved views（personal/team scope）、表头集成菜单（升降序+过滤+固定+隐藏）、row flash、`pinnedSticky` 列固定、`bulkSlot` 表内 sticky bottom、framed surface（`bg2/border/r-3`）
   - **不引入** `TableFrame` 新抽象层（reference.md §0-7）
   - 阈值：DataTable props > 20 才退回 frame 路线
   - **验收**：视频库 + 用户/审计页可消费新 props，视觉对齐设计稿；arch-reviewer PASS

3. **CHG-DESIGN-03** — 全站 scrollbar 6px 统一
   - `packages/design-tokens` 引入 `--scrollbar-size: 6px`；webkit + Firefox 双轨样式
   - 搜索仓库去除所有自定义 scrollbar 宽度 override
   - **验收**：grep 0 命中 `::-webkit-scrollbar { width: ... }` 单独覆盖（除全局）；视觉签收

4. **CHG-DESIGN-04** — Sidebar 过渡动效 + 分区标题等高占位
   - `aside width 200ms cubic-bezier(0.4,0,0.2,1)` + 内容 opacity 150ms ease-out
   - 分区标题 `<h4>` **不卸载**，折叠态用 `max-height: 0 + opacity: 0` 渐隐
   - active 配色从 `--state-warning-fg` 改为 `--accent-default`（蓝 brand）
