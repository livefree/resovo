...
| 15 | **CHG-SN-6-RETRO-3** | ultrareview P0-3 + P1-1 + P2 系统补齐：4 写端点 audit_log 扩展（POST settings / config / DELETE cache / POST import）+ AdminAuditActionType union 扩 + audit-log-coverage EXEMPT→PAYLOAD_REQUIRED 迁移 + CSS shorthand+longhand stylelint guard + verify:style-shorthand-conflict.mjs + AuditClient 521 行拆 cell 沉淀 + SettingsContainer button 改 AdminButton + ImageHealthClient 缺图列扩展 | sonnet（实施 + 跨边界 Opus 评审）| 0.3-0.5w | 🟡 P1 |
