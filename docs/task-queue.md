...
### CHG-UX-07 · 业务页面元素 hover 接入（用户验收问题 2）
- **状态**：📝 待开工
- **依赖**：CHG-UX-05c ✅
- **建议模型**：sonnet
- **触发**：用户 2026-05-03 验收"除 sidebar / topbar 4 按钮外，其他元素均无 hover：tab / 按钮 / 列表项"
- **范围**：调研 apps/server-next 业务页所有可点击元素清单（admin tabs / detail 页按钮 / list 行 / table cell action 等），按 §6.1 契约加 `data-interactive`
- **不在范围**：apps/web-next 前台（不在 admin Shell 内）
- **完成判据**：用户实测业务页可点击元素均出 hover；列表项 / tab / 按钮均生效
