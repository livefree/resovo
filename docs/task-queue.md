...
2. **CHG-SN-9-CW2-B-ADR** — Gantt 时间轴重设计 ADR-153 起草
   ...
   - 关键决策点：
     - D-153-1：每站显示条数 — rn=1 → rn≤N（推荐 rn≤3）
     - D-153-2：status 四态映射 — ok / warn / danger / neutral（paused/cancelled → neutral；区别于 ok）
     - D-153-3：range select 实装方案（30m/1h/2h/6h；透传到 SQL；前端 select 组件位置）
     - D-153-4：pending 起点修正（用 scheduled_at 而非 COALESCE(started_at, window_start)）
     - D-153-5：tick 时区统一（后端 UTC ISO 传输；前端 toLocaleTimeString 仅展示层）
     - D-153-6：health N+1 消除（inline subquery → 一次 CTE LEFT JOIN）
   - 文件范围：docs/decisions.md（追加 ADR-153）
   - 注意：CW2-B-EP 强依赖本 ADR Opus PASS；**Gantt 多 lane layout 决策也在本 ADR**（D-153-1 隐含 CSS auto-height multi-bar 方案）
   - 估时：0.15w
   - 状态：⬜
...
5. **CHG-SN-9-CW2-C-EP** — Fix-D5 定时增强实施（ADR-154 落地）
   - 创建时间：2026-05-25
   - 建议模型：sonnet
   - 依赖：CW2-C-ADR PASS
   - 文件范围：
     - `apps/api/src/db/migrations/075_auto_crawl_schedule_extend.sql`：
       - 新增 `auto_crawl_schedule_type TEXT NOT NULL DEFAULT 'daily'`
       - 新增 `auto_crawl_interval_minutes INT NULL`（interval 模式）
       - 新增 `auto_crawl_cron_expression TEXT NULL`（cron 模式）
       - `auto_crawl_last_trigger_date DATE` → `auto_crawl_last_trigger_at TIMESTAMPTZ NULL`（迁移现有 date 值）
     - `apps/api/src/db/queries/systemSettings.ts`：新字段读写（getAutoCrawlConfig / saveAutoCrawlConfig）
     - `apps/api/src/routes/admin/crawler.ts`：auto-config zod schema 扩展（scheduleType + intervalMinutes + cronExpression）
     - `apps/api/src/workers/crawlerScheduler.ts`：
       - `runSchedulerTick` 改为 dispatch 三分支（checkDaily / checkInterval / checkCron）
       - `checkInterval(config, now, lastTriggerAt)` 计算 intervalMinutes 是否到期
       - `checkCron(config, now)` 引入 cron-parser 判断（若 ADR PASS 引入）
       - 防重触发：用 `auto_crawl_last_trigger_at` timestamp 精确比对
     - `apps/server-next/src/app/admin/crawler/_client/SchedulerConfigDrawer.tsx`：
       - 顶部加 scheduleType AdminSelect（每日定时 / 间隔触发 / Cron 表达式）
       - 条件渲染：daily → dailyTime input；interval → intervalMinutes number input；cron → cronExpression input + next-run 预览
       - 全部字段受控 + zod 前端校验
   - 验收要点：
     - migration 回滚 SQL 测试
     - crawlerScheduler 单测：interval 模式 60min 到期 → 触发；未到期 → 不触发；cron 模式 next 时刻到 → 触发
     - SchedulerConfigDrawer 单测：scheduleType 切换 → 显示/隐藏对应 input + 重置联动字段
   - 估时：0.4w
   - 状态：⬜
