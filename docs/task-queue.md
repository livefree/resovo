...
- **状态**：🔄 进行中（8/11 — Phase 0 ✅（0A/0B）+ **Phase 1 ✅ 全收口**：1A ✅ candidates + 1B ✅ apply-candidate + 1C ✅ resolve-event + ids 重扫 + 1D ✅ missing-videos 筛选+行契约。**下一阶段 Phase 2**：2A/2B 共享组件〔ImageCompare / ImageCandidatePicker，**强制 Opus arch-reviewer 子代理**定 Props 契约〕）
...
| **IMGH-P2-2A** ⬜ | **共享组件：`ImageCompare`**（admin-ui 新）：当前图 vs 候选新图并排，标注两侧尺寸/比例/可达性（候选客户端探活 + 尺寸校验后才允许确认，§6.2）。Opus 子代理定 Props 契约（current/candidate/onConfirm/校验态/降级）+ 窄屏纵向堆叠 + 颜色零硬编码。 | 2（admin-ui 契约 + 组件实装） | **opus** | 0A（候选 DTO 定型） | **Opus 子代理 + arch-reviewer PASS + Subagents trailer** + admin-ui 组件测试 |
