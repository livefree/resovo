...
14. **NTLG-P2-c** — 「消息中心」页（全量历史 + 检索 + 归档）+ 未读 SSE 实时推送（替代 60s 轮询）（状态：🔄 进行中 2026-06-09 — 拆 -ADR/-A/-B/-C，原子化判据 Q2 跨 schema/api/service/BFF/UI 多层 + SSE 新传输架构需 ADR）
    ...
    - **拆卡蓝图**：
      ...
      - **NTLG-P2-c-B** — SSE 未读实时推送实装。依赖 -ADR（F1/F2/F3）。建议模型：sonnet（架构已 ADR 锁）。**拆卡（>5 项 + 跨 api-service/UI）→ -B-1（后端）/ -B-2（前端）**：
        ...
        - **NTLG-P2-c-B-2** — 前端 fetch-stream 客户端（`fetch` + ReadableStream/TextDecoderStream 解析 SSE 帧 + 手动指数退避重连）+ `admin-shell-notifications.ts` SSE 优先 + 60s 轮询 fallback 双模式（D-196-6，不删轮询）。依赖 -B-1。建议模型：sonnet。
