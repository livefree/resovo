...
| 9 | **CHG-SN-6-01** | `/admin/audit` 全局审计日志视图实施 — queries 2 + Service 3 方法 + Route 3 端点 + 视图 + 4 cell 组件 + 抽屉 + 视图测试 ≥ 9 + integration ≥ 6 + 共享原语 ≥ 80%（5 项硬清单首次正式验证）| sonnet（实施） + opus 评审（plan §5.1 任务级 PASS）| 0.4w | 🔴 P0 |

**后续候选卡**（未拆解，按用户授权依序起）：
- `/admin/system landing` + 5 子 tab 容器
- `/admin/image-health`
- `/admin/system/settings` 8 Tab
- `/admin/analytics`（触发 recharts vs visx 选型 → 候选依赖 ADR）
- `/admin/crawler`（触发 reactflow vs dagre-d3 选型 → DAG 待 reference A2）
- 通知 + 后台任务双面板 + Toast 系统
- 大数据原语（触发 react-virtual vs react-window 选型）
