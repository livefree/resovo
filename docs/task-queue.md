...
1. **CHG-SN-9-DT-HEADER-REDESIGN-ADR** · 表格头重设计 ADR-149 起草 — 状态：✅ 已完成（2026-05-23）
   ...
   **ADR-149 AMENDMENT 1（2026-05-24）— D-149-13/14/15 + EP 序列重写 4→7 段**：
   ...
   - **CHG-SN-9-DT-AUTOFILTER-AMD2-EP** · AMENDMENT 2 实施（共享层 + 4 消费方 opt-out）— 状态：🚫 待启动（~0.6w / 共享层 types.ts kind union + DataTable.tsx kind 筛选 + useFilterKindInference 触发条件 + 4 消费方 opt-out review）
