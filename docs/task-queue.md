...
- **状态**：🔄 进行中（1/3 卡完成：CHG-SN-8-04-ADR ✅ 2026-05-21；解锁 CHG-SN-8-04-EP 实施）
- **创建时间**：2026-05-21
...
2. **CHG-SN-8-04-EP** · 端点 + Service + Queries 实施（ADR-137 落地）— 状态：⬜ 待开始
   - **依赖**：CHG-SN-8-04-ADR PASS
   - **建议模型**：sonnet（实施）
   - **范围**：apps/api/src/routes/admin/moderation.ts 新增 `GET /admin/moderation/:id/similar` + ModerationService.similar + queries + R-MID-1 7 文件框架（route / service / queries / types / audit-log-coverage / audit-payload / audit-set-equal）+ 端点测试
   - **工时估算**：0.2w

3. **CHG-SN-8-04-VIEW** · TabSimilar 实装 + 测试 — 状态：⬜ 待开始
   - **依赖**：CHG-SN-8-04-EP PASS
   - **建议模型**：sonnet
   - **范围**：apps/server-next/src/app/admin/moderation/_client/RightPane/TabSimilar.tsx 真实化 + use-similar hook + 「发起合并」深链 → /admin/merge?candidate_a=<active>&candidate_b=<sim>&from=moderation
   - **W1 反例 #3 ✅ 落地**
   - **工时估算**：0.15w
