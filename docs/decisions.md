...
videos 表已含 `title_normalized` 字段（migration 007）+ TitleNormalizer Service 提供 `normalizeTitle()` / `buildMatchKey()` 函数，DB 层有 `idx_videos_normalized_year_type ON videos(title_normalized, year, type) WHERE deleted_at IS NULL` 部分索引（migration 007:40）覆盖按 normalized 三元组聚合的主路径。media_catalog 层（migration 026）+ media_catalog_aliases（migration 030）承载作品元数据 + 别名。爬虫归并由 CrawlerService 内部 buildMatchKey 自动执行（规则 A：title_normalized + year + type 三元组完全相同才合并）。 ⚠️ **本背景段事实已部分过时**：`videos.year` / `videos.title_normalized` 已由 migration 029 DROP 下沉至 `media_catalog`，候选改 JOIN media_catalog 取 `mc.title_normalized` / `mc.year`；详见 ADR-105 AMENDMENT 2026-06-02。
...
4. **candidate 算法基线 v1**（R-105-3 修订，arch-reviewer 第 1 轮）：
   ...
   - **未来扩展**：fuzzy match 需求出现 → ADR-105a 引入 pg_trgm 索引 + 多维加权评分公式（title_similarity 0.4 + year_diff 0.2 + type_match 0.2 + source_overlap 0.2）；v1 严格三元组匹配先满足保守 false-positive 低需求 （⚠️ **此 ADR-105a 方向已被 ADR-105 AMENDMENT 2026-06-02 取代**：不引入 pg_trgm，改 `core_title_key` 确定性等值 blocking + 多证据评分 + 离线候选生成）
...
- ✅ **方案 2（采纳）**：严格 title_normalized + year + type 三元组完全匹配（依赖既有 idx_videos_normalized_year_type 索引）+ source 重合度评分 — 性能 p95 ≤ 200ms 可承诺，false positive 低；如未来 fuzzy 需求出现 → ADR-105a 修订引入 pg_trgm 索引 （⚠️ pg_trgm 方向已被 ADR-105 AMENDMENT 2026-06-02 取代）
...
3. **R-ADR-105-3**：candidate 算法严格 title_normalized 三元组完全匹配 — 漏掉 fuzzy 场景（如 "复仇者联盟" vs "复仇者联盟 4"，标题差异但同作品）。**缓解**：v1 严格保守优先；fuzzy 需求出现 → ADR-105a 引入 pg_trgm（不在本 ADR 范围）（⚠️ pg_trgm 方向已被 ADR-105 AMENDMENT 2026-06-02 取代：改 core_title_key 等值 blocking + 多证据评分）
...
- **关联触发条件（未来）**：
  - PRE-MERGE-CACHE（candidate 缓存层引入）：决策要点 10 三条触发条件任一命中 ⚠️ **已被 ADR-105 AMENDMENT 2026-06-02 取代**（离线 identity_candidate 生成 + 实时轻量读，不再单独立卡）
...
### ADR-105 AMENDMENT 2026-06-02（视频身份解析蓝图取代 ADR-105a 旧方向 / 候选事实更正）— C1/C3/C4

**触发**：`docs/designs/video-identity-resolution-redesign_20260602.md`（视频身份解析与合并/拆分升级设计；arch-reviewer Opus 审核 CONDITIONAL → R1–R3 红线已闭环）取代本 ADR 中 ADR-105a 的旧技术方向，并更正若干已被 migration 029 推翻的历史事实。**零删除原文**——原描述保留为历史，以下修订声明为准。

**C1 — ADR-105a 方向修订**（取代 §决策要点 4 「未来扩展」/ §备选方案 D / §风险 R-ADR-105-3 / §关联触发条件）：
- 原方向：fuzzy match 需求出现 → 引入 **pg_trgm 索引 + 4 维加权评分**（title_similarity 0.4 + year_diff 0.2 + type_match 0.2 + source_overlap 0.2）。
- **新方向（采纳）**：**不引入 pg_trgm**（全库无该扩展，且禁技术栈外新依赖）。改为新增并行 `core_title_key` 做**确定性归一后等值 blocking**（B-tree 可承载）+ **多证据评分**（强正/中正/强负 + 阈值分级 0.92/0.75）+ **候选离线生成持久化**（`identity_candidate`，实时端点轻量读）。字符相似召回如确需，另起独立 DB capability ADR。见设计文档 §4.2 / §4.3 / Phase 2b。

**C3 — 历史事实更正**（§背景）：本 ADR 背景（2026-05-12 撰写）称「videos 表已含 `title_normalized`… `idx_videos_normalized_year_type ON videos(title_normalized, year, type)`」。该事实已被 **migration 029 推翻**：`videos.year` / `videos.title_normalized` 已 DROP 下沉至 `media_catalog`；候选 GROUP BY 现为 `mc.title_normalized, mc.year, v.type`（`video-merge-candidates.ts` JOIN media_catalog），唯一索引为 `uq_catalog_title_year_type`（migration 026）。

**C4 — 触发卡处置**：
- `PRE-MERGE-CACHE`（candidate 缓存层）：被设计文档「离线生成 `identity_candidate` + 实时轻量读」取代，不再单独立卡。
- `PRE-MERGE-CATALOG`（重复 catalog 归并，D-161-1）：归入设计文档 **Phase 5 catalog 身份层**（catalog-catalog 合并）。
- `PRE-MERGE-AUDIT-SHARD` / `PRE-MERGE-TTL`：与身份解析蓝图无关，维持原触发条件不变。

**编号规划**：身份解析蓝图落地新增 **ADR-105a**（评分公式 / 阈值分级 / 候选持久化 / 离线生成）+ **ADR-175**（多语种标题模型）+ **ADR-176**（catalog 按季粒度 + series_group）+ **ADR-177**（外部 ID 映射真源 `catalog_external_refs`；须先定与既有 `video_external_refs` 的并存/上卷关系，关系未定不得起草）。

**不变**：ADR-105 的 merge/split/unmerge 端点契约、`video_merge_audit` schema、ADR-114-NEGATED 复合键兼容、双层 audit 时序——全部不受本 AMENDMENT 影响，继续有效。
...
## ADR-117 AMENDMENT 3 2026-06-02（CHG-VSR-3）— 视频分组派生列 + KPI② 维度 + queries 拆分

> 关联：SEQ-20260601-01 视频库/播放线路职责重定义（设计 `docs/designs/videos-sources-responsibility-redesign_20260601.md` §3.2/§3.3/§3.5）+ CHG-VSR-1 已落地的 `VideoGroupRow`/`VideoGroupStats` 派生列契约。
> 形态：纯 additive AMENDMENT（填充 CHG-VSR-1 已定义的 optional 派生列 + KPI 维度②）+ queries 文件拆分（解 759 行 > 500 硬限）。**零新 admin route / 零新 error code / 零 migration / 不触碰 user_submissions**。
> 评审：arch-reviewer Opus（claude-opus-4-8）「需修改」结论 → 2 BLOCKER + 1 HIGH + 3 MEDIUM 已纳入下列决策点。主循环 claude-opus-4-8。
> 状态：🟢 Accepted（设计定稿；实施分阶段——拆分 refactor + 派生列/KPI feature）。

### 背景

`getVideoGroupStats`（sources-matrix.ts:117）KPI 全走启停维度① `source_check_status`；`listVideoGroups`（:165）无 CHG-VSR-1 契约的派生列；`sources-matrix.ts` 759 行混 video-groups / matrix / 别名+routes 三关注点，超 500 硬限。本卡填充派生列 + KPI 维度②（保留①兼容，rename 留卡 5）+ 拆分。

### 决策点

**D-117-VSR3-1：派生列单趟聚合（无子查询）+ `QUALITY_RANK_EXPR` 单一 SQL 常量三处共用。**
- 所有派生列在现有 `GROUP BY v.id, mc.year, mc.cover_url`（追加 `v.is_published`）单趟聚合内完成（现查询已 `JOIN video_sources vs ON deleted_at IS NULL` 逐源展开，FILTER 落 `vs` 上口径正确）：
  - `COUNT(vs.id) FILTER (WHERE vs.is_active=true)` → active_source_count；`FILTER (WHERE vs.is_active=false)` → disabled_count（**维度①**）
  - `FILTER (WHERE vs.probe_status='dead')` → connect_fail_count；`render_status='dead'` → render_fail_count；`probe_status='pending'` → pending_probe_count（**维度②** 线路级计数，区别于行级 worst-of `aggregateSignal`）
  - `(COUNT(*) FILTER (WHERE quality_detected IS NOT NULL)::float / NULLIF(COUNT(*),0))` → quality_coverage（**仅 quality_detected 实测比例**，不含 quality 回退）
  - `percentile_cont(0.5) WITHIN GROUP (ORDER BY vs.latency_ms)` → latency_median_ms
  - `(COUNT(vs.id) FILTER (WHERE is_active AND probe_status<>'dead' AND render_status<>'dead') = 0)` → needs_source（bool）
  - `v.is_published` → is_published；`COALESCE(MAX(vs.last_probed_at), MAX(vs.updated_at))` → last_checked_at
- **`QUALITY_RANK_EXPR`（TS module-level const，Q1/Q2/Q3 三处共用一份，禁散落）**：
  `CASE COALESCE(vs.quality_detected, vs.quality) WHEN '4K' THEN 7 WHEN '2K' THEN 6 WHEN '1080P' THEN 5 WHEN '720P' THEN 4 WHEN '480P' THEN 3 WHEN '360P' THEN 2 WHEN '240P' THEN 1 ELSE NULL END`
  —— 即 CHG-VSR-1 注释的 `quality_detected ?? quality` 回退口径；**禁止照搬 LinesPanel `pickHighestQuality`**（aggregate.ts:55 仅 quality_detected、丢 quality 回退）。

**D-117-VSR3-2：qualityHighest 走 `CASE MAX(quality_rank)` 反查 label（方案 a）。**
- `MAX(QUALITY_RANK_EXPR)` → quality_rank_max（整数）；`CASE MAX(...) WHEN 7 THEN '4K' … WHEN 1 THEN '240P' ELSE NULL END` → quality_highest（label）。
- 禁用 `MAX(quality::text)` 字符串比较（字典序 '720P' > '1080P' 会错）。全 NULL → quality_highest=null = 「质量未知」，不并入低质量。
- Service 映射：`(row.quality_highest as ResolutionTier) ?? null`（禁裸断言）。

**D-117-VSR3-3：latencyMedianMs scope = 全部源非空 latency（不限 active）。**
- `percentile_cont` 自动忽略 NULL；未探测源 latency_ms 本就 NULL 被剔除。语义=该视频所有线路延迟体感中位，与启停①正交。
- 与 routes-by-site `avgLatencyMs`（AVG）**同 scope 异度量**（中位 vs 均值），显式落字防口径漂移（重演 CHG-VSR-2 Codex flag）。

**D-117-VSR3-4：KPI② 走 per-video 子查询 + 外层 COUNT FILTER；①口径零变更 + 等价回归。**
- **BLOCKER**：①（`v.source_check_status` videos 单行属性）与②（video_sources 聚合，lowQuality 需视频级 MAX）**禁同层 FILTER 双算**。结构：
  ```
  SELECT COUNT(*) total,
    COUNT(*) FILTER (WHERE g.source_check_status IN ('ok','partial')) active,   -- ①保留
    COUNT(*) FILTER (WHERE g.source_check_status='all_dead') dead,              -- ①保留
    COUNT(*) FILTER (WHERE g.source_check_status='all_dead' AND NOT g.is_published) orphan, -- ①保留
    COUNT(*) FILTER (WHERE g.has_abnormal) abnormal,                           -- ②新增
    COUNT(*) FILTER (WHERE g.needs_source) needs_source,                       -- ②
    COUNT(*) FILTER (WHERE g.has_pending) pending_probe,                       -- ②
    COUNT(*) FILTER (WHERE g.quality_rank_max < 4) low_quality                 -- ②（NULL<4=UNKNOWN 不计，自动满足"质量未知不并入"）
  FROM ( SELECT v.id, v.source_check_status, v.is_published,
           bool_or(vs.probe_status='dead' OR vs.render_status='dead') has_abnormal,
           bool_or(vs.probe_status='pending') has_pending,
           (COUNT(*) FILTER (WHERE vs.is_active AND vs.probe_status<>'dead' AND vs.render_status<>'dead')=0) needs_source,
           MAX(QUALITY_RANK_EXPR) quality_rank_max
         FROM videos v JOIN video_sources vs ON vs.video_id=v.id AND vs.deleted_at IS NULL
         WHERE v.deleted_at IS NULL GROUP BY v.id, v.source_check_status, v.is_published ) g
  ```
- total=有源视频数（INNER JOIN 天然），与现状 `EXISTS(video_sources)` 等价。单测须断言 total/active/dead/orphan 新旧形态**逐值相等**（防①回归）。

**D-117-VSR3-5：quickFilters 谓词全落 WHERE EXISTS（不进 HAVING）；OR 合流去重。**
- has_abnormal=`EXISTS(vs probe='dead' OR render='dead')`；pending_probe=`EXISTS(vs probe='pending')`；needs_source=`NOT EXISTS(vs is_active AND probe<>'dead' AND render<>'dead')`。
- low_quality=`EXISTS(vs COALESCE(quality_detected,quality) IS NOT NULL) AND NOT EXISTS(vs QUALITY_RANK_EXPR >= 4)`（=最高<720P 且非全未知；落 WHERE 不污染 count 慢路径，与 updatedAt-range HAVING 解耦）。
- `lowQuality` boolean param 与 quickFilters 含 'low_quality' **入口归一** `wantLowQuality = lowQuality===true || quickFilters?.includes('low_quality')`，命中 push **一份**谓词（不双 push）。单测覆盖三态等价。

**D-117-VSR3-6：sortField 新增走 SELECT 别名引用（IDENT 正则零放宽）。**
- **HIGH**：现 IDENT 正则不覆盖 `COUNT(...)FILTER` / `CASE` / `COALESCE(MAX(),MAX())`，启动断言会 throw。**禁放宽正则**（=打开注入审计面）。改 ORDER BY 引用主查询已 SELECT 的派生列**别名**（裸标识符，正则原样通过）：
  `activeSources: 'active_source_count'` / `quality: 'quality_rank_max'` / `lastChecked: 'last_checked_at'`。
- Service `VideoGroupsQuerySchema.sortField` zod enum 同步扩 `activeSources/quality/lastChecked`（否则 Route→Service strip 新值）。

**D-117-VSR3-7：queries 拆分边界（4 文件）+ Service & 测试 import/vi.mock 路径同步迁移（方案 A）。**
- 拆分：`source-line-aliases.ts`（纯别名 CRUD：listLineAliases/listAllSourceLines/findLineAlias/upsert*/retire/updateLineAliasPriority/findCodenameAssignments + DbAliasRow + mapAliasRow）/ `source-routes.ts`（routes-by-site：listRoutesBySite/selectRouteSampleSource/countRouteSources/softDeleteRouteBySite + SourceRouteBySiteRaw + DbRouteBySiteRow）/ `video-matrix.ts`（getVideoMatrix + DbEpisodeCellRow）/ `sources-matrix.ts` 保留 video-groups（stats/list）单一关注点（~390 行，留增长余量）。
- **BLOCKER**：≥5 测试 `vi.mock('@/api/db/queries/sources-matrix')` 按路径 mock 别名/route 符号；物理移出后 re-export **无法**让旧路径 mock 拦截新路径真实现。**方案 A**：SourcesMatrixService import + 受影响测试（`source-line-alias-retire-priority-audit` / `source-line-alias-mutations` / `sources-routes-by-site` / `sources-matrix` / `sources-matrix-service` / `admin-sources` 等）的 `import` & `vi.mock` 路径**同步迁移到新文件**。卡片文件范围**必须显式含这些测试文件**（否则撞「改范围外文件」红线）。

**D-117-VSR3-8：SourceSegment 兼容保留 + 不触碰 user_submissions。**
- `SourceSegment` 枚举 + segment 查询分支（含 `segment='correction'` 的 `submitted_by` 谓词）**本卡原样保留**（卡 5 删）；不扩不删。
- 全卡 SQL 不出现 `user_submissions`（§5.1 无失效举报）。

### 派生列双层透传（MEDIUM）
VideoGroupRowRaw map（sources-matrix.ts:282）与 Service raw→VideoGroupRow（SourcesMatrixService.ts:201）均显式字段枚举（非 spread），新派生列须**两处都补**，否则 Service 层静默丢弃。

### 门禁
- **无新 route** → verify:endpoint-adr 不触发；**无新 error code**（纯读+派生，复用 ADR-110 14 码）；**无 migration**（quality_rank 纯派生，quality/quality_detected/latency_ms/last_probed_at/is_active/is_published 列已存在）→ architecture.md 零同步。
- 本卡只**填充** CHG-VSR-1 已定义 optional 字段（不改 `sources-matrix.types.ts` 签名）；commit 带 `Subagents: arch-reviewer (claude-opus-4-8)` trailer（ADR 文档强制 Opus）。

**结论**：ADR-117 AMENDMENT 3 status 🟢 Accepted（arch-reviewer Opus 设计定稿 / 2 BLOCKER + 1 HIGH + 3 MEDIUM 已纳入 D-117-VSR3-1..8）。实施按「拆分 refactor（含测试 mock 路径迁移）+ 派生列/KPI feature」推进。
...
> ⚠️ **AMENDMENT 2026-06-02**：本端点的 4 维加权召回算法（type/year/country/genre）将在视频身份解析蓝图 **Phase 2c** 被**统一多证据评分引擎 + `identity_candidate` 候选表**取代——审核台「类似」tab 与 `/admin/merge` 共用同一候选来源与评分引擎。现有 4 维加权为过渡实现，不再独立演进；新方案见 `docs/designs/video-identity-resolution-redesign_20260602.md` §4.2 / §4.3 / Phase 2c。
...
- **D-161-1**：占位条目标题失配可能产生重复 catalog（见决策要点 8）。处置：accept（best-effort），不在本期闭环；未来归并卡触发条件 = 重复 catalog（同 bangumi_subject_id 多条 / 同名多条）> 5% 时起 PRE-MERGE-CATALOG 卡。 ⚠️ **AMENDMENT 2026-06-02**：PRE-MERGE-CATALOG 归入视频身份解析蓝图 **Phase 5**（catalog 身份层 / catalog-catalog 合并），见 `docs/designs/video-identity-resolution-redesign_20260602.md`。
