...
3. **`docs/decisions.md`**：架构决策记录（ADR-100..215，含 NEGATED / AMENDMENT）。
...
6. **`docs/task-queue.md`**：任务序列池（活跃：SEQ-20260524-01 M-SN-9 容器 🟡 + SEQ-20260601-01 起至最新 SEQ-20260624-02 VIDEO-PLAY-STATS 全部序列；更早分段归档见 `docs/archive/task-queue/`——M0~M-SN-0 / M-SN-1~6 / M-SN-7~META 三段）。
...
4. 活跃设计文档（`docs/designs/`）：
   - `home-operations-governance-plan_20260605.md`（首页运营治理方案：同构编辑画布 / 自动填充 / 豆瓣与 Bangumi 热榜 / Banner 横图强约束，后续 `/admin/home` UI/UX 改造依据）
   - `videos-sources-responsibility-redesign_20260601.md`（SEQ-20260601-01 ✅ 已落地，§6 拆卡表留档；被 decisions.md 引用作 ADR 定档输入，保留）
   ...
   - `notification-task-log-governance-plan_20260608.md`（后台「消息·通知·提醒·日志」综合治理；SEQ-20260609-01 ✅ 端到端闭环 2026-06-10）
   - `moderation-console-ux-plan_20260610.md`（内容审核台 UX 优化拆卡规划；SEQ-20260610-03 ✅ 全 15 卡完成 2026-06-11）
   - `source-health-feedback-loop-plan_20260610.md`（视频/线路/站点三层健康度反馈闭环；SEQ-20260610-02 🔄 15/17，P3-2 影子验证 + P3-4 时序阻塞，活跃）
   - `metadata-source-ux-handoff_20260617.md`（视频元数据多源治理 UI/UX 交接报告；配套 `audit/metadata-enrichment-investigation-20260615.md`）
   - `image-health-ux-handoff_20260618.md` + `image-health-problem-board_20260620.md`（图片健康页重构 + 问题图片治理板；SEQ-20260619-02 / SEQ-20260620-01 ✅ 全交付，IMGH-P4-A / P3-5 收尾 parked）
   - `client-video-card-sizing-audit_20260622.md`（前台卡片尺寸现状调查：三层分离 / 5 种定宽碎片化 / 死 token；卡片尺寸体系 SEQ-20260622-03 + A1/A2 已于 2026-06-24 全交付合并 `01b32abf`，本审计输入已消化）
   ...
   - `design_reference_v2.1.md`（兼容性指针 stub，内容已并入 `designs/backend_design_v2.1/reference.md`）
5. 活跃台账（`docs/audit/`）：`user-review-2026-05-23.md`（SEQ-20260524-01 M-SN-9 容器反馈登记簿）+ `adr-d-status.json`（verify:adr-contracts 生成，永不手改）+ `admin-cutover-parity-2026-06-08.md`（后台 cutover 功能重现核对）+ `cross-season-merge-audit-20260612.md`（跨季误合并存量盘点，VIDEO-NAMING-STANDARD-C）+ `identity-recall-grayzone-assessment-20260612.md`（候选阈值灰区 + 召回缺口评估，GOV-7）+ `metadata-enrichment-investigation-20260615.md`（元数据增强调查：豆瓣 / Bangumi / TMDB）。
6. 活跃调研纪要（`docs/research/`）：`image-health-codebase-survey_20260619.md`（image-health 重构事实底座，被多张 IMGH 卡引为设计真源）+ `imgh-p3-4c-safeimage-coverage_20260620.md`（前台 SafeImage 零裂图覆盖面核查）。
...
6. `docs/rules/admin-module-template.md`（⚠️ v1 章节已随 apps/server 退役〔CHG-CUTOVER-EXECUTE 2026-06-08〕，仅存历史参考；后台模块唯一现行真源 = `docs/designs/backend_design_v2.1/reference.md` §4.4 + §10）
...
14. `docs/rules/model-routing.md`（模型路由：缩写 → 完整模型 ID 映射现行版本，取代归档补丁 `model_routing_patch_20260418.md` §3 表）
