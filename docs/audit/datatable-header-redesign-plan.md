...
## v4 修订（ADR-149 AMENDMENT 1 / 2026-05-24）

EP-3 commit `1d1f635e` 后用户实测视频库发现 toolbar.search 槽位还有过滤下拉菜单。深查后发现 **5/7 消费方都把业务 filter UI 塞 search 槽位**（VideoFilterBar 5 select / crawler-runs 2 select / audit 2 select + 2 datetime / users 2 select / submissions 2 select）。这是 #UR-B1 真实根源 —— 原 ADR-149 D-149-11 只约束 trailing，**没约束 search 槽位**。

### AMENDMENT 1 关键新增决策

- **D-149-13** toolbar.search 槽位职责约定（白名单+黑名单+边界判定 / B 严约束：仅 1 search input）
- **D-149-14** toolbar 三槽位职责完整闭合声明（views/search/trailing 三槽位职责矩阵）
...
| EP | 范围 | 状态 |
|---|---|---|
| EP-1 | types deprecate + 矩阵原语 | ✅ commit `e671f498` |
| EP-2 | 列级 ⋯ + 列名 toggle | ✅ commit `aef7051e` |
| EP-3 | 删旧入口（2 文件） | ✅ commit `1d1f635e` |
| **EP-4** | DataTableSearchInput 原语 + IME + 7 消费方 search 接入 | 待启 |
| **EP-5-shared** | DataTableEnumFilter/TextFilter/DateRangeFilter 原语 + 20 单测 | 待启 |
| **EP-5-crawler-runs** | 2 select 迁移 | 待启 |
| **EP-5-submissions** | 2 select 迁移 | 待启 |
| **EP-5-users** | 1 search + 2 select 迁移 | 待启 |
| **EP-5-audit** | 2 select + 2 input + 2 datetime 迁移 | 待启 |
| **EP-5-videos** | 5 select 迁移（最复杂业务 key 桥接） | 待启 |
| **EP-6** | types.ts 一次性删 4 @deprecated prop | 待启 |
| **EP-7** | @livefree 走读 10 代表页 + #UR-B1..B4 闭合 | 待启 |

### 工时调整

CHG-SN-9-DT-HEADER-REDESIGN 总工时：2.5w → **5.2w**（+2.7w 增量）

### 兼容性

EP-1/2/3 已 commit 代码 **零回退 / 零代码逻辑改动**。AMENDMENT 1 落地仅 patch types.ts 4 处 @deprecated 注释（EP-4-B → EP-6）。

### 审核

arch-reviewer (claude-opus-4-7) 评级 **A− CONDITIONAL PASS**，9 修订建议 R-AMEND-1-1..9 全部在 AMENDMENT 1 文本内消解。

ADR-149 AMENDMENT 1 位置：`docs/decisions.md` line ~12237。

---

## v5 修订（ADR-149 AMENDMENT 2 / 2026-05-24）

EP-4-HOTFIX 完成后 @livefree 提出"表格设置三点何时接入"。深查 ADR-149 + AMENDMENT 1 发现 **§4 EP 序列从未显式安排"接入矩阵触发器到 DataTable 主组件"步骤**（与 D-149-10/11、D-149-13 同类 GAP）。

### AMENDMENT 2 关键新增决策

- **D-149-16** 矩阵触发器接入 DataTable 主组件 toolbar（选项 A toolbar 右端 ⋯ button + 始终渲染 + 独立 `[data-table-matrix-trigger]` 样式块）
- **业务 key 桥接到 onClearAllFilters**：遍历 columns 优先调 `columnMenu.onClearFilter`（**BLOCKER 修订** / 否则业务 key 维度 5 select 不会被清除）
- **合并式 resetColumnVisibility**：保留 width 字段（防消费方手工调整 width 丢失）
- **两工具函数沉淀** column-visibility.ts：`clearAllColumnFilters` + `resetColumnVisibility`
...
5.2w → **5.5w**（+0.3w EP-4.5）。

### 测试 surface

160-180 → **174-196**（+10-12 data-table.tsx + 4 column-visibility）。

### 兼容性

EP-1/2/3/4 + EP-4-HOTFIX 已 commit 代码 **零回退**。不触发 ADR-103 第 6 次 AMENDMENT。

### 审核

arch-reviewer (claude-opus-4-7) 评级 **B+ → A− CONDITIONAL PASS**，10 修订建议（含 2 BLOCKER）R-AMEND-2-1..10 全部在 AMENDMENT 2 内消解。

ADR-149 AMENDMENT 2 位置：`docs/decisions.md` line ~12435。
