...
## [INFRA-01] scripts/dev.mjs 改造：pipe + 落盘 + rotate + errors 聚合

- **完成时间**：2026-04-25
- **记录时间**：2026-04-25 18:15
- **执行模型**：claude-sonnet-4-6
- **子代理**：无
- **修改文件**：
...
- **数据库变更**：无
- **注意事项**：
  - 守门 A（人工验证）：人工启动 `npm run dev` ≥ 60 秒，肉眼确认四个 service 文件实时增长、Ctrl+C 后无丢尾行
  ...
  - client 前向兼容：见到 `service:'client'` JSON 行分流到 `logs/client/<date>.ndjson`，端点在 INFRA-04 落地
  - typecheck PASS / lint PASS / 1723 unit tests PASS

**日志格式样例（脱敏，守门 D 格式基线）：**

```ndjson
{"ts":"2026-04-25T10:31:42.123Z","level":"info","service":"api","stream":"stdout","pid":12345,"msg":"Server listening at http://0.0.0.0:4000"}
{"ts":"2026-04-25T10:32:01.456Z","level":"warn","service":"api","stream":"stdout","pid":12345,"msg":"Redis connection retry attempt 2","ctx":{"worker":"scheduler","attempt":2}}
{"ts":"2026-04-25T10:32:15.789Z","level":"error","service":"api","stream":"stdout","pid":12345,"msg":"Job processing failed","ctx":{"worker":"crawlerWorker","job_id":"job-abc123","stage":"fetch"},"err":{"name":"FetchError","message":"socket hang up"}}
```
