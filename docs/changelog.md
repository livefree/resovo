...
- **注意事项**：
  ...
  - errors 聚合规则：JSON 行 pino numeric level ≥ 50 或字符串 level = error/fatal 写入 errors；非 JSON 行仅 stderr + 保守前缀（Error: / Warning: / [ERR] / [WARN] / FATAL: / Uncaught）才写 errors
...
**真实日志格式样例（守门 D 基线，脱敏 host/IP/port，从 logs/dev 实测抓取）：**

```ndjson
// ① pino 透传行（fastify 自带，本卡当前阶段约 84% 占比）— 字段：level(numeric)+time+pid+hostname+reqId+req/res+msg
{"level":30,"time":1777167350577,"pid":22563,"hostname":"<host>","reqId":"req-3o","req":{"method":"GET","url":"/v1/videos/trending?type=anime&limit=8","hostname":"<host>","remoteAddress":"<ip>","remotePort":<port>},"msg":"incoming request"}
...
// ② wrap 行（dev.mjs 对裸文本输出包装，本卡当前阶段约 16% 占比）— 字段：ts+service+stream+msg
...
**字段契约不齐说明（INFRA-07 当前阶段预期）**：上方两类行字段不一（① 有 level 无 ts/service/stream；② 有 ts/service/stream 无 level/pid）。这是 INFRA-07 仅在 dev.mjs 编排层做"按行分流 + 落盘"的预期产物——pino 透传行保留 fastify 原生 schema，wrap 行只补最小必要字段。**统一字段契约（每行都有 ts/level/service/stream/pid/msg/ctx?/err?）由 INFRA-08（API logger 接入）+ INFRA-09（packages/logger 抽公共序列化器）逐步完成。logging-rules.md（INFRA-11）将沉淀此分阶段规则。**
