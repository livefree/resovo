...
- **修改文件**：
  - `apps/api/src/services/TmdbConfirmService.ts` — `CROSS_VALIDATION_GROUPS` + `isEmptyValue` + `filterCrossValidation` + autoMatch 接入 + import CATALOG_SOURCE_PRIORITY/MediaCatalogRow
  ...
  - `tests/unit/api/tmdb-confirm-service.test.ts` — autoMatch 交叉验证 2 用例（bangumi 同级不覆盖仅补空 / douban 低于权威覆盖）+ MediaCatalogService mock 补 CATALOG_SOURCE_PRIORITY
...
- **质量门禁**：typecheck 7 workspace 全过 / lint 4 successful / test:changed 14 文件 245 passed + 定向回归 37 文件 528 passed（enrich/tmdb/douban/bangumi/moderation/safeUpdate/textMatch 零回归 + 新单测 autoMatch 交叉验证 2 + stepTmdb 5）/ verify:adr-contracts VERIFY=0；e2e N/A（纯 service 无 UI/route）。
- **注意事项**：① interim 交叉验证（Option A）是 reconcile 前的轻量近似——等/高优先级源（anime 的 bangumi）内容不被 tmdb 覆盖、仅补空 + 绑 ref/cache；**META-49-B reconcile 上线后** 各源统一改产 proposal + 一致性加权裁决（D-205-1），`filterCrossValidation` 届时被 reconcile 取代。② 同级 fill-empty 时 safeUpdate 仍会把 metadataSource 翻成 tmdb（功能等价，4==4，benign）。③ enrichmentWorker 文件未改（埋点在 service 内，worker 已记 job 生命周期）。
