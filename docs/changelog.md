...
## [MERGE-DEV-MAIN-INTEGRATION-20260710] 合并 main→dev 集成 + ADR-216 编号撞车重编（dev META → ADR-219）
- **完成时间**：2026-07-10
- **记录时间**：2026-07-10
- **执行模型**：claude-opus-4-8（主循环）
- **子代理**：`arch-reviewer` (claude-opus-4-8) — 重编号消歧 + 合并解决独立复核
- **触发**：用户「调查 main 和 dev，考虑合并 dev 到 main」→ 授权「重编号后完成合并」。
- **问题/根因**：dev 与 main 并行 track 期各自独立创建内容不同的 **ADR-216**——main STATS「视频级播放量统计体系」（SEQ-20260624-02，已交付 + migration 128 深引用）vs dev META「per-video 状态列退役、external refs 收口」（SEQ-20260627-01）。上个 commit CHG-368 已修 ADR-217 撞车（dev PLAYER-12 217→218）却漏掉 ADR-216 同样撞车。直接合并会致 decisions.md 双 ADR-216、账本破损。
- **方案**：延续 CHG-368「合入方 dev 让位主干 main」惯例——STATS ADR-216 保留 216；dev META ADR-216 整体重编为 **ADR-219**（`ADR-216→219` / `D-216-→D-219-` / `DC-216-→DC-219-`），**仅在 META 语境替换**（两 ADR 各有同号 D-216-N，按文件/行语境消歧）。
- **修改文件**：
  - `docs/decisions.md` — ADR 块重排数字升序（216 STATS→217 STATS→218 PLAYER→219 META）+ ADR-219 头加编号溯源注 + ADR-150 AMENDMENT 4 块 META 引用改 219；STATS ADR-216/D-216-N 原样保留
  - `docs/changelog.md` — union 解冲突（dev+main 条目并存，零重复）+ META 9 条（META-55-ADR…META-56）语境重编号；STATS 条目 + CHG-368 内「main ADR-216」保留
  - `docs/task-queue.md` — SEQ-20260627-01（META）段重编号；SEQ-20260624-02（STATS）段保留 216
  - `docs/tasks.md` — SEQ-20260627-01 进行中注重编号
  - `docs/audit/adr-d-status.json` — `verify:adr-d-numbers` 重新生成（含 ADR-216 STATS + ADR-219 META）
  - 代码/测试 META 语境注释重编号（video-ref-applied / moderation / staging / videos.status / BangumiService / distinct-whitelist / enrich-status-guard / MetadataEnrichService / align-douban-status-to-refs + 6 对应测试）；混合文件 `videos.ts` 行级消歧（STATS 行 118/205/206 留 216、META-58-B-1 行 469/679/696 改 219）
  - 4 个 both-modified 代码冲突解决（`videos.ts` / `routes/videos.ts` / `SearchService.ts` / `DetailHero.tsx`，合并 dev SEARCH/PLAYER + main STATS 逻辑）
- **新增依赖**：无
- **数据库变更**：无（仅注释/文档/ADR 编号，零功能逻辑改动）
- **验证**：全仓审计——decisions.md ADR-216 唯一、四 ADR 数字升序；STATS 文件零 219 误入 / META 文件零 216 残留；含 219 文件全 META 语境；混合文件（videos.ts + 4 docs）语境正确。arch-reviewer 独立复核。
- **门禁**：typecheck=0（7 workspace）/ lint=0（仅 web-next:272 既有 warning）/ verify:adr-contracts=0（endpoint-adr ✅ 253 路由 / sql-schema ✅ / admin-shell-mirror ✅；adr-d-numbers advisory）/ **全量单测 627 文件 8551 passed 零回归**。
- **范围声明**：合并 dev→main **前置集成**（main→dev merge commit）+ 编号撞车处置。**dev→main 最终合并 + push 待用户确认**。
- **注意事项**：`docs/designs/client_handoff/` 为 dev 未提交的 untracked 交付物，非本次合并范围，未纳入本 commit。
- **自审**：[AI-CHECK] 代码功能改动 NA（仅注释/文档/编号）；账本完整性 YES（ADR 唯一性 + 语境消歧全审计）；回归 NO（8551 单测零回归）。
...
## [META-55-ADR-20260627] ADR-219 立案 + 定稿：per-video 状态列退役、external refs 收口为元数据匹配状态单一真源（SEQ-20260627-01）
- **完成时间**：2026-06-27
...
- **产出文件**：
  - `docs/decisions.md` — ADR-219 **Accepted**（方向 A 变体：消费方迁 `videoRefAppliedSql` video 级 applied 谓词〔含 auto_matched=等价旧 matched〕非 catalog overall；两谓词语义分工；enrich 一致性守卫；软删纯读期 join；列 DROP 拆独立 ADR）+ 软删处置表 + 两轮 arch-reviewer/Codex 摘要
  - `docs/task-queue.md` — SEQ-20260627-01 立案 + 卡序（DC-219-1 谓词契约 / DC-219-2 软删处置表 / META-56 DROP-prep / META-57 守卫 / META-58 迁移 / META-59 软删过滤 / META-60 冻结）
...
- **注意事项**：用户 2026-06-27 裁可方向 A + matched 取 video 级语义，解锁 DC-219-1。ADR 文档随实现卡一并 commit（用户选项）。后续 DC-219-1→META-57→58→DC-219-2→59→56→60→独立 DROP ADR。

---

## [DC-219-1-20260627] videoRefAppliedSql 谓词契约（ADR-219 D-219-9 / SEQ-20260627-01）
- **完成时间**：2026-06-27
- **记录时间**：2026-06-27 22:40
- **执行模型**：claude-opus-4-8（主循环）
- **子代理**：无新调用（谓词契约决策由 ADR-219 `arch-reviewer`(claude-opus-4-8) 两轮裁定背书 D-219-2/D-219-9）
...
## [META-57-20260628] enrich 写列一致性守卫（ADR-219 D-219-3 / SEQ-20260627-01）
- **完成时间**：2026-06-28
- **记录时间**：2026-06-28 00:35
- **执行模型**：claude-opus-4-8（主循环）
- **子代理**：无（守卫逻辑非新架构决策，判据由 DC-219-1 / ADR-219 D-219-3 定）
- **内容**：根因封堵——enrich 写 `videos.{douban,bangumi}_status` 列前一致性守卫。`guardEnrichStatusAgainstAppliedRef`：computed='unmatched' 但该 video 已有 applied primary ref → 返回 'matched'（反映 ref 真相），消除「ref(linked_at) ≤ enriched_at」覆写竞态（60/60 漂移产生机制）；**仅守 unmatched 降级**，matched/candidate/pending 原样（不完全停写，列随 refs 演进——完全停写留 META-60）。`hasAppliedVideoRef` 复用既有 `findPrimaryVideoExternalRef` + DC-219-1 `isVideoRefApplied`（单一真源，不造判据/查询）。
- **修改文件**：
...
## [META-58-A-20260628] boolean applied 消费方迁移：moderation/staging 就绪判定（ADR-219 D-219-2 / SEQ-20260627-01）
- **完成时间**：2026-06-28
- **记录时间**：2026-06-28 00:46
- **执行模型**：claude-opus-4-8（主循环）
- **子代理**：无（迁移判据 = DC-219-1 `videoRefAppliedSql` 单一真源）
...
- **实现期拆卡发现（契约 gap）**：`douban_status`/`bangumi_status` 消费方分两类——boolean applied（本卡）与**多值过滤**（审核台/视频库 4 态下拉：moderation:216 / videos:434/437 / videos.status:180）。后者需 **video 级 per-state 谓词（candidate/missing/pending）**，超 DC-219-1（只定义 applied boolean）→ 拆 **META-58-B**（需 arch-reviewer 裁定契约扩展）。`videos` backfill 的 `unmatched` 态（≠ NOT applied）亦归 58-B。索引 `idx_video_external_refs_video_provider_status` 随 58-B（避免本卡 architecture.md 同步范围扩张）。
- **修改文件**：
...
## [META-58-B-1-20260628] 后端 douban 4 态过滤谓词 + 多值过滤迁移（ADR-219 D-219-10/13 / SEQ-20260627-01）
- **完成时间**：2026-06-28
- **记录时间**：2026-06-28 03:35
- **执行模型**：claude-opus-4-8（主循环）
- **子代理**：arch-reviewer (claude-opus-4-8)〔第三轮 4 态谓词契约裁定 D-219-10/11/12，已记 c388d25e〕、codex-rescue〔对抗审 2 处 BLOCK 修正，agentId a187e6f3〕
...
- **Codex 对抗审 2 处 BLOCK 修正**：① **穷尽性**——D-219-10 原 pending `meta_quality IS NULL OR dms IS NULL` 非穷尽，漂移中间态（dms∈{auto/candidate/manual} 但无 ref）4 态皆 false → 「全选 4 态仍漏行」bug；改 pending 兜底 `IS DISTINCT FROM 'unmatched'`（含 never enrich NULL + 中间态），保穷尽互斥。② **投影迁移整体延 META-60**——D-219-12 漏列 moderation:344 + VIDEO_FULL_SELECT 投影，单独迁部分投影造成口径分裂 + 耦合 derive `statusColumnState`，故投影（含 videos.status:207 回退）整体延 META-60。
- **修改文件**：
...
- **内容 + 事实修正**：移除后台 DataTable distinct 端点白名单 `DT_DISTINCT_COLUMN_SQL.videos.douban_status`。修正 D-219-12 假设：前端 `VideoColumns.tsx:504` 早用静态 `DOUBAN_STATUS_OPTIONS`（列未声明 `filterDistinctTable`→从不查 distinct 端点），白名单键为死配置。前端零改动；bangumi 保留（D-219-11）。
...
- **注意事项**：bangumi distinct 键同为死配置但列暂留（D-219-11），随 META-61 一并清理。code 已在 commit 291244b4，本次补 changelog + 工作流收尾。SEQ-20260627-01 下一卡 DC-219-2。

## [DC-219-2-20260628] refs 读路径软删处置表穷举归类（ADR-219 D-219-4/D-219-9 / SEQ-20260627-01）
- **完成时间**：2026-06-28
...
- **内容**：穷举全仓 `grep video_external_refs` = 13 文件，三类逐路径软删处置裁定落 ADR-219 处置表（META-59 实施清单）。**类A**（必过滤）metadata-status.derive.ts 8 子查询；**类B**（外层 gate）externalData/externalIdLoader/split-suggestions/video-merge-candidates/`video-ref-applied.ts`（谓词 invariant：EXISTS 不自 gate、依赖外层）/moderation；**类C**（聚合/gap 须改）home-autofill-douban/-bangumi/external-resources-stats；**N/A**（写/审计）MetadataEnrichService:146/BangumiService:196/DoubanService:239,362。
...
- **修改文件**：`docs/decisions.md`（处置表补全 + video-merge-candidates 修正 + 穷举闭合注）/ `docs/tasks.md`（删卡 + SEQ 注释 DC-219-2 ✅）
- **新增依赖**：无
- **数据库变更**：无（纯分析/文档）
...
## [META-59-20260628] refs 软删纯读期过滤——类C gate + derive 类A→类B 修正（ADR-219 D-219-4 / SEQ-20260627-01）
- **完成时间**：2026-06-28
- **记录时间**：2026-06-28 18:00
- **执行模型**：claude-opus-4-8（主循环）
- **子代理**：arch-reviewer (claude-opus-4-8, ab76495e) — derive 类A→类B 改判 + 类C 确认 + META-59 范围收窄裁定
- **内容**：按 DC-219-2 处置表 + arch-reviewer 第二轮裁定，**只 gate 类C 三路径**（软删视频的 refs 不计入跨 video 聚合/gap）：
...
## [META-56-20260628] DROP-prep 对齐脚本：applied ref 但 douban_status 欠计→matched（ADR-219 D-219-2 / SEQ-20260627-01）
- **完成时间**：2026-06-28
- **记录时间**：2026-06-28 19:00
- **执行模型**：claude-opus-4-8（主循环；META-56 卡建议 sonnet，本会话 opus 连续推进同 SEQ）
- **子代理**：无
- **内容**：新增 `scripts/align-douban-status-to-refs.ts` — DROP-prep 一次性对齐脚本。圈定有 applied douban ref（`videoRefAppliedSql` DC-219-1 单一真源：is_primary + match_status IN auto_matched/manual_confirmed）但 `douban_status != matched` 的 live video（欠计漂移，立案 60 行）→ UPDATE `douban_status='matched'`。
...
## [HANDOFF-46-20260629] 动效 token 基座对齐：落地设计稿 canonical motion 词表（Motion Spec 设计稿 / SEQ-20260629-02）
- **完成时间**：2026-06-29
- **记录时间**：2026-06-29
- **执行模型**：claude-opus-4-8（主循环）
...
- **内容**：
  ...
  - **hover 浮起规则**：`globals.css` 新增 `.video-card-poster`（transition `transform/box-shadow` 时长 `calc(var(--duration-base) * var(--motion-scale,1))`、easing `--easing-ease-out`）+ `.video-card-lift:hover .video-card-poster`（`transform: translateY(-6px) scale(1.03)` 固定 + `box-shadow: var(--shadow-card-hover)`）+ `.video-card-lift:hover { z-index:1 }`。**门控**：`@media (hover:hover) and (pointer:fine)`〔触屏不触发持留态〕；`@media (prefers-reduced-motion: reduce)` 禁 transform、保留 shadow 作静态深度 cue；动效强度 `--motion-scale`（HANDOFF-26 滑块 0/1/1.5=关闭/正常/增强）按既有惯例（globals.css:522/532）缩放 transition **时长**（非位移）。_（注：初版误将 `--motion-scale` 乘到位移幅度且 duration 未接 motion-scale，与既有 duration 惯例语义相悖，Codex stop-gate 标记；已改为缩放 duration、位移固定，见下方 [HANDOFF-47-A-FIX]。）_
...
### [HANDOFF-47-A-FIX-20260629] 修正：hover 的 --motion-scale 改为缩放 transition 时长（对齐既有惯例）
- **完成时间**：2026-06-29 ｜ **记录时间**：2026-06-29 ｜ **执行模型**：claude-opus-4-8（主循环）｜ **子代理**：无
- **触发**：Codex stop-gate 复审标记「hover motion-scale semantics are inverted」。
- **根因（源核实）**：`--motion-scale` 真实语义为「动效强度」滑块 `MOTION_VALUES=[0,1,1.5]`（关闭/正常/增强，SettingsDrawer.tsx:31；非初版假设的「0.25=减弱」），且代码库**唯一既有惯例**（globals.css:522/532 + 注释 line 221）是 `--motion-scale` 缩放 **transition 时长** `calc(<duration> * var(--motion-scale,1))`。初版 47-A 反将 `--motion-scale` 乘到**位移幅度**（`translateY(-6px*scale) scale(1+0.03*scale)`）且 transition 时长未接 `--motion-scale` → 与既有 duration 惯例语义相悖（强度越高位移越大但时长不变=速度反向），且注释误写「0.25 减弱」（无此档）。
- **修**（CLAUDE.md 价值排序 #4 一致性）：位移幅度固定 `translateY(-6px) scale(1.03)`（逐字对齐设计稿）；`--motion-scale` 改为按既有惯例缩放 transition 时长 `calc(var(--duration-base) * var(--motion-scale,1))`（transform + box-shadow 两条均接）。注释改写为真实档位 0/1/1.5 语义。`prefers-reduced-motion` 仍禁 transform 保 shadow（不变）。
- **修改文件**：`apps/web-next/src/app/globals.css`（hover 浮起块 + 注释）/ `docs/changelog.md`（47-A 条目过时描述同步修正）
- **新增依赖**：无 ｜ **数据库变更**：无
