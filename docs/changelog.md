...
- **Codex stop-time review 修复**：自动兜底切线（switchAwayFromFailedSource 环形扫描）原仅跳过 `deadLineKeys`（运行时失败）+ 检查集存在，**漏了旧 PlayerShell `!isDead && !isPending` 守卫** → 可能切入 route-scoring 已判 dead/pending 的 known-bad 线路。修复：环形扫描加 `themedLinesRef`（baseThemedLines / route-scoring 派生），同时跳过 themed dead/pending；新增 on-error #6 用例（dead 线路被跳过，切到下一健康线路）。
