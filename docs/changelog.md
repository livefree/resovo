...
- **e2e**：N/A（纯类型）。
- **[AI-CHECK]**：六问全过——复用既有 canonical（`ResolutionTier` 类型 + `QUALITY_ORDER` 档位序 + `qualityHighest` 命名）；契约面收敛（VideoGroupRow 质量单一真源 qualityHighest，rank 纯 producer 内部）；零回归（grep 确认无引用）；无 any/颜色/越层。**注意事项（已含二次 FIX 修正）**：producer 卡 3 派生 `qualityHighest`/低质量 rank 须**逐源 `quality_detected ?? quality` 回退后**按 `QUALITY_ORDER` 取最高档——**勿照搬 LinesPanel `pickHighestQuality`（仅 quality_detected、丢回退）**；低质量排序/谓词的 quality_rank 在服务端 ORDER BY CASE / FILTER 内派生，不回写 DTO。
