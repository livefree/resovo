...
- **Codex stop-time review FIX（展示与排序契约）**：
  ...
  - **e2e smoke 同步修复（不留破坏）**：`sources-sort-filter-smoke.spec.ts` test 1 断言由「首请求 sortField=null」改为「sortField=lastChecked + sortDir=desc」（随默认排序契约更新）。实跑（直连已运行 server-next:3003 绕过 web-next:3000 webServer 冲突）：**test 1（我修的默认排序）+ test 2（video sort）+ test 4（siteKey distinct）3 PASS**；test 3（probeStatus filter「应用」按钮 outside viewport）失败为**既有 env 问题**——取 5-A 前版本（042a43ef）同样失败，与本卡布局改动无关（DataTableAutoFilter popover 本机 viewport 渲染问题，留独立排查）。
