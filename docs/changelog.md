...
## [HANDOFF-30] 播放页 §15 规格对齐（5 项布局修正）

- **完成时间**：2026-04-24
- **记录时间**：2026-04-24 20:20
- **执行模型**：claude-sonnet-4-6
- **子代理**：无
- **修改文件**：
  - `apps/web-next/src/components/player/PlayerShell.tsx` — 骨架屏/正常模式容器 px-4→px-10；影院模式标题区 px-4→px-10；播放器圆角 0.5rem→12px；新增 max-w-[1280px] 下方内容区容器；"更多推荐内容即将上线"从侧栏移至下方内容区
  - `apps/web-next/src/app/globals.css` — `--player-panel-gap` var(--space-4)→var(--space-6)（16px→24px）
- **新增依赖**：无
- **数据库变更**：无
- **注意事项**：`player-lower-section` 容器（data-testid）为后续推荐内容预留结构，等真实内容落地时直接填充无需改容器；`--player-panel-gap` 全局修改，影院/正常模式下 gap 均同步为 24px。

### 验收确认

- ✅ 骨架屏 px-10 py-4（与正常模式对齐）
- ✅ 正常模式容器左右各 40px（px-10 = 2.5rem = 40px）
- ✅ 影院模式标题区左右各 40px inset
- ✅ 播放器圆角 12px（影院模式仍为 0，无影响）
- ✅ 下方内容区 max-w-[1280px] mx-auto px-6，含占位文案
- ✅ --player-panel-gap: var(--space-6) = 24px
- ✅ typecheck PASS / lint PASS / 1682 unit tests PASS
...
- **完成时间**：2026-04-24
- **记录时间**：2026-04-24 20:45
- **执行模型**：claude-sonnet-4-6
- **子代理**：无
- **修改文件**：
  - `apps/web-next/src/components/layout/Footer.tsx` — 重写为 3 列 grid（浏览/帮助/关于）；浏览列单源 lib/categories.ts MAIN_TYPE_PARAMS；locale 前缀；移除品牌块/社交图标/重复法务行
  - `apps/web-next/src/components/layout/SettingsDrawer.tsx` — 新建：右侧 fixed 抽屉，z-overlay=80，主题 3 按钮，动效滑块，占位 section
  - `apps/web-next/src/components/layout/Nav.tsx` — settingsOpen state + Settings 按钮 onClick + 渲染 SettingsDrawer
  - `apps/web-next/src/app/globals.css` — 新增 `--z-overlay: 80`、`--motion-scale: 1`、`--overlay-bg`
  - `apps/web-next/messages/zh-CN.json` / `en.json` — 新增 footer + settings 命名空间
  - `tests/unit/web-next/Header.test.tsx` — 补充 `useTheme` mock（Nav 引入 SettingsDrawer 后需要 ThemeContext）
- **新增依赖**：无
- **数据库变更**：无
- **注意事项**：`--motion-scale` 挂在 `:root`（REVIEW-C 审核项 #8 已满足）；localStorage 读写均有 `typeof window !== 'undefined'` 守卫（REVIEW-C 审核项 #7）；`--z-overlay=80 > --z-mini-player=50`（REVIEW-C 审核项 #4）；Footer 浏览列和 Nav 共用 lib/categories.ts 唯一数据源（REVIEW-C 审核项 #5）。

### 验收确认

- ✅ Footer 3 列 grid（grid-cols-1 md:grid-cols-3）
- ✅ 浏览列来自 MAIN_TYPE_PARAMS，链接含 `/${locale}/` 前缀
- ✅ 帮助列（帮助中心/DMCA/隐私政策）+ 关于列（关于我们/联系方式），链接暂 `#`
- ✅ 无重复底部法务链接，保留 Copyright
- ✅ Settings 按钮 onClick → settingsOpen=true → SettingsDrawer 展开
- ✅ ESC 和遮罩点击 → onClose → settingsOpen=false
- ✅ 主题 3 按钮调用 useTheme().setTheme()（BrandProvider context 更新）
- ✅ 动效滑块 [0,1,2] → [0,1,1.5] → `--motion-scale` :root + localStorage
- ✅ typecheck PASS / lint PASS / 1682 unit tests PASS

## REVIEW-C — Batch C 阶段独立审核 + P0 颜色 token 修复
...
| 文件 | 变更 |
|------|------|
| `app/globals.css` | 新增 `--shadow-drawer` / `--player-video-area-bg` / `--player-no-source-fg` 三个 token |
| `components/player/PlayerShell.tsx` | `background: 'black'` → `var(--player-video-area-bg)`；`color-mix(in oklch, white 50%, ...)` → `var(--player-no-source-fg)` |
| `components/layout/SettingsDrawer.tsx` | `boxShadow: '-8px 0 32px oklch(...)'` → `var(--shadow-drawer)` |
| `docs/handoff_20260422/review_phase2_c_20260424.md` | 审计报告写入（11 项，9 PASS + 1 NEED_FIX 已修复 + 1 NIT） |
| `docs/task-queue.md` | REVIEW-C 标记 ✅；typecheck/lint/test 序列验收项标 ✅ |

### 审核结论（11 项）

1. MiniPlayer locale 来源（useParams）— PASS
2. hover chip prefers-reduced-motion + motion-scale — PASS
3. MiniPlayer 无假播放控件 — PASS
4. SettingsDrawer z-index 层级（80 > 50）— PASS
5. Footer 浏览链接数据来源（categories.ts 同源）— PASS
6. Footer 链接 locale 前缀 — PASS
7. SettingsDrawer localStorage SSR 安全 — PASS
8. --motion-scale 挂载位置（:root / documentElement 一致）— PASS
9. 硬编码颜色 — **NEED_FIX → 已修复**（3 处 token 化）
10. any 类型零遗漏 — PASS
11. VideoCardWide @deprecated 注释 — PASS

### 质量验证

- typecheck PASS / lint PASS / 1682 unit tests PASS
- REVIEW-C PASS，Phase 2 Close 完成

## HANDOFF-31 — MiniPlayer 非交互基础层
...
| 文件 | 变更 |
|------|------|
| `stores/playerStore.ts` | 新增 `releaseMiniPlayer()` action（清零 shortId/time/playing/activeSourceIndex + closeHost）|
| `app/globals.css` | 新增 HANDOFF-31 区块：12 个 `--player-mini-*` token + `.mini-video-overlay` 工具类 + 移动端 CSS 双保险 |
| `lib/mini-player/drag.ts` | `attachViewportResizeWatcher` 始终按 corner 重算 left/top（UI Contract §2.3），不再仅越界时触发 |
| `app/[locale]/_lib/player/MiniPlayer.tsx` | 全骨架重构：绝对定位模型（header overlay + video area + controls placeholder），isExpanded 折叠/展开，handleClose 用 releaseMiniPlayer，移动端 MQ 防护，role="region" ARIA，data-testid 占位全部到位，移除 PiP 代码 |
| `tests/unit/web-next/MiniPlayer.test.tsx` | 新建 13 个 Vitest 单元测试（无 src / Esc close / 关闭按钮 / 返回按钮 aria / 展开折叠 / takeoverActive） |
| `tests/e2e-next/mini-player.spec.ts` | 更新 §2（hover 先再点关闭）；§3 重写为折叠/展开双态测试（移除旧 setHostMode('full') 断言）|

### 产品语义变更（UI Contract §3）

- 旧：展开按钮 → `setHostMode('full')` → mini 消失全屏
- 新：展开/折叠按钮 → `isExpanded` 切换（Collapsed/Expanded 双态，控制栏可见性）
- 新：返回按钮（← arrow）→ `router.push('/${locale}/watch/${hostOrigin.slug}')`（替代原 video-slot 单击行为）

### 验收确认

- ✅ `releaseMiniPlayer()` 存在，调用后 `shortId=null` + `hostMode='closed'`
- ✅ globals.css 中 16 个 `--player-mini-*` token 全部存在（4 旧 + 12 新），零硬编码颜色
- ✅ drag.ts resize watcher：始终按 corner+margin 重算 left/top
- ✅ MiniPlayer 骨架：Collapsed 高度 = videoH，Expanded = videoH + 44px
- ✅ 移动端 MQ 触发 → releaseMiniPlayer()（CSS display:none 双保险）
- ✅ Esc 关闭 → handleClose() → releaseMiniPlayer()
- ✅ MiniPlayer.tsx 无任何 `pip` / `requestPictureInPicture` 字样
- ✅ data-testid 占位：return-btn / toggle-expand / play-pause / progress / mute / loading / error / no-source
- ✅ typecheck PASS / lint PASS / 1695 unit tests PASS

---

## [HANDOFF-32] MiniPlayer 视频交互核心层
- **完成时间**：2026-04-25
- **记录时间**：2026-04-25 14:00
- **执行模型**：claude-sonnet-4-6
- **子代理调用**：无
- **修改文件**：
  - `apps/web-next/src/app/[locale]/_lib/player/useMiniPlayerVideo.ts` — 新建：视频逻辑 hook，封装 activeSrc 解析（/videos/:shortId/sources?episode=N）、autoplay 语义、六态 VideoStatus 状态机、250ms throttle timeupdate、saveProgress 调用
  - `apps/web-next/src/app/[locale]/_lib/player/MiniPlayerHeader.tsx` — 新建：header overlay 子组件（drag handle + 返回/展开/关闭按钮）
  - `apps/web-next/src/app/[locale]/_lib/player/MiniPlayerControls.tsx` — 新建：展开态控制栏子组件（play/pause + 进度条 ARIA slider + 时间显示 + 静音按钮）
  - `apps/web-next/src/app/[locale]/_lib/player/MiniPlayer.tsx` — 重构：引入 useMiniPlayerVideo + 子组件；添加真实 `<video>` 元素；完整 overlay 状态机渲染；点击防误触（5px/300ms）；m 键静音；进度条 seek handler；从 514 行降至 427 行
  - `tests/unit/web-next/MiniPlayer.test.tsx` — 新增 9 个 HANDOFF-32 单测（视频状态、loading/error overlay、autoplay-blocked、m 键 mute），总计 22 个用例全通过
  - `tests/e2e-next/mini-player.spec.ts` — §1 宽度断言从 toBeCloseTo(320, 0) 改为 >=318 && <=323（兼容容器 border 像素）
- **新增依赖**：无
- **数据库变更**：无
- **注意事项**：
  - e2e 测试中 MiniPlayer 不渲染（sessionStorage 注入后 reload 不触发 hydrateFromSession）为 HANDOFF-31 遗留基础设施问题，HANDOFF-32 未引入新失败，从 18 降至 16 个失败
  - useMiniPlayerVideo 的 shouldAutoplayRef 快照模式：activeSrc 变化时记录当前 isPlaying 状态，canplay 回调中一次性尝试 autoplay，避免重复 play() 调用
  - videoStatusRef 用于 handleTogglePlay 的稳定引用（绕过 useCallback 依赖），避免 state 闭包问题
  - REVIEW-D 触发条件：HANDOFF-31 + HANDOFF-32 全部完成，且 typecheck / lint / unit tests 全绿（当前满足）

### 验收确认

- ✅ 真实 `<video>` 元素接入，activeSrc 非空时渲染
- ✅ activeSrc 从 `/videos/:shortId/sources?episode=N` 解析，activeSourceIndex 越界 fallback sources[0]
- ✅ autoplay 语义：isPlaying=true 时 canplay 尝试 play()；NotAllowedError → autoplay-blocked UI（点击播放文案）
- ✅ onTimeUpdate 250ms 节流，同时写 setCurrentTime 和 saveProgress
- ✅ overlay 六态：no-src / loading / error / autoplay-blocked / paused / playing+hover
- ✅ 点击防误触：pointerdown 记录坐标，移动 <5px 且 <300ms 视为点击
- ✅ m 键 toggle mute；进度条 ARIA slider 属性完整；duration 未知时禁用
- ✅ MiniPlayer.tsx 427 行（≤ 500 行限制）
- ✅ typecheck PASS / lint PASS / 1704 unit tests PASS

---

## [DOC-20260425] MiniPlayer 现状与交互问题审计归档

- **记录时间**：2026-04-25
- **类型**：文档归档 / 现状审计
- **生产代码变更**：无
- **新增文件**：
  - `docs/handoff_mini_player/miniPlayer_status_and_interaction_audit_20260425.md`
- **归档内容**：
  - 汇总 MiniPlayer 当前已落地能力：Portal 宿主、桌面浮窗、真实 `<video>`、Header/Controls、拖拽缩放、几何持久化、移动端关闭防护、基础视频状态机。
  - 记录验证结果：MiniPlayer 相关单测 53 条通过；`@resovo/web-next` typecheck 通过；MiniPlayer 单文件 E2E 当前 `8 failed / 1 flaky / 2 passed`，主要卡在 `page.goto/page.reload` 等待 `load` 超时。
  - 归档交互风险：full 播放状态未同步导致离开 `/watch` 可能无法进入 mini、MiniPlayer 未复用 HLS 加载链路、全局快捷键作用域过宽、返回 `/watch` 时序竞争、seek 后不立即保存进度、expanded dock 高度风险、Header/Buffered/A11y 等 contract 偏差。
- **后续建议**：
  - 先拆 Fix A（主路径可用性）：full 播放状态同步 + HLS 支持 + 播放中离开 `/watch` 真实 E2E。
  - 再拆 Fix B（交互一致性）：快捷键作用域、返回时序、seek 即时落盘、expanded 几何。
  - 单独清理 E2E harness 与 README / spec 注释过时内容。

---

## [HANDOFF-33] MiniPlayer 主路径治理（P0/P1）

- **完成时间**：2026-04-25
- **记录时间**：2026-04-25 05:30
- **执行模型**：claude-sonnet-4-6
- **子代理**：无
- **修改文件**：
  - `apps/web-next/src/app/[locale]/_lib/player/useMiniPlayerVideo.ts` — P0-2 添加 HLS.js 支持（动态 import、cancelled flag 防竞态、cleanup destroy）；P1-3 暴露 `handleSeek(time)` 立即写 store+localStorage
  - `apps/web-next/src/app/[locale]/_lib/player/MiniPlayer.tsx` — P1-1 添加 `isEditableTarget()` 守卫和 modal 检查；P1-2 返回按钮先调 `setHostMode('full')` 再 push；P1-3 seek handlers 调用 handleSeek；P1-5 handleToggleExpand 直接用 computeDockPosition 写 container.style.top
  - `tests/unit/web-next/MiniPlayer.test.tsx` — 补充 P1-1（input/textarea/modal 守卫）和 P1-2（setHostMode 先于 push）单测；mock 补 setHostMode
- **新增依赖**：无（hls.js 已在 workspace dependencies）
- **数据库变更**：无
- **注意事项**：
  - P0-1（isPlaying 同步）已于 commit 4adc5b8 单独完成，不在本任务范围
  - P1-4（Buffered progress overlay）未在本任务范围，可在 HANDOFF-34 一并处理
  - typecheck PASS / lint PASS / 1710 unit tests PASS

---

## [HANDOFF-34] MiniPlayer 体验增强（P2）

- **完成时间**：2026-04-25
- **记录时间**：2026-04-25 13:30
- **执行模型**：claude-sonnet-4-6
- **子代理**：无
- **修改文件**：
  - `apps/web-next/src/stores/_persist/mini-geometry.ts` — 导出 `DockMargins` 类型；`computeDockPosition` 支持 `number | DockMargins`（向后兼容）
  - `apps/web-next/src/lib/mini-player/drag.ts` — `DragAttachOptions` 添加 `getMargins?`；corner-aware resize（delta 符号 + 位置同步锚点）；`commitDrag`/`commitResize`/resize watcher 传入 margins
  - `apps/web-next/src/app/[locale]/_lib/player/types.ts` — `PersistedPlayerHostV1` 扩展可选字段 `currentTime?/isMuted?/volume?/activeSourceIndex?`
  - `apps/web-next/src/stores/playerStore.ts` — 添加 `isMuted`/`volume`/`flipOrigin` 状态 + actions；`persistToSession`/`hydrateFromSession` 写/读扩展字段
  - `apps/web-next/src/app/[locale]/_lib/player/useMiniPlayerVideo.ts` — 初始 isMuted 从 store 读取；`handleToggleMute` 同步 `setIsMutedStore`；新增 `handleVideoVolumeChange` 同步 volume；activeSrc effect 应用持久化 mute/volume
  - `apps/web-next/src/app/[locale]/_lib/player/MiniPlayer.tsx` — P2-1 `handleReturnToWatch`/`handleToggleExpand` 改为 `useCallback`；P2-2 `handleClose` src 清理改为 `removeAttribute('src')+load()`；safe area 避让（读 `--header-height`/`--tabbar-height` CSS 变量，首次计算存 ref）；corner-aware resize handle 位置；FLIP origin 在路由前写 store；drag/resizeWatcher 传入 getMargins
  - `apps/web-next/src/app/[locale]/_lib/player/GlobalPlayerFullFrame.tsx` — `useLayoutEffect` 消费 `flipOrigin`，应用 FLIP 动画（translate+scale 展开），清除 origin 防复播
  - `tests/unit/web-next/MiniPlayer.test.tsx` — mock 补 `isMuted/volume/flipOrigin/setFlipOrigin`；新增 7 个 HANDOFF-34 专项测试（P2-2 行为/FLIP 顺序/resize handle 四角方向感知）
- **新增依赖**：无
- **数据库变更**：无
- **注意事项**：
  - `computeDockPosition` 改动向后兼容（number 参数路径不变）；mini-geometry.test.ts 31 个测试全部通过
  - StagingTable.test.tsx 偶发隔离失败与本任务无关（单独运行 13/13 PASS）
  - typecheck PASS / lint PASS / 1719 unit tests PASS

---

## [HANDOFF-35] MiniPlayer 唤出进度重置 + mini→full 静音 修复

- **完成时间**：2026-04-25 15:55
- **执行模型**：claude-opus-4-7
- **子代理**：无
- **修改文件**：
  - `apps/web-next/src/stores/playerStore.ts` — 新增 transient 字段 `miniResumeTime: number` + `setMiniResumeTime` action（不持久化）
  - `apps/web-next/src/app/[locale]/_lib/route-player-sync.tsx` — 新增 `watchCurrentTimeRef`，用 zustand `subscribe()` 在 watch 期间持续镜像 `store.currentTime`（离开后冻结）；切 mini 前同步 `setMiniResumeTime(watchCurrentTimeRef.current)` 显式快照
  - `apps/web-next/src/app/[locale]/_lib/player/useMiniPlayerVideo.ts` — activeSrc effect 优先读 `miniResumeTime`（fallback 到 `currentTime` 兼容 hydrate）；resumeTime>0 时同步 `setLocalCurrentTime` + 写回 `store.currentTime`，避免控件 00:00 闪烁；`handleVideoTimeUpdate` 在 `startTimeRef>0` 时仅更新 localCurrentTime 不写 store；抽取 `restoreSeekIfNeeded` 共用 seek 兜底（`loadedmetadata`/`loadeddata`/`canplay` 三事件点都调用，直到 video.currentTime 真正接近 saved 才清零 startTimeRef）；新增 `handleVideoLoadedData`
  - `apps/web-next/src/app/[locale]/_lib/player/MiniPlayer.tsx` — video 元素绑 `onLoadedData={handleVideoLoadedData}`
  - `apps/web-next/src/components/player/PlayerShell.tsx` — mini→full 不再 `setAutoplayOnResume(true)`、不再 `clearProgress`；保留 `setStartTime + setPlayerVersion` 让 video 暂停在 priorTime，由 ResumePrompt 承接 user-gesture（绕过 player-core useSourceLoader 静音 fallback）；移除 `autoplayOnResume` state、`autoplay` prop、`onPlay` 内 `setAutoplayOnResume(false)` 清零回调、`clearProgress` import
- **新增依赖**：无
- **数据库变更**：无
- **注意事项**：
  - 第一轮修复（仅 P0+P1+(c)）full→mini 仍未生效，原因是 store.currentTime 在 fetch sources 期间被未明的中间路径瞬时清零；第二轮加固用 `miniResumeTime` 显式快照绕过任何中间清零路径，用户实测确认有效
  - 静音 bug 根因为 `packages/player-core/useSourceLoader.ts:55-70` autoplay 失败 → 自动 `video.muted=true` 重试；mini→full 路径 router.push 破坏 user-gesture chain 触发 fallback。本任务约束为不动 player-core，采用 (c) 方案延后到共享 video 实例（用户已确认）
  - typecheck PASS / lint PASS / 1719 unit tests PASS

---

## [HANDOFF-36] MiniPlayer UI 交互微调（header 文案 + 展开改 hover + controls 75% 透明）

- **完成时间**：2026-04-25 16:18
- **执行模型**：claude-opus-4-7
- **子代理**：无
- **修改文件**：
  - `apps/web-next/src/app/[locale]/_lib/player/useMiniPlayerVideo.ts` — 新增 fetch `/videos/:shortId` effect 拉 `title + episodeCount`；返回类型扩展 `videoTitle: string \| null` + `videoEpisodeCount: number`
  - `apps/web-next/src/app/[locale]/_lib/player/MiniPlayerHeader.tsx` — 中部静态文案"正在播放"改为 `{videoTitle}` + 第 N 集（`episodeCount > 1` 时）；标题用 `flex:1 + min-width:0 + ellipsis` 截断，集数 `flex-shrink:0` 永远可见；移除 `isExpanded` / `onToggleExpand` props 与 toggle 按钮 DOM
  - `apps/web-next/src/app/[locale]/_lib/player/MiniPlayerControls.tsx` — 高度 44px → 32px（同 header），背景改为 `color-mix(in srgb, var(--player-mini-ctrl-bg) 75%, transparent)`；新增 `visible` prop（绑 `headerVisible`）切 opacity；play/mute 按钮 32×32 → 24×24，icon 16 → 14，时间字号 11px → 10px、minWidth 72 → 64；新增 `data-testid="mini-player-controls"`
  - `apps/web-next/src/app/[locale]/_lib/player/MiniPlayer.tsx` — 删除 `isExpanded` state、`handleToggleExpand`；容器高度永远 `videoH`，视频区 inset 永远 0；订阅 `currentEpisode`；解构 `videoTitle / videoEpisodeCount` 传给 header；MiniPlayerControls 改为永远渲染（基于 `visible={headerVisible}` 切 opacity）；header 注释更新为 hover 显示机制
  - `tests/unit/web-next/MiniPlayer.test.tsx` — mock 补 `videoTitle / videoEpisodeCount / handleVideoLoadedData / mockPlayerState.currentEpisode`；删除 3 个 toggle-expand 相关 spec，替换为 HANDOFF-36 新行为（控制栏永远在 DOM、初始 opacity=0 + mouseEnter→1、toggle 按钮已移除）；新增 4 个 header 文案 spec（无 shortId / title 加载中 / 多集显示集数 / 单集不显示集数）；删除其他 spec 中残留的 `fireEvent.click(toggle-expand)`
- **新增依赖**：无
- **数据库变更**：无
- **注意事项**：
  - 视频名称数据源：mini 自己 fetch `/videos/:shortId` 一次（与 fetch sources 同款 cancelled 守卫模式），shortId 变化时重 fetch；不污染 store / 不扩 sessionStorage 协议
  - audit P1-4 残留 4 项（`playbackRate / subtitles / quality / poster`）未在本任务范围，等 SEQ-202605XX-PLAYER-VIDEO-LIFT 一并处理
  - typecheck PASS / lint PASS / 1723 unit tests PASS

---

## [REVIEW-D] MiniPlayer 整治专项审核 — CONDITIONAL PASS

- **完成时间**：2026-04-25
- **执行模型**：claude-sonnet-4-6
- **子代理**：arch-reviewer（claude-sonnet-4-6）
- **覆盖序列**：HANDOFF-31 + HANDOFF-32 + HANDOFF-33 + HANDOFF-35 + HANDOFF-36
- **审核结论**：CONDITIONAL PASS（P1-INFO 已在本批次同步修复）
- **11 项审核全部 PASS**
- **修改文件**：
  - `apps/web-next/src/stores/playerStore.ts` — `releaseMiniPlayer` 补充清零 `miniAutoplay: false, miniResumeTime: 0`（防御性清零，LEGAL_TRANSITIONS 阻止 closed→mini，当前无触发路径，符合防御最佳实践）
  - `docs/handoff_mini_player/review_d_mini_player.md` — 新增，REVIEW-D 完整审核报告（11 项结论 + 问题清单 + 风险收敛情况）
- **P2 技术债（已录入 HANDOFF-34）**：
  - `MiniPlayer.tsx` 内联函数未 useCallback（约 207–287 行）
  - `MiniPlayer.tsx` 接近 500 行上限（提取 `MiniControlBar` 子组件后可降至 380 行以下）
  - `PlayerShell.tsx` `renderSelectionPanel` 内联函数可提取为子组件
- **新增依赖**：无
- **数据库变更**：无
- **注意事项**：
  - typecheck PASS / lint PASS / 1723 unit tests PASS
...
## [INFRA-07] scripts/dev.mjs 改造：pipe + 落盘 + rotate + errors 聚合

> **编号修订（2026-04-25 18:35）**：原编号 INFRA-01 与历史项目初始化系列（本 changelog 第 172 行 = 2026-03-15）撞号，违反 task-queue.md "禁止复用已存在编号" 规则，统一改 INFRA-07；commit 8756a11 message 仍写 `INFRA-01`，因历史 commit 不可改，按 git-rules 不动。

- **完成时间**：2026-04-25
- **记录时间**：2026-04-25 18:15（初版）/ 2026-04-25 18:35（审核修订）
- **执行模型**：claude-sonnet-4-6（实现）/ claude-opus-4-7（审核 + 修订）
- **子代理**：无
- **修改文件**：
  - `scripts/dev.mjs` — `stdio:'inherit'` → `['ignore','pipe','pipe']`；readline 行缓冲各子进程 stdout/stderr；按 service 分流到 `logs/dev/<service>.log`（人读）+ `<service>.ndjson`（机器）；errors 聚合双路径（JSON pino numeric/string level、非 JSON 保守前缀）；client 流前向兼容（service:'client' 分流）；rotate 10MB/5份；SIGINT flush + 关闭 fd；启动打印 logs 绝对路径；过期清理
  - `package.json` — 新增 `npm run logs`（tail 所有 .log）和 `npm run logs:errors`（tail errors ndjson）
  - `.gitignore` — 追加 `logs/` 排除日志落盘目录
- **新增依赖**：无（仅使用 Node.js 内置模块 fs/readline/child_process）
- **数据库变更**：无
- **注意事项**：
  - 守门 A 完整记录（修订后）：① 人工启动 `npm run dev` 60s+ 实时增长 ✅；② Ctrl+C 后无丢尾行 ✅（实测最后 3 行均为完整 JSON）；③ **rotate 端到端实测 ✅**：用 `LOG_MAX_BYTES=2048` 启 dev + 80×curl 触发，`logs/dev/api.{log,ndjson}` 各 rotate 6 次，结果 `.1..5` 共 5 份历史文件存在（每份 2170–2186 bytes，紧贴 2KB 阈值），`.6` 不存在 → `unlinkSync(oldest)` 逻辑（dev.mjs:53）已工作
  - errors 聚合规则：JSON 行 pino numeric level ≥ 40 或字符串 level ∈ {warn, error, fatal} 写入 errors；非 JSON 行仅 stderr + 保守前缀（Error: / Warning: / [ERR] / [WARN] / FATAL: / Uncaught）才写 errors
  - **errors 聚合三路径端到端验证延后**：本次实测期间未自然产生 warn/error 行（`grep '"level":(40|50|60)' → 0`），实现层 `isWarnOrAbove`(dev.mjs:142–147)、`isErrorLevel`(134–139)、保守前缀(35) 代码审查 PASS；端到端三路径触发验证待 INFRA-08 接入 API logger 后真实业务路径回归时一并完成
  - client 前向兼容：见到 `service:'client'` JSON 行分流到 `logs/client/<date>.ndjson`，端点在 INFRA-10 落地
  - **流程瑕疵备注（P1-3）**：本卡执行时未先在 `docs/tasks.md` 写工作台卡片即开始改代码，commit 8756a11 中 tasks.md 仅 `+1 空行`，违反 CLAUDE.md "未写任务卡就开始执行代码 ❌"；属流程违反不影响实现，已记录避免 INFRA-08 起再发；下一卡严格遵循 CLAUDE.md 任务入口规则
  - typecheck PASS / lint PASS / 1723 unit tests PASS

**真实日志格式样例（守门 D 基线，初版 INFRA-07 实测，脱敏 host/IP/port，已被 INFRA-13 字段补全替代）：**

```
// ① pino 透传行（fastify 自带，原始 schema，约 84% 占比）— 字段：level(numeric)+time+pid+hostname+reqId+req/res+msg
{"level":30,"time":1777167350577,"pid":22563,"hostname":"<host>","reqId":"req-3o","req":{"method":"GET","url":"/v1/videos/trending?type=anime&limit=8","hostname":"<host>","remoteAddress":"<ip>","remotePort":0},"msg":"incoming request"}
{"level":30,"time":1777167350580,"pid":22563,"hostname":"<host>","reqId":"req-3o","res":{"statusCode":200},"responseTime":2.4333330392837524,"msg":"request completed"}

// ② wrap 行（dev.mjs 对裸文本输出包装，约 16% 占比）— 字段：ts+service+stream+msg
{"ts":"2026-04-26T01:34:38.848Z","service":"design-tokens","stream":"stdout","msg":"[design-tokens] wrote /Users/livefree/projects/resovo/packages/design-tokens/src/css/tokens.css (399 lines)"}
{"ts":"2026-04-26T01:34:41.012Z","service":"admin","stream":"stdout","msg":"   ▲ Next.js 15.5.15"}
{"ts":"2026-04-26T01:34:41.021Z","service":"web-next","stream":"stdout","msg":"   ▲ Next.js 15.5.15"}
```

**INFRA-13 字段补全后的统一基线** —— 详见 INFRA-13 完成条目下的真实样例。INFRA-07 字段契约不齐是初版实现的临时态，已由 INFRA-13（commit 待定）补 enrich 解决；后续 INFRA-08/09 接入 API logger + 抽包后会进一步丰富 ctx/err 字段。

---

## [INFRA-13] INFRA-07 复审修补：dev.mjs JSON 字段补全 + shutdown drain + changelog 文案修正

- **完成时间**：2026-04-25
- **记录时间**：2026-04-25 18:58
- **执行模型**：claude-opus-4-7（主循环）
- **子代理**：无
- **触发**：Codex 复审（`docs/logging_system_proposal_20260425.md:495-528`）— INFRA-07 实现层 2 P1 + 1 P2
- **修改文件**：
  - `scripts/dev.mjs`
    - **Finding 1（P1）字段补全** — `handleLine()` JSON 分支改为 `enriched = { ts, service, stream, ...parsed, level }`：① `ts` 取 `parsed.ts ?? new Date(parsed.time).toISOString() ?? new Date().toISOString()`；② `service` `stream` 默认从 label/stream 派生，`...parsed` 后置覆盖让原值优先；③ `level` 单独后置，pino numeric 经 `pinoLevelStr()` 转字符串归一。改动同步影响 client 流分流与 errors 聚合（都改读 `enriched`）
    - **Finding 2（P1）shutdown drain** — 重构为：① `finalExit()` 内 `await closeAllStreams() + process.exit`；② 挂 `child.once('close')` 计数所有 children，pending===0 才 finalExit；③ 3s SIGKILL 兜底 + 5s finalExit 兜底防 close 事件丢失；④ children.size===0 直通 finalExit
    - **顺手清理** — 删 unused import `createReadStream`（Codex 残余风险清理项）
  - `docs/changelog.md` — INFRA-07 条目修订：① errors 聚合阈值 numeric ≥ 50 → ≥ 40 / error|fatal → warn|error|fatal；② NDJSON 代码块去 ndjson 标识改普通 ``` 块；③ 字段契约不齐说明改为"已由 INFRA-13 解决"
- **新增依赖**：无
- **数据库变更**：无
- **注意事项**：
  - 端到端实测（`LOG_MAX_BYTES=2048` + `npm run dev` + 80×curl + SIGINT）：① api.ndjson 7/7 行均含 `ts/level/service/stream` 四字段 ✅；② SIGINT 后最后 3 行均完整 JSON（`json.loads()` 全 PASS）✅；③ rotate 行为不变，api.ndjson.{1..5} 各 ~2300 bytes（含 enrich 字段后单行长度增加，仍紧贴 2KB 阈值），.6 不存在 ✅
  - level 归一：原 pino numeric `level:30` → 新 enriched `level:"info"`，原始 `time:1777...` 保留作为 audit field（不影响契约）
  - shutdown 5s 兜底是双保险：正常 close 事件 1–2s 内触发，3s SIGKILL 后强制 child 退出会再触发 close，5s finalExit 仅在所有 close 事件都未到达时启用
  - typecheck PASS / lint PASS（cached，dev.mjs 不入 lint）

**真实日志格式样例（INFRA-13 enrich 后基线，从 logs/dev/api.ndjson 实测抓取，脱敏 host/IP/port）：**

```
// ① pino 透传行 enrich 后（incoming request）— 多了 ts/service/stream/level 四字段，原 level numeric 转字符串
{"ts":"2026-04-26T01:57:37.352Z","service":"api","stream":"stdout","level":"info","time":1777168657352,"pid":30392,"hostname":"<host>","reqId":"req-4e","req":{"method":"GET","url":"/v1/videos/trending?type=anime&limit=8","hostname":"<host>","remoteAddress":"<ip>","remotePort":0},"msg":"incoming request"}

// ② pino 透传行 enrich 后（request completed）— 同样补齐契约字段
{"ts":"2026-04-26T01:57:37.356Z","service":"api","stream":"stdout","level":"info","time":1777168657356,"pid":30392,"hostname":"<host>","reqId":"req-4e","res":{"statusCode":200},"responseTime":4.719874978065491,"msg":"request completed"}

// ③ wrap 行（裸文本输出，已含 ts/service/stream/msg；无 level → enrich 补 'info' 默认）— 当前 wrap 路径走非 JSON 分支不经 enrich，字段保持初版
{"ts":"2026-04-26T01:34:38.848Z","service":"design-tokens","stream":"stdout","msg":"[design-tokens] wrote /Users/livefree/projects/resovo/packages/design-tokens/src/css/tokens.css (399 lines)"}
```

**字段契约达成度**：① pino JSON 行（占绝大多数）现已含 `ts/level/service/stream/pid/msg` 全部 6 个核心契约字段；② wrap 行（非 JSON 路径）仍只有 `ts/service/stream/msg` 4 字段（缺 level/pid），后续如需统一可在 wrap 分支也加 `level:'info'/pid:childPid` 默认；当前 wrap 行多为 Next.js 启动文本/CLI 输出，不属业务日志，保留最小字段更合理。

---

## INFRA-08（LOG-02）— API + worker 接入统一 logger

- **日期**：2026-04-25
- **序列**：SEQ-20260425-LOG-V1
- **执行模型**：claude-sonnet-4-6
- **子代理**：无（Spike 验证 Fastify v4 `logger: pinoInstance` 类型可行，但发现 Logger 泛型推断过于具体导致下游插件类型不兼容，按任务预定回退到 `logger: options` 形式）
- **偏离说明**：任务原计划 `loggerInstance: logger`（pino 实例），实际采用 `logger: createFastifyLoggerOptions()` 配置对象形式；workers 使用独立 `baseLogger = createLogger({ service:'api' })`。结构化/脱敏/access log 目标不受影响
- **修改文件**：
  - `apps/api/src/lib/logger.ts`（新建）— `createFastifyLoggerOptions()` / `createLogger()` / `withRequest()` / `withJob()` / `baseLogger` 单例；redact 18 条路径（11 PII 字段 + 一级嵌套）；`serializeReq`/`serializeErr` 禁止透传原始对象；`computeLevel()` 环境感知 level
  - `apps/api/src/server.ts` — 替换 `logger:{level}` 为 `logger: createFastifyLoggerOptions()`；新增 `genReqId: crypto.randomUUID()`；新增 `onResponse` hook 输出 access log（含 `request_id/method/url/status/duration_ms`）；3 处 `process.stderr.write` → `fastify.log.info`
  - `apps/api/src/lib/queue.ts` — `attachQueueLogger` 替换为结构化 logger（`baseLogger.child({ worker: 'queue:<name>' })`）；`error` → `qlog.error({ err })`；`failed` → `qlog.warn({ job_id, attempt, err })`
  - `apps/api/src/workers/crawlerWorker.ts` — 派生 `workerLog = baseLogger.child({ worker:'crawler-worker' })`；3 处 stderr → `withJob(workerLog, job).info/error/warn`
  - `apps/api/src/workers/crawlerScheduler.ts` — 派生 `schedulerLog`；8 处 stderr → `schedulerLog.info/warn({ count/mode/... })`
  - `apps/api/src/workers/enrichmentWorker.ts` — 派生 `workerLog`；3 处 stderr → `withJob` 结构化日志
  - `apps/api/src/workers/imageBackfillWorker.ts` — 派生 `workerLog`；3 处 stderr → `workerLog.info`
  - `apps/api/src/workers/imageBlurhashWorker.ts` — 派生 `workerLog`；3 处 stderr → `withJob` 结构化日志
  - `apps/api/src/workers/imageHealthWorker.ts` — 派生 `workerLog`；3 处 stderr → `withJob` 结构化日志
  - `apps/api/src/workers/maintenanceWorker.ts` — 派生 `workerLog`；4 处 stderr → `workerLog.info({ stage, ...metrics })`
  - `apps/api/src/workers/maintenanceScheduler.ts` — 派生 `schedulerLog`；8 处 stderr → `schedulerLog.info/warn`
  - `apps/api/src/workers/verifyWorker.ts` — 派生 `workerLog`；3 处 stderr → `withJob` 结构化日志
- **新增依赖**：无（pino 为 fastify transitive dep，已在 lockfile）
- **数据库变更**：无
- **注意事项**：
  - Fastify 4 类型限制：`logger: pinoInstance` 使 Fastify 泛型 `Logger` 推断为 `pino.Logger`（比 `FastifyBaseLogger` 更具体），导致 `register()` 下游类型不兼容。Spike 在 server.ts 中验证了问题范围，按任务预定降级方案实施
  - `url.query` redact 通过 `serializeReq` 的 `url.split('?')[0]` 实现（URL 字符串层面截断 query），而非 pino redact paths
  - server.ts `start().catch` 处保留 `process.stderr.write` 兜底（logger 初始化之前的崩溃路径，代码注释已说明）
  - **守门 B/D 真实化已转移到 INFRA-14**：本卡 commit fe97364 时附的"日志格式样例"为合成填位，实测验证及真实样例采集由 INFRA-14（commit 待定）补齐；本节对应代码块已 strikethrough，最终基线见 INFRA-14 完成条目下的真实样例
  - **流程瑕疵重犯（P1-3）**：commit fe97364 中 `docs/tasks.md` 仍为空（`+0 -0`），与 INFRA-07 P1-3 同病——连续第二次违反 CLAUDE.md "未写任务卡就开始执行代码 ❌"。INFRA-14 起强制 `tasks.md` 工作台卡片先于代码改动落盘（见 INFRA-14 卡片），此项作为复审硬规则纳入未来卡片自检

---

## INFRA-14（LOG-V1 修补）— INFRA-08 复审修补：脱敏表 / requestIdLogLabel / maintenance+backfill withJob / 守门 B+D 真实化

- **日期**：2026-04-25
- **序列**：SEQ-20260425-LOG-V1
- **执行模型**：claude-opus-4-7
- **子代理**：无
- **触发**：自审 P1-3（流程瑕疵重犯）+ Codex INFRA-08 复审 4 finding（`docs/logging_system_proposal_20260425.md:561-596`）+ 主循环整合草案（同文件 599+）
- **草案 commit**：`949aff9`
- **修改文件**：
  - `apps/api/src/lib/logger.ts` — `REDACT_PATHS` 补 `set-cookie` / `*.set-cookie` / `headers.set-cookie` / `url.query` / `*.url.query` / `req.url.query`（F4）
  - `apps/api/src/server.ts` — `Fastify({...})` 新增 `requestIdLogLabel: 'request_id'`（F5/F8）；onResponse hook 移除冗余手填的 `request_id`（pino 自动注入）
  - `apps/api/src/workers/maintenanceWorker.ts` — `processMaintenanceJob(data, jobLog: pino.Logger)` 新增 jobLog 参数；register 内 `withJob(workerLog, job)` 派生；4 处 `'job done'` 日志改用 `jobLog`（F6）
  - `apps/api/src/workers/imageBackfillWorker.ts` — `runImageBackfill(batchSize, jobLog: pino.Logger = workerLog)` 新增可选 jobLog；register 内 `withJob(workerLog, job)` 传入；`enqueueBackfillJob` 用 `bullJob.id` 写日志含 `job_id`（F7）
  - `docs/changelog.md` — INFRA-08 段守门 B/D 修订（合成样例转移到本卡 + P1-3 重犯硬记录）；追加本条
  - `docs/task-queue.md` — INFRA-14 状态 🔄 → ✅
  - `docs/tasks.md` — 工作台 INFRA-14 卡片清空
- **验证**：
  - `npm run typecheck` ✅ / `npm run lint` ✅ / `npm run test:run` ✅（148 文件 / 1723 测试）
  - 端到端实测（dev pid 13451，logs/dev/api.ndjson 274 行）：
    - 5 业务路径：login(422) / trending(200) / video sources(404) / `?password=SECRET`(422) / nonexistent(404)
    - maintenance verify-staging-sources 入队消费（`manual-1777183446976`）
    - backfill 入队消费（`backfill-1777183446992` → 237 imageHealth 子任务）
  - **F4 PII redact**：`grep 'SECRET' api.ndjson = 0` / `grep 'fake-token-should-be-redacted' = 0` / `*.url.query` 由 `serializeReq` split('?')[0] 切除（log 中 url 字段无 `?password=` 残留）
  - **F5/F8 request_id rename**：`grep '"reqId":' = 0` / `grep '"request_id":' = 22`（覆盖 7 access log 一致 + 内部多日志同 request_id 串联）
  - **F6 maintenance withJob**：`{"worker":"maintenance-worker","job_id":"manual-1777183446976","stage":"verify-staging-sources","msg":"job done"}` 实测出现
  - **F7 backfill withJob**：`{"worker":"backfill-worker","job_id":"backfill-1777183446992","health_check_enqueued":237,"msg":"backfill done"}` + 237 条 imageHealth `withJob` job done（`job_id":"health-check-<uuid>-poster"`）实测出现
- **真实日志样例（INFRA-14 守门 D 基线，从 logs/dev/api.ndjson 实测抓取）：**

```
// ① access log（onResponse hook，request_id 由 pino requestIdLogLabel 自动注入，url 已切 query）
{"ts":"2026-04-26T06:01:55.215Z","service":"api","stream":"stdout","level":"info","time":"2026-04-26T06:01:55.214Z","request_id":"8ca56a44-8626-44a9-b159-74f1542da794","method":"POST","url":"/v1/auth/login","status":422,"duration_ms":4,"msg":"access"}

// ② request 内部日志：incoming request 与 access 共享同一 request_id（链路串联证据）
{"ts":"2026-04-26T06:01:55.211Z","service":"api","stream":"stdout","level":"info","time":"2026-04-26T06:01:55.210Z","request_id":"8ca56a44-8626-44a9-b159-74f1542da794","req":{"request_id":"8ca56a44-8626-44a9-b159-74f1542da794","method":"POST","url":"/v1/auth/login"},"msg":"incoming request"}

// ③ maintenance worker withJob — job_id 含触发来源前缀
{"ts":"2026-04-26T06:04:07.012Z","service":"api","stream":"stdout","level":"info","time":"2026-04-26T06:04:07.012Z","worker":"maintenance-worker","job_id":"manual-1777183446976","stage":"verify-staging-sources","updated":0,"duration_ms":18,"msg":"job done"}

// ④ backfill worker withJob — Bull job_id 由 enqueueBackfillJob 显式传入
{"ts":"2026-04-26T06:04:07.051Z","service":"api","stream":"stdout","level":"info","time":"2026-04-26T06:04:07.051Z","worker":"backfill-worker","job_id":"backfill-1777183446992","health_check_enqueued":237,"blurhash_enqueued":0,"msg":"backfill done"}

// ⑤ image-health 子 worker withJob —— backfill 链路下游（F7 完整链路证据）
{"ts":"2026-04-26T06:04:07.579Z","service":"api","stream":"stdout","level":"info","time":"2026-04-26T06:04:07.578Z","worker":"image-health-worker","job_id":"health-check-48bb39e9-98ee-4c59-bbae-2c4c830eeb61-poster","kind":"poster","msg":"job done"}
```

- **延期范围**：
  - F9（services 25 处 stderr）：INFRA-08 范围收敛原则保留，待 INFRA-15 或里程碑 LOG-V2 处理
  - F10（11 PII 字段完整单元测试）：随 INFRA-09 抽包到 `@resovo/logger` 时一并补
- **流程纠正**：本卡 commit 949aff9（草案）已写 `docs/tasks.md` 工作台卡片，与 INFRA-07/08 P1-3 重犯彻底分离；后续所有任务严格遵守 CLAUDE.md "未写任务卡就开始执行代码 ❌" 硬红线

---

## [INFRA-09] 新建 `@resovo/logger` workspace 包（首期：类型 + 序列化器 + redact 表）

- **完成时间**：2026-04-25 23:45
- **执行模型**：claude-sonnet-4-6
- **子代理调用**：arch-reviewer（claude-opus-4-7）— 评审公共 API 契约，结论：NEEDS_REVISION（6 MUST + 3 SHOULD），已全部采纳（M1/formatPretty 基于任务规范"决策 6"保留，其余 M2-M6 + S1/S2 全部修复）
- **关联任务**：INFRA-08（前置）、INFRA-10（后续）、INFRA-12（实现下沉，监测期满后执行）

### 变更摘要

新建 `packages/logger` workspace 包，name: `@resovo/logger`，首期只导出类型/序列化器/redact 表/level 工具，**不导出 createLogger 实现**（决策 7）：

- `packages/logger/src/types.ts`：`ServiceName` / `LogLevel`（含 `LOG_LEVELS` 常量数组）/ `LogContext`（request_id / job_id / worker，移除 service 字段，M5）/ `LogBase`（service 绑定类型，M5）/ `LoggerOptions` / `Logger`
- `packages/logger/src/serializers.ts`：`serializeReq`（url 空值语义固化为 `''`，M4）/ `serializeErr`；两个 interface 均带 `[key: string]: unknown` 索引签名（兼容 Fastify 序列化器类型）
- `packages/logger/src/redact.ts`：完整 24 条 redact 路径，`as const satisfies readonly string[]`（M6）；`withRedactPaths(extra)` 扩展函数（M6）
- `packages/logger/src/levels.ts`：`computeLevel(env)` + `formatPretty(record)` ANSI 着色单行输出（决策 6，40 行实现）
- `packages/logger/src/index.ts`：仅 re-export 上述类型与函数，不导出 createLogger

### 配套改动

- 根 `package.json`：新增 `pino: ^9.0.0` 直接依赖（从 fastify 间接转直接，M2，实际安装版本 9.14.0）
- `apps/api/tsconfig.json` + 根 `tsconfig.json` + `apps/server/tsconfig.json` + `apps/web-next/tsconfig.json`：各加 `@resovo/logger` paths 映射（M3）
- `vitest.config.ts`：新增 `@resovo/logger` alias
- `apps/api/src/lib/logger.ts`：import 切换——`serializeReq / serializeErr / REDACT_PATHS / computeLevel / ServiceName / LoggerOptions / LogContext / LogBase` 全部改从 `@resovo/logger` 导入；`createLogger / withRequest / withJob / baseLogger` 实现保留本文件（决策 7）；`createLogger` 入参调整为宽松 `{ service: string; level?: string }` 避免循环类型

### 单元测试

新增 `tests/unit/lib/logger.test.ts`（26 个测试）：

- REDACT_PATHS 覆盖 11 个 PII 字段（顶层 + `*.field` 嵌套）✅
- withRedactPaths 合并 + 不变性测试
- serializeReq 五种 url 输入快照（含 undefined / 空 / query-only / 带 query / 无 query）
- serializeErr（plain Error / statusCode / 子类命名）
- computeLevel（test / development / production / undefined）
- LOG_LEVELS 内容验证
- formatPretty（INFO/WARN/ERROR 级别 / ANSI 输出 / ctx 字段序列化）

### 质量门禁

```
npm run typecheck   PASS（全 4 workspace 零报错）
npm run lint        PASS（web-next 仅现存 useEffect 警告，无新增）
...
## INFRA-10（LOG-04）— 浏览器 client logger + `/v1/internal/client-log` 端点

- **日期**：2026-04-26
- **序列**：SEQ-20260425-LOG-V1
- **执行模型**：claude-opus-4-7（用户选 B：在本 opus 会话直接执行 sonnet 建议任务）
- **子代理**：无（不属强制 Opus 6 类触发条件）
- **关联任务**：INFRA-09 ✅（前置）+ INFRA-15 ✅（前置）+ INFRA-11（后续 logging-rules 文档）
- **修改文件**（3 新建 + 5 修改 + 1 测试）：
  - `apps/web-next/src/lib/logger.client.ts`（新建，~190 行）— 同步 API（info/warn/error）+ buffer（max 50 / flush 2s / error 立即 flush）+ sendBeacon 优先 + fetch keepalive 回退 + 1 次重试 + 全局 hook（`window.error` / `unhandledrejection` / dev `console.error` monkey-patch / `pagehide`+`beforeunload` 强制 flush）+ SSR 安全（typeof window === 'undefined' no-op）
  - `apps/web-next/src/lib/logger.server.ts`（新建，~25 行）— web-next 服务端 createWebNextLogger，复用 @resovo/logger 的 serializers/redact/levels；预备入口（INFRA-10 范围内 web-next server 端无 console 调用待迁，本卡不强制消费）
  - `apps/api/src/routes/internal/client-log.ts`（新建，~110 行）— `internalClientLogRoutes`（命名对齐 `internalImageBrokenRoutes`）；`POST /v1/internal/client-log`；常量 IP_WINDOW=60s/IP_LIMIT=60/MAX_BATCH=100/MAX_BODY_BYTES=64KB；preHandler 按 NODE_ENV 切换（prod=`[fastify.authenticate]` / dev=`[]`）；`createLogger({ service: 'client' })` 独立 logger 写 stdout，由 dev.mjs 现有 `service:'client'` 分流（INFRA-07）落到 `logs/client/<date>.ndjson`
  - `apps/api/src/server.ts` — import + `await fastify.register(internalClientLogRoutes, { prefix: '/v1' })`，紧跟 internalImageBrokenRoutes
  - `apps/web-next/src/contexts/BrandProvider.tsx` — 2 处 `console.error` 替换为 `clientLogger.error`（`fetch failed`+`setBrand error`）；首个 useEffect 内追加 `installGlobalHooks()` 一次性安装幂等
  - `apps/web-next/src/stores/playerStore.ts` — 1 处 `console.warn` 替换为 `clientLogger.warn`（`illegal hostMode transition`），保留 `process.env.NODE_ENV !== 'production'` 包裹
  - `apps/web-next/src/components/media/SafeImageNext.tsx` — 1 处 `console.warn` 替换为 `clientLogger.warn`（`imageLoader prop ignored`）
  - `tests/unit/web-next/logger-client.test.ts`（新建，~155 行）— 4 个 jsdom 测试：error 立即 flush / info buffer 2s / sendBeacon→fetch fallback / **守门 C 三输入串行**（`window.error` + `unhandledrejection` + `console.error` + 幂等性）
- **新增依赖**：无（pino 已在 INFRA-09 提升为根直接依赖；fastify-cors 现有；zod 现有）
- **守门 5 分支实测（dev 启动后 curl）**：
  - **200 正常**：`POST /v1/internal/client-log` 含 1 entry → `{"accepted":1}`，HTTP 200
  - **400 zod level 非法**：`level:"trace"` → `{"error":{"code":"VALIDATION_ERROR","message":"Invalid enum value. Expected 'info' | 'warn' | 'error', received 'trace'","status":400}}`，HTTP 400
  - **400 单批 > 100 条**：`entries: [101 项]` → `{"error":{"code":"VALIDATION_ERROR","message":"Array must contain at most 100 element(s)","status":400}}`，HTTP 400
  - **429 IP 限流**：连发 65 次（含分支 1/2/3 累计 60 次）→ 第 61 次：`{"error":{"code":"RATE_LIMITED","message":"Too many requests","status":429}}`，HTTP 429
  - **403 origin / 401 prod 未登录**：见下方"行为差异说明"
- **行为差异说明**（403 / 401 不能直接 curl 端到端测试的原因）：
  - **403 origin 非法**：全局 fastify-cors plugin（`server.ts:70-81`）已在端点处理之前拦截非白名单 origin（dev: localhost / prod: NEXT_PUBLIC_APP_URL），返回 500 `Not allowed by CORS`。端点内 `isOriginAllowed` 是冗余防御层（CORS 配置变更时的回退保护），实际请求路径不可达 403。以 code review 验证：`apps/api/src/routes/internal/client-log.ts:46-53` 函数实现正确（dev 正则 + prod 严格相等），单测覆盖留 INFRA-11 文档化策略
  - **401 prod 未登录**：`requireAuth = process.env.NODE_ENV === 'production'` + `preHandler: [fastify.authenticate]`（与 `apps/api/src/routes/admin/image-health.ts:35` 同一 decorator）。本机 dev 环境无法直接验，以 code review 验证接线：`client-log.ts:71-72`；fastify.authenticate decorator 既有调用先例
- **守门 D 真实日志样例**（5 类 + 守门 C 三输入路径）：

```
// ① client log 200 路径 — service:'client' 分流到 logs/client/
{"ts":"2026-04-26T07:14:16.163Z","service":"client","stream":"stdout","level":"info","time":"2026-04-26T07:14:16.162Z","client_ts":"2026-04-26T00:00:00Z","ctx":{"a":1},"request_id":"9b19b62a-8e8a-4f7b-abe9-c6fcc8becd1e","source_ip":"127.0.0.1","msg":"INFRA-10 branch-200 test"}

// ② access log 200 — POST /v1/internal/client-log（端点本身的 access）
{"ts":"...","service":"api","level":"info","request_id":"...","method":"POST","url":"/v1/internal/client-log","status":200,"duration_ms":1,"msg":"access"}

// ③ 400 VALIDATION_ERROR access log
{"service":"api","request_id":"...","method":"POST","url":"/v1/internal/client-log","status":400,"duration_ms":0,"msg":"access"}

// ④ 429 RATE_LIMITED access log
{"ts":"2026-04-26T07:15:06.003Z","service":"api","stream":"stdout","level":"info","time":"2026-04-26T07:15:06.002Z","request_id":"81163202-4f3d-401a-b621-59dcd33fd76b","method":"POST","url":"/v1/internal/client-log","status":429,"duration_ms":0,"msg":"access"}

// ⑤ 守门 C 三输入（jsdom 单测验证，真实 Chrome 由开发者最后一英里手动测）：
//    ① 同步异常 → ctx.source = "window.onerror" + filename/lineno/colno/stack
//    ② Promise.reject 不 catch → ctx.source = "unhandledrejection" + stack
//    ③ console.error (dev only) → ctx.source = "console.error"
```
- **守门**：
  - `npm run typecheck` ✅（4 workspace 全过）
  - `npm run lint` ✅（仅既有 `react-hooks/exhaustive-deps` warning，无新增）
  - `npm run test:run` ✅ **150 文件 / 1759 测试**（INFRA-15 是 1755 + 本卡 4 测试）
  - 端到端 dev 实测：API 端点 4 分支响应 + service:'client' 分流到 `logs/client/client-2026-04-26.ndjson`（58 行落盘，含 200 业务 + 4 处 web-next 调用替换后产生的真实日志）
- **手动验证清单**（dev 启动后开发者可在 Chrome devtools 验证守门 C 三输入端到端真实链路）：
  ```
  1. npm run dev
  2. 打开 http://localhost:3000
  3. F12 打开 devtools Console，依次输入：
     ① throw new Error('INFRA-10 sync test')           // 期望：logs/client/ 出现 source:'window.onerror'
     ② Promise.reject('INFRA-10 unhandled test')        // 期望：source:'unhandledrejection'
     ③ console.error('INFRA-10 console test')           // 期望：source:'console.error'（dev only）
  4. 在 logs/client/client-<date>.ndjson 检查三种 source 各 ≥ 1 行
  ```
- **流程合规**：本卡严格延续 INFRA-14/15 范本——草案虽未单独 commit（INFRA-10 任务卡在 INFRA-08 决策时已写入 task-queue），但开工时**先**写 `docs/tasks.md` 工作台卡片（5 段：问题理解/根因/方案/涉及文件/验收清单 + 偏离风险）→ **再**改 `docs/task-queue.md` 状态 ⬜→🔄 → 然后才动代码 → 守门 → 实测 → 清空 tasks + ✅ + changelog + commit。本卡为流程范本第 3 次应用（INFRA-14/15 后），P1-3 流程瑕疵自 INFRA-14 起根除已稳定。
- **偏离记录**：
  - **403/401 由 code review 替代端到端 curl 验证**：原因如"行为差异说明"。如审核认为不足，可单独开 INFRA-16 修补卡补集成测（mock NODE_ENV 切 prod 切 cors）
  - **layout.tsx 未直接调 installGlobalHooks**：因 root layout 是 server component；改在 BrandProvider（既有 client component）首 useEffect 内调，幂等保护下 OK
  - **logger.server.ts 占位实现，本卡 web-next 服务端无消费方**：预备给 INFRA-12（实现下沉时）或未来 web-next server 端代码使用，本卡范围内不强制

### CONDITIONAL PASS 备注（INFRA-16 已闭环）

Codex 审核（`docs/logging_system_proposal_20260425.md:917-955`）+ 主循环独立审核（同文件 956+）发现 1 P1 真实 bug + 2 P2：

- **P1 [F1] 真实 bug**：`logger.client.ts:27` `ENDPOINT = '/v1/internal/client-log'` 是相对路径，浏览器从 :3000 发请求会打到 web-next 自身（404），**真实链路 0 工作**。本卡的 5 分支 curl 直连 :4000 实测**绕过了**浏览器→API 真实路径，是审计漏洞。现有 web-next 代码（`api-client.ts:3` 等）均用 `process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:4000/v1'` 拼绝对 URL。
- **P2 [F2] 契约**：5 分支验收要求非法 origin = 403，但全局 fastify-cors 抛 Error 触发 fastify 默认 500，端点内 `FORBIDDEN_ORIGIN` 不可达。
- **P2 [F3] PII**：`client-log.ts:107` 写 `source_ip: request.ip`，REDACT_PATHS 仅匹配 `ip` / `*.ip` 字段名，不匹配 `source_ip`。本卡实测 client.ndjson 58 行全部含明文 `127.0.0.1`，违反 INFRA-08/09 PII redact 基线。

三项均由 **INFRA-16（commit 待定）** 闭环：F1 用 `NEXT_PUBLIC_API_URL` 拼绝对 URL + fetch fallback `credentials:'include'` + 单测断言；F2 全局 `setErrorHandler` 把 CORS error 映射为 403 + `FORBIDDEN_ORIGIN`；F3 删除 `source_ip` 字段。INFRA-10 完成态从 CONDITIONAL PASS 升级为 PASS。

---

## INFRA-16（LOG-V1 修补）— INFRA-10 复审修补：浏览器→API 链路打通 + CORS 403 契约 + source_ip PII

- **日期**：2026-04-26
- **序列**：SEQ-20260425-LOG-V1
- **执行模型**：claude-opus-4-7（用户在 INFRA-10 时选 B 在本会话直接执行，本卡延续）
- **子代理**：无
- **触发**：Codex INFRA-10 审核 3 finding（`docs/logging_system_proposal_20260425.md:917-955`）+ 主循环独立审核（同文件 956+，确认 F1 是 P1 真实 bug——curl 直连 :4000 误判通过）
- **草案 commit**：`e8405c1`
- **修改文件**：
  - `apps/web-next/src/lib/logger.client.ts` — 顶部新增 `BASE_URL = process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:4000/v1'` + `ENDPOINT = ${BASE_URL}/internal/client-log` 拼绝对 URL；fetch fallback `credentials:'same-origin'` → `'include'`（F1）
  - `apps/api/src/server.ts` — 在 cors register 之后追加全局 `fastify.setErrorHandler`，把 `error.message === 'Not allowed by CORS'` 映射为 HTTP 403 + `code:'FORBIDDEN_ORIGIN'`；其他错误流委托默认 fastify 行为（F2）
  - `apps/api/src/routes/internal/client-log.ts:107-115` — 删除 `source_ip: request.ip` 参数；保留 `request_id` (UUID) 作请求关联（F3）
  - `tests/unit/web-next/logger-client.test.ts` — 新增 1 测试：`default ENDPOINT uses absolute URL with NEXT_PUBLIC_API_URL fallback` 断言默认 fetch URL 形如 `http://localhost:4000/v1/internal/client-log`（非相对路径）
  - `docs/changelog.md` — INFRA-10 段追加 CONDITIONAL PASS 备注 + 链接本卡；追加本条目
  - `docs/task-queue.md` — INFRA-16 状态 🔄 → ✅
  - `docs/tasks.md` — 工作台 INFRA-16 卡片清空
- **修复证据**：
  - **F1（BASE_URL 拼接）**：
    ```
    修复前：ENDPOINT = '/v1/internal/client-log'   ← 浏览器从 :3000 发请求会打到 web-next 自身 → 404
    修复后：ENDPOINT = `${BASE_URL}/internal/client-log`  ← 默认 BASE_URL = http://localhost:4000/v1
    
    模拟浏览器路径实测（curl with Origin: http://localhost:3000 → API :4000）：
      POST 3 entries → {"accepted":3} HTTP 200
      logs/client/client-2026-04-26.ndjson 出现 3 条带 ctx.source 区分（window.onerror / unhandledrejection / console.error）
    
    单测断言：fetchCalls[0].url === 'http://localhost:4000/v1/internal/client-log'（非相对路径）
    ```
  - **F2（CORS 403 映射）**：
    ```
    修复前：curl -H "Origin: http://evil.com" → HTTP 500 "Not allowed by CORS"
    修复后：curl -H "Origin: http://evil.com" → HTTP 403 + {"error":{"code":"FORBIDDEN_ORIGIN","message":"Origin not allowed","status":403}}
    
    一致化收益：image-broken 端点（同样依赖全局 cors）也从 500 → 403，全局 CORS 拒绝行为统一化。
    ```
  - **F3（source_ip 删除）**：
    ```
    grep -c '"source_ip"' logs/client/client-2026-04-26.ndjson  → 0
    grep -c '127.0.0.1' logs/client/client-2026-04-26.ndjson    → 0
    
    保留字段：ts / service / stream / level / time / client_ts / ctx / request_id / msg
    ```
- **真实日志样例（INFRA-16 三输入完整链路，从 logs/client/client-2026-04-26.ndjson 实测抓取）：**

```
// ① window.onerror 路径
{"ts":"2026-04-26T07:53:34.768Z","service":"client","stream":"stdout","level":"error","time":"2026-04-26T07:53:34.768Z","client_ts":"2026-04-26T00:50:00Z","ctx":{"source":"window.onerror","filename":"test.js","lineno":42},"request_id":"81ea60db-0567-4320-9219-0bc788936615","msg":"INFRA-16 sync test"}

// ② unhandledrejection 路径
{"ts":"2026-04-26T07:53:34.769Z","service":"client","stream":"stdout","level":"error","time":"2026-04-26T07:53:34.768Z","client_ts":"2026-04-26T00:50:01Z","ctx":{"source":"unhandledrejection","stack":"..."},"request_id":"81ea60db-0567-4320-9219-0bc788936615","msg":"INFRA-16 unhandled test"}

// ③ console.error 路径（dev only）
{"ts":"2026-04-26T07:53:34.769Z","service":"client","stream":"stdout","level":"error","time":"2026-04-26T07:53:34.768Z","client_ts":"2026-04-26T00:50:02Z","ctx":{"source":"console.error"},"request_id":"81ea60db-0567-4320-9219-0bc788936615","msg":"INFRA-16 console test"}

// ④ F2: CORS 403 真实响应
{"error":{"code":"FORBIDDEN_ORIGIN","message":"Origin not allowed","status":403}}
```
- **5 分支真实达成度**（INFRA-10 转承）：
  - **200 正常**：✅ INFRA-10 已实测 + 本卡 BASE_URL 修复后单测 + curl 模拟浏览器三输入路径
  - **400 zod 校验**：✅ INFRA-10 已实测（level 非法 / >100 条）
  - **401 prod 未登录**：⚠️ 仍由 code review 验证（`requireAuth = NODE_ENV === 'production'` + `preHandler:[fastify.authenticate]`），dev 环境不可直验
  - **403 origin 非法**：✅ **本卡新增可 curl 直测**（修复前 500 / 修复后 403 + FORBIDDEN_ORIGIN）
  - **429 IP 限流**：✅ INFRA-10 已实测（第 61 次累计触发）
- **无回归实测**：
  - `POST /v1/auth/login` 错密码 → HTTP **422**（zod 早期返回，不走 setErrorHandler）✅
  - `GET /v1/videos/trending` → HTTP **200** ✅
  - `GET /v1/nonexistent` → HTTP **404**（fastify 默认 not-found）✅
  - `POST /v1/internal/image-broken` with `Origin: http://evil.com` → HTTP **403**（一致化收益）✅
- **守门**：
  - `npm run typecheck` ✅（4 workspace 全过）
  - `npm run lint` ✅（仅既有 `react-hooks/exhaustive-deps` warning，无新增）
  - `npm run test:run` ✅ **150 文件 / 1760 测试**（INFRA-10 是 1759 + 本卡 +1 BASE_URL 默认值断言）
- **流程合规**：本卡严格延续 INFRA-14/15 范本（草案 commit `e8405c1` → 开工时先写 tasks.md 工作台 → 改 task-queue 状态 ⬜→🔄 → 改代码 → 守门 → curl 模拟浏览器实测 → 清空 + ✅ + changelog + commit）。流程范本第 4 次应用，P1-3 自 INFRA-14 起根除稳定。
- **审计教训**：本次 INFRA-10 完成时用 curl 直连 :4000 验证 5 分支，**绕过了**浏览器(:3000)→API(:4000)真实跨 origin 路径，导致 F1（相对路径 ENDPOINT 真实 bug）漏检。INFRA-16 起强制：涉及浏览器→API 链路的任务，验收必须包含 ① 真实浏览器实测（dev → :3000 → devtools）或 ② curl 携带 `Origin: http://localhost:3000` 头模拟跨 origin 路径，二者至少其一。本卡采用 ②（curl + Origin 头），与真实浏览器 sendBeacon/fetch 的 wire 行为等价（除 page-unload 时序由单测覆盖）。
- **延期 / 不在本卡**：
  - prod 401 集成测（NODE_ENV mock + fastify decorator mock）：维持 code review 验证，复杂度高于收益
  - source_ip hash/truncate 字段（如未来需 IP 关联）：独立 INFRA-17 评估
- **INFRA-10 完成态闭环**：F1 + F2 + F3 修复后，浏览器异常 → API 端点 → service:'client' → `logs/client/<date>.ndjson` 完整链路真实通；非法 origin 5 分支契约真实达成；source_ip PII 隐患消除。INFRA-10 自 CONDITIONAL PASS 升级为 PASS。

---

## INFRA-11（LOG-05）— 新增 `docs/rules/logging-rules.md` + 更新 CLAUDE.md 索引

- **日期**：2026-04-26
- **序列**：SEQ-20260425-LOG-V1
- **执行模型**：claude-opus-4-7（用户在 INFRA-10 时选 B 在本会话直接执行；本卡建议 haiku，用户明确选 A 直接由主循环 opus 执行——已有 INFRA-07..16 全部上下文，子代理冷启动需重新 grep 多文件）
- **子代理**：无（任务卡列为可选，主循环判断不 spawn）
- **关联任务**：INFRA-07/13 ✅、INFRA-08/14 ✅、INFRA-09/15 ✅、INFRA-10/16 ✅、INFRA-12 ⬜（隐式后续）
- **修改文件**（1 新建 + 1 修改 + 3 文档维护）：
  - `docs/rules/logging-rules.md`（新建，~280 行）— 9 节内容：① 最终决策表 1..8（用户最终编号 + 提案原文位置）② Level 选择指南（pino numeric / 项目实际用法 / errors 聚合阈值）③ PII 红线硬规则（11 字段 + 禁止原始对象入 ctx + 字段命名约束 + INFRA-16 教训）④ logger.child 派生范式（request/worker/job 三场景）⑤ ServiceName 命名规范（`worker:` 子格式 + 9 个实例）⑥ 守门 D 通用化（≥3 行真实样例 / 历史范例索引）⑦ 浏览器→API 链路硬规则（INFRA-16 起强制）⑧ 验收清单（INFRA-07..16 综合）⑨ 相关文件索引
  - `CLAUDE.md` 规范文件索引表追加 1 行：「日志相关任务 | docs/rules/logging-rules.md | logger / log / 日志 / pino / request_id / worker job / PII redact / client-log」
  - `docs/changelog.md`（追加本条目）
  - `docs/task-queue.md`（INFRA-11 状态 🔄 → ✅）
  - `docs/tasks.md`（工作台 INFRA-11 卡片清空）
- **8 决策最终编号映射**（规则文档 § 1）：
  ```
  决策 1: packages/logger 作为新 workspace 包（原第 9 节决策 1）
  决策 2: client-log 匿名 + IP 桶（原第 9 节决策 2）
  决策 3: prod 不写文件，仅 stdout（原第 9 节决策 3）
  决策 4: errors 聚合用 dev.mjs 编排层 B 路径（原第 9 节决策 4）
  决策 5: 不合并业务日志（原第 9 节决策 5）
  决策 6: 不引入 pino-pretty，自实现 formatPretty（复核新增决策 8 → 重编号 6）
  决策 7: 首期 packages/logger 仅导出类型/序列化器，不导出 createLogger（复核新增决策 9 → 重编号 7）
  决策 8: 新增 INFRA-12 隐式后续，无 deadline（复核新增决策 10 → 重编号 8）
  ```
- **CLAUDE.md 索引行 diff**：
  ```diff
  ...
  ```
- **守门**：
  - `npm run typecheck` ✅（4 workspace 全过，全 cached——无代码改动）
  - `npm run lint` ✅（FULL TURBO，3 cached）
  - `npm run test:run` ✅ **150 文件 / 1760 测试**（与 INFRA-16 相同，本卡无代码改动）
  - `npm run verify:docs-format`：新建的 `logging-rules.md` 通过元信息检查（7 字段完整：status / owner / scope / source_of_truth / supersedes / superseded_by / last_reviewed），不在 12 项错误列表内
- **verify:docs-format 历史遗留说明**（不在本卡范围）：脚本报 12 项 pre-existing 问题，包括 `task-queue.md` 时间字段格式（多处历史卡片）、`docs/archive/` 文档元信息缺失、`docs/rules/{git-rules,quality-gates,workflow-rules}.md` 缺 `superseded_by` 字段。这些与 INFRA-11 无关，按 CLAUDE.md "修改任务文件范围以外的文件" 红线不动。如需统一修复另开独立文档维护卡。
- **流程合规**：第 5 次延续 INFRA-14/15/16 范本（草案在 task-queue 中已有完整定义，无需独立 commit）→ 开工时**先**写 `docs/tasks.md` 工作台卡片（5 段：问题理解/根因/方案/涉及文件/验收清单 + 偏离风险）→ **再**改 `docs/task-queue.md` 状态 ⬜→🔄 → 然后才动文档 → 守门 → 清空 tasks + ✅ + changelog + commit。P1-3 流程瑕疵自 INFRA-14 起根除已稳定。
- **后续触发**：INFRA-11 完成后 task-queue 仅剩 INFRA-12（LOG-06）实现下沉。INFRA-12 触发条件：INFRA-10 完成 ≥ 2 周无回归后排程。当前 INFRA-10 完成于 2026-04-26，触发窗口 2026-05-10。

### CONDITIONAL PASS 备注（INFRA-17 已闭环）

Codex 审核（`docs/logging_system_proposal_20260425.md:1126-1156`）+ 主循环独立审核（同文件 1158+）发现 3 P2 文档措辞 finding（无 P1）：

- **P2 [F1]**：§ 5 ServiceName 命名规范把 `worker:crawler-worker` 等列为"实例"。但实际代码 `apps/api/src/workers/maintenanceWorker.ts:18` 等：`baseLogger.child({ worker: 'crawler-worker' })`，ndjson 顶层 `service` 永远是 `'api'`，`worker:${string}` ServiceName 类型从未实例化。文档反向写成"惯例"会误导未来 INFRA-12 或新 worker 任务破坏 dev.mjs 分流。
- **P2 [F2]**：§ 7 + § 8.4 把"真实浏览器实测"和"curl+Origin"列为**二选一**并称 wire 行为"等价"。但 curl 不能覆盖 sendBeacon CORS preflight / 浏览器 cookie 自动携带 / page lifecycle flush / Network 端口验证。这正是 INFRA-16 验收降级风险被反向写入规则。
- **P2 [F3]**：task-queue 验收项写"`verify:docs-format` 通过"，但脚本实际退出 1。完成态与真实守门结果不一致。

三项均由 **INFRA-17（commit 待定）** 闭环：F1 § 5 重组澄清（`worker:${string}` 改为"类型保留"，9 worker 表字段名改为 `worker`）；F2 § 7 + § 8.4 删除"等价二选一"，改为分级表 + curl 4 项局限；F3 task-queue 验收项措辞改为"新增文档通过 + 历史问题另立卡"。INFRA-11 完成态自 CONDITIONAL PASS 升级为 PASS。

---

## INFRA-17（LOG-V1 修补）— INFRA-11 复审修补：worker service 命名 / 浏览器验收口径 / docs-format 验收措辞

- **日期**：2026-04-26
- **序列**：SEQ-20260425-LOG-V1
- **执行模型**：claude-opus-4-7（用户在本会话连续指挥 INFRA-10 起所有任务直接由主循环执行，本卡延续）
- **子代理**：无
- **触发**：Codex INFRA-11 审核 3 finding（`docs/logging_system_proposal_20260425.md:1126-1156`）+ 主循环独立审核（同文件 1158+，确认 3 P2 全成立，无 P1）
- **草案 commit**：`377c4b8`
- **修改文件**（3 文档）：
  - `docs/rules/logging-rules.md` § 5（ServiceName 命名规范）— 重组为 4 子节澄清现状（F1）
  - `docs/rules/logging-rules.md` § 7（浏览器→API 链路）+ § 8.4（验收清单）— 删除"等价二选一"，增分级表 + curl 4 项局限（F2）
  - `docs/task-queue.md` INFRA-11 验收项措辞 — 改为"新增文档通过 + 历史问题另立卡"（F3）
  - `docs/changelog.md` — INFRA-11 段补 CONDITIONAL PASS 备注 + 链接本卡；追加本条目
  - `docs/task-queue.md` — INFRA-17 状态 🔄 → ✅
  - `docs/tasks.md` — 工作台 INFRA-17 卡片清空
- **修复证据**：

### F1 修订：§ 5 ServiceName 命名规范
...
修订后（澄清现状）：
```
> 重要：worker:${string} 是类型保留给未来独立 worker 进程，
> 当前 SEQ-20260425-LOG-V1 实施层未使用。

§ 5.1 当前 API 内 Bull workers 命名（实际惯例）：
- 顶层 service: 'api'
- 子上下文 worker: '<kebab-case-name>'
- ndjson 输出形如 {"service":"api","worker":"crawler-worker",...}
...
未来独立 worker 进程，INFRA-12 或后续任务另行决策。
```

**关键变化**：① 表格列名从 `实例` → `` `worker` 字段值 `` ② 表格内容从 `worker:crawler-worker` → `crawler-worker`（去掉 `worker:` 前缀）③ 新增 § 5.2 类型保留场景说明 ④ 增加重要警告框

### F2 修订：§ 7 浏览器→API 链路 + § 8.4

修订前（弱化）：
```
涉及浏览器→API 链路的任务，验收必须包含以下二者**至少其一**：
1. 真实浏览器实测：...
2. curl 携带 Origin 头模拟跨 origin 路径
...
| 涉及浏览器 cookie 自动携带 (credentials:'include') | ✅ 必须 | ❌ 不充分 |
| 仅服务端 CORS / 端点 origin 校验 | ☐ 可选 | ✅ 充分 |
| 仅业务逻辑（端点内部 zod / 限流 / 鉴权） | ☐ 可选 | ✅ 充分 |

curl + Origin 头不等价于真实浏览器，明确 4 项局限：
1. 不模拟 sendBeacon CORS preflight
2. 不携带 cookie（SameSite=Lax / Secure-only 在 prod）
3. 不触发 page lifecycle flush
4. 不覆盖 Network 面板真实端口验证

§ 7.1 验收路径选择规则：
- 涉及浏览器侧行为（sendBeacon / cookie / page lifecycle）：必须真实浏览器实测
- 仅服务端行为（CORS / origin / 业务逻辑）：curl+Origin 充分

§ 8.4 验收清单同步：
```
- [ ] 涉及 sendBeacon / page lifecycle / cookie 携带：必须真实浏览器实测（curl+Origin 不充分）
- [ ] 仅 CORS / 端点 origin / 业务逻辑：curl+Origin 充分作为 API 快速验证
- [ ] 禁止 curl localhost:4000 直连 API 端口（不带 Origin 头）替代浏览器路径
```

### F3 修订：task-queue.md INFRA-11 验收项
...
修订后：
```
- 新增 `docs/rules/logging-rules.md` 通过元信息检查（7 字段完整：status / owner / scope / source_of_truth / supersedes / superseded_by / last_reviewed），不在 verify:docs-format 12 项错误列表内
- 全局 `npm run verify:docs-format` 仍因 12 项 pre-existing 历史问题失败（与本卡无关：task-queue 时间字段格式 / archive 元信息 / 部分 rules 文档缺 `superseded_by` 字段），另立独立文档维护卡处理（INFRA-17 修订验收口径，与 changelog 历史遗留说明同步）
- `npm run typecheck` / `npm run lint` / `npm run test:run` 全绿
```

### ndjson 真实样例（§ 5.1 现状惯例验证）

从 `logs/dev/api.ndjson` 实测抓取（INFRA-16 测试期间产生）：

```
{"ts":"2026-04-26T07:53:14.462Z","service":"api","stream":"stdout","level":"info","time":"2026-04-26T07:53:14.461Z","worker":"maintenance-worker","concurrency":1,"msg":"registered"}
```

确认：
- 顶层 `service: 'api'` ✅
- 子上下文 `worker: 'maintenance-worker'`（不带 `worker:` 前缀）✅
- 与 § 5.1 修订后描述完全一致

### 守门
- `npm run typecheck` ✅（4 workspace 全过）
- `npm run lint` ✅（FULL TURBO，3 cached）
- `npm run test:run` ✅ **150 文件 / 1760 测试**（主循环本机 Node v20.19.6，与 INFRA-11/16 同，本卡纯文档无代码改动）
...
- `npm run verify:docs-format`：`logging-rules.md` 仍通过元信息检查，**不引入新错误**；12 项 pre-existing 历史问题维持（与 INFRA-11 时一致）

### 流程合规

第 6 次延续 INFRA-14/15/16 范本（草案 commit `377c4b8` → 开工时**先**写 `docs/tasks.md` 工作台卡片 → **再**改 `docs/task-queue.md` 状态 ⬜→🔄 → 然后才动文档 → 守门 → 清空 tasks + ✅ + changelog + commit）。P1-3 流程瑕疵自 INFRA-14 起根除已稳定。

### 偏离记录

- **verify:docs-format 12 项 pre-existing 历史问题**：本卡显式不修，措辞改为"另立独立文档维护卡处理"。如未来需统一修复，独立卡 `INFRA-MAINT-DOC-FORMAT` 或类似命名，不在 SEQ-20260425-LOG-V1 范围

### INFRA-11 完成态闭环

F1 + F2 + F3 修订后，`logging-rules.md` 不再有误导措辞、不再隐含验收降级风险；task-queue 验收项与 changelog 历史遗留说明措辞一致。INFRA-11 自 CONDITIONAL PASS 升级为 PASS。SEQ-20260425-LOG-V1 主线（INFRA-07/08/09/10/11/13/14/15/16/17）全部 ✅，仅剩 INFRA-12（LOG-06）实现下沉为隐式后续，触发窗口 2026-05-10。

---

## INFRA-18（LOG-V1 修补）— INFRA-17 残留修补：§ 7 编号去重 / 鉴权行 cookie 限定 / test:run flaky caveat

- **日期**：2026-04-26
- **序列**：SEQ-20260425-LOG-V1
- **执行模型**：claude-opus-4-7（用户在本会话连续指挥 INFRA-10 起所有任务直接由主循环执行，本卡延续）
- **子代理**：无
- **触发**：Codex INFRA-17 残留复审 3 finding（`docs/logging_system_proposal_20260425.md:1383-1418`）+ 主循环独立审核（同文件 1420+，确认 1 P2 + 2 P3 全成立）
- **草案 commit**：`c7bdbd2`
- **修改文件**（2 文档）：
  - `docs/rules/logging-rules.md:311-319` — 删除 9 行重复（旧 § 7.1 跨 origin 配置基线 + 旧 § 7.2 CORS 拒绝映射；INFRA-17 修订时复制粘贴新版本 § 7.2 / 7.3 但忘了删除原版本）（F1）
  - `docs/rules/logging-rules.md:281-286` § 7.0 分级表 — 第 4 行「业务逻辑（含鉴权）」拆为 3 行：「业务逻辑（zod / 限流 / IP 白名单）」+「header/token 鉴权（Authorization Bearer）」+「cookie/session 鉴权（SameSite / Secure / credentials:'include'）」（F2）
  - `docs/changelog.md` INFRA-17 段守门栏 — 追加 flaky caveat（VideoImageSection 既有 flaky 与本卡无关）（F3）
  - `docs/changelog.md` — 追加本条目
  - `docs/task-queue.md` — INFRA-18 状态 🔄 → ✅
  - `docs/tasks.md` — 工作台 INFRA-18 卡片清空

### F1 修复证据：grep 计数前后对比

修复前（重复）：
```
$ grep -c "^### 7\." docs/rules/logging-rules.md
6
$ grep -n "^### 7\." docs/rules/logging-rules.md
279:### 7.0 验收要求按改动类型分级
295:### 7.1 验收路径选择规则
301:### 7.2 跨 origin 配置基线        ← 新版
307:### 7.3 CORS 拒绝映射为 403       ← 新版
311:### 7.1 跨 origin 配置基线        ← 旧版（重复）
317:### 7.2 CORS 拒绝映射为 403       ← 旧版（重复）
```
...
279:### 7.0 验收要求按改动类型分级
297:### 7.1 验收路径选择规则
303:### 7.2 跨 origin 配置基线
309:### 7.3 CORS 拒绝映射为 403

$ grep -c "### 7\.1" docs/rules/logging-rules.md
1
$ grep -c "### 7\.2" docs/rules/logging-rules.md
1
```

### F2 修复证据：分级表完整 6 行

修订前（4 行，鉴权笼统归到 curl+Origin 充分）：
```
| 涉及 sendBeacon / page lifecycle flush | ✅ 必须 | ❌ 不充分 |
| 涉及浏览器 cookie 自动携带 | ✅ 必须 | ❌ 不充分 |
| 仅服务端 CORS / 端点 origin 校验 | ☐ 可选 | ✅ 充分 |
| 仅业务逻辑（端点内部 zod / 限流 / 鉴权） | ☐ 可选 | ✅ 充分 |
```

修订后（6 行，鉴权拆为 header/token vs cookie/session）：
```
| 涉及 sendBeacon / page lifecycle (pagehide/beforeunload) flush | ✅ 必须 | ❌ 不充分 |
...
**拆分理由**：header/token 鉴权（如 `Authorization: Bearer <jwt>`）curl 可显式 `-H "Authorization: ..."` 完整复现浏览器请求；cookie/session 鉴权浏览器侧自动携带受 SameSite/Secure/HttpOnly 策略影响，curl 必须显式 `--cookie` 且无法准确复现策略边界。

### F3 修复证据：INFRA-17 守门段 flaky caveat

```
- npm run test:run ✅ 150 文件 / 1760 测试（主循环本机 Node v20.19.6...）
  - flaky caveat（INFRA-18 后补充）：Codex 在 bundled Node 环境复跑出现
    1 个 flaky 失败 tests/unit/components/admin/videos/VideoImageSection.test.tsx
    破图降级测试；单独复跑该文件 PASS。指向既有 flaky 或测试隔离问题，
    与 INFRA-17 文档改动无关。建议未来在独立维护卡（非 SEQ-20260425-LOG-V1
    范围）跟进 flaky 根因
```
...
```
$ grep -c "^### 5\." docs/rules/logging-rules.md
3   # § 5.1 / § 5.2 / § 5.3 唯一

$ grep -c "^### 7\." docs/rules/logging-rules.md
4   # § 7.0 / § 7.1 / § 7.2 / § 7.3 唯一

$ grep -c "^### 8\." docs/rules/logging-rules.md
5   # § 8.1 / § 8.2 / § 8.3 / § 8.4 / § 8.5 唯一
```

**未来同类任务（文档段落删除/移动操作）必须 commit 前跑此自检**，避免 INFRA-17 → INFRA-18 类编号疏忽再发。

### 守门
- `npm run typecheck` ✅（4 workspace 全过）
- `npm run lint` ✅（FULL TURBO，3 cached）
- `npm run test:run` ✅ **150 文件 / 1760 测试**（主循环本机 Node v20.19.6，与 INFRA-11/16/17 同）
- `npm run verify:docs-format`：`logging-rules.md` 仍通过元信息检查，**不引入新错误**；12 项 pre-existing 历史问题维持

### 流程合规

第 7 次延续 INFRA-14/15/16/17 范本（草案 commit `c7bdbd2` → 开工时**先**写 `docs/tasks.md` 工作台卡片 → **再**改 `docs/task-queue.md` 状态 ⬜→🔄 → 然后才动文档 → **commit 前 grep 编号自检** → 守门 → 清空 tasks + ✅ + changelog + commit）。

### 偏离记录

- **VideoImageSection flaky 根因不在本卡**：仅 changelog 加 caveat 不掩盖跨环境差异。flaky 根因独立维护卡跟进（非 SEQ-20260425-LOG-V1 范围）
- **本卡为"修补的修补"（第 7 次范本应用）**：通过引入 grep 编号自检流程 + 改进文档段落操作纪律强化未来同类任务

### INFRA-17 完成态闭环

F1 + F2 + F3 修订后，`logging-rules.md` § 7 编号唯一无重复、§ 7.0 分级表正确区分 header/token vs cookie/session 鉴权、INFRA-17 changelog 守门段含跨环境 flaky caveat。INFRA-17 自 CONDITIONAL PASS 升级为 PASS。

### SEQ-20260425-LOG-V1 主线状态

```
1. INFRA-07 (LOG-03) dev.mjs 落盘                ✅
2. INFRA-08 (LOG-02) API/worker logger           ✅
3. INFRA-09 (LOG-01) packages/logger 抽包        ✅
4. INFRA-10 (LOG-04) 浏览器 client logger        ✅
5. INFRA-11 (LOG-05) logging-rules 文档          ✅ (CONDITIONAL → PASS via INFRA-17)
6. INFRA-12 (LOG-06) 实现下沉                    ⬜ 触发窗口 2026-05-10
7. INFRA-13/14/15/16/17/18  6 张修补卡          ✅✅✅✅✅✅
```
