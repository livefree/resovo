...
### 9.10 CODENAME-MATRIX-PICKER（CHG-SN-9-CODENAME-MATRIX / Wave 3 验收期补丁 / 2026-05-28）

用户验收期反馈 3 项：希望看到 52 山名字库预览 / codename 分配改为单元格内操作 / 字库重复使用解决方案 → ship 完整方案。

**真源**：
- 前端纯函数：`apps/server-next/src/lib/sources/codename-utils.ts`（parseCodename / joinCodename / buildCodenameMatrix / computeMatrixStats）
- Modal 组件：`apps/server-next/src/app/admin/source-line-aliases/_client/CodenameMatrixPicker.tsx`
- 触发点：`SourceLineAliasesClient.tsx` codename 列 cell 按钮

#### 9.10.1 单元格内联代号分配

codename 列每行渲染按钮（虚线边框 / "＋ 分配代号"或当前 codename）：
...
点击 → 打开 `CodenameMatrixPicker` modal（52 山名 grid + 状态色 + 后缀变种 chip）。

#### 9.10.2 山名状态机（3 态）

| 状态 | 触发条件 | 视觉 | 交互 |
|---|---|---|---|
| `available` | 字库内 / 未被任何活跃 alias 占用 / 不在 90 天冷却 | 绿底 success | 点击 → 立即 `onPick(base)` PUT codename |
| `occupied` | 字库内 / 已被某活跃 alias 占用 | 灰底 muted | 点击 → 弹"已占用 + 建议 base-N"浮层（suggested next）|
| `cooling` | 字库内 / 退役 < 90 天前 | 黄底 warning | `disabled=true` + title 显示"剩 X 天可复用" |

#### 9.10.3 后缀建议算法（重复使用解决方案）

`buildCodenameMatrix(rows).suggestedNext` 计算：
- base 占用时从后缀 1 递增找第一个**空缺**位置（防递增浪费）
- 例：`泰山 + 泰山-1 + 泰山-3` 已占用 → 建议 `泰山-2`（而非 `泰山-4`）

#### 9.10.4 用户交互流程

**Flow A: 分配新 available 名**
1. 点击行内 codename cell → matrix open
2. 点击 available 山名（如 `昆仑`）
3. PUT `/admin/source-line-aliases/{siteKey}/{sourceName}` `{codename: '昆仑', displayName, priority}`
4. modal close + 列表 reload + toast 成功

**Flow B: 占用名 → 接受建议**
1. 点击 codename cell → matrix open
2. 点击 occupied 山名（如 `泰山`）
3. 弹 `codename-suggest-modal`："泰山 已被 X 占用 / 是否使用 泰山-2？"
4. 点击 "使用 泰山-2" → PUT codename='泰山-2'

**Flow C: 冷却名禁用**
1. 点击 codename cell → matrix open
2. cooling 山名 button `disabled` + title 显示"剩 X 天可复用"
3. 强点不触发 PUT / modal 保持打开

### 9.11 e2e 测试（CHG-SN-9-WAVE3-FOLLOWUP-CODENAME-MATRIX-E2E / Wave 4 #6 / 2026-05-28）

补 Wave 3 验收期补丁 CODENAME-MATRIX 的 e2e 覆盖：

**真源**：`tests/e2e/admin/sources/codename-matrix-picker.spec.ts`

4 case（playwright headless / page.route mock）：

| # | case | 验证点 |
|---|---|---|
| 1 | page-load | 进入 `/admin/source-line-aliases` → 行渲染 + 点击 codename cell → matrix modal 打开 + 5 山名 testid 可见 |
| 2 | available-pick | 点击 `codename-slot-昆仑` → PUT 请求触发 + body 含 `codename:'昆仑'` + modal close |
| 3 | occupied-suggest | 点击 `codename-slot-泰山`（mock 数据已占用）→ `codename-suggest-modal` 弹 + accept → PUT body 含 `codename:/^泰山-\d+$/` |
| 4 | cooling-disabled | mock retired_at 30 天前的 `衡山` → button `disabled=true` + title 含"冷却"/"剩 X 天" + 强点不触发 PUT |

mock 数据范式：4 SourceLineRow（已占用泰山 / 已占用华山-1 / 未分配 youku/默认 / 退役 30 天前衡山）。endpoint mock：GET `/admin/source-line-aliases/all` + PUT upsert。
