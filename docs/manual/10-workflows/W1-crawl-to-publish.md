...
| 反例 | 现象 | 当前状态 |
|---|---|---|
| 「全站全量」是主按钮 + 一键触发 | 误触爆炸性损耗 | ❌ 需修复（CHG-SN-8-01）|
| 采集完成后没有跳转入口 | 用户要手动开 moderation | ❌ 需修复（CHG-SN-8-03）|
| 审核台右栏「类似」是占位 | 无法找重复视频 | ❌ 需修复（CHG-SN-8-04）|
| 审核台「探/播 待测」无测试入口 | 用户无法手动重测线路 | ❌ 需修复（CHG-SN-8-05）|
| 通过后还要去 staging 再点一次 | 多走一步 | ⚠️ 设计待定（CHG-SN-8-06）|
