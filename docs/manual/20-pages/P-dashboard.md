...
| RecentActivity 头部「示例数据」warn chip | 真端点失败 fallback mock（dashboardStats.activitiesDataSource='mock'） | **✅ 已实装** `GET /admin/dashboard/activities`（ADR-141 + CHG-SN-8-FUP-DASH-ACTIVITY-LIVE 2026-05-22）— admin_audit_log 派生最近 N 条 + 60s TTL Map 缓存 + 新索引 idx_admin_audit_log_created + 前端 i18n 37 项 audit-action-labels.ts；DashboardClient 拉真端点 fallback null → mock；端点正常时 chip 自动消失（GAPS.md #G-dashboard-activities-mock ✅）|
