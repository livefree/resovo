...
- **来源**：plan v1.3 §8.1 任务卡总览（SEQ-20260501-01 第 1 张 / M-SN-4 milestone）
- **真源**：`docs/designs/backend_design_v2.1/M-SN-4-moderation-console-plan.md` v1.3 §2 + §3.0 + §11.5
...
M-SN-4 plan v1.3 §2 共 9 张 migration（052 audit_log + 053 状态机 + 054–060 业务字段）是后续所有 M-SN-4 卡的硬阻塞前置。本卡完成 schema + types 同步 + architecture.md / ADR 文档同步 + 053 状态机回归测试集；不含任何路由 / Worker / 前端代码。v1.3 重排消除"audit 优先 deploy 不可强制"硬伤：052 = 序列首位，runner 字典序自动保证。

**2. 根因判断**

- **052 admin_audit_log**：M-SN-2 欠账（D-18），M-SN-4 所有写端点必须能写入审计；本期前置补建（实测仓内 grep `admin_audit_log` 0 命中）；v1.3 重排占用编号 052 = M-SN-4 序列首位
- **053 状态机扩展**：plan §1 D-01"approved → pending_review"两条新转换；现有 `enforce_videos_state_machine()` trigger 白名单需改写
- **054–059**：probe/render 双轨信号 + staff_note + review_labels + crawler_sites.user_label + source_health_events.source_id + 实测分辨率，全部为 plan §3 端点 + §5 操作流必需字段
- **060**：v1.0 §6 显示补全表已列 `review_source` 但 v1.0 无对应 migration；v1.1 §2.8 补齐为 Migration 059；v1.3 §2.9 重排占用编号 060 = M-SN-4 序列尾位
- **部署顺序**（plan v1.3 §2.10）：052 → 053 → 054 → … → 060，**runner 字典序自动保证**（`scripts/migrate.ts:50–52`）；053 状态机回归集 staging 验证后再 prod
...
- `tests/unit/db/migrations/053_state_machine_regression.test.ts`（新建子目录）：053 之前所有合法转换路径白名单回归集（确保 053 改 trigger 后旧路径仍通；v1.3 重排：状态机 migration 编号从原 052 → 053）

**4. 涉及文件（边界严格 — 修正版对齐仓内约定）**

可写：
- 新增 `apps/api/src/db/migrations/052_admin_audit_log.sql`（v1.3 重排，已落地）
- 新增 `apps/api/src/db/migrations/053_state_machine_add_staging_revert.sql`
- 新增 `apps/api/src/db/migrations/054_video_sources_signal_columns.sql`
- 新增 `apps/api/src/db/migrations/055_videos_moderation_fields.sql`
- 新增 `apps/api/src/db/migrations/056_review_labels.sql`
- 新增 `apps/api/src/db/migrations/057_crawler_sites_user_label.sql`
- 新增 `apps/api/src/db/migrations/058_source_health_events_line_detail.sql`
- 新增 `apps/api/src/db/migrations/059_video_sources_resolution_detection.sql`
- 新增 `apps/api/src/db/migrations/060_videos_review_source.sql`
- 新增 `packages/types/src/admin-moderation.types.ts`
- 新增 `tests/unit/db/migrations/053_state_machine_regression.test.ts`
...
- `npm run lint` — 必须通过
- `npm run test -- --run` — 含 053 状态机回归集，必须全绿
...
- [ ] 053 状态机回归测试集 100% 通过（含现有所有合法转换路径）
...
- [ ] commit trailer 含完整字段：`Plan-Source: ...v1.3` / `Main-Model: claude-opus-4-7` / `Sub-Agents: arch-reviewer (claude-opus-4-7)` / `Migration: 052,053,054,055,056,057,058,059,060`（v1.3 重排：052=audit_log）/ `Architecture-Sync: docs/architecture.md`
...
Task(subagent_type: "arch-reviewer", model: "claude-opus-4-7",
     prompt: "评审 M-SN-4 schema 卡（CHG-SN-4-03）：(1) 9 张 migration up/down 完整性与幂等性；
              (2) 053 状态机白名单改动是否破坏现有合法转换；
              (3) 052 admin_audit_log schema 与 §3.0.5 写入位点表对齐；
