...
- **计划开始时间**：2026-03-20 12:47
- **实际开始时间**：2026-03-20 12:48
- **完成时间**：2026-03-20 12:48
- **问题**：需要将新增的 Python 开发规范正式落盘，形成后续协作执行基线。
- **影响的已完成任务**：CHG-92
- **文件范围**：
  - `docs/python_tooling_adoption_plan.md`（新增）
- **修复内容**：
  - 新增 Python 渐进接入方案文档，明确：
    - 仅在 Python 任务触发时执行
    - 保持最小侵入，不影响现有 TS/Node 主流程
    - 默认工具链与命令规范（`uv + ruff + ty`）
    - 首次接入最小步骤、检查模板、风险边界与渐进治理路线
- **测试要求**：
  - 文档一致性检查（编号、时间戳、尾部追加）
- **完成备注**：
  - 已按记录规范追加到 `tasks/task-queue/changelog/run-logs`。
- **问题说明**：_（无）_

---

#### CHG-94 Python 规范改为条件触发式（首次引入业务 Python 代码才接入）

- **状态**：✅ 已完成
- **创建时间**：2026-03-20 12:55
- **计划开始时间**：2026-03-20 12:56
- **实际开始时间**：2026-03-20 12:56
- **完成时间**：2026-03-20 12:56
- **问题**：需要明确规范触发边界，避免仓库尚无 Python 业务代码时做无效“空接入”。
- **影响的已完成任务**：CHG-93
- **文件范围**：
  - `docs/python_tooling_adoption_plan.md`
- **修复内容**：
  - 新增“条件触发规则（强约束）”：仅当仓库首次引入 Python 业务代码时，才要求立即完成最小接入。
  - 明确触发前不做空接入；触发后需在当次任务完成 `pyproject + uv/ruff/ty` 可执行链路与检查。
  - 将“最小接入方案”章节改为“触发后立即执行”。
- **测试要求**：
  - 文档一致性检查（编号、时间戳、尾部追加）
- **完成备注**：
  - 已同步更新治理记录文件。
...
- **计划开始时间**：2026-03-20 15:41
- **实际开始时间**：2026-03-20 15:41
- **完成时间**：2026-03-20 15:44
- **问题**：后台切页时偶发出现“需要登录”，实际会话常仍有效；需先去前台登出再登录才能恢复。
- **影响的已完成任务**：CHG-37、CHG-44
- **文件范围**：
  - `src/lib/api-client.ts`
- **修复内容**：
  - 修复 refresh 响应解析不一致：兼容 `{ accessToken }` 与 `{ data: { accessToken } }` 两种结构，避免 token 刷新成功却被误判失败。
  - 新增统一 `handleUnauthorized`：401 且无法恢复时统一清理本地登录态。
  - 在 admin 路径下统一跳转登录页并带 `callbackUrl`，避免卡在“需要登录”错误状态。
  - 保持业务页面逻辑不变，仅修认证流与登录态同步逻辑。
- **测试要求**：
  - `npm run typecheck`
  - `npm run lint`
  - `npm run test:run`
- **完成备注**：
  - typecheck/lint/test:run 全部通过（37 files / 476 tests）。
  - 已完成文档闭环：`tasks/changelog/task-queue/run-logs` 尾部追加。
- **问题说明**：_（无）_
...
- **计划开始时间**：2026-03-20 15:49
- **实际开始时间**：2026-03-20 15:49
- **完成时间**：2026-03-20 15:52
- **问题**：采集入口分散、职责边界不清，缺少可执行的集中管理重构文档基线。
- **影响的已完成任务**：CHG-96
- **文件范围**：
  - `docs/admin_crawl_control_center_plan.md`（新增）
- **修复内容**：
  - 固化“自动采集配置唯一入口为采集控制台”的原则。
  - 输出页面重命名、职责重划分、run/task 统一模型、Phase A-D 计划与回滚策略。
  - 补充兼容期约束、生效范围说明、冲突策略口径。
- **测试要求**：
  - 文档一致性检查（编号、时间戳、尾部追加）
- **完成备注**：
  - 作为 CHG-96 后续实施任务的执行基线。
- **问题说明**：_（无）_

---

#### CHG-98 采集控制台命名迁移（含旧入口兼容）

- **状态**：✅ 已完成
- **创建时间**：2026-03-20 15:50
- **计划开始时间**：2026-03-20 15:52
- **实际开始时间**：2026-03-20 15:52
- **完成时间**：2026-03-20 15:58
- **问题**：页面与导航仍使用“视频源配置/源站与爬虫”旧命名，不符合新心智模型。
- **影响的已完成任务**：CHG-47、CHG-96、CHG-97
- **文件范围**：
  - `src/components/admin/AdminSidebar.tsx`
  - `src/app/[locale]/admin/crawler/page.tsx`
  - `src/components/admin/AdminCrawlerTabs.tsx`
- **修复内容**：
  - 导航与页面标题统一迁移为“采集控制台”。
  - Tab 主入口由“视频源配置”改为“采集控制台”。
  - 保留“原视频源配置”兼容语义提示，降低迁移期认知断层。
- **测试要求**：
  - `npm run typecheck`
  - `npm run lint`
- **完成备注**：
...
#### CHG-99 自动采集入口收口（站点配置/任务记录去编辑化）

- **状态**：✅ 已完成
- **创建时间**：2026-03-20 15:52
- **计划开始时间**：2026-03-20 15:58
- **实际开始时间**：2026-03-20 15:58
- **完成时间**：2026-03-20 16:03
- **问题**：自动采集开关仍分散在任务记录与站点配置页面，违反单点配置原则。
- **影响的已完成任务**：CHG-36、CHG-96、CHG-97、CHG-98
- **文件范围**：
  - `src/components/admin/AdminCrawlerPanel.tsx`
  - `src/components/admin/AdminCrawlerTabs.tsx`
  - `src/components/admin/system/site-settings/SiteSettings.tsx`
- **修复内容**：
  - 任务记录页移除自动采集开关编辑能力，仅保留任务列表与手动触发。
  - 站点配置页移除自动采集编辑区，改为只读迁移说明 + 跳转“采集控制台”。
  - 任务记录 Tab 增加“自动采集已迁移”提示与回到控制台入口。
- **测试要求**：
  - `npm run typecheck`
  - `npm run lint`
  - `npm run test:run`
...
- **计划开始时间**：2026-03-20 17:53
- **实际开始时间**：2026-03-20 17:53
- **完成时间**：2026-03-20 17:54
- **问题**：需要明确“暂停/恢复/中止”入口位置与操作方式，减少后台使用歧义。
- **影响的已完成任务**：CHG-109
- **文件范围**：
  - `README.md`
- **修复内容**：
  - 将后台入口描述更新为“采集控制台”。
  - 新增“触发与控制采集任务”说明（入口位置 + 暂停/恢复/中止按钮语义 + 单站/批量/全站触发）。
  - 新增 run 级调试 API 示例（trigger/pause/resume/cancel）。
- **测试要求**：
  - 文档改动，无代码测试要求
- **完成备注**：
  - 仅文档更新，不涉及业务逻辑变更。
