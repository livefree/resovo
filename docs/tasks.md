...
- **状态**：🔄 进行中（49-D-A ✅ 7fa00181 / 49-D-B ✅ 75d87e40 / 49-E-A ✅ 待提交 → 下一步 49-E-B）｜ **创建/开始**：2026-06-30 ｜ **执行模型**：claude-opus-4-8（主循环）｜**子代理**：arch-reviewer（claude-opus-4-8，已出 `<DropdownMenu>` + `<SlidingUnderline>` 双契约设计蓝图 + 12 条硬约束）
...
- **子卡（5）**：
  ...
  - **49-E-B**：Nav 桌面主导航 `NavLinkItem` 迁移（`ref={register(key)}`，删各项静态 span，容器挂 `<SlidingUnderline insetPx=14 offset=calc(-1*--header-underline-offset)>`）。**MoreMenu 下划线保持独立**（下拉 trigger 非导航 Link，不混入 registry，arch-reviewer 推荐）。+ e2e。
