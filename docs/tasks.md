...
- **状态**：🔄 进行中（49-D-A ✅ commit 待提交 → 下一步 49-D-B）｜ **创建/开始**：2026-06-30 ｜ **执行模型**：claude-opus-4-8（主循环）｜**子代理**：arch-reviewer（claude-opus-4-8，已出 `<DropdownMenu>` + `<SlidingUnderline>` 双契约设计蓝图 + 12 条硬约束）
...
- **子卡（5）**：
  ...
  - **49-D-B**：MoreMenu 改渲染 `<DropdownMenu>`（items 由 MORE_CATS.map，label 调用方 `t()` 算好传入；trigger render-prop 渲 "更多"按钮+chevron(消费 open)+active 下划线；保留 `nav-more-trigger/menu/${typeParam}` testid）+ **删除零调用 MegaMenu.tsx**（用户同意）+ e2e 回归。
