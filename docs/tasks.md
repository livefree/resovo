...
### 🔄 CHG-VIR-18-B（进行中）· merge 来源单一化 · 前端 + 测试

- **序列**：SEQ-20260612-04 / 创建 2026-06-12 23:55 / 实际开始 2026-06-12 23:55
- **建议模型**：opus（前端 legacy 触点清理 + GOV-2 警示保留 + 测试改写）
- **依赖**：CHG-VIR-18-A2（已完成）——后端 source 恒 identity + staleIdentityPending route 已透传。
- **问题理解**：`MergeCandidatesSection` 清 legacy 前端触点；source 列回显 `effectiveSource` 恒 `'identity'` → 整列零信息量 → 整列退役（D-105-20）；保留 GOV-2 stale 警示（读独立 staleIdentityPending）。
- **方案**：
  - ① source 列整列退役：删 source 列定义 + `effectiveSource` state + `CandidateSource` type + `SOURCE_CHIP_STYLE`。
  - ② 删降级提示条（`FALLBACK_NOTE_STYLE`「多证据候选为空，已降级实时聚合」）。
  - ③ 删 minScore 控件（legacy 专属，MergeCandidatesFilters 视情况）。
  - ④ 空态文案统一 identity 口径（删 `effectiveSource==='legacy'` 分支）。
  - ⑤ **保留** GOV-2 stale 警示条（staleIdentityPending 读取保留，route 已透传）。
  - ⑥ 测试：`MergeCandidatesSection.test.tsx` 2a/2a-fix/3 用例（source 列 + 降级回显）删除/重构；保留 stale 警示用例。
- **涉及文件**：`apps/server-next/src/app/admin/merge/_client/MergeCandidatesSection.tsx`（+ `MergeCandidatesFilters.tsx` 视 minScore 控件归属）/ `tests/unit/components/server-next/admin/merge/MergeCandidatesSection.test.tsx`。
- **门禁**：typecheck/lint/test:changed/verify:adr-contracts；D-105-20 闭环。
