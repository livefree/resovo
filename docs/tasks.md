...
_（**SEQ-20260629-02 动效对齐进行中**：HANDOFF-46 + AMEND ✅（motion token 基座，`globals.css` 镜像 `tokens.css` 10 duration + 5 easing）→ **卡片 hover 浮起 47-A + 47-B ✅ 全交付 2026-06-29**：47-A 镜像 `--shadow-card-hover`〔逐字 tokens.css line 74 rgb〕+ `.video-card-lift`/`.video-card-poster` 浮起规则〔`translateY(-6px) scale(1.03)` + shadow，门控 `(hover:hover)`/`prefers-reduced-motion`/`--motion-scale` + `z-index:1`〕+ VideoCard 两分支加类〔无 Props 变更〕+ `.scroll-row` padding-top；47-B 补内联横滚容器（Shelf/VideoGrid-scroll/TopTenRow）`paddingTop: var(--space-3)`，DailyAnimeRow 排除〔SafeImage 非 VideoCard〕；**47-C** 按用户反馈删除既有 `PosterHoverDim` hover 暗化遮罩（FloatingPlayButton 不依赖暗化、移除后仍可见）。全站 VideoCard（首页/分类/搜索/相关）grid + 全部横滚容器浮起对齐设计稿、不裁顶、hover 不变暗；门禁全绿 + 跨上下文 Playwright 回归（含 navigate→详情 / takeover→/watch 无回归），详见 changelog [HANDOFF-47-A/B/C + 47-A-FIX-20260629]。后续 HANDOFF-48（ADR-044·048 曲线收敛，需评审）/ HANDOFF-49（微交互审计）见 task-queue SEQ-20260629-02。取卡前先查 🚨 BLOCKER。）_


### 🔄 HANDOFF-49 — 微交互动效补全（D/E 共享 primitive 重构，SEQ-20260629-02）

- **状态**：🔄 进行中（49-D-A ✅ 7fa00181 / 49-D-B ✅ 75d87e40 / 49-E-A ✅ 9d7d003a / 49-E-B ✅ 待提交 → 下一步 49-E-C 收尾）｜ **创建/开始**：2026-06-30 ｜ **执行模型**：claude-opus-4-8（主循环）｜**子代理**：arch-reviewer（claude-opus-4-8，已出 `<DropdownMenu>` + `<SlidingUnderline>` 双契约设计蓝图 + 12 条硬约束）
- **依据**：HANDOFF-49 微交互审计 + Motion Spec §2 + arch-reviewer 蓝图。**关键转折**：原 Tier-1「快速 token 微调」经 arch-review 揭露为 throwaway——① MegaMenu 是僵尸代码（web-next 内零调用方）；② 进场动画须内联 transition 不复用 `menuFadeIn`；③ 时长须 `calc(var(--duration-X) * var(--motion-scale,1))`（既有惯例 globals.css:522/532 + 47-A-FIX）；④ A/B ⊂ 49-D、C-下划线 ⊂ 49-E-C、仅 C-图标-spring 净新增。interim 补丁已回退回干净基线，转按蓝图建正式 primitive（用户 2026-06-30 拍板「按蓝图实装全部」+「同意删除 MegaMenu」）。
- **硬约束（arch-reviewer 12 条，全程遵守）**：时长 `calc(var(--duration-X) * var(--motion-scale,1))` + `var(--easing-X)`，禁字面值 ／ 禁复用 menuFadeIn（留历史不改不删）／ 进场幅度 `translateY(-6px)` ／ 颜色零硬编码 ／ 禁 any（registry 用 `ReadonlyMap<string,HTMLElement|null>`）／ reduce 用 CSS `@media`（新增 `.dropdown-panel`/`.sliding-underline` 进 globals reduce 区，零首帧闪）／ SSR 安全（panel 仅 client 挂载；underline span SSR `opacity:0`+无 transform，几何 `useLayoutEffect` 注入）／ hover-intent 桥接区组件内建（`paddingTop:8px`+panel onMouseEnter cancelTimers）+ 专项测试 ／ 职责边界（不碰 i18n/路由/trigger 外观，回传 `open`）／ Props 独立 `types.ts` + commit 带 `Subagents: arch-reviewer` trailer ／ 逻辑属性优先（`inset-inline-start/end`、underline 用 `transform: translateX` 不用 `left`）／ 保留全部既有 data-testid。
- **子卡（5）**：
  - **✅ 49-D-A**（2026-06-30，changelog [HANDOFF-49-D-A-20260630]）：新建 `primitives/dropdown-menu/`（`DropdownMenu.tsx` + `types.ts`）——render-prop trigger 回传 `{open,toggle}` + 数据驱动 items（统一 role=menuitem + Link）+ hover-intent 双延时（默认 open120/close240，可配，仅 `(hover:hover)` 启用）+ 键盘 ArrowDown/Enter/Space 开并聚焦首项/Esc 关回焦 + a11y(aria-haspopup/expanded/controls + useId)+ 内联进场(opacity+translateY(-6px)，`calc(--duration-fast * motion-scale)` ease-out，rAF data-state)+ 几何桥接区内建。globals.css 加 `.dropdown-panel` reduce 类。+ 单测（hover-intent 桥接 / 键盘 / Esc 回焦 / items 渲染）。
  - **✅ 49-D-B**（2026-06-30，changelog [HANDOFF-49-D-B-20260630]）：MoreMenu 改渲染 `<DropdownMenu>`（items 由 MORE_CATS.map，label 调用方 `t()` 算好传入；trigger render-prop 渲 "更多"按钮+chevron(消费 open)+active 下划线；保留 `nav-more-trigger/menu/${typeParam}` testid）+ **删除零调用 MegaMenu.tsx**（用户同意）+ e2e 回归。
  - **✅ 49-E-A**（2026-06-30，changelog [HANDOFF-49-E-A-20260630]）：新建 `primitives/sliding-underline/`（`SlidingUnderline.tsx` + `useUnderlineRegistry.ts` + `types.ts`）——ref-registry 范式（项渲染权留调用方）+ `offsetLeft/offsetWidth` 测量（不受横滚影响）+ `useLayoutEffect`+`data-ready` 首次无 transition 定位 + 默认 `opacity:0` 防闪 + ResizeObserver 重测 + `transform: translateX`+width `calc(--duration-base * motion-scale)` ease-in-out。globals.css 加 `.sliding-underline` reduce 类。+ 单测（首帧/reduce/滑动）。
  - **✅ 49-E-B**（2026-06-30，changelog [HANDOFF-49-E-B-20260630]）：Nav 桌面主导航 `NavLinkItem` 迁移（`ref={register(key)}`，删各项静态 span，容器挂 `<SlidingUnderline insetPx=14 offset=calc(-1*--header-underline-offset)>`）。**MoreMenu 下划线保持独立**（下拉 trigger 非导航 Link，不混入 registry，arch-reviewer 推荐）。+ e2e。
  - **49-E-C**：MobileTabBar 迁移（`ref={register(key)}` + 删 opacity 显隐 span + 挂 `<SlidingUnderline insetPercent=25>`）+ **并入 C 图标 scale spring**（`.tabbar-icon` CSS 类：`transform calc(--duration-base*motion-scale) var(--easing-spring)` + active scale(1.12) + reduce 降级）+ 移动 e2e。
- **不做**：49-F scrubber（暂缓，player-core 跨包需 Opus）/ 49-G 长按环（暂缓，待产品定义目标动作）。demo「tabbar view 淡入淡出」属页面过渡域（ADR-044），N/A。
- **门禁（每子卡）**：typecheck=0 / lint=0 / test:changed / 单测 + 真实 dev server Playwright（D：NavMoreMenu hover 进场 -6px+ease-out 解析、键盘/桥接区不收起；E：两处下划线滑动、首屏不闪、reduce 即时定位、横滚不抖）。e2e 按 SEARCH/通用 + mobile project 选跑。**关键路径**：导航非播放器关键路径。
- **提交**：逐子卡 1 commit（D-A→D-B→E-A→E-B→E-C），共享组件 commit 带 `Subagents: arch-reviewer (claude-opus-4-8)` trailer。
...
_（**当前无 🔄 进行中卡片。SEQ-20260627-01 进行中**：META-55-ADR ✅ → DC-216-1 ✅〔92aa118f〕→ META-57 ✅〔4b443649〕→ META-58-A ✅〔b8443a09〕→ META-58-B 契约裁定 ✅〔c388d25e〕→ **META-58-B-1 ✅**（后端 douban 4 态过滤谓词 + 多值迁移；过滤侧迁 refs+meta_quality 真源；Codex 对抗审 2 处 BLOCK 修正：pending 兜底穷尽 `IS DISTINCT FROM` + 投影迁移整体延 META-60；门禁 typecheck=0/lint=0/test:changed 1202/verify=0）。**META-58-B-2 ✅**（distinct 死配置移除 + ADR-150 AMENDMENT 4）→ **DC-216-2 ✅**（refs 读路径软删处置表穷举归类，13 文件三类裁定）→ **META-59 ✅**（类C 软删 gate：external-resources-stats 3 COUNT + home-autofill douban/bangumi gap；derive 类A→类B 修正，arch-reviewer 改判）→ **META-56 ✅**（DROP-prep 对齐脚本 `align-douban-status-to-refs`，applied ref 但列欠计→matched，幂等 dry-run）→ **SEQ 编码工作完成（8 代码卡全交付）**；剩余 **META-60 deploy-blocked**（待 META-56 对齐脚本 deploy 实跑 + 软泡期 ≥1 enrich 周期）→ META-63-DROP-ADR（Opus）/ META-61（bangumi 退役，前置补信号）/ META-62（覆盖索引，性能触发）均 deploy/soak/prerequisite-gated，详见 task-queue「deploy 收尾序列」。**B-1 延后项（D-216-13）已并入 META-60**：投影迁移（moderation:344 / VIDEO_FULL_SELECT / videos.status:207）+ derive douban 列兜底清理（cache-only 误升 applied，须列停写后做）。**bangumi 退役另起 META-61**（无 `meta_quality.bangumi_match_status` 信号）。取卡前先查 🚨 BLOCKER。）_
