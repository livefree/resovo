...
### 🔄 CHG-VIR-18-A2（进行中）· merge 来源单一化 · 后端实施

- **序列**：SEQ-20260612-04 / 创建 2026-06-12 23:30 / 实际开始 2026-06-12 23:30
- **建议模型**：opus（删降级链路 + GOV-2 解耦，跨 service/schemas/queries/types/test，回归风险高）
- **依赖**：CHG-VIR-18-A1 ADR PASS（已提交，见 changelog）。
- **问题理解**：按 ADR-105 AMENDMENT 2026-06-12（D-105-17~22）删 `source=legacy` 检索路径，identity 成唯一来源。
- **方案**：
  - ① `VideoMergesService.listCandidates`：删 legacy 分支（`fetchRawCandidateGroups`/`computeOverlapScore` 段）；`listIdentityCandidates` 返回 null（真空表）时直接返回 identity 空 envelope `{ data:[], total:0, source:'identity', staleIdentityPending? }` + 独立查 `hasStaleVersionPending`（GOV-2 解耦，D-105-21）。
  - ② route `video-merges.ts`：envelope 透传 `staleIdentityPending`（D-105-22，修 GOV-2 既有缺口）。
  - ③ schemas：`source: z.enum(['identity']).default('identity')`（D-105-18，传 legacy → 422）。
  - ④ 删 legacy 专属：`fetchRawCandidateGroups`/`countRawCandidateGroups`（video-merge-candidates.ts）/ `computeOverlapScore`（schemas）。
  - ⑤ types：`source`/`truncated` 注释口径更新。
  - ⑥ 测试：`identity-source-switch.test.ts` 整组重构 + `video-merge-candidates.test.ts` GOV-2 用例改写 + route 单测新增（staleIdentityPending 透传）。
- **保留（误删即回归）**：`score` 字段 / `legacy_score` 列 / `minScore` zod / identityScore·videoCount·q 筛选 / `truncated` / `pickRecommendedTarget`·`mapVideoRow`·`groupMatchesFilters`·`buildGroupFromCluster`·`scoreGroup`·`fetchVideoDetailsForCandidates`。
- **涉及文件**：`apps/api/src/services/VideoMergesService.ts` / `VideoMergesService.schemas.ts` / `routes/admin/video-merges.ts` / `db/queries/video-merge-candidates.ts` / `packages/types/src/video-merge.types.ts` / `tests/unit/api/{identity-source-switch,video-merge-candidates}.test.ts`。
- **门禁**：typecheck/lint/test:changed/verify:adr-contracts；D-105-17~22 changelog 闭环。
