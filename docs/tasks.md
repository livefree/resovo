...
- **状态**：🔄 进行中（49-D-A ✅ 7fa00181 / 49-D-B ✅ 待提交 → 下一步 49-E-A）｜ **创建/开始**：2026-06-30 ｜ **执行模型**：claude-opus-4-8（主循环）｜**子代理**：arch-reviewer（claude-opus-4-8，已出 `<DropdownMenu>` + `<SlidingUnderline>` 双契约设计蓝图 + 12 条硬约束）
...
- **子卡（5）**：
  ...
  - **49-E-A**：新建 `primitives/sliding-underline/`（`SlidingUnderline.tsx` + `useUnderlineRegistry.ts` + `types.ts`）——ref-registry 范式（项渲染权留调用方）+ `offsetLeft/offsetWidth` 测量（不受横滚影响）+ `useLayoutEffect`+`data-ready` 首次无 transition 定位 + 默认 `opacity:0` 防闪 + ResizeObserver 重测 + `transform: translateX`+width `calc(--duration-base * motion-scale)` ease-in-out。globals.css 加 `.sliding-underline` reduce 类。+ 单测（首帧/reduce/滑动）。
