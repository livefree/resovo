...
| updated_at | timestamptz | 触发器维护 |
  ...
  - 端点（ADR-182；已落地 #1/#2/#3）：`GET /admin/home/preview`（整页预览聚合，跳缓存；Phase 1 正式配置预览无草稿叠加）+ `GET /admin/home/sections` + `PATCH /admin/home/sections/:section/settings`（`apps/api/src/routes/admin/home.ts` → `HomeCurationService`）
