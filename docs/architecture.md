...
- `(retired_at) WHERE retired_at IS NOT NULL`（Migration 079）— 加速「已退役行查询」路径：① SourceLineAliasService 90 天冷却期判定（`retired_at < NOW() - INTERVAL '90 days'`，D-164-11）② admin UI"已退役" tab 视图筛选（CHG-368-B-B）。listSources 主路径谓词 `sla.retired_at IS NULL` 由 `idx_source_line_aliases_codename_active` 部分唯一索引 WHERE 条件覆盖（含 `retired_at IS NULL`），不依赖本索引
