...
**source_line_aliases（Migration 063 / 079）：**
...
- `UNIQUE (codename) WHERE codename IS NOT NULL AND retired_at IS NULL`（Migration 079 / D-164-9）— 索引覆盖在役行 codename 列唯一性 + 排序结构。候选路径：① PUT upsert 冲突检测（在役 codename 全局唯一 / DB 强制）② listSources JOIN 主路径 `retired_at IS NULL` 谓词（部分索引 WHERE 子句含同条件）。不适用：cooling lookup（`retired_at IS NOT NULL` 反向条件 / 部分索引不可用）/ 已退役行查询。
- `(retired_at) WHERE retired_at IS NOT NULL`（Migration 079）— 索引覆盖已退役行 retired_at 列排序结构。候选路径（规划器选用取决于数据 selectivity / 留实测 EXPLAIN ANALYZE 验证）：① admin UI "已退役" tab `WHERE retired_at IS NOT NULL ORDER BY retired_at DESC`（CHG-368-B-B）② GET codename-pool cooling 段 `WHERE retired_at >= NOW() - 90 days` 范围扫描（CHG-368-B-A2 / 按 retired_at 范围 / 不按 codename 单值）。不适用：listSources 主路径 `retired_at IS NULL`（反向 / `idx_codename_active` 覆盖）/ 按 codename 查询 cooling（`WHERE codename = $1 AND retired_at IS NOT NULL` / **当前 schema 无适用索引** / 全表扫描或经 idx_retired_at 范围扫后应用层过滤 codename / 未来如成为热路径可独立加 `(codename) WHERE retired_at IS NOT NULL` 部分索引）
...
**未 ship 业务路径（→ CHG-368-B-A2/-A3 排期）**：3 新 admin 写端点（POST `/admin/source-line-aliases/:siteKey/:sourceName/retire` + PUT `/.../priority` + GET `/.../codename-pool`）+ SourcesMatrixService 3 方法 + R-MID-1 audit RETRO 7 文件框架（D-164-7 / 2 新 actionType `source_line_alias.retire` + `.priority_update`）+ route-scoring priority 通道激活（priority/100 替代 Phase 1 默认 0）+ listSources JOIN 加 `sla.retired_at IS NULL` 谓词。

**未 ship UI（→ CHG-368-B-B/-C 排期）**：admin UI 独立路径 `/admin/source-line-aliases`（DataTable 一体化）+ LinesPanel 行级 codename 标签 + 退役行 opacity + docs/manual/route-labeling.md §"Layer B 实施记录"。
