...
- `video_external_refs`：记录内部视频与外部条目的匹配关系（Migration 041，META-03）
  ...
  - 查询函数：`upsertVideoExternalRef()` / `findPrimaryVideoExternalRef()`（`src/api/db/queries/externalData.ts`）
...
- `video_metadata_provenance`：追踪 `media_catalog` 每个字段最后一次写入来源（Migration 043）
  ...
  - 查询函数：`batchUpsertFieldProvenance()` / `getProvenanceByCatalogId()`（`src/api/db/queries/metadataProvenance.ts`）
