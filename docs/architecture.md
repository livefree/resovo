...
- `UNIQUE (codename) WHERE codename IS NOT NULL AND retired_at IS NULL`（Migration 079 / D-164-9）— 活跃 codename 全局唯一，退役后可复用
- `(retired_at) WHERE retired_at IS NOT NULL`（Migration 079）— 索引覆盖已退役行 retired_at 列的排序结构。候选未来查询路径（规划器选用取决于实际数据 selectivity / 留 CHG-368-B-B 上线后 EXPLAIN ANALYZE 验证）：① admin UI "已退役" tab `WHERE retired_at IS NOT NULL ORDER BY retired_at DESC`（CHG-368-B-B）② GET codename-pool cooling 段 `WHERE retired_at >= NOW() - INTERVAL '90 days'` 范围扫描（CHG-368-B-A2）。不适用：listSources 主路径 `retired_at IS NULL`（反向条件 / 由 `idx_source_line_aliases_codename_active` 覆盖）/ `isCodenameInCooling(codename)` 按 codename 查询（codename 索引覆盖）
