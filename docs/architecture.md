...
**`catalog_external_refs`（ADR-177 规划草案 / 未落 migration / Phase 5 CHG-VIR-12 落地）— catalog 级 canonical 外部身份映射真源**

> ⚠️ **规划中，非现状基线**：当前生产无此表，catalog 外部身份仍由 `media_catalog` 四列（`imdb_id`/`tmdb_id`/`douban_id`/`bangumi_subject_id`）承载。真源详见 `docs/decisions.md` ADR-177 + 关系定档 `docs/designs/adr177-external-refs-relation_20260602.md`；落地时迁出本标注。

- **字段**：`catalog_id FK→media_catalog ON DELETE CASCADE` / `provider`(imdb/tmdb/douban/bangumi) / `external_id TEXT`（统一文本，tmdb/bangumi 数值转存）/ `external_kind`(show/season/movie/subject) / `relation`(exact/parent/candidate/rejected) / `season_number INT`(NULL/CHECK>0，复用 ADR-176 哨兵口径) / `confidence` / `source`(auto/manual) / `is_primary` / 审计列。
...
### 5.15 视频身份解析层（Entity Resolution / ADR-105a / Phase 1a·2a·2b·2c 写路径已落地）

> **现状基线（部分落地）**：SEQ-20260602-03 视频身份解析升级（设计 `docs/designs/video-identity-resolution-redesign_20260602.md` §4.2/§4.3）。**已落地**：`core_title_key`/facets 纯函数（Phase 1a CHG-VIR-5）+ 多证据评分 `apps/api/src/services/identity/` 模块（Phase 2a CHG-VIR-7）+ **`identity_candidate` 表（Phase 2b CHG-VIR-8 / Migration 086）** + 离线重算 Bull job + 候选来源读切换（Phase 2c CHG-VIR-9-A / `?source=identity|legacy`）+ **`identity_decisions` 人工裁定写路径（Phase 2c CHG-VIR-9-B / Migration 087+088 / ADR-178）**。**留后续**：UI 切换 + confirm/reject 操作（CHG-VIR-9-C）+ 自动绑定开关（默认 OFF）。真源详见 `docs/decisions.md` ADR-105a + ADR-178。
...
- **决策写路径**（Phase 2c CHG-VIR-9-B 落地 / ADR-178）：① confirmed→merge——`POST /admin/video-merges` Body optional `candidateId`，merge 事务内（单 BEGIN/COMMIT / R8）挂 candidate `status='confirmed'`（from-state 守卫）+ decision(confirmed, audit_id)；② reject——`POST /admin/identity-candidates/:id/reject`（`IdentityCandidatesService.reject` 单事务 candidate rejected + decision(rejected)，复活归离线 job R6 链）；③ unmerge 联动——经 `video_merge_audit_id` 反查 decision 原地置 `reverted_at`（candidate 保持 confirmed，避撞 `uq_identity_candidate_pending`）。admin_audit_log：actionType `identity_candidate.reject` + targetKind `identity_candidate`（**Migration 088** CHECK 14→15）。
