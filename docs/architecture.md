...
> **触发器 `trg_media_catalog_clear_image_signals`（ADR-213 D-213-10 / Migration 122）**：BEFORE UPDATE，任一 `<kind>_url` 变更即 NULL 掉该 kind 的 `<kind>_client_error_at` + `<kind>_checked_at`（信号/checked_at 是裸时间戳、不绑 URL → URL 替换后旧信号失效须清，否则已替换的新图被误判 client_error/fresh-ok）。路径无关（覆盖 apply-candidate/手填/crawler/enrichment 所有 URL 写入）；与 `trg_media_catalog_updated_at` 独立共存。
