...
- `episode_count`：`INT NOT NULL DEFAULT 1`（Migration 001 / 爬虫推算）— **三层集数语义的第 1 层**："已收录最大集数"（单向递增高水位 / 近似 *activeEpisodes* 语义 / 详 ADR-163 §3 D-163-2 命名保留依据 + §11 advisory）。**写入路径**（口径统一：活跃非投稿源 `MAX(COALESCE(episode_number,1))`，`GREATEST` 只增不减）：① 爬虫 `videos.crawler.ts:248 bumpEpisodeCountIfHigher`；② 合并/拆分转移 source 后 `video-merge-mutations.ts recalcEpisodeCountFromSources`（merge → target / split → 各新建·已有 target，FIX-MERGE-EPCOUNT）；③ 历史漂移修复 Migration 024 / 114。撤销（unmerge）不回退（高水位语义 / snapshot 未存旧值）。
...
- **语言双维度（Migration 112，ADR-199 / LANG-DIM-A）**：语音与字幕分维度行级存储——语言归 source 层（D-176-12），入库时同 vod 各行同值是「多语言版本合并为同一 video 后变异构」的合法去范式化：
  - `audio_language TEXT NULL`：语音规范词单值（封闭枚举 国语/粤语/日语/韩语/英语/国配…，真源 `AUDIO_LANGUAGE_CANONICALS`）。NULL=未知；「国粤双语」归一取首个规范词（不建数组，评审裁定 over-modeling）。
  - `subtitle_languages TEXT[] NULL`：**三态语义，消费方勿 COALESCE 抹平**——NULL=未知（含「双语但不知具体」）/ `{}`=明确无字幕 / `{中文,英文}`=已知具体语言。禁占位词「双语」入数组。
...
- `metadata_field_proposals`：多源交叉验证字段级载体（Migration 119 / ADR-205 D-205-2 / META-49-A）
  - 主键：`(catalog_id, field_name, source_kind)`；`catalog_id FK → media_catalog(id) ON DELETE CASCADE`
  - 🔴 M6 不变量：PK 隐含「同源同字段至多一 proposal」（douban step1/step2 互斥当前安全；未来 douban 多路并行须重启评估）
  - 关键字段：`source_kind`（开放字符串；trust 派生 `CATALOG_SOURCE_PRIORITY`，禁另立平行硬编码，D-205-3）、`source_ref`（外部 ID，可空）、`proposed_value JSONB NOT NULL`（标量/数组，reconcile 比较前经 canonical 归一）、`confidence NUMERIC`、`is_winner BOOLEAN`（逻辑 winner）、`applied BOOLEAN`（实际落 catalog——M1 降级 skip 时 `is_winner=true, applied=false` 解耦，D-205-4）、`conflict_state TEXT`（跨源冲突标记，derive 读注入 needs_review，D-205-6）、`proposed_at`
  - 索引：`idx_metadata_field_proposals_conflict`（partial `(catalog_id) WHERE conflict_state IS NOT NULL`，49-C 冲突行批量读避 N+1）
  - 设计：reconcile（gather→reconcile→write）逐字段裁决的候选/winner/冲突载体；与 `video_metadata_provenance`（last-writer SSOT）**正交**——后者记最终落库来源，本表记多源候选全貌。否决扩 provenance（破坏单行 last-writer 契约）/ 否决 JSON 列（无法对 conflict 态建索引 / 无法批量 LATERAL）
  - 查询函数：`batchUpsertFieldProposals()` / `getFieldProposalsByCatalogId()`（`apps/api/src/db/queries/metadata-field-proposals.ts`）；reconcile 写侧编排（49-B）+ 冲突行批量 LATERAL 读注入 derive（49-C）另卡

- `catalog_blocking_alias_keys`：alias_normalized blocking 桶预计算键派生表（Migration 120 / ADR-206 §META-50-2A / META-50-2A-1）
  - 主键：`(catalog_id, normalized_key)`；`catalog_id FK → media_catalog(id) ON DELETE CASCADE`；索引 `idx_cbak_normalized_key (normalized_key)`（离线分桶 GROUP BY）
  - 关键字段：`normalized_key`（`normalizeForExternalMatch(已知名)` 归一键，简繁不归一 ADR-175 R1）、`source`（胜出源 catalog 哨兵/manual/tmdb/bangumi/douban，crawler 写键时已排除）、`kind`（KnownName.kind）、`confidence NUMERIC`（catalog/manual 恒 1.0）
  - 设计（方案 A 派生表，M-2A-1）：`normalizeForExternalMatch` 含 normalizeTitle（HTML/季数/画质 lookbehind）**SQL 不可复刻** → TS 单一真源预计算落本表（写键 `services/metadata/catalogBlockingKeys.ts recomputeCatalogBlockingKeys`：loadKnownNames→`qualifiesForBlockingBucket` 阈值〔M-2A-2 catalog/manual 恒进 + 非manual 白名单 kind+conf≥0.80 + crawler 排除〕→归一→replace）。blocking 召回（blockingRecall 段③，2A-2）离线分桶 + 单 video 召回 `JOIN` 本表读预计算键，口径一致（D-105a-16）。**仅扩召回不成正证据**（D-206-6a）——只供召回 + evidence_hash blockingKeys 并集，不进 scorePair 评分（不激活休眠 `external_alias_match`）。否决方案 B（多表多列多写点 → META-22 同构回归）/ 禁复用 `title_normalized` 列（= `normalizeMergeKey`，语义分立防漂移，M-2A-1）
  - 写键接线（加性，D-206-8 不开第二写入方）：enrich reconcile settle 后增量重算（`MetadataEnrichService`）+ 回填脚本 `scripts/backfill-catalog-blocking-keys.ts` 补存量；manual 标题/别名编辑重算归 WS3-3A
  - 查询函数：`replaceCatalogBlockingAliasKeys()` / `listCatalogBlockingAliasKeys()`（`apps/api/src/db/queries/catalogBlockingAliasKeys.ts`）
...
| `<kind>_checked_at` ×4 | timestamptz NULL | **ADR-213 / Migration 121**：最近一次确定性健康判定时间（worker ok/low_quality/broken 出口写，瞬态不写）；NULL=从未巡检→读端判 `unknown`。**禁用 updated_at 回填代理**。|
| `<kind>_client_error_at` ×4 | timestamptz NULL | **ADR-213 / Migration 121**：浏览器上报加载失败时间（前台 beacon last-write-wins，自过期）；读端 7 天窗口内计入真破损。仅 4 受治理 kind（stills/thumbnail 无）。|

> **触发器 `trg_media_catalog_clear_image_signals`（ADR-213 D-213-10 / Migration 122+123）**：BEFORE UPDATE，任一 `<kind>_url` 变更即重置该 kind 所有 **url 派生列**——`<kind>_client_error_at` + `<kind>_checked_at`（时间戳信号）+ `<kind>_status`（健康真源，**尊重显式写**：调用方未在同 UPDATE 改 status 才重置 `pending_review`/`missing`；worker 写 verified status 不改 url 故不触发）+ `<kind>_blurhash`/`<kind>_primary_color`/`poster_width`/`poster_height`（渲染占位/尺寸缓存）。这些列均绑旧 URL → URL 替换后失效须清，否则已替换的新图被误判（client_error/fresh-ok/旧 status/陈旧占位）。路径无关（覆盖 apply-candidate/手填/crawler/enrichment 所有 URL 写入）；与 `trg_media_catalog_updated_at` 独立共存。

> **ADR-213「双真源溶解」（方案 C，supersede ADR-212 / refine ADR-211 D-211-2）**：problem-images 健康判定**不再读 `broken_image_events`**，收敛为 `media_catalog` 当前态——`<kind>_status` + `<kind>_checked_at`（server 确定性判定）+ `<kind>_client_error_at`（browser 自过期信号）。读端单一谓词 `problemFilterSqlV2` = `url 非空 AND (status<>'ok' OR client_error_at 在 7d 窗口 OR stale-ok)`，counts/list 逐字共用（total 不漂移）。`broken_image_events` **降级为纯遥测**（趋势/域名/brokenLast7Days），schema 零改动。实施 Migration 121（0M，本表 +8 列）+ P4-A（worker 写 checked_at）+ P4-B（beacon 写 client_error_at，URL 同源守卫）+ P4-C（读端谓词 + DTO）+ P4-S（周期巡检收尾），时序 `0M→A→A-SCAN门→C`。
...
| `source_hostname` | 107 | TEXT NULL CHECK 小写 | 规范化 hostname（P3-3-B `host_health` JOIN key，SRCHEALTH-P3-3-A）。语义真源 = `@resovo/media-probe` `extractHostname`（`new URL().hostname`：小写/去端口/去 userinfo/IDN punycode/IPv6 含方括号）；NULL=URL 不可解析（不参与 host 降权）。写路径 3 处维护（`upsertSource`/`replaceSourcesForSite` DO UPDATE/`replaceSourceUrl` 换源重算）；**存量回填走 `scripts/backfill-source-hostname.ts`**（migration 不回填：IDN punycode SQL 不可复制 + 单事务 55.7 万行锁风险，arch-reviewer 裁决 D）+ 末尾 ANALYZE；partial index `idx_video_sources_hostname WHERE deleted_at IS NULL AND source_hostname IS NOT NULL` |
| `last_admin_verified_at` | 109 | TIMESTAMPTZ NULL | admin 审核台真实播放亲验时间戳（ADR-198）。NULL=从未被 admin 真实播放亲验；非 NULL=最近 `playback-verify` 成功时刻，溯源「谁验证的/何时」，区别于爬虫/众包 `last_rendered_at`/`last_feedback_at`。admin 验证是确定性状态写入（成功直更 `render_status='ok'`+probe 复活，**不写 EMA 三字段** ADR-198 D-198-4），驱动 P3-1 双时钟 render 维度衰减回升语义 |
| `audio_language` | 112 | TEXT NULL | 语音规范词单值（ADR-199 D-199-1，封闭枚举 国语/粤语/日语…）。NULL=未知；「国粤双语」取首个规范词不建数组。详见 §5.2 语言双维度小节 |
| `subtitle_languages` | 112 | TEXT[] NULL | 字幕语言数组**三态**：NULL=未知（含双语未知具体）/ `{}`=明确无字幕 / `{中文,英文}`=已知。勿 COALESCE 抹平 |
| `audio_language_source` | 112 | TEXT **NOT NULL** DEFAULT 'unknown' CHECK 5 值 | 语音 provenance：source_name_token/vod_lang/title_token/region_inferred/unknown（五级推断链 D-199-3；后两级可被前三级覆盖，反向禁止）|
| `subtitle_language_source` | 112 | TEXT **NOT NULL** DEFAULT 'unknown' CHECK 4 值 | 字幕 provenance：同上**无 region_inferred**（字幕不做地区推断）|
...
注：037 既有 origin 列实际值为 `'island_detected' / 'auto_refetch_success' / 'auto_refetch_failed'`（无 CHECK 约束）；M-SN-4 worker 新增写入 `'scheduled_probe' / 'render_check' / 'manual_recheck' / 'feedback_driven' / 'circuit_breaker'`，无需 schema 迁移即可兼容。Migration 106（SRCHEALTH-P2-4-A）再增 `'manual_route_reprobe'`（运营线路级重探信号，reprobeRoute 批量入队 + partial index `idx_source_health_events_route_reprobe_unprocessed`；jobId 落 `triggered_by` 溯源）。Migration 109（ADR-198）再增 `'admin_playback'`（admin 审核台真实播放**失败**定向 recheck 信号——成功直更 render/probe 不入队；失败不直接置 dead〔D-198-2 红线〕改记本 origin + `processed_at=NULL`，复用 feedback-driven-recheck worker 定向消费〔D-198-8〕；partial index `idx_health_events_admin_playback_pending WHERE origin = 'admin_playback' AND processed_at IS NULL`）。types 见 `SourceHealthEventOriginLegacy ∪ SourceHealthEventOriginWorker` union。
...
**待审队列过滤维度扩展（MODUX-P3-1-A / item 3，无 schema 迁移）：** `GET /admin/moderation/pending-queue` 的 `PendingQueueQuerySchema` 加性补 3 个可选 query 参数（后向兼容，非新端点）：`year`（精确年份，`mc.year = year`）/ `decade`（年代起始年，`mc.year ∈ [decade, decade+10)`，如 2020→2020s）/ `enrichmentStatus`（富集完整度枚举）。**`enrichmentStatus` 枚举 `ENRICHMENT_STATUSES = ['missing','partial','complete']`（真源 `packages/types/src/admin-moderation.types.ts`，value 导出）** —— **从 raw 字段/provenance 派生，不按 UI `EnrichmentSummary` 反推**（后者已默认填 'pending'/0，丢失"从未富集"信号）。派生语义（raw：`videos.meta_quality->>'enriched_at'` / `videos.douban_status` / `media_catalog.bangumi_subject_id` / `media_catalog.douban_id·tmdb_id·imdb_id`）：① `complete` = `enriched_at IS NOT NULL AND (douban_status='matched' OR bangumi_subject_id IS NOT NULL)`（富集已跑 + 主源匹配）；② `missing` = `enriched_at IS NULL AND douban_id/tmdb_id/imdb_id/bangumi_subject_id 全 NULL`（无富集痕迹）；③ `partial` = 其余。三态互斥穷尽。**SQL WHERE 实现 = MODUX-P3-1-B（`listPendingQueue`）**；本卡（P3-1-A）仅契约层（schema + 共享 `PendingQueueQuery` 类型扩 `year?/decade?/enrichmentStatus?`）。
...
- **`identity_candidate`**（Phase 2b 落地 / **Migration 086**）：video-pair 候选 shadow 表 + 状态机 `pending`/`confirmed`/`rejected`/`superseded`。字段（D-105a-7）：`left_video_id`/`right_video_id`（FK CASCADE）/ `canonical_pair_key`（min\|max 有序）/ `status` / `parser_version` / `scorer_version` / `evidence_jsonb` / `evidence_hash`（D-105a-8 确定性 sha256）/ `legacy_score`（nullable）/ `identity_score` / `strong_negative_reasons[]` / `trigger_source`（`ingest`/`offline-rescore`/`manual-search`/`enrichment`——**Migration 111** 外部 ID 绑定后定向重评，BUGFIX-IDENTITY-ENRICH-RESCORE；`title_change`——**Migration 113** 标题变更后定向重评，GOV-4/SEQ-20260612-03）/ `group_key` / `revived_from_candidate_id`·`superseded_by_candidate_id`（自引用 FK SET NULL，保复活链）/ `created_at`·`updated_at`。约束：`CHECK(left<>right)` + `CHECK(left::text<right::text)`（canonical 有序兜底）。索引：`uq_identity_candidate_pending`（partial unique `WHERE status='pending'` / R5 幂等）+ pair_key 反查 + `(status,scorer_version,parser_version)` + left/right video FK 反查 + `idx_title_observations_core_key`（blocking 召回支撑，表达式索引）。
- **离线生成**（Phase 2b 落地 / Bull `identity-candidate-queue` · `apps/api/src/workers/identityCandidateWorker.ts`）：Blocking **多 key 并集召回**（CHG-VIR-10 D-105a-17：段 ① `title_observations.coreTitleKey` 分桶 + 段 ② external_id `provider:id` 分桶〔双源 Y-105a-4：media_catalog 外部 ID 列上卷 ∪ video_external_refs manual_confirmed〕，禁 pairwise 全量 + MAX_BUCKET 护栏 + 全局 seen 去重 / `services/identity/blockingRecall.ts` 真源）→ Scoring（复用 `scorePair`）→ 单事务幂等 upsert（hash 比对 noop / 腾位 supersede + 新建 / 复活链；评分→hash→upsert 共享层 `pairScoringPersist.ts`，blockingKeys = 双方 core key + 共享 ext 桶 key 并集）。与现有实时 group-by 候选并行对照，实时端点轻量读 + 旧 group fallback；实时 p95 继承 ADR-105 ≤200ms，离线 job 另立 SLO。手动触发 `scripts/enqueue-identity-rescore.ts`（无自动 scheduler / shadow 阶段）；对比报表 `scripts/identity-compare-report.ts`（CHG-VIR-10 扩 trigger_source 切片 + `--source=` 过滤）。**enrichment 定向重评**（BUGFIX-IDENTITY-ENRICH-RESCORE / Migration 111）：外部 ID 绑定完成位点（MetadataEnrichService/DoubanService/BangumiService 的 auto_matched·manual_confirmed ref 落库后，事务路径在 COMMIT 后）fire-and-forget `enqueueVideoRescore` → worker `video-rescore` job → `services/identity/videoRescore.ts` 单视频双键召回重评（`trigger_source='enrichment'`；candidate 降级不触发——externalIdLoader 双源不认 candidate 证据面不变），修复 ingest 一次性快照与异步 enrichment 的证据时差空窗（实证案例：bangumi id 晚 5-9 分钟绑定 → 同名对纯标题分低于 0.75 门槛永久缺席候选）。**标题变更定向重评**（GOV-4 / Migration 113）：admin 标题实变位点（VideoService 更新冗余副本后 fire-and-forget）+ 批量清洗脚本（backfill-standard-titles 逐行）写当前版本观测（`insertObservationIfAbsent` 查询层真源）→ admin 位点另入队 `video-rescore`（`trigger_source='title_change'`），批量路径由每日 reconcile 兜底消化。**版本对账 + 周期重扫**（GOV-3 / `services/identity/versionReconcile.ts` + `workers/identityReconcileScheduler.ts`）：boot 自愈（失配检测 → 入队）+ 每日兜底 tick → `version-reconcile-rescan` job 四步幂等编排（失配检测/观测重写/runIdentityRescore/残留 supersede），修复 parser/scorer bump 搁浅候选 + 召回数据源同钉版本的双联缺陷（2026-06-12 实证）；scheduler 必须 api 进程内（ADR-107 §4，arch-reviewer BLOCKER 裁决）。
...
### 5.19 前台卡片尺寸体系（ADR-214 / ADR-215，Migration 124）

- `card_size_settings`（Migration 124，ADR-214 D-214-3）：DB 驱动、后台可配的前台卡片尺寸体系数据底座（3 档 seed 恒存在；不可删，档位新增/退役走 ADR-214 amendment + migration）

| 列名 | 类型 | 说明 |
|------|------|------|
| id | uuid | 主键 DEFAULT gen_random_uuid()；audit `card_size.update` 的 target_id 锚点（仿 home_section D-182-5.3） |
| size_class | text | UNIQUE，`CardSizeClass` 封闭枚举 3 值 CHECK（`'standard'｜'compact'｜'scroll'`） |
| desktop_columns | int | 网格档（standard/compact）列数，CHECK BETWEEN 2 AND 8；scroll 档 NULL（D-214-4 存列数不存卡宽，弹性列由容器派生） |
| card_width_px | int | scroll 档横滚卡定宽 px，CHECK BETWEEN 120 AND 280；网格档 NULL（横滚无列概念、本就定宽） |
...
| updated_at | timestamptz | 触发器 `card_size_settings_set_updated_at_trg` 维护 |
  ...
  - **档位×单位绑定 CHECK**（`card_size_settings_unit_by_class_check`，ADR-214 D-214-3 / Codex-R1 HIGH）：`(size_class IN ('standard','compact') AND desktop_columns IS NOT NULL AND card_width_px IS NULL) OR (size_class='scroll' AND desktop_columns IS NULL AND card_width_px IS NOT NULL)`——单位绑定到具体 size_class（非仅"二选一"），拒绝语义倒置行（scroll 带 columns / 网格档带 width）。DB 级倒置行测见 `tests/integration/api/card-size-settings-schema.test.ts`。
  - seed 3 行（SQL 字面量，`ON CONFLICT (size_class) DO NOTHING`，运营可改）：standard(列 5, gap 16) / compact(列 3, gap 12) / scroll(卡宽 170, gap 16)。默认值真源 `CARD_SIZE_DEFAULTS`（`@resovo/types`，CARD-SIZE-TYPES-QUERIES 落地）+ seed 一致性单测守 SQL↔TS 同步（D-214-5；migration 纯 SQL 不能 import TS）。
...
- `119_metadata_field_proposals.sql`（ADR-205 D-205-2 / META-49-A：新建 metadata_field_proposals 多源交叉验证字段级载体——每 catalog 每字段每源一行 proposal，承载候选值/逻辑 winner/实际 applied/冲突态，喂 reconcile 逐字段裁决；与 video_metadata_provenance last-writer SSOT 正交。PK(catalog_id,field_name,source_kind) M6 不变量 + conflict partial index；116–118 列表 drift 待清理卡补全）
