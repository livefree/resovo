...
- `catalog_characters` + `catalog_character_actors`：作品角色阵容 + 配音(CV)（动漫角色↔声优 N:M，来源 Bangumi `/v0/subjects/:id/characters`；Migration 083 / ADR-161 AMENDMENT / META-19）
  ...
  - 写入：`BangumiService.gatherEnrichmentData` 拉 `/v0/subjects/:id/characters` → `applyEnrichmentDb` 经 `replaceCatalogCharacters` **delete-by-catalog-then-insert 全量替换（事务内 / 仅 REST 命中非降级时，degraded 守卫防瞬时故障误删）**；覆盖 auto + 人工 confirmMatch 两路径
