...
注：037 既有 origin 列实际值为 `'island_detected' / 'auto_refetch_success' / 'auto_refetch_failed'`（无 CHECK 约束）；M-SN-4 worker 新增写入 `'scheduled_probe' / 'render_check' / 'manual_recheck' / 'feedback_driven' / 'circuit_breaker'`，无需 schema 迁移即可兼容。Migration 106（SRCHEALTH-P2-4-A）再增 `'manual_route_reprobe'`（运营线路级重探信号，reprobeRoute 批量入队 + partial index `idx_source_health_events_route_reprobe_unprocessed`；jobId 落 `triggered_by` 溯源）。types 见 `SourceHealthEventOriginLegacy ∪ SourceHealthEventOriginWorker` union。
