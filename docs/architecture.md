...
- `current_episodes`：`INT NULL`（Migration 078，ADR-163 / CHG-367-B-A）— **第 3 层**：当前已播集数（外部 metadata 真源 / NULL=未取到 / 连载中持续更新 / 完结后等于 total_episodes 或外部源独立提供）。CHECK 正整数（同 total_episodes）。
  - 写入路径：MetadataEnrichService step2/step3 自动 enrich（按 catalog.status 派发 / completed→total / ongoing→current / `updateVideoEpisodes(auto)` COALESCE 仅写 NULL 字段 + WHERE 守卫 no-op 不触 updated_at）+ DoubanService.confirmSubject/confirmFields 手动覆盖（CHG-367-B-B 落地）
  - 显示规约（CHG-367-B-B）：审核台 TabDetail 三维显示 "已收 {episode_count} / 已播 {current_episodes} / 共 {total_episodes}" / `currentEpisodes > totalEpisodes` 数据异常防御（仅显示 current + 异常标记 / DB 层不强制 `total >= current` 不变式 / Y1）
