...
interface DataTableProps<T> {
  ...
  toolbar?: ReactNode | ToolbarConfig;  // 内置 toolbar（替代原拟 searchSlot 单一插槽，扩成完整 toolbar 编排）
  bulkActions?: ReactNode | BulkActionsConfig;  // 选中后表内底部 sticky bulk bar（原拟名 bulkSlot）
