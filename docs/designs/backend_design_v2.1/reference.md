...
| Table language | **【已闭合 Step 1–6 + 7A】** framed surface / toolbar / saved views / header menu(enableHeaderMenu) / bulk(.dt__bulk) / row flash / body 独立滚动 / `.dt__foot` pagination / 隐藏列 chip / filter chips slot 全部进入 DataTable 一体化；后续列表页可消费内置 props，无需外置 PaginationV2 / FilterToolbar / SelectionActionBar 编排（视频库切换路径 = CHG-DESIGN-08 + CHG-DESIGN-02 Step 7B） | DataTable 一体化骨架已完整；列表页接入新 props 即可与视频库标杆对齐 |
| 表格密度 | 当前 DataTable 13px body、`.dt__foot` 24px 高（复用 `--row-h-compact`）；设计稿 table/dataTable 主体 12px、th 11px 仍待独立 token 卡收紧 | 视觉显松，行高与 poster/操作列比例不稳 |
