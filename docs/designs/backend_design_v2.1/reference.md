...
- ✅ **已实现**（CHG-DESIGN-02 Step 1–6 + Step 7A 落地，单测覆盖）：
  - **Step 1–6**（types/framed surface / 表头菜单 / toolbar+views / bulkActions+flashRowKeys / saved views 持久化）：`toolbar: ToolbarConfig` / `bulkActions: React.ReactNode`（`.dt__bulk` sticky bottom） / `flashRowKeys: ReadonlySet<string>` / `enableHeaderMenu: boolean`（含 sort / hide / clear filter） / saved views menu（`toolbar.viewsConfig` + sessionStorage 持久化）
...
- 外层 `.dt` 是一个完整 framed surface：`bg2/border/r-3/overflow:hidden/height:100%`。【✅ Step 7A 落地：framed surface + `flex-direction: column` + 防御性 `min-height: 240px`；消费方提供 height 约束时 body 独立滚动完整生效】
- 内置 `.dt__toolbar`：search、saved views、filter chips、hidden-columns chip、toolbar slot。【✅ Step 1–6 + 7A 全部落地：toolbar slot / saved views（Step 4–6） + filter chips slot / hidden-columns chip（Step 7A）】
...
- 表头 sticky，body 独立滚动。【✅ Step 7A 落地：thead `position: sticky; top: 0` + body wrapper `overflow-y: auto + flex: 1 + min-height: var(--row-h)`；完整体验需消费方在父级提供 height 约束】
- 行支持 hover、selected、乐观更新 flash。【✅ Step 5 落地（flashRowKeys prop）】
- 页脚 `.dt__foot` 内置 pagination 和计数。【✅ Step 7A 落地：`pagination?: PaginationConfig` prop + `.dt__foot` 渲染（24px 高页码按钮，复用 `--row-h-compact` token）；**三态语义**：省略 prop → summary-only foot（与外置 PaginationV2 零冲突）；显式 `pagination={...}` → 完整 foot（含 pager + pageSize select）；`pagination={{ hidden: true }}` → 完全不渲染】
...
interface DataTableProps<T> {
  ...
  // ✅ Step 1–6 已落地（CHG-DESIGN-02）：
  flashRowKeys?: ReadonlySet<string>;   // 乐观更新闪烁（原拟名 flashIds，落地名 flashRowKeys）
  toolbar?: ToolbarConfig;              // 内置 toolbar 编排（search / trailing / viewsConfig + Step 7A: hideHiddenColumnsChip / hideFilterChips 兜底）
  ...
  // ✅ Step 7A 已落地（CHG-DESIGN-02 / 2026-04-30）：
  pagination?: PaginationConfig;        // .dt__foot 内置；page/pageSize 控制态走 query.pagination + onQueryChange
                                        //   PaginationConfig：{ hidden? / pageSizeOptions? / summaryRender? }
                                        //   三态语义：省略 prop → summary-only foot（避免与外置 PaginationV2 双 pager）
                                        //              显式 pagination={...} → 完整 foot（pager + pageSize 激活）
                                        //              pagination={{ hidden: true }} → 完全不渲染
                                        //   summaryRender ctx 含 selectedCount（含选区文案上下文）
  // 隐藏列 chip 自动启用（toolbar.hideHiddenColumnsChip 兜底）；HiddenColumnsMenu popover 列出 hidable 列；pinned 显示"已锁定"
  // filter chips slot 自动启用（toolbar.hideFilterChips 兜底）；6 种 FilterValue.kind 默认 formatter；column.renderFilterChip 完全接管逃生口

  // 🔄 后续阶段（CHG-DESIGN-02 Step 7C / 独立卡）：
  // pinnedSticky?: boolean;            // 列左侧固定物理 sticky 渲染（当前 column.pinned 仅作"恒可见"标记）
  // 12px 表格密度 / cell 复合组件沉淀 → CHG-DESIGN-12
}

interface PaginationConfig {
  readonly hidden?: boolean;
  readonly pageSizeOptions?: readonly number[];   // 缺省 [10, 20, 50, 100]
  readonly summaryRender?: (ctx: PaginationSummaryContext) => ReactNode;
}

interface PaginationSummaryContext {
...
**阈值**：当 props 数量超过 ~20 个时，退回 frame 组合路线（仅作为兜底）。Step 7A 后顶层 props 数 ≈ 17（< 20，安全余量充足）。
...
| 位置 | 已对齐 | 差异 |
|---|---|---|
| `DataTable` | server/client mode、sticky header、selection、sort、column visibility、loading/error/empty、row density；**【已闭合 Step 1–6 + 7A】**：`.dt` framed surface（`dt-styles.tsx`）；toolbar / bulk bar 一体化；表头菜单（enableHeaderMenu sort+hide+clear filter）；saved views（sessionStorage）；row flash（flashRowKeys）；body 独立滚动（thead sticky + body overflow-y + 防御性 min-height: 240px）；`.dt__foot` 内置 pagination（`pagination?: PaginationConfig`，24px 高页码按钮）；隐藏列 chip + popover；filter chips slot + 6 种 FilterValue 默认 formatter | **【后续】**：column.pinned 物理 left-sticky 渲染（当前仅"恒可见"标记）；body cell 12px 密度（当前 13px，token 卡）；selected/hover 引用未定义 token `--accent-subtle` / `--bg-subtle`（独立 token 卡补齐） |
| `Toolbar` | 有 leading、columnSettings、trailing slot；**【已闭合 Step 4 + 7A】**：DataTable 内置 toolbar slot（ToolbarConfig 支持 search / saved views / 自定义 ReactNode）；filter chips slot（独立第二 flex row）；隐藏列 chip（toolbar 内 views 之后、trailing 之前固定位置） | 外置 8px gap 容器形态保留作嵌入式场景兜底，外部 export 不取消 |
| `ColumnSettingsPanel` | 有 portal、focus trap、ESC、外部点击、列显隐；**【已闭合 Step 3 + 7A】**：列显隐入口已迁到表头菜单（enableHeaderMenu）+ 隐藏列 chip popover（HiddenColumnsMenu 列出全部 hidable 列；pinned 列显示"已锁定"标签 + disabled） | 外部 ColumnSettingsPanel 仍可独立 export 但不作首选 |
| `Pagination` | 有页码窗口、pageSize、计数；**【已闭合 Step 7A】**：`.dt__pager` 内置 footer 与表格一体（24px 高页码按钮，复用 `--row-h-compact` token；page window 算法 ±2 + 首末页 + ellipsis）；缺省渲染最简 foot（仅 summary）；`pagination={{ hidden: true }}` 完全不渲染 | 外置 `PaginationV2` 仍可独立 export 但首选 DataTable 内置 pagination |
...
- 封面是 64x36 横图，设计稿是 48×72 竖版（CHG-UX2-03 升级，原 32×48 废弃）。
...
实现提醒：**system/settings 子页如果继续存在，侧栏不应暴露多个 system 子项**；设计稿是设置页内部 tab。

**现状（ADR-125 / M-SN-7 REDO-03-A 收敛后）**：
- 真源路径 `/admin/settings`（顶级）— SettingsContainer + 5 Tab（settings / cache / monitor / config / migration）
- 6 个旧 URL 全部 308 永久重定向（`permanentRedirect`）：`/admin/system` / `/admin/system/settings` / `/admin/system/cache` / `/admin/system/config` / `/admin/system/migration` / `/admin/system/monitor` → `/admin/settings(?tab=X)`
- sidebar 仅暴露 1 个入口「站点设置 → `/admin/settings`」（`admin-nav.tsx`）
- 后续 REDO-03-B 扩展至 8 Tab（补图片 / 通知 / API·Webhook / 登录会话）
...
- framed surface（外层 bg2 + border + radius r-3 + overflow hidden）【✅ Step 7A 落地】
- toolbar 模式（DataTable 内部含 search / saved views / filter chips / 隐藏列 chip / toolbar slot）【✅ Step 1–6 + 7A 全部落地】
- sticky thead；密度（th 8×12 padding / fs 11 uppercase / td 8×12 padding / fs 12）【✅ thead sticky + body 独立滚动 Step 7A 落地；🔄 12px 密度待独立 token 卡】
- hover bg3 / selected accent-soft / active 左侧 2px accent
- empty / error / loading 三态文案与样式
- pagination（`.dt__foot` 内置，24px 高页码按钮）【✅ Step 7A 落地（PaginationConfig + PaginationFoot 24px 复用 `--row-h-compact`）】
- bulk bar（表格内 sticky bottom + accent border-top）【✅ Step 5 bulkActions prop 已落地】
...
| `thumb` | 封面 | 60 | `<img class="tbl-thumb tbl-thumb--sm">` 48×72 竖版 radius 4 | ❌ | ❌ |
...
> **2026-05-01 更新**：SEQ-20260429-02（12 张设计对齐卡）已全部完成。以下以 ✅ 标注已闭合项，剩余行为当前实际差距。  
> 详见 `docs/archive/2026Q2/design-iterations/implementation-gap-report-2026-04-30.md`（已更新至 2026-05-01 状态）。

| 范围 | 状态 | 说明 |
|---|---|---|
| Token 未定义引用 | ✅ CHG-DESIGN-01 | 12 个未定义 token 全部修复；verify:token-references 脚本守门 |
| Brand 颜色 | ✅ 裁决接受 | 实现侧蓝系 accent，不回迁 amber；crux 裁决锁定 |
| Shell / Sidebar collapse | ✅ CHG-DESIGN-04 + 05 | 过渡动效 / 分区标题等高占位 / NavTip / footer / 折叠按钮文案全部对齐 |
| Scrollbar 混用 | ✅ CHG-DESIGN-03 | 全局 6px，统一 CSS 规则注入 admin-shell-styles |
| DataTable 框架扩展 | ✅ CHG-DESIGN-02（Steps 1-7B） | toolbar / saved views / header menu / bulk bar / pagination / filter chips / body 独立滚动 全部一体化 |
| Poster 48×72 竖版 | ✅ CHG-DESIGN-08 + CHG-UX2-03 | Thumb 组件 + `size="poster-sm"` 入 admin-ui；视频库已消费；尺寸由 32×48 升级至 48×72 |
| 视频库 row actions | ✅ CHG-DESIGN-08 + 12 | InlineRowActions 5 按钮（查看/编辑/可见性/审核/发布）入 admin-ui |
| 视频库 DualSignal / VisChip | ✅ CHG-DESIGN-08 + 12 | DualSignal / VisChip 入 admin-ui；视频库已消费（probe/render 暂 unknown，API 待扩展） |
| VideoEditDrawer 540px 基础表单 | ✅ CHG-DESIGN-10 | 680px + fullscreen + 4 Tab + quick header + footer |
| Dashboard StatCard 占位 | ✅ CHG-DESIGN-07 | 8 卡浏览态（AttentionCard/WorkflowCard/MetricKpiCard/RecentActivity/SiteHealth）+ KpiCard/Spark 入 admin-ui |
| Analytics 空内容 | ✅ CHG-DESIGN-09 | AnalyticsView（period select / KPI×4 / SVG 图表 / 爬虫任务表）落地 |
| Settings 多入口 | ✅ CHG-DESIGN-06 | 收敛单「站点设置」入口 + 双栏容器（180px tab list + 1fr content） |
| Settings Tab 内容 | ⚠️ 仍为占位 | 5 Tab（settings/cache/monitor/config/migration）内容为"正在迁移中"占位文案；reference §5.11 要求 8 类真实表单 |
| 占位页面（10 个） | ⚠️ 未启动 | moderation（P0）/ crawler / sources / subtitles / image-health / merge / home / users / submissions / audit |
| admin-ui Popover | ⚠️ 缺失 | HiddenColumnsMenu 私有实现；无通用 Popover 原语 |
| admin-ui SplitPane | ⚠️ 缺失 | Moderation 三栏前置依赖（reference §4.6） |
| admin-ui PageHeader / AdminButton | ⚠️ 缺失 | 各页面 inline style 自拼；视觉碎片化风险 |
| FilterChipBar 外置妥协 | ⚠️ 已知妥协 | 视频库 `hideFilterChips: true` + toolbar.trailing 注入外置 FilterChipBar；follow-up 待专项卡 |
| 表格密度精调 | ⚠️ 低优 | body 13px（目标 12px）/ th 无独立 token（目标 11px） |
| Notifications/Tasks 真实 API | ⚠️ mock | Shell 接入 mock 数据；M-SN-4+ 替换真端点 |
| Dashboard 编辑态 / CardLibrary | ⚠️ 后置 | 仅浏览态；编辑态/CardLibrary/全屏 M-SN-3+ |
| 开发者模式三栏 | ⚠️ 低优 | 只有 components demo；Tokens/Semantic 面板未实现 |
...
4. 视频类素材默认**竖版 48×72 poster**（CHG-UX2-03 固化，原 32×48 废弃）。只有 Home Ops banner / 前台预览等横向运营位使用横图。
