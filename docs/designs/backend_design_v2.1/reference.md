...
- 外层 `.dt` 是一个完整 framed surface：`bg2/border/r-3/overflow:hidden/height:100%`。
- 内置 `.dt__toolbar`：search、saved views、filter chips、hidden-columns chip、toolbar slot。
- 搜索为 280px 固定基础宽度，输入左侧有 search icon。
- Saved views 支持 personal/team scope 和保存当前视图。
- 表头内置菜单：排序、筛选入口、固定/取消固定、隐藏列；popover 使用 `bg4/border-strong/shadow-lg`。
- 表头 sticky，body 独立滚动。
- 行支持 hover、selected、乐观更新 flash。
- 页脚 `.dt__foot` 内置 pagination 和计数。
- 批量操作 `.dt__bulk` sticky bottom 在表格内部，顶部 accent border。
- 选择全部匹配是 DataTable/SelectionActionBar 的一等交互，需要有 query-based bulk API 或明确的 ids 获取策略。

#### 4.4.2 改造路径（§0-7 裁决）

**扩展现有 `DataTable`，不引入 TableFrame 新抽象层**。建议增量加入的字段：
...
interface DataTableProps<T> {
  ...
  // 待补：
  flashIds?: ReadonlySet<string>;       // 乐观更新闪烁
  views?: TableView[];                  // 个人/团队 视图保存
  onViewChange?: (id: string) => void;
  searchSlot?: ReactNode;               // toolbar 内置 280px 搜索
  enableHeaderMenu?: boolean;           // 表头集成菜单（升降序+过滤+固定+隐藏）
  pinnedSticky?: boolean;               // 列固定为左侧 sticky
  bulkSlot?: ReactNode;                 // 选中后表内底部 sticky bulk bar
}
```
...
| 位置 | 已对齐 | 差异 |
|---|---|---|
| `DataTable` | server/client mode、sticky header、selection、sort、column visibility、loading/error/empty、row density | 外层不是 `.dt` framed surface；使用 div grid 而非 table；toolbar/pagination/bulk bar 分散在外部；表头没有菜单、筛选、pin/hide popover；没有 saved views；没有 row flash；body cell 13px 比设计稿 12px 松；selected/hover 仍引用未定义 `--accent-subtle`、`--bg-subtle` |
| `Toolbar` | 有 leading、columnSettings、trailing slot | 设计稿 toolbar 是 DataTable 内部结构，含 search/views/filter chips；当前只是外部 8px gap 容器 |
| `ColumnSettingsPanel` | 有 portal、focus trap、ESC、外部点击、列显隐 | 设计稿列控制在表头菜单和隐藏列 chip 中出现；当前为独立"列设置"按钮，交互入口和视觉不同 |
| `Pagination` | 有页码窗口、pageSize、计数 | 设计稿 `.dt__pager` 更紧凑，页码 24px 高，footer 与表格一体；当前 nav 在表格外部，28px 按钮，信息靠右 |
| `SelectionActionBar` | 有 selected count、page/all-matched 文字、actions、confirm | 设计稿有 `.sel-bar` 浮动形态和 `.dt__bulk` 表格内 sticky 形态；当前 bar 是表格外部 sticky，视觉层级和边界不同 |
...
- toolbar 模式（DataTable 内部含 search / saved views / filter chips / 隐藏列 chip / toolbar slot）
- sticky thead；密度（th 8×12 padding / fs 11 uppercase / td 8×12 padding / fs 12）
- hover bg3 / selected accent-soft / active 左侧 2px accent
- empty / error / loading 三态文案与样式
- pagination（`.dt__foot` 内置，24px 高页码按钮）
- bulk bar（表格内 sticky bottom + accent border-top）
...
| Table language | API 能用，但设计稿旗舰能力没有收束到一个组件：toolbar、views、header menu、bulk、pager、frame 分散 | 视频库和后续列表页继续自由拼装，每页像不同表格 |
| 表格密度 | 当前 DataTable 13px body、外部分页 28px 按钮，设计稿 table/dataTable 主体 12px、th 11px | 视觉显松，行高与 poster/操作列比例不稳 |
