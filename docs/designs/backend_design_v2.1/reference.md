...
- 页脚 `.dt__foot` 内置 pagination 和计数。【✅ Step 7A 落地：`pagination?: PaginationConfig` prop + `.dt__foot` 渲染（24px 高页码按钮，复用 `--row-h-compact` token；缺省渲染最简 foot；显式 `{ hidden: true }` 才完全不渲染）】
...
interface DataTableProps<T> {
  ...
  pagination?: PaginationConfig;        // .dt__foot 内置；page/pageSize 控制态走 query.pagination + onQueryChange
                                        ...
                                        //   缺省渲染最简 foot（仅 summary）；显式 hidden=true 才不渲染
