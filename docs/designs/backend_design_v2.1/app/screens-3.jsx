    ...
    <>
      ...
      <div style={{display:"grid", gridTemplateColumns:"180px 1fr", gap:16}}>
        ...
        <div className="card">
          ...
          {tab === "filter" && <div className="card__body" style={{display:"flex", flexDirection:"column", gap:12}}>
            {[
              ["启用关键词过滤", "屏蔽敏感分类标题", true],
              ["显示成人内容", "仅对标记 is_adult 的隐藏源生效", false],
              ["新视频默认隐藏", "需手动审核后才公开", true],
              ["豆瓣低分自动隐藏", "评分 < 4.0 自动设为 internal", false],
            ].map(([t,d,on], i) => (
              <div key={i} style={{display:"flex", gap:12, alignItems:"center", padding:10, background:"var(--bg3)", borderRadius:4}}>
                <div style={{width:36, height:20, borderRadius:999, background: on?"var(--accent)":"var(--muted-2)", position:"relative", flexShrink:0}}>
                  <div style={{position:"absolute", top:2, left: on?18:2, width:16, height:16, borderRadius:"50%", background:"white", transition:"left .15s"}}/>
                </div>
                <div style={{flex:1}}><div style={{fontWeight:600, fontSize:13}}>{t}</div><div style={{fontSize:11, color:"var(--muted)", marginTop:2}}>{d}</div></div>
              </div>
            ))}
          </div>}

          {tab === "images" && <div className="card__body" style={{display:"flex", flexDirection:"column", gap:10}}>
            <div style={{fontSize:11, color:"var(--muted)"}}>当 P0 封面 4xx/5xx 时的回退顺序：</div>
            {["img.doubanio.com → cdn.proxy.io", "img.maoyan → mirror.maoyan", "豆瓣 ID 已知 → 豆瓣 API 重新拉取", "全部失败 → 显示占位图"].map((s, i) => (
              <div key={i} style={{display:"flex", gap:8, alignItems:"center", padding:"8px 10px", background:"var(--bg3)", borderRadius:4}}>
                <span style={{color:"var(--muted)", fontFamily:"monospace", fontSize:11}}>{i+1}.</span><span style={{flex:1, fontSize:12}}>{s}</span><button className="btn btn--xs">{I.edit}</button>
              </div>
            ))}
          </div>}

          {(tab === "notify" || tab === "api" || tab === "cache" || tab === "auth") && <div className="empty" style={{padding:80}}>这一栏配置正在加载…</div>}
        </div>
      </div>
    </>
  );
