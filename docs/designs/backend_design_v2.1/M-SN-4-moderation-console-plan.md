...
> 版本：v1.3（v1.0 → v1.1 → v1.2 → v1.3 修订日志见文末 §12）  
> 日期：2026-05-01（v1.0 / v1.1 / v1.2 / v1.3 同日）  
> 作者：Engineering（claude-sonnet-4-6 辅助生成；v1.1 缺漏补全；v1.2 审核意见 12 项；v1.3 修复"runner 字典序 ≠ 设计部署顺序"硬伤）  
> 状态：待工程确认 → 实施  
...
| 059 | video_sources.quality_detected + 实测分辨率（v1.3 重排自原 058）| 默认空，按 §4.2 双轨写入 |
| 060 | videos.review_source（v1.3 重排自原 059）| 默认 'manual'，CHECK 3 值；DEFAULT 不补 NOT NULL（types 标 nullable）|
...
Plan-Source: docs/designs/backend_design_v2.1/M-SN-4-moderation-console-plan.md v1.3
...
*本文档（**v1.3**）为 M-SN-4 开发真源，代码改动须对照本文档验证覆盖度。任务卡入队前请先核对 §8 任务卡映射 + §11 收口准入条件。*
