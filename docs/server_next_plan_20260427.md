...
- **范围**：
  ...
  - ~~视频编辑 Drawer 4 Tab（基础 / 线路 / 图片 / 豆瓣）+ 全屏模式 + 全局入口~~ ✅ 已完成（CHG-DESIGN-10，2026-05-01）
  - VideoEditDrawer 线路 / 图片 / 豆瓣三 Tab 真实 API 集成（依赖后端扩展 VideoAdminDetail 字段）
- **前置依赖（v2.5 新增）**：`SplitPane` admin-ui 原语先于 moderation 首张任务卡完成
- **完成标准**：
...
| ADR-104 | home_modules admin API 协议 — **2026-05-12 CHG-SN-5-04 已采纳（PASS / 2 轮 Opus 评审 / 1 红线 + 3 黄线 + 3 advisory 全闭环）** | 推荐 3 落地所需 6 端点（list/create/update/delete/reorder/publish-toggle）+ admin only 鉴权 + 错误码复用 ADR-110 零新增 + audit log 扩 5 actionType+1 targetKind + 首版零缓存 + zod 双层校验（Service + DB CHECK）+ DISCUSS-6 闭合 | CHG-SN-5-05/-06 端点实施卡解锁条件 = ADR-104 PASS（已满足）；CHG-SN-5-07 `/admin/home` 视图卡依赖 -05/-06 端点就位 |
| ADR-105 | merge candidate / split / unmerge API 协议 — **2026-05-12 CHG-SN-5-08 已采纳（PASS / 3 轮 Opus 评审 / 3 红线 R-105-1/-2/-3 + 5 黄线 Y-105-1..5 全闭环）** | 推荐 5 落地所需 4 端点（candidates 预览 + merge 执行 + unmerge + split）+ admin only 鉴权 + 错误码复用 ADR-110 零新增 + audit log 扩 3 actionType + video_merge_audit 表（migration 062）+ ADR-114 复合键约束兼容（merge 前置探测 + STATE_CONFLICT 409）+ v1 评分简化 score = source_overlap_ratio ∈ [0,1] | CHG-SN-5-09 candidates 预览 + CHG-SN-5-10 merge + split + unmerge + migration 062 解锁条件 = ADR-105 PASS（已满足）；CHG-SN-5-11/-12 视图卡依赖 -09/-10 端点就位 |
...
>
> **M-SN-6 启动条件已满足**，原"推迟 M-SN-6"风险解除。

- ~~用户已说明 v2.1 仍在补"设置补全 / 采集展开 / 开发者模式 / 弹层规范"~~（已完工，见上）
- M-SN-4 审核台的前置依赖已从"设计稿"转为"admin-ui SplitPane 原语"（§6 M-SN-4 前置依赖）
...
> **v2.5 更新（2026-05-01）**：设计稿四项（设置补全 / 采集展开 / 开发者模式 / 弹层规范）已完工。M-SN-6 启动条件已满足，无需推迟。下一步优先级：M-SN-4 审核台（需先落 SplitPane 原语）→ M-SN-5 P1 视图 → M-SN-6 周边视图 + 设计稿对齐。
