...
resovo/
├── apps/
│   ├── web-next/    # 前台 Next.js（@resovo/web-next，port 3000）
│   ├── server-next/ # 后台管理 Next.js（@resovo/server-next，port 3003；CUTOVER 后独占 /admin）
...
```
/* (默认)   → web:3000        前台
/admin/*   → server-next:3003  后台管理（CUTOVER 2026-06-08：apps/server v1 退役）
/v1/*      → api:4000         API
```
...
```bash
npm run dev     # 同时启动 apps/web-next:3000 + apps/server-next:3003 + apps/api:4000
```
...
npm --workspace @resovo/web-next run dev

# 终端 2：后台管理（Next.js，端口 3003）
npm --workspace @resovo/server-next run dev
...
| 管理后台 | http://localhost:3003/admin | 需要 admin 账号 |
...
1. 访问 http://localhost:3003/admin（或代理模式 http://localhost:8080/admin）
...
# E2E 测试（需要 web:3000 和 server-next:3003 在运行）
npm run test:e2e
```
...
| 前台 | Next.js 15 + TypeScript + Tailwind CSS（apps/web-next） |
| 后台管理 | Next.js 15 + TypeScript（内联样式，无 Tailwind）+ packages/admin-ui（apps/server-next） |
