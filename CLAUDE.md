...
- ❌ 函数满足以下任一条件，必须先拆分再继续，不得以"先实现再重构"或"功能完整"为由跳过：
  1. 包含 2 个以上独立逻辑阶段（如：校验 → 变换 → 写库，各阶段可独立命名即算独立）
  2. 条件 / 循环 / try 嵌套层数达到 3 层
  3. 超过 80 行，且不属于**纯声明性结构**（定义：函数体仅为 JSX return 块或静态数组 / 对象字面量，其中不含 Service 调用、store 写入、async 操作或副作用）
- ❌ 文件满足以下任一条件，必须先拆分再继续添加代码：
  1. 导出 2 个以上主要概念（主要概念：组件 / Service / Hook / Store / 独立功能函数集合，每种各算一个）
  2. 超过 500 行，且不属于**纯声明性文件**（类型定义文件、静态映射数据文件）
  - **400 行触发门禁**：向已超过 400 行的文件继续写入前，必须先在当前输出中用一句话完成声明：「本文件的唯一职责是 ___」。能完成且职责单一 → 可继续；无法用一句话表达 → 必须立即拆分，不得继续写入。
