...
describe('ImageHealthProblemBoard — load-more 竞态守卫（IMGH-P4-LOADMORE-RACE）', () => {
  it('在途 load-more + 切 reason → 过期响应被丢弃，不污染新视图', async () => {
    let resolveStale: (v: ReturnType<typeof pageOf>) => void = () => {}
    getProblemImagesMock.mockImplementation((p: { offset?: number; reason?: string }) => {
      if (p.reason === 'all' && p.offset === 0) {
        return Promise.resolve(pageOf([makeRow({ videoId: 'a', problemReason: 'low_quality' })], 60))
      }
      if (p.reason === 'all' && p.offset === 48) {
        return new Promise<ReturnType<typeof pageOf>>((r) => { resolveStale = r }) // load-more 在途挂起
      }
      if (p.reason === 'broken') {
        return Promise.resolve(pageOf([makeRow({ videoId: 'b', problemReason: 'broken' })], 1))
      }
      return Promise.resolve(pageOf([], 0))
    })
    const { container } = render(<ImageHealthProblemBoard />)
    await waitFor(() => expect(container.querySelectorAll('[data-problem-card]').length).toBe(1))
    // 触发 load-more（reason=all, offset=48）→ 在途挂起（seq=N）
    fireEvent.click(screen.getByTestId('problem-board-load-more'))
    // 立即切 reason=broken → useEffect 重置（seq=N+1），rows=[b]
    fireEvent.click(screen.getByText('真破损'))
    await waitFor(() => {
      const reasons = Array.from(container.querySelectorAll('[data-problem-card]')).map((el) => el.getAttribute('data-problem-reason'))
      expect(reasons).toEqual(['broken'])
    })
    // 过期 load-more（reason=all 数据）现在 resolve → seq 不匹配 → 必须丢弃，不追加
    resolveStale(pageOf([makeRow({ videoId: 'a2', problemReason: 'low_quality' })], 60))
    await new Promise((r) => setTimeout(r, 0))
    const reasonsAfter = Array.from(container.querySelectorAll('[data-problem-card]')).map((el) => el.getAttribute('data-problem-reason'))
    expect(reasonsAfter).toEqual(['broken']) // 未被过期 a2(low_quality) 污染
  })
})
