...
describe('POST /v1/admin/videos/:id/douban-sync', () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks()
    // 默认：视频存在，无 douban_id，搜索无结果（降级 no_match）
    mockVQ.findAdminVideoById.mockResolvedValue({
      id: VIDEO_ID,
      title: '测试电影',
      year: 2020,
      douban_id: null,
  ...
  it('匹配成功时返回 200 updated:true 和 fields', async () => {
    mockSearch.mockResolvedValue([{ id: '26266893', title: '测试电影' }])
    mockDetail.mockResolvedValue({
      ...
      directors: ['导演名'],
      casts: ['演员A'],
      posterUrl: 'https://img.douban.com/poster.jpg',
    })
    mockVQ.updateDoubanData.mockResolvedValue(true)
...
describe('DoubanService.syncVideo', () => {
  ...
  it('相似度不足 80% 时返回 no_match', async () => {
    mockSearch.mockResolvedValue([{ id: '12345', title: '完全不相关的电影' }])
    const result = await service.syncVideo(VIDEO_ID)
    expect(result).toEqual({ updated: false, reason: 'no_match' })
  })

  it('匹配成功时更新 DB，返回 updated:true', async () => {
    ...
    expect(mockVQ.updateDoubanData).toHaveBeenCalledWith(
      expect.anything(),
      VIDEO_ID,
      expect.objectContaining({ doubanId: '26266893', rating: 7.9 })
    )
  })
