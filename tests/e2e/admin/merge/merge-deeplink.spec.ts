...
async function installMergeMocks(page: Page) {
  ...
  // VideoPicker 搜索（q = 标题关键词；真实后端 q 不匹配 UUID）
  await page.route(new RegExp('/v1/admin/videos\\?'), (route) =>
    route.fulfill(json({ data: [VID_A, VID_B].map(videoRow), total: 2, page: 1, limit: 20 })))
  // CHG-VIR-13-FIX-PREFILL：深链成员预填 = GET /admin/videos/:id 精确查（真实契约对齐）
  await page.route(new RegExp('/v1/admin/videos/[0-9a-f-]{36}$'), (route) => {
    const id = route.request().url().split('/').pop()!
    if (id === VID_A || id === VID_B) return route.fulfill(json({ data: videoRow(id) }))
    return route.fulfill({ status: 404, ...json({ error: { code: 'NOT_FOUND', message: 'not found', status: 404 } }) })
  })
  // split 自动加载线路矩阵 + 拆分建议
...
test.describe('merge 工作台深链升级映射（CHG-VIR-13 系列收口 ②）', () => {
  ...
  test('1. 无参数 → 默认 candidates + Segment 常驻 3 区（CHG-VIR-15-UX-C 收编）', async ({ page }) => {
    await page.goto('/admin/merge')
    await expect(page.getByTestId('merge-mode-segment')).toBeVisible()
    // UX-C：常驻 3 区（候选更名「合并工作区」顶替旧 merge tab）
    for (const label of ['合并工作区', '拆分工作区', '操作记录']) {
      await expect(page.getByTestId('merge-mode-segment').getByText(label)).toBeVisible()
    }
    await expect(page.getByTestId('merge-mode-segment').getByText('待审候选')).toHaveCount(0)
    await expect(page.getByTestId('merge-mode-segment').getByText('批量合并（深链）')).toHaveCount(0)
  })

  test('2. ?candidate_a&candidate_b → mode=merge 工作区 + 成员预填（by-id 标题充实）', async ({ page }) => {
    ...
    // 深链成员预填（CHG-VIR-13-FIX-PREFILL：GET /admin/videos/:id 充实真实标题，非「加载失败」占位）
    await expect(page.getByTestId(`merge-member-${VID_A}`)).toBeVisible()
    await expect(page.getByTestId(`merge-member-${VID_A}`)).toContainText('E2E 视频')
    await expect(page.getByTestId(`merge-member-${VID_A}`)).not.toContainText('加载失败')
  ...
  test('3. ?ids=csv → mode=merge（batch 形态升级映射 + UX-C 深链动态项）', async ({ page }) => {
    ...
    // UX-C：深链上下文 Segment 动态补「批量合并（深链）」项
    await expect(page.getByTestId('merge-mode-segment').getByText('批量合并（深链）')).toBeVisible()
  })
