 ...
 *
 * size variant 设计稿映射（数值真源已迁至 packages/design-tokens/src/admin-layout/cover.ts，
 * 由 CHG-UX2-01 / SEQ-20260505-01 落地；本组件 sizeSpec 用 var(--cover-*-{w|h}) 引用）：
 *   - `poster-sm`（默认）：**32×48 竖版**（reference §10 视频类素材默认）；用于视频库表格行 thumb 列
 *     **CHG-UX2-03 拟把视频库主列表 thumb 升级到 poster-md 解决"显示过小"痛点**
 *   - `poster-md`：**48×72 竖版**（CHG-UX2-01 校准 38×56 → 48×72；视觉量级与 sm/lg 拉开）；
 *     用于视频库列表（CHG-UX2-03 起）/ 详情页 / Drawer header
 ...
 *   - `poster-xl`：**120×180 竖版**（CHG-UX2-01 新增）；预留给详情页 hero / 全屏预览；
 *     触发型升级：当 PendingCenter `poster-lg` 仍嫌小时切换（CHG-UX2-EXT-A）
...
/**
 * Thumb 尺寸变体（数值真源 packages/design-tokens/src/admin-layout/cover.ts）
 *
 * 设计稿 §10 + components.css 映射；6 种规格满足视频库 / 审核台 / banner / dashboard 全场景。
 *
 * - `poster-sm`（默认）：32×48 竖版 radius sm — reference §6.1 视频库 thumb 列 + 审核台列表行
 * - `poster-md`：48×72 竖版 radius sm — 视频库列表（CHG-UX2-03） / Drawer header
 * - `poster-lg`：80×120 竖版 radius sm — 审核台中央海报 / 详情页主图（v1.6 G6 引入）
 * - `poster-xl`：120×180 竖版 radius md — 详情页 hero / 全屏预览（CHG-UX2-01 新增）
 * - `banner-sm`：64×36 横版 radius sm — Home Ops banner / 横向运营位
 * - `square-sm`：28×28 正方形 radius md — sev icon box / 头像类圆角方块
 */
export type ThumbSize = 'poster-sm' | 'poster-md' | 'poster-lg' | 'poster-xl' | 'banner-sm' | 'square-sm'
