...
import { adminShell, adminTable, adminDensity, adminShellZIndex, adminShellSurfaces } from './src/admin-layout/index.js'
...
function buildLayoutVars(): Array<[string, string]> {
  ...
  for (const group of [adminShell, adminTable, adminDensity, adminShellZIndex, adminShellSurfaces]) {
    for (const [varName, value] of Object.entries(group as Record<string, string>)) {
      out.push([`--${varName}`, value])
    }
  }
...
function buildJs(): string {
  const tokens = {
    ...
    adminLayout: { adminShell, adminTable, adminDensity, adminShellZIndex, adminShellSurfaces },
  }
...
function buildDts(primitiveVars: Array<[string, string]>, lightVars: Array<[string, string]>): string {
  ...
  return [
    ...
    '  adminLayout: { adminShell: object; adminTable: object; adminDensity: object; adminShellZIndex: object; adminShellSurfaces: object }',
    '}',
