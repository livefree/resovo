...
function buildSemanticVars(theme: 'light' | 'dark'): Array<[string, string]> {
  const sources: Array<[string, Record<'light' | 'dark', unknown>]> = [
    ...
    ['state-fg-on-soft', stateFgOnSoft],
...
function buildJs(): string {
  const tokens = {
    ...
    semantic: { bg, fg, border, accent, surface, state, stateFgOnSoft, dualSignal, layout },
...
function buildDts(primitiveVars: Array<[string, string]>, lightVars: Array<[string, string]>): string {
  ...
  return [
    ...
    '  semantic: { bg: object; fg: object; border: object; accent: object; surface: object; state: object; stateFgOnSoft: object; dualSignal: object; layout: object }',
