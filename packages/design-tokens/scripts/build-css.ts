...
import { adminShell, adminTable, adminDensity, adminShellZIndex } from '../src/admin-layout/index.js'
...
function buildThemeIndependentVars(): Array<[string, string]> {
  ...
  for (const group of [adminShell, adminTable, adminDensity, adminShellZIndex]) {
    for (const [varName, value] of Object.entries(group as Record<string, string>)) {
      out.push([`--${varName}`, value])
    }
  }
