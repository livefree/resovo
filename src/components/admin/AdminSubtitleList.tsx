 ...
 * ADMIN-03: 通过 → is_verified=true；拒绝 → 软删除
 */
...
export function AdminSubtitleList() {
  ...
  return (
    <div data-testid="admin-subtitle-list">
      {error && (
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-[var(--subtle)]">
            ...
            {!loading && subtitles.map((sub) => (
              <tr key={sub.id} className="bg-[var(--bg)] hover:bg-[var(--bg2)]" data-testid={`admin-subtitle-row-${sub.id}`}>
                <td className="max-w-48 truncate px-4 py-3 text-[var(--text)]">
                  {sub.video_title ?? sub.video_id}
                </td>
                <td className="px-4 py-3 text-[var(--muted)]">{sub.label} ({sub.language})</td>
                <td className="px-4 py-3 text-[var(--muted)] uppercase">{sub.format}</td>
