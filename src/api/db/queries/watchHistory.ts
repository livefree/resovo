...
/**
 * Upsert 观看历史：同一 (user_id, video_id, episode_number) 已存在时更新进度，
 * 不存在时插入新记录。
 */
export async function upsertWatchHistory(
  ...
  await db.query(
    `INSERT INTO watch_history (user_id, video_id, episode_number, progress_seconds, watched_at)
     VALUES ($1, $2, $3, $4, NOW())
     ON CONFLICT (user_id, video_id, episode_number)
     DO UPDATE SET
       progress_seconds = EXCLUDED.progress_seconds,
       watched_at = NOW()`,
    [input.userId, input.videoId, input.episodeNumber ?? null, input.progressSeconds]
  )
}
...
export async function getUserHistory(
  ...
  const [rows, countResult] = await Promise.all([
    db.query<WatchHistoryRow>(
      `SELECT wh.video_id, wh.episode_number, wh.progress_seconds, wh.watched_at,
